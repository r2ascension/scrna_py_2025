#!/usr/bin/env Rscript
# ==============================================================================
# T/NK Cell Subcluster LLM Interpretation - PRODUCTION VERSION v1.1
# ==============================================================================
#
# Version: v1.1 (2026-01-29)
# Status: Production-ready, aligned with B cell v2.6
#
# Key Features (same as B cell pipeline):
...
#
# T/NK-specific modifications:
#   - Input: T/NK cell h5ad from subcluster pipeline
#   - LLM context: Emphasizes CD4/CD8 distinction
#   - Output: T/NK cell interpretation reports
#
# Bug fixes in v1.1:
#   - P0-1: ✅ compareCluster instead of global pooled enrichment
#   - P0-3: ✅ API key from environment variable
#   - P0-4: ✅ multisession instead of multicore (fork safety)
#   - P1-5: ✅ Auto-detect logFC column name
#   - P1-6: ✅ Fail-fast validation for required columns
#   - P1-7: ✅ Unified toupper for TERM2GENE
#   - P2-10: ✅ Thread limiting (OMP/MKL/BLAS)
#   - P2-11: ✅ Enhanced statistics reporting
#
# ==============================================================================
...
OUTPUT_DIR <- "/home/h2048/data/R/0129/tcell_interpret_v1_1"
...
# ======================================================================
# Deep Mode (interpret_agent) - Dual-track settings
# ======================================================================
RUN_INTERPRET_AGENT <- TRUE

INTERPRET_AGENT_MODEL <- "deepseek-reasoner"   # or "deepseek-reasoner"
...
# ==============================================================================
# P2-10: Thread limiting (prevent CPU oversubscription)
# ==============================================================================
...
# P0-4 fix: Use multisession (spawn) instead of multicore (fork)
# This prevents fork + Python initialization conflicts
...
# P1-8: Check for double-normalization risk
...
top_markers <- all_markers %>%
  ...
  mutate(
    gene = toupper(gene),
    cluster = as.character(cluster) # P0-1: Ensure character for compareCluster
  ) %>%
...
# P2-11: Report marker counts per cluster
...
if (!is.null(cellmarker_db)) {
  ...
  for (i in 1:nrow(cellmarker_db)) {
    ...
    if (!is.na(markers_raw) && markers_raw != "") {
      ...
      markers <- toupper(markers) # P1-7: Unified toupper
...
if (!is.null(panglaodb_db)) {
  panglaodb_term2gene <- panglaodb_db %>%
    ...
    mutate(gene_symbol = toupper(trimws(gene_symbol))) %>% # P1-7: Unified toupper
...
# P1-7: Unified toupper for GMT genes
...
# ==============================================================================
# P0-1 FIX: compareCluster-based GO Enrichment (per-cluster, not global pooled)
# ==============================================================================

cat("=== GO Enrichment (compareCluster, GMT-based, zero gene loss) ===\n")
...
# ==============================================================================
# MSigDB Enrichment (Hallmark + KEGG) - compareCluster
# ==============================================================================
...
tryCatch(
  {
    ...
    term2gene_list <- lapply(gene_sets_list, function(gs) {
      if (length(gs$genes) > 0) {
        data.frame(
          ...
          gene = toupper(gs$genes), # P1-7: Unified toupper
          stringsAsFactors = FALSE
        )
      }
    })
    ...
    # Hallmark enrichment (compareCluster)
    ...
    # KEGG enrichment (compareCluster)
...
# ==============================================================================
# CellMarker Enrichment - compareCluster
# ==============================================================================
...
# ==============================================================================
# PanglaoDB Enrichment - compareCluster
# ==============================================================================
...
# ==============================================================================
# LLM-based Interpretation: Annotation Task
# ==============================================================================

cat("\n=== LLM Interpretation: Annotation Task ===\n")

# Prepare enrichment results (compareClusterResult objects)
  ...
  annotation_results <- tryCatch(
    {
      interpret(
        ...
        save_history = TRUE,
        history_dir = file.path(OUTPUT_DIR, "reports"),
        additional_context = paste(
          ...
          "   - MAIT (TRAV1-2+, KLRB1+): MR1-restricted, bacterial recognition",
          "   - NKT (ZBTB16+, TRAJ18+): Lipid-reactive, rapid responders",
          "   - γδ T (TRDC+, TRG+): Unconventional, epithelial barrier surveillance",
          "",
          "4. MEMORY STATES:",
          ...
          "",
          "6. TISSUE CONTEXT (Respiratory tissues - lung, airway, sinus):",
          "   - TRM cells are abundant in mucosal tissues",
          "   - Type 2 inflammation: Enhanced Th2, ILC2 in CRSwNP",
          "   - Barrier immunity: Th17, MAIT, γδ T at epithelial surfaces",
          "",
          "ANNOTATION INSTRUCTIONS:",
          "For EACH cluster, your annotation must explicitly specify:",
          "  a) Lineage: CD4+ / CD8+ / NK / Innate-like T (MAIT/NKT/γδ)",
          ...
          "  - 'CD8+ tissue-resident memory cytotoxic T cells (TRM, activated)'",
          "  - 'CD4+ regulatory T cells (Treg, FOXP3+)'",
          "  - 'CD8+ effector memory T cells (TEM, GZMB+)'",
          "  - 'CD16+ mature NK cells (cytotoxic)'",
          "  - 'MAIT cells (innate-like, TRAV1-2+)'",
          "",
          "Always base your reasoning on:",
          "  1. Specific marker gene expression (cite genes)",
          "  2. Enriched pathways (GO/Hallmark/KEGG)",
          "  3. Consistency across multiple databases",
          "  4. Biological plausibility in respiratory tissue context",
          sep = "\n"
        )
      )
    },
...
# ==============================================================================
# LLM-based Interpretation: Phenotype Task
# ==============================================================================
  ...
  phenotype_results <- tryCatch(
    {
      interpret(
        ...
        additional_context = paste(
          ...
          "",
          "4. INNATE-LIKE T FUNCTIONS:",
          "   - MAIT: Rapid antibacterial responses, mucosal immunity",
          "   - NKT: Bridging innate/adaptive immunity, lipid recognition",
          "   - γδ T: Epithelial surveillance, tissue repair, IL-17 production",
          "",
          "5. ACTIVATION VS EXHAUSTION:",
          ...
          "",
          "6. MIGRATION AND LOCALIZATION:",
          "   - Circulation: CCR7+, S1PR1+ for lymphoid/blood trafficking",
          "   - Tissue residency: CD69+, CD103+ for local retention and surveillance",
          "   - Inflammatory recruitment: Chemokine receptor expression (CXCR3, CCR4, CCR6)",
          "",
          "7. DISEASE RELEVANCE (CRSwNP context):",
          "   - Type 2 inflammation: Th2, ILC2 drive eosinophilia and tissue remodeling",
          "   - Tissue damage: Cytotoxic T/NK cells may contribute to epithelial injury",
          "   - Immune dysregulation: Treg dysfunction, Th17 imbalance",
          "   - Chronic inflammation: Exhausted T cells, persistent activation",
          "",
          "PHENOTYPING INSTRUCTIONS:",
          "For EACH cluster, describe:",
          "  a) Primary effector function (cytotoxic/helper/regulatory/innate)",
          "  b) Key biological processes (e.g., 'IFN-γ-mediated antiviral immunity')",
          ...
          "",
          "Always connect phenotype to:",
          "  1. Specific marker genes and pathways",
          "  2. Expected functional outcomes",
          "  3. Disease mechanisms in CRSwNP",
          "  4. Therapeutic implications if relevant",
          sep = "\n"
        )
      )
    },
...
if (isTRUE(RUN_INTERPRET_AGENT)) {
  ...
  # ---- build a global gene_fold_change named vector (optional but recommended)
  fc_tbl <- all_markers %>%
    dplyr::filter(!gene %in% genes_to_filter) %>%
  ...
  # ---- combine TERM2GENE across databases (keep DB prefix for traceability)
  term2gene_blocks <- list()
  
  if (exists("go_bp_gmt") && !is.null(go_bp_gmt) && nrow(go_bp_gmt) > 0) {
    term2gene_blocks[["GO_BP"]] <- go_bp_gmt %>%
      dplyr::select(term, gene) %>% dplyr::mutate(term = paste0("GO_BP|", term))
  }
  if (exists("go_mf_gmt") && !is.null(go_mf_gmt) && nrow(go_mf_gmt) > 0) {
    term2gene_blocks[["GO_MF"]] <- go_mf_gmt %>%
      dplyr::select(term, gene) %>% dplyr::mutate(term = paste0("GO_MF|", term))
  }
  if (exists("go_cc_gmt") && !is.null(go_cc_gmt) && nrow(go_cc_gmt) > 0) {
    term2gene_blocks[["GO_CC"]] <- go_cc_gmt %>%
      dplyr::select(term, gene) %>% dplyr::mutate(term = paste0("GO_CC|", term))
  }
  if (exists("hallmark_term2gene") && !is.null(hallmark_term2gene) && nrow(hallmark_term2gene) > 0) {
    term2gene_blocks[["HALLMARK"]] <- hallmark_term2gene %>%
      dplyr::select(term, gene) %>% dplyr::mutate(term = paste0("HALLMARK|", term))
  }
  if (exists("kegg_term2gene") && !is.null(kegg_term2gene) && nrow(kegg_term2gene) > 0) {
    term2gene_blocks[["KEGG"]] <- kegg_term2gene %>%
      dplyr::select(term, gene) %>% dplyr::mutate(term = paste0("KEGG|", term))
  }
  if (exists("cellmarker_term2gene") && !is.null(cellmarker_term2gene) && nrow(cellmarker_term2gene) > 0) {
    term2gene_blocks[["CELLMARKER"]] <- cellmarker_term2gene %>%
      dplyr::select(term, gene) %>% dplyr::mutate(term = paste0("CELLMARKER|", term))
  }
  if (exists("panglaodb_term2gene") && !is.null(panglaodb_term2gene) && nrow(panglaodb_term2gene) > 0) {
    term2gene_blocks[["PANGLAODB"]] <- panglaodb_term2gene %>%
      dplyr::select(term, gene) %>% dplyr::mutate(term = paste0("PANGLAODB|", term))
  }
  
  combined_term2gene <- dplyr::bind_rows(term2gene_blocks) %>%
    dplyr::mutate(term = as.character(term), gene = toupper(as.character(gene))) %>%
    dplyr::distinct()
  ...
  if (nrow(combined_term2gene) == 0) {
    ...
    # ---- use the same big context you already crafted
    # You already have a long string in additional_context for interpret().
    # For interpret_agent, just reuse it as "context".
    deep_context <- paste(
      "DEEP MODE CONTEXT (interpret_agent):",
      "This is T/NK cell subclusters from respiratory tissues (CRSwNP).",
      "Emphasize CD4/CD8/NK/innate-like, memory states, activation/exhaustion, TRM, and disease relevance.",
      sep = "\n"
    )
    ...
    for (cid in clusters_for_agent) {
      ...
      genes_c <- top_markers %>%
        dplyr::filter(cluster == cid) %>% dplyr::pull(gene) %>% unique()
      ...
      # per-cluster enrichResult (multi-DB, with DB prefix retained)
      er <- tryCatch(
        {
      ...
      # interpret_agent returns either a single interpretation (Default) or a list;
      # we normalize to a single list object "res_one"
      res_one <- ia
      if (inherits(ia, "interpretation_list") && length(ia) >= 1) {
        res_one <- ia[[1]]
      }
      ...
      # save per-cluster txt using the package print method
  ...
  cat("[INFO] RUN_INTERPRET_AGENT=FALSE, skip deep mode\n")
}
...
if (!is.null(seurat_obj$cell_type_L2)) {
  ...
  for (celltype in unique_celltypes) {
    ...
    # P0-2 FIX: Filter enrichment results to this celltype's subclusters
    ...
    for (db_name in names(enrich_databases)) {
      ...
      if (!is.null(db_result) && "compareClusterResult" %in% class(db_result)) {
        # Subset compareClusterResult to only subclusters of this celltype
        ccr <- db_result@compareClusterResult
        ccr_filtered <- ccr %>% filter(Cluster %in% subclusters)

        if (nrow(ccr_filtered) > 0) {
          # Create a new compareClusterResult object with filtered data
...
tryCatch(
  {
    ...
    cat("# T/NK Cell Subcluster Interpretation Report v1.1\n\n")
    ...
    # Deep Mode interpret_agent results
...
cat("T/NK CELL ANALYSIS COMPLETE - v1.1 (PRODUCTION)\n")
cat(
  "================================================================================\n\n"
)
...
cat("RDS objects:\n")
cat("  - reports/annotation_results.rds     LLM annotation ⭐\n")
cat("  - reports/phenotype_results.rds      LLM phenotype ⭐\n")
cat(
  "  - reports/celltype_interpretations.rds Per-celltype detailed interpretations\n"
)
cat(
  "  - reports/[database]_enrich.rds      Individual enrichment results (compareClusterResult)\n"
)
cat("\n")
...
cat("Bug fixes in v1.1:\n")
cat("  ✅ P0-1: compareCluster instead of global pooled enrichment\n")
cat("  ✅ P0-2: Per-celltype enrichment properly filtered\n")
cat("  ✅ P0-3: API key from environment variable (security)\n")
cat("  ✅ P0-4: multisession instead of multicore (fork safety)\n")
cat("  ✅ P1-5: Auto-detect logFC column name\n")
cat("  ✅ P1-6: Fail-fast validation for required columns\n")
cat("  ✅ P1-7: Unified toupper for TERM2GENE\n")
cat("  ✅ P1-8: Double-normalization detection\n")
cat("  ✅ P2-10: Thread limiting (OMP/MKL/BLAS)\n")
cat("  ✅ P2-11: Enhanced statistics reporting\n")
cat("\n")
