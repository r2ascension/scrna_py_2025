#!/usr/bin/env Rscript
# ==============================================================================
# Epithelial Cell Subcluster LLM Interpretation - PRODUCTION VERSION v2.8
# ==============================================================================
#
# Version: v2.8-PRODUCTION (2026-02-05)
# Status: Production-ready with critical interpret_agent fixes
# Previous: v2.7-FIXED (2026-01-29)
#
# Key Fixes from v2.7 → v2.8:
#   ✅ FIXED: PPI parameter conflict (set to FALSE for epithelial)
#   ✅ IMPROVED: Enhanced deep_context with disease/marker vocabulary
#   ✅ IMPROVED: Robust to_scalar() function for complex nested structures
#   ✅ IMPROVED: Better error tracking for interpret_agent failures
#   ✅ IMPROVED: gene_fold_change validation before interpret_agent
#   ✅ IMPROVED: TERM2GENE validation and gene overlap checks
#   ✅ IMPROVED: REPORT.md includes failure summaries
#   ✅ FIXED: Memory optimization hints for 48 subclusters
#
# Key Features:
...
#   ✅ interpret_agent deep mode with regulatory network analysis
...
OUTPUT_DIR <- "/home/h2048/data/R/0205/epithelial_interpret_v2_8_PRODUCTION"
...
INTERPRET_AGENT_ADD_PPI <- FALSE   # ⚠️ CRITICAL FIX: 上皮分析关闭PPI，避免引入额外依赖和波动
...
if (isTRUE(RUN_INTERPRET_AGENT)) {
  ...
  # ---- Prepare fold-change vector (gene -> logFC) ----
  cat("\n[INFO] Preparing gene fold-change vector for interpret_agent...\n")
  ...
  if (is.na(lfc_col)) {
    ...
    if (!is.null(gene_fc) && length(gene_fc) > 0) {
      cat(sprintf("[OK] Prepared gene_fold_change: %d genes\n", length(gene_fc)))
    } else {
      cat("[WARN] gene_fold_change is NULL or empty\n")
      gene_fc <- NULL
    }
  }
  
  # ---- Combine TERM2GENE with DB prefix ----
  cat("\n[INFO] Combining TERM2GENE databases for interpret_agent...\n")
  ...
  if (nrow(combined_term2gene) == 0) {
    cat("[ERROR] combined_term2gene is empty! Skip interpret_agent.\n")
    RUN_INTERPRET_AGENT <- FALSE
  } else {
    combined_universe <- unique(combined_term2gene$gene)
    
    cat(sprintf("[OK] interpret_agent TERM2GENE combined: %d pairs, %d unique genes\n",
                nrow(combined_term2gene), length(combined_universe)))
    
    # ---- Enhanced context for epithelial cells (v2.8) ----
    deep_context <- paste(
      "Respiratory epithelium subclusters from human nasal, sinus, and airway tissues.",
      "",
      "Cell Type Vocabulary:",
      "- Basal (TP63+KRT5+KRT14+): Stem/progenitor cells",
      "- Suprabasal (KRT13+SPRR1A+): Intermediate differentiation",
      "- Secretory-Goblet (MUC5AC+MUC5B+TFF3+): Mucin-producing cells",
      "- Secretory-Club (SCGB1A1+SCGB3A1+): Clara/Club cells",
      "- Ciliated (FOXJ1+RSPH1+DNAI1+): Motile cilia for mucociliary clearance",
      "- Deuterosomal (DEUP1+CCNO+): Pre-ciliated transitional state",
      "- Ionocyte (FOXI1+CFTR+ATP6V0D2+): Ion/pH regulation",
      "- Tuft/Brush (POU2F3+TRPM5+): Chemosensory cells",
      "- AT1 (AGER+PDPN+HOPX+): Alveolar type 1 (gas exchange)",
      "- AT2 (SFTPC+SFTPB+LAMP3+): Alveolar type 2 (surfactant)",
      "- SMG Basal/Duct/Mucous/Serous: Submucosal gland components",
      "",
      "Functional States:",
      "- Basal cycling (MKI67+TOP2A+): Proliferation/regeneration",
      "- Mucin hypersecretion (MUC5AC/MUC5B high): Goblet cell metaplasia",
      "- Active ciliogenesis (FOXJ1+RSPH1+): Ciliated cell maturation",
      "- Differentiation programs: Basal → Goblet/Ciliated/Club",
      "",
      "Disease Context (Type 2 Inflammation):",
      "- CRSwNP, allergic rhinitis → IL-4/IL-13 signaling",
      "- Goblet cell metaplasia, mucus hypersecretion",
      "- Epithelial barrier dysfunction (tight junction loss)",
      "- Impaired mucociliary clearance",
      "",
      "Analysis Goals:",
      "1. Identify epithelial subtypes based on marker evidence",
      "2. Distinguish cell identity from transient functional states",
      "3. Link subcluster phenotypes to Type 2 inflammation mechanisms",
      "4. Identify regulatory drivers of epithelial remodeling",
      "",
      "Interpretation Guidelines:",
      "- Use marker gene evidence and enriched terms rigorously",
      "- Avoid overinterpreting housekeeping pathways (ribosome, spliceosome)",
      "- Distinguish stable identity (e.g., Basal) from transient state (e.g., cycling)",
      "- Link findings to disease mechanisms when relevant",
      sep = "\n"
    )
    ...
    # ---- Improved to_scalar function (v2.8) ----
    to_scalar <- function(x) {
      ...
      if (is.list(x)) {
        ...
        if (!is.null(json_str)) return(as.character(json_str))
    ...
    agent_results <- list()
    agent_rows <- list()
    agent_failed_clusters <- character(0)  # Track failures (v2.8)
    
    clusters_for_agent <- sort(unique(top_markers$cluster))
    
    for (cid in clusters_for_agent) {
      cat(sprintf("\n[interpret_agent] Processing Cluster: %s\n", cid))
      ...
      if (length(genes_c) < 10) {
        ...
        agent_failed_clusters <- c(agent_failed_clusters, paste0(cid, " (too few genes)"))
        next
      }
      
      # ---- Validate gene overlap (v2.8) ----
      overlap <- length(intersect(genes_c, combined_universe))
      if (overlap < 5) {
        cat(sprintf("[WARN] Low gene overlap (%d/%d) for cluster %s\n", overlap, length(genes_c), cid))
      }
      ...
      if (is.null(er) || nrow(as.data.frame(er)) == 0) {
        ...
        agent_failed_clusters <- c(agent_failed_clusters, paste0(cid, " (no enrichment)"))
        next
      }
      ...
      ia <- tryCatch(
        ...
        error = function(e) {
          cat("[ERROR] interpret_agent failed for", cid, ":", conditionMessage(e), "\n")
          agent_failed_clusters <<- c(agent_failed_clusters, paste0(cid, " (agent error)"))
          NULL
        }
      )
      if (is.null(ia)) next
    ...
    # ---- Report failures (v2.8) ----
    if (length(agent_failed_clusters) > 0) {
      cat(sprintf("\n[WARN] interpret_agent failed for %d clusters:\n", length(agent_failed_clusters)))
      for (fc in agent_failed_clusters) {
        cat(sprintf("  - %s\n", fc))
      }
      
      # Save failure log
      writeLines(
        agent_failed_clusters,
        file.path(OUTPUT_DIR, "reports", "interpret_agent_failures.txt")
      )
    }
...
tryCatch(
  {
    ...
    cat("# Epithelial Cell Subcluster Interpretation Report v2.8-PRODUCTION\n\n")
    cat("**Generated:** ", format(Sys.time()), "\n\n", sep = "")
    cat("**Status:** Production-ready with interpret_agent improvements\n\n")
    cat(
      "**Databases:** Epithelial Markers, CellMarker, PanglaoDB, GO BP/MF/CC, Hallmark, MSigDB KEGG\n\n"
    )
    cat("**LLM Models:** DeepSeek Chat (annotation/phenotype), DeepSeek Reasoner (interpret_agent)\n\n")
    cat(sprintf("**PPI Integration:** %s (epithelial-optimized)\n\n", 
    ...
    if (file.exists(agent_csv_path)) {
      ...
      agent_failure_path <- file.path(OUTPUT_DIR, "reports", "interpret_agent_failures.txt")
      if (file.exists(agent_failure_path)) {
        failed <- readLines(agent_failure_path)
        cat(sprintf("**Failed Clusters:** %d\n\n", length(failed)))
        cat("Failed clusters:\n")
        for (fc in failed) {
          cat(sprintf("  - %s\n", fc))
        }
...
cat("ANALYSIS COMPLETE - Epithelial Subcluster Interpretation v2.8-PRODUCTION\n")
cat(
  "================================================================================\n\n"
)
...
cat(
  "  - annotation_results.csv             Cell_Type + Markers + Regulatory_Drivers\n"
)
cat(
  "  - phenotype_results.csv              Functional_Phenotype + Key_Processes\n"
)
cat(
  "  - interpret_agent_results.csv        Deep mode analysis + regulatory networks ⭐ NEW\n"
)
...
cat("v2.7 → v2.8 Critical Improvements:\n")
cat("  ✅ FIXED: PPI parameter set to FALSE (epithelial-optimized)\n")
cat("  ✅ Enhanced deep_context with disease/marker vocabulary\n")
cat("  ✅ Robust to_scalar() function for complex structures\n")
cat("  ✅ Better error tracking for interpret_agent failures\n")
cat("  ✅ gene_fold_change validation before interpret_agent\n")
cat("  ✅ TERM2GENE validation and gene overlap checks\n")
cat("  ✅ REPORT.md includes failure summaries\n")
cat("\n")
