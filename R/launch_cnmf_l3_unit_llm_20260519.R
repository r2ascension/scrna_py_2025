...
# Launch per-unit LLM interpretation for L3 cell-type cNMF outputs created by
# script/py/run_cnmf_by_l3_celltype_20260519.py.
# Preferred layout is `<run_root>/<lineage>/cnmf_by_celltype/<safe_celltype>/cnmf_full/`.
# Legacy `cnmf_<safe_celltype>/cnmf_full/` directories are still discovered.

UNIT_LLM_SCRIPT <- "/home/h2048/script/R/launch_program_unit_llm_20260513.R"
source(UNIT_LLM_SCRIPT)

RUN_ROOT <- Sys.getenv("CNMF_L3_RUN_ROOT", unset = "/home/h2048/output/program_full_parallel_methods_20260507")
OUTPUT_SUBDIR <- Sys.getenv("CNMF_L3_OUTPUT_SUBDIR", unset = "cnmf_by_celltype")

split_env_l3 <- function(name, default = character()) {
...
source_method_l3 <- function(method) {
  switch(method, cnmf = "cnmf_full", method)
}

discover_l3_cnmf_sources <- function(l3_root) {
  candidate_dirs <- sort(unique(c(
    Sys.glob(file.path(l3_root, "*", "cnmf_full")),
    Sys.glob(file.path(l3_root, "cnmf_*", "cnmf_full"))
  )))
  ...
  records <- lapply(candidate_dirs, function(source_dir) {
    ...
    status <- read_json_unit_llm(file.path(source_dir, "cnmf_l3_status.json"), default = list())
    ...
    celltype_l3 <- as.character(status$celltype_l3 %||% NA_character_)[1]
    list(
      key = if (!is.na(celltype_l3) && nzchar(celltype_l3)) paste0("celltype:", celltype_l3) else paste0("safe:", safe_celltype),
      status_ok = status_ok,
      is_legacy = is_legacy,
...
pick_primary_value_l3 <- function(x) {
  x <- as.character(x)
  x <- trimws(x)
...
infer_l3_hierarchy <- function(source_dir, status = list(), fallback_l3 = NA_character_) {
  ...
  l2 <- as.character(status$celltype_l2 %||% status$cell_type_L2 %||% NA_character_)[1]
  l3 <- as.character(status$celltype_l3 %||% status$cell_type_L3 %||% fallback_l3 %||% NA_character_)[1]
  if (is.data.frame(meta) && nrow(meta) > 0L) {
    if ((!nzchar(l1) || is.na(l1)) && "cell_type_L1" %in% colnames(meta)) l1 <- pick_primary_value_l3(meta$cell_type_L1)
    if ((!nzchar(l2) || is.na(l2)) && "cell_type_L2" %in% colnames(meta)) l2 <- pick_primary_value_l3(meta$cell_type_L2)
    if ((!nzchar(l3) || is.na(l3)) && "cell_type_L3" %in% colnames(meta)) l3 <- pick_primary_value_l3(meta$cell_type_L3)
  }
  hierarchy <- c()
  if (!is.na(l2) && nzchar(l2)) hierarchy <- c(hierarchy, l2)
  if (!is.na(l3) && nzchar(l3)) hierarchy <- c(hierarchy, l3)
  list(
    celltype_l1 = if (!is.na(l1) && nzchar(l1)) l1 else NA_character_,
    celltype_l2 = if (!is.na(l2) && nzchar(l2)) l2 else NA_character_,
    celltype_l3 = if (!is.na(l3) && nzchar(l3)) l3 else fallback_l3,
    hierarchy_label = if (length(hierarchy) > 0L) paste(hierarchy, collapse = " > ") else fallback_l3,
    metadata_path = if (file.exists(meta_path)) normalizePath(meta_path, winslash = "/", mustWork = FALSE) else NA_character_
  )
}

build_l3_cnmf_unit_rows <- function(run_root = RUN_ROOT,
                                    lineages = character(),
                                    celltypes = character(),
                                    all_cnmf_k = TRUE) {
  ...
  for (lineage in lineages) {
    l3_root <- file.path(run_root, lineage, OUTPUT_SUBDIR)
    if (!dir.exists(l3_root)) next
    source_records <- discover_l3_cnmf_sources(l3_root)
    for (record in source_records) {
      source_dir <- record$source_dir
      status <- record$status
      if (!isTRUE(record$status_ok)) next
      ct_basename <- basename(dirname(source_dir))
      celltype <- as.character(status$celltype_l3 %||% if (grepl("^cnmf_", ct_basename)) sub("^cnmf_", "", ct_basename) else ct_basename)
      hierarchy <- infer_l3_hierarchy(source_dir, status = status, fallback_l3 = celltype)
      celltype_l3 <- as.character(hierarchy$celltype_l3 %||% celltype)
      celltype_l2 <- as.character(hierarchy$celltype_l2 %||% NA_character_)
      ...
      for (score_path in score_files) {
        ...
        for (gep in unique(as.character(score_tbl$gep))) {
          unit_id <- safe_unit_id(sprintf("cnmf_l3_%s_k%s_%s", safe_celltype, k, gep))
          out_dir <- file.path(l3_root, "llm_parallel", "cnmf", "units", unit_id)
          ...
          rows[[length(rows) + 1L]] <- new_task_row(
            ...
            extra = list(
              ...
              celltype_hierarchy = hierarchy_label,
              ...
              cnmf_l3_status_json = normalizePath(file.path(source_dir, "cnmf_l3_status.json"), winslash = "/", mustWork = FALSE),
...
launch_cnmf_l3_unit_llm_main <- function() {
  lineages <- split_env_l3("CNMF_L3_LLM_LINEAGES", split_env_l3("CNMF_L3_LINEAGES", character()))
  celltypes <- split_env_l3("CNMF_L3_LLM_CELLTYPES", split_env_l3("CNMF_L3_CELLTYPES", character()))
  all_cnmf_k <- !(Sys.getenv("CNMF_L3_LLM_ALL_K", unset = "1") %in% c("0", "false", "FALSE", "no", "NO"))
  max_parallel <- suppressWarnings(as.integer(Sys.getenv("CNMF_L3_LLM_MAX_PARALLEL", unset = "8")))
  if (length(max_parallel) == 0L || is.na(max_parallel) || max_parallel <= 0L) max_parallel <- 8L
  force <- Sys.getenv("CNMF_L3_LLM_FORCE", unset = "0") %in% c("1", "true", "TRUE", "yes", "YES")
  enable_live <- !(Sys.getenv("CNMF_L3_LLM_ENABLE_LIVE", unset = "1") %in% c("0", "false", "FALSE", "no", "NO"))
  model <- Sys.getenv("CNMF_L3_LLM_MODEL", unset = "deepseek-reasoner")
  timeout_sec <- suppressWarnings(as.integer(Sys.getenv("CNMF_L3_LLM_TIMEOUT_SEC", unset = "240")))
  if (length(timeout_sec) == 0L || is.na(timeout_sec) || timeout_sec <= 0L) timeout_sec <- 240L

  idx <- build_l3_cnmf_unit_rows(RUN_ROOT, lineages = lineages, celltypes = celltypes, all_cnmf_k = all_cnmf_k)
  index_path <- file.path(RUN_ROOT, sprintf("cnmf_l3_unit_llm_task_index_%s.tsv", format(Sys.time(), "%Y%m%d_%H%M%S")))
  ...
  cat(sprintf("[INFO] L3 cNMF unit LLM task index: %s (%d tasks)\n", index_path, nrow(idx)))
  ...
  summary_path <- file.path(RUN_ROOT, sprintf("cnmf_l3_unit_llm_summary_%s.tsv", format(Sys.time(), "%Y%m%d_%H%M%S")))
  ...
  if (is.data.frame(combined$master_manifest_df) && nrow(combined$master_manifest_df) > 0L) {
    cat(sprintf("[OK] L3 cNMF celltype combined LLM manifest: %s (%d celltypes)\n", combined$master_manifest_tsv, nrow(combined$master_manifest_df)))
  }
  cat(sprintf("[OK] L3 cNMF unit LLM launcher finished: %s\n", summary_path))
