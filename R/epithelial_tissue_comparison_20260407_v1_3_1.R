#!/usr/bin/env Rscript
# ==============================================================================
# Epithelial Tissue Comparison Pipeline v1.3.1 - PRODUCTION
# ==============================================================================
#
# Purpose:
...
#
# v1.3.1 Changes (vs v1.3.0):
#   [CHOIR-1] Raise CHOIR alpha from 0.05 to 0.20 to reduce over-fragmented clusters
#   [CHOIR-2] Prefer `scanvi_major` over `scanvi_fine` for CHOIR clustering
#   [CHOIR-3] Align CHOIR variable-feature handling with B cell v2.6 (auto-find + cap)
#   [LLM-4] Disable interpret_agent PPI by default for faster, more stable full runs
#
# v1.3.0 Changes (vs v1.2.0):
#   [PB-1] One-shot pseudobulk cache by sample x tissue x group; avoid repeated subset()
#   [LLM-1] Add validated gene->pathway map from enrichment geneID hits into LLM context
#   [LLM-2] Add per-group ssGSEA LLM interpretation outputs
#   [LLM-3] Add per-cluster CHOIR OFA LLM interpretation outputs
#   [WX-1] Exploratory wilcox outputs now carry explicit evidence-tier metadata
#   [P2-1] stratified_downsample() now ignores NA/blank groups explicitly
#   [P2-2] h5ad sparse export avoids COO data.frame materialization
#   [P2-3] NormalizeData provenance recorded under obj@misc$normalization_for_report
#
...
SCRIPT_BASENAME  <- "epithelial_tissue_comparison_20260407_v1_3_1.R"
OUTPUT_TAG       <- "epithelial_tissue_comparison_v1_3_1"

H5AD_PATH  <- "/home/h2048/data/core20260322/epithelial_scanvi_v2_7_HOTFIX_SELF_final.h5ad"
OUTPUT_DIR <- file.path("/home/h2048/data/R/0407", OUTPUT_TAG)
...
env_flag <- function(name, default = FALSE) {
  ...
  tolower(trimws(raw)) %in% c("1", "true", "t", "yes", "y", "on")
}

env_int <- function(name, default) {
  raw <- trimws(Sys.getenv(name, unset = as.character(default)))
  val <- suppressWarnings(as.integer(raw))
  if (is.na(val)) as.integer(default) else val
}

env_num_or_auto <- function(name, default = "auto") {
  raw <- trimws(Sys.getenv(name, unset = as.character(default)))
  if (!nzchar(raw) || identical(tolower(raw), "auto")) return("auto")
  val <- suppressWarnings(as.numeric(raw))
  if (is.na(val) || !is.finite(val) || val <= 0 || val > 1) default else val
}

PIPELINE_TEST_MODE <- identical(tolower(Sys.getenv("PIPELINE_TEST_MODE", "false")), "true")
PIPELINE_RESUME_MODE <- env_flag("PIPELINE_RESUME_MODE", FALSE)
PIPELINE_SKIP_PAIRWISE_DE <- env_flag("PIPELINE_SKIP_PAIRWISE_DE", PIPELINE_RESUME_MODE)
PIPELINE_SKIP_GROUPED_SSGSEA <- env_flag("PIPELINE_SKIP_GROUPED_SSGSEA", PIPELINE_RESUME_MODE)
PIPELINE_SKIP_WILCOX <- env_flag("PIPELINE_SKIP_WILCOX", PIPELINE_RESUME_MODE)
PIPELINE_SKIP_SUMMARY_WRITE <- env_flag("PIPELINE_SKIP_SUMMARY_WRITE", PIPELINE_RESUME_MODE)
PIPELINE_SKIP_REPORT_WRITE <- env_flag("PIPELINE_SKIP_REPORT_WRITE", PIPELINE_RESUME_MODE)
PIPELINE_SKIP_FINAL_EXPORT <- env_flag("PIPELINE_SKIP_FINAL_EXPORT", FALSE)
...
N_CORES            <- env_int("PIPELINE_N_CORES", 4L)  # BiocParallel SnowParam workers
...
CHOIR_N_CORES <- env_int("CHOIR_N_CORES", if (PIPELINE_RESUME_MODE) 1L else N_CORES)
CHOIR_SAMPLE_MAX <- env_int("CHOIR_SAMPLE_MAX", 5000L)
CHOIR_DOWNSAMPLING_RATE <- env_num_or_auto("CHOIR_DOWNSAMPLING_RATE", "auto")
CHOIR_SUBTREE_REDUCTIONS <- env_flag("CHOIR_SUBTREE_REDUCTIONS", !PIPELINE_RESUME_MODE)
...
build_tissue_pair_context <- function(t1, t2, celltype_label, dir_name,
  ...
  validated_map_block <- if (nzchar(trimws(gene_pathway_map_text))) {
    paste(
...
cat(sprintf("[INFO] Resume mode: %s\n", ifelse(PIPELINE_RESUME_MODE, "ON", "OFF")))
if (PIPELINE_RESUME_MODE) {
  cat(sprintf("[INFO] Skip pairwise DE: %s | Skip grouped ssGSEA: %s | Skip Wilcoxon: %s\n",
              ifelse(PIPELINE_SKIP_PAIRWISE_DE, "TRUE", "FALSE"),
              ifelse(PIPELINE_SKIP_GROUPED_SSGSEA, "TRUE", "FALSE"),
              ifelse(PIPELINE_SKIP_WILCOX, "TRUE", "FALSE")))
  cat(sprintf("[INFO] Skip summary write: %s | Skip report write: %s | Skip final export: %s\n\n",
              ifelse(PIPELINE_SKIP_SUMMARY_WRITE, "TRUE", "FALSE"),
              ifelse(PIPELINE_SKIP_REPORT_WRITE, "TRUE", "FALSE"),
              ifelse(PIPELINE_SKIP_FINAL_EXPORT, "TRUE", "FALSE")))
}
...
build_pseudobulk_cache <- function(obj, group_col) {
  ...
  meta_keep <- meta_sub[, group_cols, drop = FALSE]
  meta_keep$cell <- rownames(meta_sub)
  meta_keep$group_value <- as.character(meta_keep[[group_col]])
  meta_keep$pb_group <- apply(meta_keep[, group_cols, drop = FALSE], 1,
                              function(v) paste(normalize_meta_values(v), collapse = "__"))

  extra_cols <- setdiff(group_cols, c(SAMPLE_COL, TISSUE_COL, group_col))
  grouping_cols <- c("pb_group", SAMPLE_COL, TISSUE_COL, "group_value", extra_cols)
  pb_meta <- meta_keep %>%
  ...
  keep_idx <- meta_keep$pb_group %in% pb_meta$pb_group
  meta_keep <- meta_keep[keep_idx, , drop = FALSE]
...
aggregate_pseudobulk <- function(pseudobulk_cache, group_value, min_total_cells = 50L) {
  ...
  valid <- colnames(pb_counts)[Matrix::colSums(pb_counts) > 0 &
                                  !is.na(pb_meta_sub[colnames(pb_counts), "tissue"])]
...
run_deseq2_pairwise <- function(pb, t1, t2) {
  ...
  counts_int   <- round(as.matrix(counts_sub)); storage.mode(counts_int) <- "integer"
...
split_gene_ids <- function(x) {
  ...
  unique(trimws(toupper(unlist(strsplit(x, "/", fixed = TRUE)))))
}

build_gene_pathway_map <- function(enrich_obj, gene_fc = NULL, n_terms = 8L,
                                   n_top_genes = 12L, max_pathways_per_gene = 3L) {
  enrich_df <- tryCatch(as.data.frame(enrich_obj), error = function(e) NULL)
  ...
  map_rows <- lapply(seq_len(nrow(top_terms)), function(i) {
    ...
    data.frame(
      gene = genes_i,
      log2FC = unname(gene_fc[genes_i]),
      pathway = safe_trim(top_terms$Description[i]),
      pathway_padj = top_terms$p.adjust[i],
      stringsAsFactors = FALSE
    )
  })
...
build_multi_database_gene_pathway_map <- function(enrich_list, gene_fc = NULL,
                                                  n_terms_per_db = 5L,
                                                  n_top_genes = 12L,
                                                  max_pathways_per_gene = 4L) {
  if (is.null(enrich_list) || length(enrich_list) == 0 || is.null(gene_fc) || length(gene_fc) == 0)
    return(data.frame())
  map_rows <- lapply(names(enrich_list), function(db) {
    er <- enrich_list[[db]]
    map_df <- build_gene_pathway_map(
      er,
      gene_fc = gene_fc,
    ...
    if (is.null(map_df) || nrow(map_df) == 0) return(NULL)
    mutate(map_df, source_db = db)
  })
  out <- bind_rows(map_rows)
  if (nrow(out) == 0) return(data.frame())
  out %>%
    mutate(abs_log2FC = abs(log2FC)) %>%
    arrange(desc(abs_log2FC), pathway_padj, source_db, pathway) %>%
    group_by(gene, pathway) %>% slice_head(n = 1) %>% ungroup() %>%
    group_by(gene) %>% slice_head(n = max_pathways_per_gene) %>% ungroup() %>%
    select(gene, log2FC, pathway, pathway_padj, source_db)
}
...
build_multi_database_evidence_text <- function(enrich_list, gene_fc = NULL, n_terms_per_db = 5L,
  ...
  gene_pathway_lines <- format_gene_pathway_map_text(gene_pathway_map)
  paste(c("Integrated multi-database evidence:", unlist(sections),
          if (length(gene_pathway_lines) > 0) "Validated gene-pathway hits:" else NULL,
          gene_pathway_lines, gene_lines), collapse = "\n")
}
...
build_multi_database_interpretation_record <- function(multi_payload, enrich_list,
  ...
  gene_pathway_map  <- build_multi_database_gene_pathway_map(enrich_list[selected_dbs], gene_fc = gene_fc)
  gene_pathway_map_text <- paste(format_gene_pathway_map_text(gene_pathway_map), collapse = "\n")
  evidence_text     <- build_multi_database_evidence_text(
    enrich_list[selected_dbs],
    gene_fc = gene_fc,
...
standardize_result_with_llm <- function(raw_text, context_str, evidence_text,
  ...
  prompt <- paste(
    ...
    "7. Gene->pathway links must be conservative: only use the validated gene-pathway map provided below.",
    "   If a highlighted gene lacks a validated mapping, describe the gene and enriched programs separately.",
    "8. In key_mechanisms and key_drivers, prioritize genes with the largest absolute log2FoldChange values.",
    "   When a validated mapping exists, use the format: gene(log2FC=X.X) -> pathway: explanation.",
    "",
    ...
    for (pair in combn(as.character(pb_tissues), 2, simplify = FALSE)) {
      ...
      for (dir_name in c("up", "down")) {
        ...
        if (ENABLE_LLM) {
          ...
          if (length(selected_dbs) > 0) {
            gene_pathway_map <- build_multi_database_gene_pathway_map(
              enr_list[selected_dbs],
              gene_fc = gene_fc
            )
            gene_pathway_map_text <- paste(format_gene_pathway_map_text(gene_pathway_map), collapse = "\n")
            if (nrow(gene_pathway_map) > 0) {
              fwrite(
                gene_pathway_map,
                file.path(comp_dir, paste0("validated_gene_pathway_map_", dir_name, ".csv"))
              )
            }
            tissue_ctx <- build_tissue_pair_context(
              t1, t2, group_name, dir_name,
            ...
            if (!is.null(ia_structured$gene_pathway_map) && nrow(ia_structured$gene_pathway_map) > 0) {
              saveRDS(ia_structured$gene_pathway_map,
                      file.path(comp_dir, paste0("validated_gene_pathway_map_", dir_name, ".rds")))
            }
...
build_ssgsea_evidence_text <- function(top_df_group, n_top = 20L) {
  ...
  top_df_group <- top_df_group[order(top_df_group$z_score, decreasing = TRUE), , drop = FALSE]
  top_rows <- utils::head(top_df_group, n_top)
  term_lines <- vapply(seq_len(nrow(top_rows)), function(i) {
    sprintf("- %s | z_score=%.3f | score=%.3f | rank=%d",
            safe_trim(top_rows$pathway[i]),
            top_rows$z_score[i], top_rows$score[i], top_rows$rank[i])
  }, character(1))
  paste(c("Top ssGSEA pathways (by Z-score):", term_lines), collapse = "\n")
}

build_ssgsea_group_context <- function(group_id, method, level_name) {
  ...
  if (is.null(tissue_ctx)) tissue_ctx <- paste(tissue_label, ": no specific epithelial context available.")
  sprintf(
    paste(
      "%s",
      ...
      "tissue x epithelial group reveal about site-adapted epithelial biology?",
      "\n\nOutput requirements:",
      ...
      "3. Focus on the most biologically significant epithelial programs.",
      "4. Do not invent gene->pathway links not supported by the evidence."
    ),
    EPITHELIAL_BASE_CONTEXT,
    level_name, group_id,
    tissue_label, ct_label, method,
    tissue_ctx
  )
}

run_ssgsea_group_llm <- function(group_id, top_df_group, method,
  ...
  ctx_str <- build_ssgsea_group_context(group_id, method, level_name)
  std_payload <- standardize_result_with_llm(
    raw_text = "",
    context_str = ctx_str,
    evidence_text = evidence_text,
    ...
    celltype_level = level_name,
    comp_name = group_id,
    dir_name = paste0("ssgsea_", method),
    source_db = method,
    warnings = character(),
    error_message = NULL
  )
  rec <- if (!is.null(std_payload$result)) {
    std <- std_payload$result
    list(
      celltype_level = level_name, celltype_label = ct_label, celltype_l2 = ct_label,
      comparison = group_id, direction = paste0("ssgsea_", method), source_db = method,
  ...
  if (!is.null(out_dir)) {
    ...
    meth_dir <- file.path(out_dir, sprintf("grp_%s_%s", method, group_safe))
    dir.create(meth_dir, recursive = TRUE, showWarnings = FALSE)
    saveRDS(rec, file.path(meth_dir, "interpretation.rds"))
    writeLines(
      c(sprintf("# ssGSEA LLM: %s | %s | %s", level_name, group_id, method), "",
...
collect_ssgsea_llm_records <- function(ssgsea_results) {
  rows <- list()
  for (method in names(ssgsea_results)) {
    llm_list <- ssgsea_results[[method]][["llm"]]
...
build_choir_cluster_context <- function(cluster_name, dir_name, gene_pathway_map_text = "") {
  validated_map_block <- if (nzchar(safe_trim(gene_pathway_map_text))) {
    paste("\nValidated gene-pathway hits:\n", gene_pathway_map_text, sep = "")
  } else {
    "\nValidated gene-pathway hits: none available from current evidence. Do not invent gene->pathway links."
  }
  sprintf(
    paste(
      "%s",
      "\nCHOIR cluster %s vs rest (one-vs-rest OFA).",
      "Direction: %s-regulated genes in cluster %s relative to all other epithelial cells.",
      "\nBiological question: What transcriptional programs define the identity and function of CHOIR cluster %s?",
      "\n\nOutput requirements:",
      "1. Write ALL interpretive content in Chinese (Simplified Chinese characters).",
      "2. key_drivers must remain as English gene symbols.",
      "3. Only use the validated gene->pathway map for explicit gene->pathway claims.",
      "%s"
    ),
    EPITHELIAL_BASE_CONTEXT,
    cluster_name, dir_name, cluster_name,
    cluster_name,
    validated_map_block
  )
}
...
run_grouped_ssgsea <- function(obj, group_col, level_name, methods,
  ...
  for (method in methods) {
    ...
    ssgsea_llm_method <- list()
    if (ENABLE_LLM) {
      cat(sprintf("[LLM] Interpreting ssGSEA groups (method=%s, level=%s)...\n", method, level_name))
      ...
      all_group_ids <- unique(top_df$group)
      for (gid in all_group_ids) {
        gid_df <- top_df[top_df$group == gid, , drop = FALSE]
        rec_gid <- tryCatch(
          run_ssgsea_group_llm(gid, gid_df, method, level_name, ssgsea_llm_out),
          error = function(e) {
            cat(sprintf("    [WARN] ssGSEA LLM failed for %s: %s\n", gid, e$message))
            NULL
          }
        )
        if (!is.null(rec_gid)) {
          ssgsea_llm_method[[gid]] <- rec_gid
          cat(sprintf("    [OK] %s | %s | status=%s\n", gid, method, rec_gid$status))
        }
        Sys.sleep(STANDARDIZE_LLM_RETRY_SLEEP_SEC)
      }
      cat(sprintf("[OK] LLM interpretations: %d / %d groups (method=%s)\n",
                  length(ssgsea_llm_method), length(all_group_ids), method))
    }
    results_all[[method]][["llm"]] <- ssgsea_llm_method
  ...
  for (mn in names(results_all)) {
    cat(sprintf("  - %s: %d pathways x %d groups (%d LLM records)\n",
                mn, nrow(results_all[[mn]]$scores), ncol(results_all[[mn]]$scores),
                length(results_all[[mn]]$llm)))
  }
  cat("\n")
...
pb_de_all <- list(); enrich_all <- list(); agent_all <- list(); agent_structured_all <- list()
pb_de_l3_all <- list(); enrich_l3_all <- list(); agent_l3_all <- list(); agent_structured_l3_all <- list()

if (PIPELINE_SKIP_PAIRWISE_DE) {
  cat("[INFO] Skipping pairwise pseudobulk DE / enrichment / interpret_agent in resume mode\n\n")
} else {
...
if (RUN_SSGSEA && !PIPELINE_SKIP_GROUPED_SSGSEA) {
...
} else if (RUN_SSGSEA && PIPELINE_SKIP_GROUPED_SSGSEA) {
  cat("[INFO] Skipping grouped ssGSEA in resume mode\n\n")
}
...
if (RUN_CHOIR) {
  ...
  if (is.null(choir_reduction)) {
    ...
    if (length(choir_var_features) == 0 && isTRUE(CHOIR_FIND_VAR_FEATURES_IF_MISSING)) {
      cat(sprintf("[INFO] CHOIR var_features missing; running FindVariableFeatures(method=%s, nfeatures=%d)\n",
                  CHOIR_VAR_FEATURES_METHOD, min(CHOIR_VAR_FEATURES_MAX, nrow(obj))))
      obj <- tryCatch(
        FindVariableFeatures(
          obj,
          selection.method = CHOIR_VAR_FEATURES_METHOD,
          nfeatures = min(CHOIR_VAR_FEATURES_MAX, nrow(obj)),
          verbose = FALSE
        ),
        error = function(e) {
    ...
    if (length(choir_var_features) == 0) {
      ...
      if (length(choir_var_features) > CHOIR_VAR_FEATURES_MAX) {
        choir_var_features <- choir_var_features[seq_len(CHOIR_VAR_FEATURES_MAX)]
        cat(sprintf("[INFO] CHOIR var_features capped at %d\n", length(choir_var_features)))
      } else {
  ...
  if (RUN_CHOIR) {
    cat(sprintf("[INFO] Running CHOIR (alpha=%.3f, n_cores=%d, sample_max=%d, downsampling_rate=%s, subtree_reductions=%s)...\n",
                CHOIR_ALPHA, CHOIR_N_CORES, CHOIR_SAMPLE_MAX,
                if (is.character(CHOIR_DOWNSAMPLING_RATE)) CHOIR_DOWNSAMPLING_RATE else sprintf("%.4f", CHOIR_DOWNSAMPLING_RATE),
                ifelse(CHOIR_SUBTREE_REDUCTIONS, "TRUE", "FALSE")))
    obj_choir <- tryCatch(
      CHOIR::CHOIR(obj, use_assay = "RNA", reduction = choir_embedding,
                   var_features = choir_var_features, n_cores = CHOIR_N_CORES,
                   alpha = CHOIR_ALPHA, sample_max = CHOIR_SAMPLE_MAX,
                   downsampling_rate = CHOIR_DOWNSAMPLING_RATE,
                   subtree_reductions = CHOIR_SUBTREE_REDUCTIONS,
                   random_seed = 42),
    ...
    if (is.null(choir_col)) {
      ...
      if (RUN_OFA) {
        ...
        for (cl in choir_clusters) {
          ...
          choir_ofa_llm <- list()
          if (ENABLE_LLM) {
            gene_fc_ofa_all <- setNames(de_ofa$avg_log2FC, toupper(de_ofa$gene))
            for (dir_name_ofa in c("up", "down")) {
              if (is.null(ofa_enrich[[dir_name_ofa]]) || length(ofa_enrich[[dir_name_ofa]]) == 0) next
              gene_fc_dir <- if (dir_name_ofa == "up") {
                gene_fc_ofa_all[gene_fc_ofa_all > 0]
              } else {
                gene_fc_ofa_all[gene_fc_ofa_all <= 0]
              }
              selected_dbs_ofa <- select_interpret_agent_databases(ofa_enrich[[dir_name_ofa]])
              if (length(selected_dbs_ofa) == 0) next

              gene_pathway_map_ofa <- build_multi_database_gene_pathway_map(
                ofa_enrich[[dir_name_ofa]][selected_dbs_ofa],
                gene_fc = gene_fc_dir
              )
              gene_pathway_map_text_ofa <- paste(
                format_gene_pathway_map_text(gene_pathway_map_ofa),
                collapse = "\n"
              )
              if (nrow(gene_pathway_map_ofa) > 0) {
                fwrite(
                  gene_pathway_map_ofa,
                  file.path(ofa_dir, sprintf("validated_gene_pathway_map_%s.csv", dir_name_ofa))
                )
              }

              choir_ctx <- build_choir_cluster_context(
                cl_name,
                dir_name_ofa,
                gene_pathway_map_text = gene_pathway_map_text_ofa
              )
              cat(sprintf("    [LLM] CHOIR %s %s integrating %d db(s): %s\n",
                          cl_name, dir_name_ofa, length(selected_dbs_ofa),
                          paste(selected_dbs_ofa, collapse = ", ")))
              ia_ofa_payload <- tryCatch(
                run_interpret_agent_multi(ofa_enrich[[dir_name_ofa]][selected_dbs_ofa], choir_ctx, dir_name_ofa, gene_fc_dir),
                error = function(e) {
                  cat(sprintf("    [WARN] interpret_agent CHOIR failed: %s\n", e$message))
                  list(selected_dbs = character(), per_db_payloads = list(), combined_raw_text = "",
                       combined_warnings = character(), combined_error = e$message)
                }
              )
              ia_ofa_struct <- build_multi_database_interpretation_record(
                multi_payload = ia_ofa_payload,
                enrich_list = ofa_enrich[[dir_name_ofa]],
                celltype_label = paste0("CHOIR_", cl_name),
                celltype_level = "CHOIR",
                comp_name = paste0("cluster_", cl_name, "_vs_rest"),
                dir_name = dir_name_ofa,
                context_str = choir_ctx,
                gene_fc = gene_fc_dir
              )
              choir_ofa_llm[[dir_name_ofa]] <- ia_ofa_struct
              saveRDS(ia_ofa_payload, file.path(ofa_dir, sprintf("interpret_agent_%s_payload.rds", dir_name_ofa)))
              saveRDS(ia_ofa_struct,  file.path(ofa_dir, sprintf("interpret_agent_%s_structured.rds", dir_name_ofa)))
              if (nzchar(ia_ofa_struct$raw_text)) {
                writeLines(ia_ofa_struct$raw_text,
                           file.path(ofa_dir, sprintf("interpret_agent_%s_raw.txt", dir_name_ofa)))
              }
              cat(sprintf("    [OK] CHOIR cluster %s %s LLM status=%s\n",
                          cl_name, dir_name_ofa, ia_ofa_struct$status))
              Sys.sleep(STANDARDIZE_LLM_RETRY_SLEEP_SEC)
            }
          }
...
wilcox_all <- list()
if (PIPELINE_SKIP_WILCOX) {
  cat("[INFO] Skipping exploratory Wilcoxon in resume mode\n")
} else {
...
cat("\n=== Saving Summary ===\n")
if (PIPELINE_SKIP_SUMMARY_WRITE) {
  cat("[INFO] Skipping summary/aggregate writes in resume mode\n")
} else {
...
ssgsea_llm_df <- bind_rows(ssgsea_llm_df_l2, ssgsea_llm_df_l3)
if (!PIPELINE_SKIP_SUMMARY_WRITE) {
  ...
  if (nrow(ssgsea_llm_df) > 0) {
    ...
    write_interpretation_markdown(
      ...
      "# LLM ssGSEA Interpretation (Tissue x Epithelial Groups)",
      include_raw = FALSE
    )
  }
}
...
if (!PIPELINE_SKIP_SUMMARY_WRITE) {
  ...
  if (nrow(choir_llm_df) > 0) {
    ...
    write_interpretation_markdown(
      ...
      "# LLM CHOIR Cluster Interpretation (One-vs-Rest)",
      include_raw = FALSE
    )
  }
}
...
if (PIPELINE_SKIP_REPORT_WRITE) {
  cat("\n=== Generating REPORT.md ===\n")
  cat("[INFO] Skipping REPORT.md generation in resume mode\n")
} else {
cat("\n=== Generating REPORT.md ===\n")
...
if (!RUN_SSGSEA || (length(ssgsea_results_all) == 0 && length(ssgsea_results_l3_all) == 0)) {
  ...
  add("| Method | Pathways | Groups | LLM records | Output |"); add("|---|---|---|---|---|")
  for (mn in names(ssgsea_results_all))
    add(sprintf("| %s | %d | %d | %d | `reports/ssgsea_scores_%s.rds` |",
                mn, nrow(ssgsea_results_all[[mn]]$scores), ncol(ssgsea_results_all[[mn]]$scores),
                length(ssgsea_results_all[[mn]]$llm), mn))
  add("")
  ...
  for (mn in names(ssgsea_results_l3_all))
    add(sprintf("| %s | %d | %d | %d | `reports/ssgsea_l3_scores_%s.rds` |",
                mn, nrow(ssgsea_results_l3_all[[mn]]$scores), ncol(ssgsea_results_l3_all[[mn]]$scores),
                length(ssgsea_results_l3_all[[mn]]$llm), mn))
  add("")
  ...
  if (exists("ssgsea_llm_df") && nrow(ssgsea_llm_df) > 0) {
    add("### 4.5.3 ssGSEA LLM Summary"); add("")
    add(sprintf("- **Structured rows:** %d", nrow(ssgsea_llm_df)))
    add("- `reports/ssgsea_llm_structured.tsv`")
    add("- `LLM_SSGSEA_INTERPRETATION.md`")
    add("")
  }
}
...
if (!RUN_CHOIR || length(choir_ofa_all) == 0) {
  ...
  add(sprintf("- **CHOIR variable features cap:** %d | **Auto-find if missing:** %s | **Method:** `%s`",
              CHOIR_VAR_FEATURES_MAX,
              ifelse(isTRUE(CHOIR_FIND_VAR_FEATURES_IF_MISSING), "TRUE", "FALSE"),
              CHOIR_VAR_FEATURES_METHOD))
  add("")
  ...
  add("| Cluster | n_focal | n_rest | Sig markers | LLM up | LLM down |"); add("|---|---|---|---|---|---|")
  for (cl_name in names(choir_ofa_all)) {
    ...
    llm_up <- if (!is.null(r$llm$up)) r$llm$up$status else "skipped"
    llm_dn <- if (!is.null(r$llm$down)) r$llm$down$status else "skipped"
    add(sprintf("| %s | %d | %d | %d | %s | %s |", cl_name, r$n_focal, r$n_rest, n_sig, llm_up, llm_dn))
  }
  ...
  if (exists("choir_llm_df") && nrow(choir_llm_df) > 0) {
    add("### 4.6.4 CHOIR LLM Summary"); add("")
    add(sprintf("- **Structured rows:** %d", nrow(choir_llm_df)))
    add("- `reports/choir_llm_structured.tsv`")
    add("- `LLM_CHOIR_INTERPRETATION.md`")
    add("")
  }
}
...
if (!ENABLE_LLM) {
  ...
  add("- LLM emphasis: top-|log2FC| genes linked to site-relevant epithelial pathways using validated gene-pathway hits when available.")
  add("- LLM evidence integration: all eligible databases merged into one consensus summary for tissue-pair DE.")
  add("")
  add("### 5.1 Tissue-pair DE LLM"); add("")
  if (nrow(agent_structured_df) == 0) {
    add("No interpret_agent results generated.")
  } else {
  ...
  add("### 5.2 ssGSEA Group LLM"); add("")
  if (!exists("ssgsea_llm_df") || nrow(ssgsea_llm_df) == 0) {
    add("No ssGSEA LLM results generated.")
  } else {
    add(sprintf("- **Structured rows:** %d", nrow(ssgsea_llm_df)))
    add("- `reports/ssgsea_llm_structured.tsv`")
    add("- `LLM_SSGSEA_INTERPRETATION.md`")
  ...
  add("### 5.3 CHOIR Cluster LLM"); add("")
  if (!exists("choir_llm_df") || nrow(choir_llm_df) == 0) {
    add(if (!RUN_CHOIR) "CHOIR was not run; cluster LLM skipped." else "No CHOIR cluster LLM results generated.")
  } else {
    add(sprintf("- **Structured rows:** %d", nrow(choir_llm_df)))
    add("- `reports/choir_llm_structured.tsv`")
    add("- `LLM_CHOIR_INTERPRETATION.md`")
...
add("- **ssGSEA LLM:** secondary DeepSeek standardization per tissue x epithelial group and per method")
add("- **CHOIR cluster LLM:** multi-database interpret_agent integration for each cluster up/down OFA direction")
add(sprintf("- **CHOIR tuning:** alpha=%.2f, reduction priority=%s, variable-feature cap=%d",
            CHOIR_ALPHA,
...
if (ENABLE_LLM) {
  ...
  add("- **Validated mapping:** direct gene->pathway claims restricted to enrichment-supported geneID hits")
} else {
...
add("- `LLM_INTERPRETATION.md` -- tissue-pair DE LLM summary")
add("- `LLM_SSGSEA_INTERPRETATION.md` -- ssGSEA group LLM summary")
add("- `LLM_CHOIR_INTERPRETATION.md` -- CHOIR cluster LLM summary")
...
if (PIPELINE_SKIP_FINAL_EXPORT) {
  cat("\n=== Saving Final Object (RDS + h5ad) ===\n")
  cat("[INFO] Skipping final export by configuration\n")
} else {
cat("\n=== Saving Final Object (RDS + h5ad) ===\n")
...
cat("  LLM_SSGSEA_INTERPRETATION.md             <- ssGSEA group LLM summary (Chinese)\n")
cat("  LLM_CHOIR_INTERPRETATION.md              <- CHOIR cluster LLM summary (Chinese)\n")
...
cat("  reports/ssgsea_llm_structured.tsv        <- structured ssGSEA LLM table\n")
...
cat("  reports/choir_llm_structured.tsv         <- structured CHOIR cluster LLM table\n")
...
cat(sprintf("%s Changes (vs v1.3.0):\n", PIPELINE_VERSION))
cat("  [CHOIR-1] Raise CHOIR alpha from 0.05 to 0.20 to reduce over-fragmented clusters\n")
cat("  [CHOIR-2] Prefer scanvi_major over scanvi_fine for CHOIR clustering\n")
cat("  [CHOIR-3] Align CHOIR variable-feature handling with B cell v2.6 (auto-find + cap)\n")
cat("  [LLM-4] Disable interpret_agent PPI by default for faster, more stable full runs\n")
cat("\n")
cat("v1.3.0 Changes (vs v1.2.0):\n")
cat("  [PB-1] One-shot pseudobulk cache by sample x tissue x group; avoid repeated subset()\n")
cat("  [LLM-1] Validated gene->pathway map derived from enrichment geneID hits and passed to LLM evidence\n")
cat("  [LLM-2] ssGSEA per-group LLM interpretation with structured TSV + markdown outputs\n")
cat("  [LLM-3] CHOIR per-cluster OFA LLM interpretation with structured TSV + markdown outputs\n")
cat("  [WX-1] Exploratory wilcox outputs include evidence-tier metadata and exploratory_* filenames\n")
cat("  [P2-1] stratified_downsample() ignores NA/blank groups explicitly\n")
cat("  [P2-2] h5ad sparse export uses dgCMatrix slots directly (no COO data.frame materialization)\n")
cat("  [P2-3] NormalizeData provenance recorded under obj@misc$normalization_for_report\n")
cat("\n")
cat("v1.2.0 Changes (vs v1.1.1):\n")
