...
OUTPUT_STEM <- "llm_outlier_review_20260506"
...
process_target <- function(target) {
  ...
  for (spec in screen_specs) {
    ...
    draw_outlier_dotplot(
      ...
      genes = unique(head(top_gene_union, max(8L, length(top_gene_union)))),
      plot_col = PLOT_CLUSTER_COL,
...
cat("=== Tissue Comparison LLM Outlier Visual Batch (2026-05-06) ===\n")
batch_rows <- list()
...
for (target in TARGETS) {
  ...
  cat(sprintf("[result] %s | %s | outliers=%s\n", res$lineage_tag, res$status, res$n_outlier_clusters))
  gc(verbose = FALSE)
}
