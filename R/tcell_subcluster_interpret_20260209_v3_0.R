#!/usr/bin/env Rscript
# ==============================================================================
# T Cell Subcluster Interpretation Pipeline - v3.1-TCELL
# ==============================================================================
#
# Version: v3.1-TCELL (2026-02-09)
# Status: Production-ready T cell specialized version  
# Base: v3.0-HOTFIX + Multi-ssGSEA + Load existing enrichment
#
# Key Features:
#   ✅ T cell expert context (CD4/CD8/TRM/exhaustion)
...
OUTPUT_DIR <- "/home/h2048/data/R/0209/tcell_interpret_v3_0"
...
# Optional: Custom T cell marker database
CUSTOM_MARKERS_CSV <- NULL  # e.g., "/path/to/tcell_markers.csv"
...
# CELLTYPE_TO_ANALYZE <- "T cells"  # Or "T/NK cells"
...
generate_tcell_context <- function() {
  paste(
    "CRITICAL CONTEXT FOR T CELL ANNOTATION:",
    "",
    "These are T cell subclusters from respiratory tissues (CRSwNP study).",
    "",
    "=== LINEAGE GATE VERIFICATION (Important) ===",
    "Before finalizing annotation, verify lineage markers:",
    "• T cells: TRAC/CD3D/CD3E/TRBC (if absent → potential contamination)",
    "• NK cells: NKG7/GNLY + (FCGR3A/NCAM1/KLRD1) without CD3",
    "• Myeloid: LYZ/S100A8/S100A9 (if dominant → myeloid contamination)",
    "",
    ...
    "   - Type 2 inflammation: Enhanced Th2 in CRSwNP",
    "   - Barrier immunity: Th17 at epithelial surfaces",
    "",
    "ANNOTATION INSTRUCTIONS:",
    "For EACH cluster, specify:",
    "  a) Lineage: CD4+ / CD8+ / NK (if present)",
    "  b) Memory state: Naïve / TCM / TEM / TEMRA / TRM",
    "  c) Functional state: Resting / Activated / Exhausted / Proliferating",
    "  d) Effector phenotype: Cytotoxic / Th1 / Th2 / Th17 / Tfh / Treg",
    "",
...
tcell_context <- generate_tcell_context()
...
interpret_tcell <- function(cluster_id, marker_genes, enrichment_data = NULL,
  ...
  cat(sprintf("\n--- T Cell Interpretation: %s (Task: %s) ---\n",
              cluster_id, task))
...
Cell Type: [specific T cell subtype]
...
CRITICAL: Include ALL FIVE fields with exact format.",
      tcell_context, markers_text, enrichment_text, ssgsea_text
    )
...
tryCatch({
  ...
  if (RUN_SSGSEA && !is.null(ssgsea_scores)) {
    ssgsea_method_label <- if (exists("primary_method") && !is.null(primary_method)) {
      primary_method
    } else {
      paste(SSGSEA_METHOD, collapse = ", ")
    }
...
cat("\n")
cat("================================================================================\n")
cat("T CELL ANALYSIS COMPLETE - v3.1-TCELL\n")
cat("================================================================================\n\n")
...
cat("Key Features (v3.1-TCELL):\n")
cat("  ✅ T cell expert context (CD4/CD8/TRM/exhaustion/memory)\n")
