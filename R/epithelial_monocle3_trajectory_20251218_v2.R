...
USE_PARTITION <- TRUE
RUN_GRAPH_TEST <- TRUE
REDUCTION_METHOD <- "PCA"  # [P1-1] Explicit specification

# Marker analysis
TOP_N_MARKERS <- 10
MIN_PSEUDOBULK_CELLS <- 50
...
plot_pseudotime_markers <- function(cds, markers, out_file) {
  ...
  # 额外输出一个 summary 表，量化早/晚 pseudotime 的 marker 表达
  summary_list <- list()
  ...
  for (lineage_name in names(markers)) {
    ...
    for (gene in genes) {
      ...
      # 早期 (0-25%) 与晚期 (75-100%) pseudotime 平均表达
      q <- quantile(pt_valid, probs = c(0.25, 0.75), na.rm = TRUE)
      early_idx <- pt_valid <= q[1]
      late_idx  <- pt_valid >= q[2]
      summary_list[[length(summary_list) + 1]] <- data.frame(
        lineage = lineage_name,
        gene = gene,
        early_mean = mean(expr[early_idx], na.rm = TRUE),
        late_mean = mean(expr[late_idx], na.rm = TRUE),
        stringsAsFactors = FALSE
      )
    }
  }
  ...
  if (length(summary_list) > 0) {
    ...
    csv_file <- sub("\\.pdf$", "_summary.csv", out_file)
    write.csv(summary_df, csv_file, row.names = FALSE)
    cat("  Saved marker summary: ", csv_file, "\n", sep = "")
  }
  cat("  Saved: ", out_file, "\n", sep = "")
}
...
run_monocle3_for_tissue <- function(seurat_obj, tissue_value, out_dir) {
  ...
  # Basal 占比统计，便于后续生物学解释
  ct_tab <- table(seu@meta.data[[LABELS_KEY]])
  basal_count <- sum(ct_tab[names(ct_tab) == root_label])
  basal_frac <- basal_count / sum(ct_tab)
  cat(
    "  tissue=", tissue_value,
    " | cells=", ncol(seu),
    " | root=", root_label,
    " | root_fraction=", sprintf("%.3f", basal_frac),
    "\n",
    sep = ""
  )
...
# 过滤掉运行失败 (NULL) 的条目，便于后续在 R 会话中统一处理
cds_list <- Filter(Negate(is.null), cds_list)
...
# 过滤掉细胞数太少的 cell types，避免不稳定的 pseudobulk
