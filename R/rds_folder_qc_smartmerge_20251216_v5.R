...
# =============================================================================
# ⭐ FIX v4: Promote RNA_restored to RNA (safe: never delete DefaultAssay)
# =============================================================================
promote_rna_restored <- function(
  obj,
  restored_assay_name = "RNA_restored",
  keep_old_rna = FALSE,
  old_rna_name = "RNA_original"
) {
  assays <- get_assay_names_safe(obj)
  ...
  log_msg(sprintf("  [Assay] Found '%s' assay - promoting to 'RNA' (safe)", restored_assay_name))

  # Helper: safe RenameAssays with fallback
  safe_rename_assay <- function(o, from, to) {
    rename_fun <- NULL
    rename_fun <- tryCatch(get0("RenameAssays", mode = "function", inherits = TRUE), error = function(e) NULL)

    if (is.function(rename_fun)) {
      mapping <- list()
      mapping[[from]] <- to
      o <- do.call(rename_fun, c(list(object = o), mapping))
      return(o)
    }

    # Fallback: copy assignment (less ideal for memory, but robust)
    o[[to]] <- o[[from]]
    return(o)
  }

  # 1) Ensure DefaultAssay is NOT "RNA" before touching/removing RNA
  if ("RNA" %in% assays && DefaultAssay(obj) == "RNA") {
    DefaultAssay(obj) <- restored_assay_name
  }

  # 2) Handle existing RNA
  assays <- get_assay_names_safe(obj)  # refresh
  if ("RNA" %in% assays) {
    if (keep_old_rna) {
      # Rename RNA -> RNA_original (or custom)
      if (old_rna_name %in% assays) {
        # avoid collision
        old_rna_name <- paste0(old_rna_name, "_", format(Sys.time(), "%Y%m%d_%H%M%S"))
      }
      obj <- safe_rename_assay(obj, from = "RNA", to = old_rna_name)
      log_msg(sprintf("  [Assay] Kept old RNA as '%s'", old_rna_name))
    } else {
      # Safe remove (DefaultAssay already moved away)
      obj[["RNA"]] <- NULL
      log_msg("  [Assay] Removed old 'RNA' assay")
    }
  }

  # 3) Rename restored -> RNA (preferred: in-place rename)
  assays <- get_assay_names_safe(obj)  # refresh
  if (restored_assay_name %in% assays) {
    obj <- safe_rename_assay(obj, from = restored_assay_name, to = "RNA")
  } else {
    # If fallback copy happened, ensure "RNA" exists
    obj[["RNA"]] <- obj[[restored_assay_name]]
  }

  # 4) Set DefaultAssay to RNA and remove restored if it still exists
  DefaultAssay(obj) <- "RNA"
  assays <- get_assay_names_safe(obj)

  if (restored_assay_name %in% assays) {
    # now safe to delete because DefaultAssay is RNA
    obj[[restored_assay_name]] <- NULL
    log_msg(sprintf("  [Assay] Removed '%s' after promotion", restored_assay_name))
  }

  log_msg("  [Assay] ✓ RNA_restored promoted to RNA and set as default (safe)")
  return(obj)
}
...
incremental_merge <- function(merge_inputs) {
  ...
  load_entry <- function(entry, idx, total) {
    ...
    log_entry <- list(
      ...
      runId = "pre-fix",
      hypothesisId = "A",
      location = "rds_folder_qc_smartmerge_20251216_v4_memory_optimized copy.R:1350",
      message = "load_entry entry",
      data = list(
        idx = idx,
        total = total,
        has_path = !is.null(entry$path),
        has_object = !is.null(entry$object),
        has_sample = !is.null(entry$sample),
        sample = entry$sample
      ),
    ...
    log_entry <- list(
      ...
      runId = "pre-fix",
      hypothesisId = "A",
      location = "rds_folder_qc_smartmerge_20251216_v4_memory_optimized copy.R:1390",
      message = "load_entry exit",
      data = list(
        sample = entry$sample,
        n_cells = ncol(obj),
        n_genes = nrow(obj)
  ...
  for (i in 2:n_inputs) {
    ...
    # ⭐ For scArches/scVI: do NOT merge normalized data, only counts
    merged_obj <- merge(x = merged_obj, y = next_obj, merge.data = FALSE)
...
record_merge_input <- function(sample_name, path = NULL, object = NULL) {
  ...
  log_entry <- list(
    ...
    runId = "pre-fix",
    hypothesisId = "B",
    location = "rds_folder_qc_smartmerge_20251216_v4_memory_optimized copy.R:1467",
  ...
  log_entry <- list(
    ...
    runId = "pre-fix",
    hypothesisId = "B",
    location = "rds_folder_qc_smartmerge_20251216_v4_memory_optimized copy.R:1509",
...
log_entry <- list(
  ...
  runId = "pre-fix",
  hypothesisId = "C",
  location = "rds_folder_qc_smartmerge_20251216_v4_memory_optimized copy.R:1620",
...
if (is.null(merged_obj)) {
  stop("FATAL: Merged object is NULL after incremental_merge()")
}
if (ncol(merged_obj) == 0) {
  stop("FATAL: Merged object has 0 cells after merge")
}
if (nrow(merged_obj) == 0) {
  stop("FATAL: Merged object has 0 genes after merge")
}
...
if (MIN_CELLS_PER_GENE > 0) {
  ...
  if (length(keep_genes) == 0) {
    log_msg("  WARNING: No genes pass the min.cells filter! Keeping all genes.")
    keep_genes <- rownames(merged_obj)
  }
