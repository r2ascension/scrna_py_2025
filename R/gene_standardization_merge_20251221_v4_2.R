...
OUTPUT_DIR  <- "/home/h2048/data/R/1215/merge/cleaned_samples_COMPLETE_v4_2"
...
apply_gene_mapping_fixed <- function(
  ...
  # ⭐ FIXED: ENSEMBL patch with validation
  ...
  if (any(needs_patch)) {
    ...
    # Get all valid ENSEMBL keys from database
    valid_ensembl_keys <- tryCatch(
      keys(org.Hs.eg.db, keytype = "ENSEMBL"),
      error = function(e) character(0)
    )
    
    # Filter to only valid keys
    keys_to_patch <- base_keys[needs_patch]
    valid_keys <- keys_to_patch %in% valid_ensembl_keys
    
    if (sum(valid_keys) > 0) {
      # Only patch keys that exist in database
      patched <- tryCatch(
        mapIds(org.Hs.eg.db, 
               keys = keys_to_patch[valid_keys],
               column = "SYMBOL", 
               keytype = "ENSEMBL", 
               multiVals = "first"),
        error = function(e) {
          if (verbose) cat(sprintf("      ⚠️  Patch failed: %s\n", e$message))
          rep(NA_character_, sum(valid_keys))
        }
      )
      ...
      idx_needs_patch <- which(needs_patch)
      idx_valid <- idx_needs_patch[valid_keys]
      official[idx_valid[ok]] <- patched[ok]
      
      if (verbose) {
        cat(sprintf("      Valid ENSG in db: %d/%d\n", sum(valid_keys), length(keys_to_patch)))
        cat(sprintf("      ✓ Patched: %d genes\n", sum(ok)))
      }
    } else {
      if (verbose) {
        cat(sprintf("      ⚠️  None of %d ENSG IDs found in org.Hs.eg.db\n", length(keys_to_patch)))
        cat("      These will remain as ENSG IDs (likely outdated/non-human)\n")
      }
    }
  }
