...
script_path <- "/home/h2048/script/R/launch_cnmf_l3_unit_llm_20260519.R"
source(script_path)

tmp_root <- tempfile("cnmf_l3_unit_llm_")
source_dir <- file.path(tmp_root, "bcell", "cnmf_by_celltype", "cnmf_Plasma_IgA", "cnmf_full")
...
writeLines(
  '{"status":"ok","lineage":"bcell","celltype_l3":"Plasma_IgA","safe_celltype":"Plasma_IgA"}',
...
utils::write.table(
  data.frame(
    ...
    rank = c(1L, 2L, 1L, 2L),
    gene = c("JCHAIN", "XBP1", "MS4A1", "CD79A"),
...
utils::write.csv(
  data.frame(
    ...
    cell_type_L3 = c("Plasma_IgA", "Plasma_IgA"),
...
unit <- idx[idx$unit_id == "cnmf_l3_Plasma_IgA_k3_GEP_1", , drop = FALSE]
stopifnot(nrow(unit) == 1L)
stopifnot(identical(as.character(unit$unit_label)[1], "Memory_B > Plasma_IgA | k3 / GEP_1"))

extra <- parse_task_extra_unit_llm(unit[1, , drop = FALSE])
stopifnot(identical(as.character(extra$celltype_l2)[1], "Memory_B"))
stopifnot(identical(as.character(extra$celltype_l3)[1], "Plasma_IgA"))
stopifnot(identical(as.character(extra$celltype_hierarchy)[1], "Memory_B > Plasma_IgA"))

prompt <- build_unit_prompt(unit[1, , drop = FALSE])
stopifnot(any(grepl("celltype_hierarchy: `L2=Memory_B | L3=Plasma_IgA`", prompt, fixed = TRUE)))
stopifnot(any(grepl("celltype_l2: `Memory_B`", prompt, fixed = TRUE)))
stopifnot(any(grepl("celltype_l3: `Plasma_IgA`", prompt, fixed = TRUE)))

cat("[OK] L3 cNMF unit index carries L2/L3 hierarchy into per-GEP prompt metadata.\n")