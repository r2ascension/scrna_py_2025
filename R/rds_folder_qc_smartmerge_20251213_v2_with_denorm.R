...
#
# VERSION: v2_DENORM (2024-12-14) ⭐ De-lognormalization Integration
# NEW FEATURES:
...
#
# PREVIOUS FIXES:
#   - DoubletFinder command dependency (NULL@params error)
#   - ensure_normalize_command function
#   - Copy NormalizeData command in rebuild
#   - Assays() container, dimnames, integer detection
#
...
# =============================================================================
# ⭐ NEW: De-lognormalization Functions
# =============================================================================
...
#'
#' @details
#' For Seurat's LogNormalize: log1p(counts / colSums(counts) * scale_factor)
#' To reverse: counts = expm1(normalized_data) * colSums(counts) / scale_factor
#'
#' Example:
#'   # Assuming normalized_data is log(counts + 1) normalized
#'   # size_factors is the per-cell normalization factor
...
restore_raw_counts_log <- function(normalized_data, size_factors) {
  ...
  # Reverse log transformation: log(counts + 1) -> counts + 1 = exp(log(counts + 1))
  # Then subtract 1 to get original counts
  # Use expm1(x) = exp(x) - 1 for numerical stability
  raw_counts <- expm1(normalized_data)

  # Multiply by size factors to restore original scale
  # Each column (cell) is multiplied by its corresponding size factor
...
#'
#' @details
#' Seurat's LogNormalize does: log1p(counts / sum(counts) * scale_factor)
#' This function reverses it: counts = expm1(data) * sum(counts) / scale_factor
#' We use nCount_RNA (stored in metadata) as sum(counts)
#'
restore_counts_from_seurat <- function(seurat_obj, assay = "RNA", scale_factor = 10000) {
  ...
  # Calculate size factors: nCount / scale_factor
  # This reverses the normalization: counts_norm = log1p(counts / nCount * scale_factor)
  # To get counts: counts = expm1(counts_norm) * nCount / scale_factor
  size_factors <- nCount / scale_factor

  # Restore raw counts
  raw_counts <- restore_raw_counts_log(normalized_data, size_factors)

  # Round to integers (raw counts should be integer UMI counts)
  # Use sparse rounding to maintain dgCMatrix format
...
snapshot_seurat_structure <- function(obj, sample_name, outdir, assay = NULL) {
  ...
  # --- write text snapshot (controlled, not str(obj) huge dump) ---
  txt <- c(
...
# =============================================================================
# Safe assay name getter (Seurat v5 compatible, avoids Assays() container issues)
# =============================================================================
get_assay_names_safe <- function(obj) {
  nm <- tryCatch(names(obj@assays), error = function(e) NULL)
  if (is.null(nm) || length(nm) == 0) {
    # fallback: try SeuratObject method, then take names()
...
pick_assay <- function(obj, preferred = "RNA") {
  ...
  # fallback to active assay if valid
...
# =============================================================================
# Fix missing dimnames in counts matrix (critical for Assay5 → Assay conversion)
# =============================================================================
...
# =============================================================================
# Check if counts matrix contains integer-like values (for DecontX validity)
# =============================================================================
is_integer_like_counts <- function(m, tol = 1e-6, max_check = 5e5) {
  ...
  frac_nonint < 0.01 # allow <1% non-integer noise
}

# =============================================================================
# CRITICAL: Seurat v5 Layers Handler
# =============================================================================
...
rebuild_assay_from_scratch <- function(seurat_obj, assay = "RNA") {
  ...
  # ⭐ CRITICAL FIX: Ensure dimnames are present (Assay5 often has NULL dimnames)
  ...
  # Step 3: Extract data (or recalculate)
  ...
  if (!is.null(data_data) && nrow(data_data) > 0) {
    ...
    # ⭐ CRITICAL FIX: Convert ALL factor columns to character before creating temp object
    # This prevents 'match' requires vector arguments error
    ...
    # ⭐ SAFETY CHECK: Ensure metadata rownames match cell names
    ...
    # Make sure temp_obs has same rownames as tmp
    ...
    # ⭐ CRITICAL: Copy NormalizeData command to original object
    # DoubletFinder reads obj@commands$NormalizeData.<assay>@params
...
# =============================================================================
# CRITICAL: Sample Column Fallback
# =============================================================================
ensure_sample_column <- function(seurat_obj, sample_column = "sample") {
  md <- seurat_obj@meta.data

  # Check if column exists AND has valid values (not all NA)
  if (sample_column %in% colnames(md) && !all(is.na(md[[sample_column]]))) {
    # Still need to reset Idents to avoid active.ident factor issues
    Idents(seurat_obj) <- sample_column
    return(seurat_obj)
  }

  if ("orig.ident" %in% colnames(md)) {
    # ⭐ KEY FIX: Force conversion to character (orig.ident is often a factor)
    # This prevents 'match' requires vector arguments error
  ...
  # Reset Idents to the new sample column (ensures active.ident is character-based)
  Idents(seurat_obj) <- sample_column
...
get_counts_matrix <- function(seurat_obj, assay = "RNA") {
  ...
  # ⭐ FIX: Ensure dimnames are present
  m <- fix_dimnames_counts(m, seurat_obj, assay = assay)

  return(m)
}

# Drop NA-corrupted cells (rare but protective)
...
# =============================================================================
# Ensure NormalizeData command exists (required by DoubletFinder)
# =============================================================================
ensure_normalize_command <- function(
  obj,
  assay = "RNA",
  ...
  # Seurat sometimes stores commands in a list; ensure it exists
...
run_doubletfinder_keep_singlets <- function(
  ...
  # ⭐ CRITICAL: DoubletFinder reads obj@commands$NormalizeData.<assay>@params
  # Even if data slot exists, ensure the command record exists
...
run_decontx <- function(obj, assay = "RNA", max_iter = 500) {
  ...
  if (!is_integer_like_counts(cnt)) {
    ...
    obj$data_type <- "raw_counts" # Mark as validated raw counts
  }
  ...
  # Only store contamination scores (not decontaminated counts)
  # Rationale: decontXcounts would be removed by DietSeurat anyway,
  # and round() can cause dense matrix memory explosion
  obj$decontX_contamination <- dx$contamination

  # NOTE: decontXcounts is NOT stored to save memory and avoid DietSeurat conflicts
  # If you need decontaminated counts, modify DietSeurat call to keep "decontXcounts" assay
...
process_one_rds <- function(rds_path) {
  ...
  # ⭐ NEW: Snapshot structure for diagnostics
  ...
  # Ensure sample metadata (force character to avoid factor issues)
  ...
  # ⭐ CRITICAL FIX: Reset active.ident to avoid factor issues
  # active.ident is often a factor, which causes 'match' errors in downstream operations
  ...
  # CRITICAL: Join layers + rebuild assay for v5 compatibility
  ...
  if (DO_DECONTX && ncol(obj) > 0) {
    b4 <- ncol(obj)
    obj <- tryCatch(
      run_decontx(obj, assay = assay_use, max_iter = DECONTX_MAX_ITER),
      error = function(e) {
    ...
    # Filter by contamination (SAFE: skip if all NA)
    if (
  ...
  # === Final Diet (reduce file size, keep only current assay) ===
  current_assay <- DefaultAssay(obj)
  obj <- DietSeurat(
    obj,
    assays = current_assay,
    counts = TRUE,
    data = TRUE,
    scale.data = FALSE,
    dimreducs = NULL,
...
log_msg("║  VERSION: v2_DENORM (De-lognormalization Support)       ║")
log_msg("╚══════════════════════════════════════════════════════════╝")
...
for (i in seq_along(rds_files)) {
  ...
  # Wrap in tryCatch to handle corrupted/incompatible RDS files
  res <- tryCatch(
    {
...
# Ensure sample column in merged object
...
if (MIN_CELLS_PER_GENE > 0) {
  ...
  # Use same assay selection logic
...
# Also save as "final" (already singlets + decontX filtered if enabled)
