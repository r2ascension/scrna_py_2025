...
CELLTYPE_L3_COL <- "cell_type_scanvi_pred"      # fine subtype (display label)
LABEL_COL       <- "cell_type_scanvi_pred"      # annotation column for visualization
...
first_nonempty_column <- function(meta, candidates, min_unique = 2) {
  for (nm in candidates) {
    if (is.null(nm) || is.na(nm) || !nm %in% colnames(meta)) next
    vals <- meta[[nm]]
...
normalize_interpret_agent_result <- function(raw_result,
                                             ct_l2,
                                             comp_name,
                                             dir_name,
  ...
  status <- dplyr::case_when(
    ...
    is.list(core) && any(nzchar(c(overview, key_mechanisms, hypothesis, narrative, key_drivers, evidence, limitations))) ~ "structured",
...
add("")
if (LABEL_COL != CELLTYPE_L2_COL) {
  add("![UMAP subtype](figures/umap_subtype.png)")
  add("")
}
...
if (!ENABLE_LLM) {
  ...
  if (nrow(agent_structured_df) == 0) {
    ...
    add(sprintf("- `%s`", file.path("reports", "LLM_INTERPRETATION_FOR_LLM.md")))
    add("")
    for (i in seq_len(nrow(agent_structured_df))) {
      ...
      if (nzchar(rec$warnings)) add(sprintf("- **Warnings:** %s", rec$warnings))
      if (nzchar(rec$error)) add(sprintf("- **Error:** %s", rec$error))
      ...
      if (nzchar(rec$key_drivers)) add(sprintf("- **Key Drivers:** %s", rec$key_drivers))
      if (nzchar(rec$limitations)) add(sprintf("- **Limitations:** %s", rec$limitations))
      add("")
    }
  }
  add("")
