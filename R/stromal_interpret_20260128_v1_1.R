#!/usr/bin/env Rscript
# ==============================================================================
# Stromal/Vascular Cell Subcluster LLM Interpretation - PRODUCTION VERSION v1.1
# ==============================================================================
#
# Version: v1.1 (2026-01-28)
...
#   ✅ 8 database support: Stromal Markers, CellMarker, PanglaoDB, GO BP/MF/CC, Hallmark, KEGG
#   ✅ Complete interpret() field extraction
#   ✅ Annotation + Phenotype dual tasks
#   ✅ Comprehensive CSV outputs and REPORT.md
#
# Production Fixes (v1.1):
#   ✅ P0-1: Fixed variable name pollution (markers_vec vs all_markers)
#   ✅ P0-2: Fixed per-celltype report filtering (membership vs grepl)
#   ✅ P0-3: Added Description/ID fallback for enrichment summary
#   ✅ P1-1: Added filtered_genes_info.csv for consistency with B cell v2.6
#
# ==============================================================================
...
OUTPUT_DIR <- "/home/h2048/data/R/0128/stromal_interpret_v1_1"
...
# ==============================================================================
table(seurat_obj$cell_type_L2)
table(seurat_obj$cell_type_L3)
...
# P1-1: Generate filtered genes info table (consistency with B cell v2.6)
...
if (!is.null(stromal_markers_db)) {
  ...
  for (i in 1:nrow(stromal_markers_db)) {
    ...
    markers_vec <- c() # P0-1 FIX: Avoid polluting outer all_markers variable
...
if (!is.null(stromal_markers_term2gene)) {
  stromal_markers_enrich <- tryCatch(
    ...
    error = function(e) {
      cat(
        "[WARN] Stromal Markers enrichment failed:",
        conditionMessage(e),
        "\n"
      )
      return(NULL)
...
if (length(enrichment_summary) > 0) {
  ...
  for (db_name in names(enrichment_summary)) {
    ...
    if (nrow(db_terms) > 0) {
      # P0-3 FIX: Fallback to ID if Description doesn't exist
      desc_col <- if ("Description" %in% colnames(db_terms)) {
        "Description"
      } else {
        "ID"
      }
      ...
      term_summary <- db_terms %>%
        group_by(Cluster) %>%
        summarize(
          top_terms = paste(.data[[desc_col]][1:min(3, n())], collapse = "; "),
          .groups = "drop"
        )
        ...
        if (is.list(interpretation)) {
          if (length(interpretation) > 0 && is.list(interpretation[[1]])) {
            ...
            for (cluster_name in names(interpretation)) {
              # P0-2 FIX: Use membership instead of grepl to avoid L2/L3 name mismatch
              # (e.g., L2="Smooth muscle" vs L3="Smooth_muscle_c0")
...
cat("ANALYSIS COMPLETE - Stromal/Vascular Cell Interpretation v1.1\n")
cat(
  "================================================================================\n\n"
)
...
cat("Production Fixes Applied (v1.1):\n")
cat("  P0 (Critical - Result Correctness):\n")
cat("    ✓ P0-1: Fixed variable name pollution (markers_vec vs all_markers)\n")
cat("    ✓ P0-2: Fixed per-celltype report filtering (membership vs grepl)\n")
cat("    ✓ P0-3: Added Description/ID fallback for enrichment summary\n")
cat("  P1 (Consistency with B Cell v2.6):\n")
cat("    ✓ P1-1: Added filtered_genes_info.csv output\n")
cat("\n")
...
cat(
  "  - figures/stromal_markers_dotplot.pdf    Stromal/vascular marker enrichment\n"
)
cat(
  "  - figures/cellmarker_dotplot.pdf         CellMarker database enrichment\n"
)
cat("  - figures/panglaodb_dotplot.pdf          PanglaoDB enrichment\n")
cat(
  "  - figures/go_bp_dotplot.pdf              GO biological process enrichment\n"
)
cat(
...
cat(
  "  - figures/hallmark_dotplot.pdf           Hallmark pathways (50 canonical)\n"
)
