...
#
# VERSION: v3_STORE_RESTORED_SKIP_EXISTING (2024-12-16) ⭐ Skip Existing Cleaned
# NEW FEATURES vs v3:
...
log_msg("║  VERSION: v3_SKIP_EXISTING (Skip Already Cleaned Files) ║")
log_msg("╚══════════════════════════════════════════════════════════╝")
...
for (i in seq_along(rds_files)) {
  ...
  # ⭐ NEW: Check if cleaned RDS already exists
  out_rds <- file.path(
    OUTPUT_DIR,
    "cleaned_samples",
  ...
  if (SKIP_EXISTING_CLEANED && !FORCE_REPROCESS && file.exists(out_rds)) {
    ...
    log_msg("  Loading existing cleaned object...")

    # Load the existing cleaned object
    obj_loaded <- tryCatch(
      {
        readRDS(out_rds)
      },
      error = function(e) {
        log_msg(sprintf("  ✗ ERROR loading existing file: %s", e$message))
        log_msg("  Will reprocess this sample...")
        NULL
      }
    )

    if (!is.null(obj_loaded)) {
      processed[[sample_name]] <- obj_loaded

      # Create a QC record for skipped sample
      qc_skipped <- data.frame(
        sample = sample_name,
        step = "LOADED_FROM_EXISTING",
        cells_before = ncol(obj_loaded),
        cells_after = ncol(obj_loaded),
        stringsAsFactors = FALSE
      )
      qc_all[[i]] <- qc_skipped

      log_msg(sprintf("  ✓ Loaded: %d cells, %d features", ncol(obj_loaded), nrow(obj_loaded)))
      log_msg("")
