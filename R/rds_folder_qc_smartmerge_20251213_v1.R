...
run_decontx <- function(obj, assay = "RNA", max_iter = 500) {
  ...
  if (!is_integer_like_counts(cnt)) {
    ...
    log_msg(
      "  [DecontX] DecontX requires raw UMI counts → SKIPPING for this sample"
    )
    obj$decontX_contamination <- NA_real_
    obj$data_type <- "non_raw_counts" # Flag for QC tracking
    return(obj)
  }
...
log_msg("║  VERSION: v5_HOTFIX_v6 (DoubletFinder Command Fix)      ║")
log_msg("╚══════════════════════════════════════════════════════════╝")
