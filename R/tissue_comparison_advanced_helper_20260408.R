...
tc_find_ambiguous_sample_tissue_keys <- function(meta_df,
  ...
  if (is.null(meta_df) || !is.data.frame(meta_df) || nrow(meta_df) == 0) return(character())
  required_cols <- c(sample_col, tissue_col)
  if (!all(required_cols %in% colnames(meta_df))) return(character())
  ...
  key_df <- unique(meta_df[, unique(c(required_cols, compare_cols)), drop = FALSE])
...
tc_resolve_pseudobulk_group_columns <- function(meta_df,
  ...
  group_cols <- required_cols
  candidate_cols <- unique(as.character(candidate_cols))
  candidate_cols <- setdiff(candidate_cols, required_cols)
  candidate_cols <- candidate_cols[candidate_cols %in% colnames(meta_df)]
  ...
  while (length(ambiguous_keys) > 0 && length(candidate_cols) > 0) {
    informative_cols <- candidate_cols[vapply(candidate_cols, function(col) {
      ...
      keep <- probe_key %in% ambiguous_keys
      probe_df <- probe_df[keep, , drop = FALSE]
      probe_key <- probe_key[keep]
...
tc_prepare_ofa_bubble_data <- function(enrich_by_direction, top_n_per_db = 6L, max_term_chars = 80L) {
  if (is.null(enrich_by_direction) || length(enrich_by_direction) == 0) return(data.frame())

  parse_gene_ratio <- function(x) {
    x <- as.character(x)
    vals <- suppressWarnings(vapply(strsplit(x, "/", fixed = TRUE), function(parts) {
      if (length(parts) == 2L) {
  ...
  trim_term <- function(x) {
    x <- as.character(x)
    over <- nchar(x) > max_term_chars
    x[over] <- paste0(substr(x[over], 1L, max_term_chars - 3L), "...")
    x
  }

  rows <- list()
  for (direction_name in names(enrich_by_direction)) {
    db_list <- enrich_by_direction[[direction_name]]
    if (is.null(db_list) || length(db_list) == 0) next
    for (db_name in names(db_list)) {
      enr <- db_list[[db_name]]
      if (is.null(enr)) next
      df <- tryCatch(as.data.frame(enr), error = function(e) NULL)
      ...
      df <- df[!is.na(df$p.adjust) & is.finite(df$p.adjust), , drop = FALSE]
      if (nrow(df) == 0) next
      df <- df[order(df$p.adjust, -as.numeric(df$Count)), , drop = FALSE]
      df <- utils::head(df, as.integer(top_n_per_db))
      df$direction <- toupper(direction_name)
      df$database <- db_name
      df$term <- trim_term(df$Description)
      df$gene_ratio_numeric <- parse_gene_ratio(df$GeneRatio)
      df$padj_plot <- pmax(df$p.adjust, .Machine$double.xmin)
      df$neg_log10_padj <- -log10(pmax(df$p.adjust, .Machine$double.xmin))
      rows[[length(rows) + 1L]] <- df[, c(
        "direction", "database", "term", "Description", "Count", "GeneRatio",
        "gene_ratio_numeric", "p.adjust", "padj_plot", "neg_log10_padj"
      ), drop = FALSE]
    }
  }

  if (length(rows) == 0) return(data.frame())
  bubble_df <- dplyr::bind_rows(rows)
  bubble_df$panel_key <- paste(bubble_df$database, bubble_df$direction, sep = " | ")
  bubble_df <- bubble_df %>%
    dplyr::group_by(.data$panel_key) %>%
    dplyr::arrange(.data$p.adjust, dplyr::desc(.data$Count), .by_group = TRUE) %>%
    dplyr::mutate(term_factor = factor(.data$term, levels = rev(unique(.data$term)))) %>%
    dplyr::ungroup()
  bubble_df
}

tc_save_ofa_bubble_plot <- function(
  enrich_by_direction,
  out_dir,
  ...
  bubble_df <- tc_prepare_ofa_bubble_data(
    enrich_by_direction = enrich_by_direction,
    top_n_per_db = top_n_per_db,
    max_term_chars = max_term_chars
  )
  csv_path <- file.path(out_dir, paste0(stem, "_data.csv"))
  if (!is.data.frame(bubble_df) || nrow(bubble_df) == 0) {
    return(list(data = bubble_df, data_csv = csv_path, plots = list(pdf = NA_character_, png = NA_character_)))
  }

  utils::write.csv(bubble_df, csv_path, row.names = FALSE)
  n_panels <- length(unique(bubble_df$panel_key))
  max_terms <- max(stats::aggregate(term ~ panel_key, data = bubble_df, FUN = function(x) length(unique(x)))$term, na.rm = TRUE)
  plot_width <- max(8, min(18, 3.8 * min(n_panels, 4L)))
  plot_height <- max(6, min(20, 2.2 * ceiling(n_panels / 2) + 0.35 * max_terms))

  p <- ggplot2::ggplot(
    bubble_df,
    ggplot2::aes(x = .data$padj_plot, y = .data$term_factor, size = .data$Count, color = .data$neg_log10_padj)
  ) +
    ggplot2::geom_point(alpha = 0.9) +
    ggplot2::facet_wrap(~panel_key, scales = "free_y", ncol = min(2L, n_panels)) +
    ggplot2::scale_x_continuous(labels = scales::label_scientific(digits = 2), name = "adj.P") +
    ggplot2::scale_color_gradient(low = "#56B1F7", high = "#D73027", name = expression(-log[10](adj.P))) +
    ggplot2::scale_size_continuous(name = "Count", range = c(2.5, 9)) +
    ggplot2::labs(
      y = NULL,
      title = if (!is.null(title) && nzchar(tc_safe_trim(title))) title else "OFA enrichment bubble plot"
    ) +
  ...
  plot_paths <- tc_save_plot(
    plot_obj = p,
    path_no_ext = file.path(out_dir, stem),
    width = plot_width,
    height = plot_height,
    dpi = dpi
  )
  list(data = bubble_df, data_csv = csv_path, plots = plot_paths)
}

tc_run_tissue_milopy <- function(
  input_h5ad = NULL,
  h5ad_path = NULL,
  output_dir,
  celltype_cols = c("cell_type_L2", "cell_type_L3"),
  sample_col = "sample",
  tissue_col = "tissue",
  conda_exe = "/home/h2048/miniconda3/bin/conda",
  env_prefix = "/home/h2048/miniconda3/envs/scarches_stable_pertpy",
  python_exe = NULL,
  script_path = "/home/h2048/script/py/tissue_subcluster_milopy_20260418_v1.py",
  latent_key_candidates = c("X_scanvi", "X_scANVI", "X_scanvi_corrected", "X_scvi", "X_harmony", "X_pca"),
  umap_key_candidates = c("X_umap_scanvi", "X_umap_scANVI", "X_umap_scanvi_corrected", "X_umap_scvi", "X_umap_harmony", "X_umap"),
  min_cells_per_celltype = 200L,
  min_cells_per_sample = 10L,
  min_samples_per_tissue = 3L,
  n_neighbors = 30L,
  nhood_prop = 0.1,
  alpha = 0.1,
  random_seed = 0L,
  max_celltypes_per_level = NULL,
  make_plots = TRUE,
  write_milo_h5ad = FALSE,
  log_path = NULL,
  stop_on_error = FALSE
) {
  if ((is.null(input_h5ad) || !nzchar(tc_safe_trim(input_h5ad))) && !is.null(h5ad_path) && nzchar(tc_safe_trim(h5ad_path))) {
    input_h5ad <- h5ad_path
  }
  input_h5ad <- tc_to_scalar(input_h5ad)
  output_dir <- tc_to_scalar(output_dir)
  conda_exe <- tc_to_scalar(conda_exe)
  env_prefix <- tc_to_scalar(env_prefix)
  python_exe <- tc_to_scalar(if (is.null(python_exe) || !nzchar(tc_safe_trim(python_exe))) file.path(env_prefix, "bin", "python") else python_exe)
  script_path <- tc_to_scalar(script_path)
  if (!tc_path_exists(input_h5ad)) {
    return(list(status = "skipped", reason = sprintf("input_h5ad not found: %s", input_h5ad)))
  }
  if (!tc_path_exists(script_path)) {
    return(list(status = "skipped", reason = sprintf("Milo script not found: %s", script_path)))
  }
  if (!dir.exists(env_prefix)) {
    return(list(status = "skipped", reason = sprintf("pertpy environment not found: %s", env_prefix)))
  }
  if (!tc_path_exists(python_exe)) {
    return(list(status = "skipped", reason = sprintf("pertpy python executable not found: %s", python_exe)))
  }

  dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)
  if (is.null(log_path) || !nzchar(tc_safe_trim(log_path))) {
    log_path <- file.path(output_dir, "milopy_run.log")
  }

  env_lib_dir <- file.path(env_prefix, "lib")
  if (!dir.exists(env_lib_dir)) env_lib_dir <- ""

  args <- c(
    "-u", "PYTHONPATH",
    "-u", "PYTHONHOME",
    if (nzchar(env_lib_dir)) sprintf("LD_LIBRARY_PATH=%s", env_lib_dir),
    "PYTHONNOUSERSITE=1",
    python_exe,
    script_path,
    "--input-h5ad", input_h5ad,
    "--output-dir", output_dir,
    "--sample-col", tc_to_scalar(sample_col),
    "--tissue-col", tc_to_scalar(tissue_col),
    "--min-cells-per-celltype", as.character(as.integer(min_cells_per_celltype)),
    "--min-cells-per-sample", as.character(as.integer(min_cells_per_sample)),
    "--min-samples-per-tissue", as.character(as.integer(min_samples_per_tissue)),
    "--n-neighbors", as.character(as.integer(n_neighbors)),
    "--nhood-prop", as.character(as.numeric(nhood_prop)),
    "--alpha", as.character(as.numeric(alpha)),
    "--random-seed", as.character(as.integer(random_seed))
  )
  for (col in unique(as.character(celltype_cols))) {
    if (!is.na(col) && nzchar(tc_safe_trim(col))) args <- c(args, "--celltype-col", col)
  }
  for (key in unique(as.character(latent_key_candidates))) {
    if (!is.na(key) && nzchar(tc_safe_trim(key))) args <- c(args, "--latent-key-candidate", key)
  }
  for (key in unique(as.character(umap_key_candidates))) {
    if (!is.na(key) && nzchar(tc_safe_trim(key))) args <- c(args, "--umap-key-candidate", key)
  }
  if (!is.null(max_celltypes_per_level) && length(max_celltypes_per_level) > 0 && !is.na(max_celltypes_per_level[[1]])) {
    args <- c(args, "--max-celltypes-per-level", as.character(as.integer(max_celltypes_per_level[[1]])))
  }
  if (!isTRUE(make_plots)) args <- c(args, "--no-plots")
  if (isTRUE(write_milo_h5ad)) args <- c(args, "--write-milo-h5ad")

  cmd_display <- paste(c("env", shQuote(args)), collapse = " ")
  exit_status <- tryCatch(
    system2("env", args = args, stdout = log_path, stderr = log_path, wait = TRUE),
    error = function(e) structure(127L, error_message = conditionMessage(e), class = "tc_milo_system_error")
  )

  read_csv_if_present <- function(path) {
    if (!file.exists(path) || isTRUE(file.info(path)$size == 0)) return(data.frame())
    tryCatch(utils::read.csv(path, check.names = FALSE, stringsAsFactors = FALSE), error = function(e) data.frame())
  }

  level_summary_path <- file.path(output_dir, "level_run_summary.csv")
  pairwise_summary_path <- file.path(output_dir, "combined_pairwise_run_summary.csv")
  contrast_summary_path <- file.path(output_dir, "combined_contrast_level_summary.csv")
  result <- list(
    status = if (isTRUE(as.integer(exit_status) == 0L)) "completed" else "failed",
    ...
    level_summary = read_csv_if_present(level_summary_path),
    pairwise_summary = read_csv_if_present(pairwise_summary_path),
    contrast_summary = read_csv_if_present(contrast_summary_path)
  )
  if (!isTRUE(as.integer(exit_status) == 0L) && isTRUE(stop_on_error)) {
    stop(sprintf("MiloPy run failed (exit=%s). See log: %s", as.integer(exit_status), log_path))
  }
  result
}
...
tc_pick_existing_reduction_name <- function(available_reductions,
                                            candidates = character(),
                                            fallback_regex = NULL) {
  available_reductions <- unique(as.character(available_reductions))
  available_reductions <- available_reductions[!is.na(available_reductions) & nzchar(trimws(available_reductions))]
  candidates <- unique(as.character(candidates))
  candidates <- candidates[!is.na(candidates) & nzchar(trimws(candidates))]
  hit <- candidates[candidates %in% available_reductions]
  if (length(hit) > 0) return(hit[[1]])
  if (!is.null(fallback_regex) && length(fallback_regex) == 1 && nzchar(tc_safe_trim(fallback_regex))) {
    regex_hit <- available_reductions[grepl(fallback_regex, available_reductions, perl = TRUE)]
    if (length(regex_hit) > 0) return(regex_hit[[1]])
  }
  NA_character_
}

tc_default_final_schema_lineage <- function(lineage_tag = NULL) {
  tag <- if (is.null(lineage_tag) || length(lineage_tag) == 0) "" else as.character(lineage_tag[[1]])
  tag <- toupper(tc_safe_trim(tag))
  if (!nzchar(tag)) tag <- "UNKNOWN"

  defaults <- switch(
    tag,
    BCELL = list(
      lineage_family = "bcell",
      lineage_branch = "bcell",
      default_latent_reduction_candidates = c("scANVI_L3", "scanvi", "scanvi_corrected", "scvi", "harmony", "pca"),
      default_umap_reduction_candidates = c("umap", "umap_scanvi", "umap_scanvi_corrected", "umap_scvi", "umap_harmony")
    ),
    TNK = list(
      lineage_family = "tnk",
      lineage_branch = "tnk",
      default_latent_reduction_candidates = c("scanvi_refined", "scanvi", "scvi", "harmony", "pca"),
      default_umap_reduction_candidates = c("umap", "umap_refined", "umap_scanvi", "umap_scanvi_corrected", "umap_scvi")
    ),
    EPITHELIAL = list(
      lineage_family = "epithelial",
      lineage_branch = "epithelial",
      default_latent_reduction_candidates = c("scanvi_major", "scanvi", "scanvi_fine", "scvi", "harmony", "pca"),
      default_umap_reduction_candidates = c("umap", "umap_fine", "umap_major", "umap_scanvi", "umap_harmony", "umap_scanvi_fine", "umap_scanvi_major")
    ),
    EPI = list(
      lineage_family = "epithelial",
      lineage_branch = "epithelial",
    ...
    MYELOID = list(
      lineage_family = "myeloid",
      lineage_branch = "myeloid",
      default_latent_reduction_candidates = c("scanvi", "scvi", "harmony", "pca"),
      default_umap_reduction_candidates = c("umap", "umap_scanvi", "umap_scanvi_corrected", "umap_scvi", "umap_bbknn")
    ),
    STROMAL_ENDOTHELIAL = list(
      lineage_family = "stromal",
      lineage_branch = "endothelial",
      default_latent_reduction_candidates = c("scanvi", "scvi", "pca"),
      default_umap_reduction_candidates = c("umap", "umap_scanvi", "umap_scanvi_corrected", "umap_scvi", "umap_bbknn")
    ),
    STROMAL_FIBROBLAST = list(
      lineage_family = "stromal",
      lineage_branch = "fibroblast",
      default_latent_reduction_candidates = c("scanvi", "scvi", "pca"),
      default_umap_reduction_candidates = c("umap", "umap_scanvi", "umap_scanvi_corrected", "umap_scvi", "umap_bbknn")
    ),
    STROMAL_SMC = list(
    ...
    list(
      lineage_family = tolower(tag),
      lineage_branch = tolower(tag),
      default_latent_reduction_candidates = c("scanvi", "scvi", "harmony", "pca"),
      default_umap_reduction_candidates = c("umap", "umap_scanvi", "umap_scanvi_corrected", "umap_scvi", "umap_harmony")
    )
  )

  defaults$lineage_tag <- tag
  defaults
}

tc_build_final_schema_contract <- function(lineage_tag = NULL,
                                           generated_by = "",
                                           final_file_prefix = "",
                                           available_reductions = character(),
                                           default_latent_reduction_candidates = NULL,
                                           default_umap_reduction_candidates = NULL,
                                           schema_version = "tc_final_v1",
                                           object_role = "final") {
  lineage_defaults <- tc_default_final_schema_lineage(lineage_tag)
  if (is.null(default_latent_reduction_candidates) || length(default_latent_reduction_candidates) == 0) {
    default_latent_reduction_candidates <- lineage_defaults$default_latent_reduction_candidates
  }
  if (is.null(default_umap_reduction_candidates) || length(default_umap_reduction_candidates) == 0) {
    default_umap_reduction_candidates <- lineage_defaults$default_umap_reduction_candidates
  }

  default_latent_reduction <- tc_pick_existing_reduction_name(
    available_reductions = available_reductions,
    candidates = default_latent_reduction_candidates
  )
  default_umap_reduction <- tc_pick_existing_reduction_name(
    available_reductions = available_reductions,
    candidates = unique(c("umap", default_umap_reduction_candidates)),
    fallback_regex = "^umap"
  )

  list(
    schema_version = tc_safe_trim(as.character(schema_version)),
    object_role = tc_safe_trim(as.character(object_role)),
    lineage_family = lineage_defaults$lineage_family,
    lineage_branch = lineage_defaults$lineage_branch,
    lineage_tag = lineage_defaults$lineage_tag,
    generated_by = tc_safe_trim(as.character(generated_by)),
    final_file_prefix = tc_safe_trim(as.character(final_file_prefix)),
    default_latent_obsm = if (!is.na(default_latent_reduction) && nzchar(default_latent_reduction)) paste0("X_", default_latent_reduction) else "",
    default_umap_obsm = if (!is.na(default_umap_reduction) && nzchar(default_umap_reduction)) paste0("X_", default_umap_reduction) else "",
    required_obs_columns = c("lineage_family", "lineage_branch", "orig.ident", "sample", "tissue", "cell_type_L2", "cell_type_L3"),
    required_layers = c("counts")
  )
}

tc_apply_final_schema_to_seurat <- function(obj,
                                            lineage_tag = NULL,
                                            generated_by = "",
                                            final_file_prefix = "",
                                            default_latent_reduction_candidates = NULL,
  ...
  if (!methods::is(obj, "Seurat")) {
    stop("tc_apply_final_schema_to_seurat() expects a Seurat object.")
  }
  if (!"orig.ident" %in% colnames(obj@meta.data) && "sample" %in% colnames(obj@meta.data)) {
    obj@meta.data[["orig.ident"]] <- as.character(obj@meta.data[["sample"]])
  }
  if (!"sample" %in% colnames(obj@meta.data) && "orig.ident" %in% colnames(obj@meta.data)) {
    obj@meta.data[["sample"]] <- as.character(obj@meta.data[["orig.ident"]])
  }

  required_obs <- c("orig.ident", "sample", "tissue", "cell_type_L2", "cell_type_L3")
  missing_obs <- setdiff(required_obs, colnames(obj@meta.data))
  if (length(missing_obs) > 0) {
    stop(sprintf(
      "Final schema requires metadata columns in Seurat object: %s",
      paste(missing_obs, collapse = ", ")
    ))
  }

  schema <- tc_build_final_schema_contract(
    lineage_tag = lineage_tag,
    generated_by = generated_by,
    final_file_prefix = final_file_prefix,
    available_reductions = tryCatch(Reductions(obj), error = function(e) character()),
    default_latent_reduction_candidates = default_latent_reduction_candidates,
    default_umap_reduction_candidates = default_umap_reduction_candidates,
    schema_version = schema_version,
    object_role = object_role
  )

  obj@meta.data[["lineage_family"]] <- schema$lineage_family
  obj@meta.data[["lineage_branch"]] <- schema$lineage_branch
  obj@misc$tc_schema <- schema
  obj
}

tc_write_final_schema_to_anndata <- function(adata, schema) {
  if (is.null(adata) || is.null(schema) || !is.list(schema)) return(invisible(NULL))
  scalar_fields <- list(
    tc_object_role = schema$object_role,
    tc_schema_version = schema$schema_version,
    tc_lineage_family = schema$lineage_family,
    tc_lineage_branch = schema$lineage_branch,
    tc_generated_by = schema$generated_by,
    tc_final_file_prefix = schema$final_file_prefix,
    tc_default_latent_obsm = schema$default_latent_obsm,
    tc_default_umap_obsm = schema$default_umap_obsm
  )
  for (nm in names(scalar_fields)) {
    value <- tc_safe_trim(as.character(scalar_fields[[nm]]))
    if (nzchar(value)) {
      adata$uns$`__setitem__`(nm, value)
    }
  }
  invisible(adata)
}

tc_validate_final_h5ad_contract <- function(adata, builtins, schema = NULL) {
  if (is.null(adata) || is.null(builtins)) {
    stop("tc_validate_final_h5ad_contract() requires both adata and builtins.")
  }
  obs_cols <- reticulate::py_to_r(builtins$list(adata$obs$columns))
  layer_keys <- reticulate::py_to_r(builtins$list(adata$layers$keys()))
  uns_keys <- reticulate::py_to_r(builtins$list(adata$uns$keys()))
  obsm_keys <- reticulate::py_to_r(builtins$list(adata$obsm$keys()))

  required_obs <- if (!is.null(schema) && !is.null(schema$required_obs_columns)) {
    as.character(schema$required_obs_columns)
  } else {
    c("lineage_family", "lineage_branch", "orig.ident", "sample", "tissue", "cell_type_L2", "cell_type_L3")
  }
  required_layers <- if (!is.null(schema) && !is.null(schema$required_layers)) {
    as.character(schema$required_layers)
  } else {
    c("counts")
  }
  required_uns <- c("tc_object_role", "tc_schema_version", "tc_lineage_family", "tc_lineage_branch", "tc_default_umap_obsm")

  missing_obs <- setdiff(required_obs, obs_cols)
  missing_layers <- setdiff(required_layers, layer_keys)
  missing_uns <- setdiff(required_uns, uns_keys)
  if (length(missing_obs) > 0) {
    stop(sprintf("Final h5ad schema missing obs columns: %s", paste(missing_obs, collapse = ", ")))
  }
  if (length(missing_layers) > 0) {
    stop(sprintf("Final h5ad schema missing layers: %s", paste(missing_layers, collapse = ", ")))
  }
  if (length(missing_uns) > 0) {
    stop(sprintf("Final h5ad schema missing uns keys: %s", paste(missing_uns, collapse = ", ")))
  }

  default_umap_obsm <- if (!is.null(schema) && !is.null(schema$default_umap_obsm)) tc_safe_trim(schema$default_umap_obsm) else ""
  default_latent_obsm <- if (!is.null(schema) && !is.null(schema$default_latent_obsm)) tc_safe_trim(schema$default_latent_obsm) else ""
  if (nzchar(default_umap_obsm) && !default_umap_obsm %in% obsm_keys) {
    stop(sprintf("Final h5ad schema default UMAP obsm is absent: %s", default_umap_obsm))
  }
  if (nzchar(default_latent_obsm) && !default_latent_obsm %in% obsm_keys) {
    stop(sprintf("Final h5ad schema default latent obsm is absent: %s", default_latent_obsm))
  }
