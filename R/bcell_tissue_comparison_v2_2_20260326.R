#!/usr/bin/env Rscript
# ==============================================================================
# B Cell Tissue Comparison Pipeline v2.2.1 - PRODUCTION
# ==============================================================================
#
# Purpose:
...
OUTPUT_DIR <- "/home/h2048/data/R/0326/bcell_tissue_comparison_v2_2_1"
...
build_tissue_pair_context <- function(t1, t2, ct_l2, dir_name) {
  ...
  sprintf(
    ...
    BCELL_BASE_CONTEXT, t2, t1, ct_l2,
    dir_name, t2, t1,
    t2, ctx2, t1, ctx1
  )
}
...
cat("B Cell Tissue Comparison Pipeline v2.2.1 (PRODUCTION)\n")
...
tryCatch({
  ...
  # Strict B cell / plasma / GC filter (avoid generic "immune" / "lymphocyte" noise)
  cm_bcell <- cm_db[
    grepl("B cell|B-cell|B lymphocyte|Plasma cell|Plasmablast|Germinal center|Memory B|Naive B|Follicular B|Marginal zone B|Breg|Age-associated B|Atypical B|Pre-B|Pro-B|Transitional B",
          cell_name, ignore.case = TRUE)
  ]
  cat(sprintf("[OK] CellMarker B-cell-specific: %d entries\n", nrow(cm_bcell)))

  if (nrow(cm_bcell) < 10) {
    cat("[WARN] Too few B cell entries in CellMarker; skipping this database\n")
  } else {
    t2g_list <- list()
    ...
    cat(sprintf("[OK] CellMarker TERM2GENE: %d pairs (%d B cell types)\n",
...
tryCatch({
  ...
  # Strict B cell / plasma filter
  pdb_bcell <- pdb[
    grepl("B cell|Plasma cell|Plasmablast|Germinal center|Memory B|Naive B|Follicular",
          cell_type, ignore.case = TRUE)
  ]
  cat(sprintf("[OK] PanglaoDB B-cell-specific: %d markers (%d cell types)\n",
              nrow(pdb_bcell), length(unique(pdb_bcell$cell_type))))

  if (nrow(pdb_bcell) < 5) {
    cat("[WARN] Too few B cell markers in PanglaoDB; skipping this database\n")
  } else {
    ...
    cat(sprintf("[OK] PanglaoDB TERM2GENE: %d pairs (%d B cell types)\n",
...
bcell_custom_t2g <- BCELL_MARKERS_DB %>%
  ...
  distinct()
cat(sprintf("[OK] Custom B cell markers: %d subtypes, %d pairs\n",
            length(unique(bcell_custom_t2g$term)), nrow(bcell_custom_t2g)))
...
aggregate_pseudobulk <- function(obj, celltype_l2) {
  ...
  # Sparse-friendly: collect columns as list, cbind at end
  pb_cols <- list()
  ...
  for (g in groups) {
    ...
    # rowSums returns dense vector; wrap as single-column sparse
  ...
  # Merge to dense only at final step (pseudobulk is small: genes x n_samples)
...
standardize_result_with_llm <- function(raw_text, context_str, evidence_text,
                                        ct_l2, comp_name, dir_name, source_db,
...
normalize_interpret_agent_result <- function(raw_result, ct_l2, comp_name, dir_name,
  ...
  list(
    celltype_l2 = ct_l2, comparison = comp_name, direction = dir_name,
    ...
    narrative = narrative, key_drivers = key_drivers, evidence = evidence,
    limitations = limitations, raw_text = raw_text, raw_result = raw_result
  )
}
...
write_interpretation_markdown <- function(records, path, title, include_raw = FALSE) {
  ...
  for (i in seq_len(nrow(records))) {
    ...
    md <- c(md, sprintf("## %s | %s | %s", rec$celltype_l2, rec$comparison, rec$direction), "")
...
run_interpret_agent_safe <- function(enrich_obj, context_str, gene_fc = NULL) {
  ...
  for (attempt in seq_len(INTERPRET_AGENT_MAX_RETRIES)) {
    ...
    attempt_warning_msgs <- payload$warnings
    if (length(attempt_warning_msgs) > 0) {
      combined_warnings <- c(
        combined_warnings,
        sprintf("attempt %d: %s", attempt, attempt_warning_msgs)
      )
    }

    retryable_issue <- is_retryable_interpret_agent_issue(payload$warnings, payload$error)
    structured_result <- !is.null(payload$result) && looks_structured_interpret_agent_result(payload$result)
    raw_text_result <- !is.null(payload$result) && nzchar(capture_object_text(payload$result))

    if (is.null(payload$error) && (structured_result || (raw_text_result && !retryable_issue))) {
      payload$warnings <- unique(c(combined_warnings, payload$warnings))
      return(payload)
    }

    if (attempt < INTERPRET_AGENT_MAX_RETRIES && (retryable_issue || is.null(payload$result))) {
...
stratified_downsample <- function(obj, group_col, n_per = HEATMAP_CELLS_PER_TYPE) {
  set.seed(42)  # reproducible downsample across sessions
  meta <- obj@meta.data
...
# Apply L3 -> L2 remapping
l3_vals <- as.character(meta[[L3_SOURCE_COL]])
l2_vals <- L3_TO_L2_REMAP[l3_vals]

# Check for unmapped L3 values -- fail fast in production
...
remap_summary <- table(L3 = l3_vals, L2 = l2_vals, useNA = "ifany")
print(remap_summary)
cat("\n")
...
# Drop cells with NA in tissue / sample / L2 / label
...
# Normalize for visualization / marker finding
...
# 7.2 Dotplot (by L3 for fine resolution)
...
p4 <- ggplot(comp_df, aes(x = !!sym(TISSUE_COL), y = pct, fill = !!sym(CELLTYPE_L2_COL))) +
  ...
  labs(x = "Tissue", y = "Percentage (%)", title = "B Cell - L2 Composition by Tissue") +
...
# 7.3b Sample-level composition boxplot
sample_comp <- meta %>%
...
# 7.4 Heatmap (top markers per L2)
...
cat(sprintf("Enrichment databases: GO_BP, GO_MF, GO_CC, KEGG, Hallmark, CellMarker, PanglaoDB, B_cell_custom\n\n"))
  ...
  for (pair in combn(as.character(pb_tissues), 2, simplify = FALSE)) {
    ...
    for (dir_name in c("up", "down")) {
      ...
      enr_list <- list(
        ...
        B_cell_custom = run_gmt_enrichment(genes, bcell_custom_t2g, "B_cell_custom", tested)
      )
      ...
      # --- interpret_agent (tissue-pair-specific context) ---
      if (ENABLE_LLM) {
        best_er <- NULL; best_db <- NA_character_
        for (db in c("GO_BP", "Hallmark", "KEGG", "GO_MF", "CellMarker")) {
          if (!is.null(enr_list[[db]]) && nrow(as.data.frame(enr_list[[db]])) >= 3) {
            best_er <- enr_list[[db]]; best_db <- db; break
          }
        }
        ...
        if (!is.null(best_er)) {
          ...
          ia_structured <- normalize_interpret_agent_result(
            raw_result = ia, ct_l2 = ct_l2, comp_name = comp_name,
            dir_name = dir_name, source_db = best_db,
            warnings = ia_payload$warnings, error_message = ia_payload$error,
            enrich_obj = best_er, context_str = tissue_ctx, gene_fc = gene_fc
          )
...
# Save L2 remapping for reference
...
add("")
add("**Pipeline:** B Cell Tissue Comparison v2.2.1 (pseudobulk DESeq2, multi-database enrichment)")
add("")
...
# Enrichment (show all databases)
add("## 4. Multi-Database Enrichment")
add("")
add("Databases: GO BP/MF/CC, KEGG, Hallmark, CellMarker, PanglaoDB, B cell custom markers")
add("")
...
if (!ENABLE_LLM) {
  ...
  if (nrow(agent_structured_df) == 0) {
    ...
    add("- Output normalization: interpret_agent result is retried first, then standardized by a secondary DeepSeek pass to a fixed JSON schema when needed.")
    add("")
...
add("- **Enrichment:** clusterProfiler::enricher() + MSigDB GMT + CellMarker + PanglaoDB + custom B cell markers (symbol-based, zero ID loss)")
add("- **Enrichment universe:** DESeq2-tested genes intersected with GMT (not full GMT)")
add("- **Enrichment databases (8):** GO BP, GO MF, GO CC, KEGG, Hallmark, CellMarker, PanglaoDB, B cell custom")
if (ENABLE_LLM) {
  add("- **LLM:** interpret_agent with DeepSeek deepseek-reasoner (tissue-pair-specific context)")
  add("- **LLM post-processing:** secondary DeepSeek deepseek-chat pass to standardize outputs to fixed JSON schema when interpret_agent output is partial, raw, or malformed")
} else {
...
add("- **L2 remapping:** L3 (cell_type_scanvi_pred) merged into 4 L2 categories")
add(sprintf("- **Context:** Normal tissue comparison across %s", paste(tissues, collapse = ", ")))
add("")
add("### Output Objects")
add("")
add("- `bcell_tissue_comparison_final.rds` — Seurat object with `cell_type_L2` + `cell_type_L3`")
add("- `bcell_tissue_comparison_final.h5ad` — AnnData object with `cell_type_L2` + `cell_type_L3`")
add("")
add("---")
add("*Generated by bcell_tissue_comparison_v2_2_1.R*")
...
# ----- 12.1 Standardize column names: cell_type_L2 + cell_type_L3 -----
# cell_type_L2 already exists from L3_TO_L2_REMAP (Section 6)
# cell_type_L3: copy from L3_SOURCE_COL if not already named cell_type_L3
...
# Verify both columns present and non-empty
...
tryCatch({
  ...
  # Extract counts (sparse)
  counts_mat <- GetAssayData(obj, layer = "counts")

  # Transpose: Seurat = genes x cells -> anndata = cells x genes
  # Convert to CSC for efficient column slicing, then transpose to CSR
  ...
  # Build scipy CSR from R dgCMatrix (t() of dgCMatrix = dgRMatrix -> convert)
  ...
  # Build obs (metadata) - only character/factor columns to avoid type issues
  meta_export <- obj@meta.data
  # Keep key columns; convert factors to character for clean h5ad
  ...
  # Build var (gene info)
  var_df <- data.frame(row.names = rownames(obj))
  ...
  cat(sprintf("[OK] h5ad verified: %d cells x %d genes, cell_type_L2 + cell_type_L3 present\n",
              reticulate::py_to_r(adata_check$n_obs),
              reticulate::py_to_r(adata_check$n_vars)))
  rm(adata, adata_check, counts_mat, counts_t, counts_csc, X_scipy, meta_export)
  gc()
...
cat("\n", paste(rep("=", 70), collapse = ""), "\n")
cat("B CELL TISSUE COMPARISON COMPLETE (v2.2.1)\n")
cat(paste(rep("=", 70), collapse = ""), "\n\n")
...
cat("  bcell_tissue_comparison_final.rds       <- Seurat object (cell_type_L2 + cell_type_L3)\n")
cat("  bcell_tissue_comparison_final.h5ad      <- AnnData object (cell_type_L2 + cell_type_L3)\n")
...
cat("v2.2 Features:\n")
cat("  [NEW] L3->L2 remapping: Memory_B (3 subtypes), GC_B (3), Plasma (2), Naive_B (1)\n")
cat("  [NEW] Tissue-pair-specific LLM context (normal tissue biology)\n")
cat("  [NEW] 8-database enrichment: GO BP/MF/CC, KEGG, Hallmark, CellMarker, PanglaoDB, B cell custom\n")
cat("  [NEW] Fixed-schema DeepSeek standardization for interpret_agent outputs\n")
cat("  [DEL] Removed column candidate auto-resolution\n")
cat("\n")
