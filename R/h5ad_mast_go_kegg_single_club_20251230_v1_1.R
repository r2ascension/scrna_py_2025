...
# ===== Configuration =====
H5AD_FILE <- "/home/h2048/data/R/1223/cnmf_batch_production_v1_2_2/Club/batch_aware/cnmf_analysis_k35/Club_with_cnmf_k35.h5ad"
...
filter_low_quality_genes_fast <- function(seurat_obj, min_cells = 3) {
  ...
  rp_count <- length(grep("^RPS|^RPL|^MRPS|^MRPL", genes_to_remove, value = TRUE))
  cat(sprintf(
    "    MT: %d, RP: %d, Other: %d\n",
    mt_count,
    rp_count,
...
check_gene_id_consistency <- function(seurat_obj, msigdb_gmt) {
  ...
  common_genes <- intersect(
    all_genes,
    c(
      "TP63", "KRT5", "KRT14",      # Basal markers
      "MKI67", "PCNA", "TOP2A",     # Proliferation
      "KRT13", "KRT4", "IVL",       # Squamous differentiation
      "IL6", "CXCL8", "TNF"         # Inflammation
    )
  )
  cat(sprintf(
