#!/usr/bin/env Rscript
# ==============================================================================
# B Cell Tissue Comparison Pipeline v2.6.2 - PRODUCTION
# ==============================================================================
#
# Purpose:
...
#
# v2.6.2 Changes (vs v2.6.1):
#   [CHOIR-7] Reintroduce explicit CHOIR tuning knobs (`CHOIR_N_CORES`,
#             `CHOIR_SAMPLE_MAX`, `CHOIR_DOWNSAMPLING_RATE`,
#             `CHOIR_SUBTREE_REDUCTIONS`) for wrapper-based lineages.
#   [CHOIR-8] Surface CHOIR tuning in runtime logs so long-running tree builds
#             can be diagnosed without editing the validated base engine.
#
...
if (!exists("PIPELINE_VERSION_LABEL"))    PIPELINE_VERSION_LABEL <- "v2.6.2"
...
if (!exists("GENERATED_BY_LABEL"))        GENERATED_BY_LABEL <- "bcell_tissue_comparison_v2_6_2_20260414.R"
...
l3_ofa_vs_rest_summary_df <- data.frame()
l3_ofa_inter_tissue_summary_df <- data.frame()
...
if (RUN_CHOIR) {
  ...
  if (is.null(choir_reduction)) {
    ...
    if (RUN_CHOIR) {
      cat(sprintf(
        "[INFO] Running CHOIR (alpha=%.3f, n_cores=%d, sample_max=%s, downsampling_rate=%s, subtree_reductions=%s)...\n",
        CHOIR_ALPHA,
        as.integer(CHOIR_N_CORES),
        if (is.infinite(CHOIR_SAMPLE_MAX)) "Inf" else as.character(as.integer(CHOIR_SAMPLE_MAX)),
        if (is.character(CHOIR_DOWNSAMPLING_RATE)) CHOIR_DOWNSAMPLING_RATE else sprintf("%.4f", CHOIR_DOWNSAMPLING_RATE),
        ifelse(isTRUE(CHOIR_SUBTREE_REDUCTIONS), "TRUE", "FALSE")
      ))
      obj_choir <- tryCatch(
        CHOIR::CHOIR(obj, use_assay = "RNA", reduction = choir_embedding,
                     var_features = choir_var_features,
                     n_cores = as.integer(CHOIR_N_CORES),
                     alpha = CHOIR_ALPHA,
    ...
    if (is.null(choir_col)) {
      ...
      choir_annotation_tables <- if (exists("tc_write_choir_annotation_count_tables", mode = "function")) {
        tc_write_choir_annotation_count_tables(
        ...
        build_choir_annotation_count_tables(
          meta_df = obj@meta.data,
          choir_col = choir_col,
          annotation_cols = c(L2 = CELLTYPE_L2_COL, L3 = CELLTYPE_L3_COL)
        )
...
# ==============================================================================
# 8.7 L3 OFA (cell-level one-vs-rest)
# ==============================================================================

if (RUN_L3_OFA && (RUN_L3_OFA_VS_REST || RUN_L3_OFA_INTER_TISSUE)) {
  ...
  if (RUN_L3_OFA_VS_REST) {
    cat("\n--- L3 OFA: each L3 vs other cell types ---\n")
    for (l3_name in l3_types) {
      focal_cells <- colnames(obj)[as.character(obj@meta.data[[CELLTYPE_L3_COL]]) == as.character(l3_name)]
      rest_cells <- colnames(obj)[as.character(obj@meta.data[[CELLTYPE_L3_COL]]) != as.character(l3_name)]
      ...
      l3_ofa_vs_rest_all[[as.character(l3_name)]] <- c(ofa_res, list(comparison = "vs_other_cell_types"))
  ...
  if (RUN_L3_OFA_INTER_TISSUE) {
    cat("\n--- L3 OFA: same L3 across tissues ---\n")
    for (l3_name in l3_types) {
      l3_cells <- colnames(obj)[as.character(obj@meta.data[[CELLTYPE_L3_COL]]) == as.character(l3_name)]
      if (length(l3_cells) == 0) next
      l3_meta <- obj@meta.data[l3_cells, , drop = FALSE]
  ...
  l3_ofa_vs_rest_summary_df <- if (length(l3_vs_rows) > 0) dplyr::bind_rows(l3_vs_rows) else data.frame()
  l3_ofa_inter_tissue_summary_df <- if (length(l3_tissue_rows) > 0) dplyr::bind_rows(l3_tissue_rows) else data.frame()
...
add("## 7. L3 OFA + MiloPy Tissue DA"); add("")
if ((!is.data.frame(l3_ofa_vs_rest_summary_df) || nrow(l3_ofa_vs_rest_summary_df) == 0) &&
    (!is.data.frame(l3_ofa_inter_tissue_summary_df) || nrow(l3_ofa_inter_tissue_summary_df) == 0)) {
  add("L3 OFA not run or no valid contrasts passed cell-count thresholds."); add("")
} else {
  add("### 7.1 L3 vs other cell types"); add("")
  if (is.data.frame(l3_ofa_vs_rest_summary_df) && nrow(l3_ofa_vs_rest_summary_df) > 0) {
    add("| L3 subtype | n_focal | n_rest | Sig markers | Bubble plot | Output |")
    add("|---|---|---|---|---|---|")
    for (i in seq_len(nrow(l3_ofa_vs_rest_summary_df))) {
      rec <- l3_ofa_vs_rest_summary_df[i, , drop = FALSE]
      bubble_md <- if (!is.na(rec$bubble_png) && nzchar(rec$bubble_png)) sprintf("`%s`", rec$bubble_png) else "NA"
      add(sprintf("| %s | %d | %d | %d | %s | `%s` |",
                  rec$celltype_label, rec$n_focal, rec$n_rest, rec$n_sig, bubble_md, rec$output_dir))
    }
  } else {
    add("No L3-vs-rest OFA results generated.")
  }
  add("")
  add("### 7.2 Same L3 across tissues"); add("")
  if (is.data.frame(l3_ofa_inter_tissue_summary_df) && nrow(l3_ofa_inter_tissue_summary_df) > 0) {
    add("| L3 subtype | Comparison | n_focal | n_rest | Sig markers | Bubble plot | Output |")
    add("|---|---|---|---|---|---|---|")
    for (i in seq_len(nrow(l3_ofa_inter_tissue_summary_df))) {
      rec <- l3_ofa_inter_tissue_summary_df[i, , drop = FALSE]
      bubble_md <- if (!is.na(rec$bubble_png) && nzchar(rec$bubble_png)) sprintf("`%s`", rec$bubble_png) else "NA"
      add(sprintf("| %s | %s | %d | %d | %d | %s | `%s` |",
                  rec$celltype_label, rec$comparison, rec$n_focal, rec$n_rest, rec$n_sig, bubble_md, rec$output_dir))
    }
  } else {
    add("No same-L3-across-tissue OFA results generated.")
  }
  add("")
}
add("### 7.3 MiloPy tissue composition DA"); add("")
if (!RUN_MILOPY) {
  add("MiloPy disabled by configuration.")
} else {
  add("MiloPy runs after final `.h5ad` export using the pertpy environment and writes outputs under `reports/milopy/`.")
  add(sprintf("Configured levels: `%s`", paste(MILOPY_LEVEL_COLS, collapse = "`, `")))
  add("Expected outputs: `milopy_run.rds`, `milopy_level_summary.tsv`, `milopy_pairwise_summary.tsv`, `milopy_contrast_summary.tsv`, plus per-celltype plots and tables.")
}
add("")
...
if (!ENABLE_LLM) {
  ...
  add("### 8.3 CHOIR Cluster LLM (One-vs-Rest)"); add("")
  if (!exists("choir_llm_df") || nrow(choir_llm_df) == 0) {
...
add("- **L3 OFA:** cell-level one-vs-rest Wilcoxon marker discovery with multi-database over-representation analysis and bubble-plot summaries")
add("- **MiloPy:** pertpy/milo differential abundance across tissues using the final exported `.h5ad`")
...
add("- `reports/l3_ofa/` -- L3 one-vs-rest and same-L3-across-tissue OFA markers, enrichment tables, and bubble plots")
add("- `reports/milopy/` -- pertpy Milo differential abundance summaries and plots")
add("")
add("---")
...
tryCatch({
  ...
  if (RUN_MILOPY && exists("tc_run_tissue_milopy", mode = "function")) {
    tryCatch({
      cat("\n=== Running MiloPy tissue DA ===\n")
      milopy_dir <- file.path(rpt_dir, "milopy")
      milopy_run <- tc_run_tissue_milopy(
        h5ad_path = h5ad_out_path,
        output_dir = milopy_dir,
        script_path = MILOPY_SCRIPT_PATH,
        conda_exe = MILOPY_CONDA_EXE,
        env_prefix = MILOPY_ENV_PREFIX,
        level_cols = MILOPY_LEVEL_COLS,
        ...
        log_filename = MILOPY_LOG_FILENAME,
        stop_on_error = MILOPY_STOP_ON_ERROR
      )
      saveRDS(milopy_run, file.path(milopy_dir, "milopy_run.rds"))
      if (is.data.frame(milopy_run$level_summary) && nrow(milopy_run$level_summary) > 0) {
        fwrite(milopy_run$level_summary, file.path(milopy_dir, "milopy_level_summary.tsv"), sep = "\t")
      }
      if (is.data.frame(milopy_run$pairwise_summary) && nrow(milopy_run$pairwise_summary) > 0) {
        fwrite(milopy_run$pairwise_summary, file.path(milopy_dir, "milopy_pairwise_summary.tsv"), sep = "\t")
      }
      if (is.data.frame(milopy_run$contrast_summary) && nrow(milopy_run$contrast_summary) > 0) {
        fwrite(milopy_run$contrast_summary, file.path(milopy_dir, "milopy_contrast_summary.tsv"), sep = "\t")
      }
      cat(sprintf("[OK] MiloPy status: %s\n", milopy_run$status))
    }, error = function(e) {
      cat(sprintf("[WARN] MiloPy post-export step failed: %s\n", e$message))
    })
  }
...
cat("  reports/choir/choir_cluster_L2_counts.csv     <- CHOIR cluster x L2 cell counts\n")
cat("  reports/choir/choir_cluster_L3_counts.csv     <- CHOIR cluster x L3 cell counts\n")
cat("  reports/choir/choir_cluster_L2_L3_counts_long.csv <- long-format CHOIR cluster annotation counts\n")
cat("  reports/choir/ssgsea_{scores|z}_{method}.rds\n")
cat("  reports/choir/ofa_{cluster}/markers.csv + enrichment + integrated LLM [LLM-5]\n")
cat("  reports/l3_ofa/                     <- L3 OFA markers, enrichment, and bubble plots\n")
cat("  reports/l3_ofa_vs_rest_summary.tsv  <- L3 vs other cell types OFA summary\n")
cat("  reports/l3_ofa_inter_tissue_summary.tsv <- same-L3-across-tissues OFA summary\n")
cat("  reports/milopy/                     <- pertpy Milo tissue DA outputs\n")
cat("  reports/milopy/milopy_*_summary.tsv <- Milo level/pairwise/contrast summaries\n")
...
if (!is.null(milopy_run)) {
  cat(sprintf("  MiloPy run status: %s\n", milopy_run$status))
}
cat("\n")
