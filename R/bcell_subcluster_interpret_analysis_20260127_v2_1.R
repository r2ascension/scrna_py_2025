#!/usr/bin/env Rscript
# ==============================================================================
# B Cell Subcluster LLM Interpretation - PRODUCTION VERSION v2.6
# ==============================================================================
#
# Version: v2.6 (2026-01-27)
...
#   ✅ 7-8 database support (from v2.5)
#   ✅ Complete interpret() field extraction (from v2.4.2)
#
# Previous Features:
#   ✅ Multi-database support: GO BP/MF/CC, Hallmark, KEGG, CellMarker, PanglaoDB, B Cell Markers
...
OUTPUT_DIR <- "/home/h2048/data/R/0129/bcell_interpret_v2_4_1"
...
# MSigDB GO GMT Files (v2.6 NEW - for zero gene loss enrichment)
GMT_GO_ALL <- "/home/h2048/data/source/reference/MSigDB/c5.all.v2025.1.Hs.symbols.gmt"
# Or use individual files if you have them:
# GMT_GO_BP <- "/home/h2048/data/source/reference/MSigDB/c5.go.bp.v2025.1.Hs.symbols.gmt"
# GMT_GO_MF <- "/home/h2048/data/source/reference/MSigDB/c5.go.mf.v2025.1.Hs.symbols.gmt"
# GMT_GO_CC <- "/home/h2048/data/source/reference/MSigDB/c5.go.cc.v2025.1.Hs.symbols.gmt"
...
if (!is.null(bcell_markers_db)) {
  ...
  cat(sprintf(
    "[OK] Prepared B Cell TERM2GENE: %d pairs\n",
...
if (!is.null(cellmarker_db)) {
  ...
  bcell_related <- cellmarker_db %>%
    filter(
      grepl(
        "B cell|B-cell|Plasma|Plasmablast|Lymphocyte|Immune",
        cell_name,
        ignore.case = TRUE
      ) |
        grepl(
          "Blood|Lymph|Spleen|Bone marrow|Tonsil",
          tissue_type,
          ignore.case = TRUE
        )
    )
  ...
  cat(sprintf(
    "[OK] Found %d B cell/immune-related entries\n",
    nrow(bcell_related)
  ))
...
# ==============================================================================
# Load GO GMT Files (v2.6 NEW - Zero Gene Loss Method)
# ==============================================================================
...
# Split by ontology (term names have GOBP_, GOMF_, GOCC_ prefixes)
...
# ==============================================================================
# 富集分析 - GO (BP + MF + CC) - GMT-based (v2.6)
# ==============================================================================

cat("=== GO Enrichment (GMT-based, zero gene loss) ===\n")

# GO Biological Process (GMT-based enricher)
go_bp_enrich <- tryCatch(
  {
    ...
    compareCluster(
      gene ~ cluster,
      data = top_markers,
      fun = enricher, # ← Changed from enrichGO
      TERM2GENE = go_bp_gmt, # ← Use GMT data directly
      pvalueCutoff = 0.05,
      pAdjustMethod = "BH",
...
# GO Molecular Function (GMT-based enricher)
...
# GO Cellular Component (GMT-based enricher)
...
tryCatch(
  {
    ...
    cat(
      "**Databases (9 total):** B Cell Markers, CellMarker, PanglaoDB, GO BP/MF/CC, Hallmark, MSigDB KEGG\n\n"
    )
...
cat(
  "  - annotation_results.csv             Cell_Type + Markers (not key_processes!) ⭐\n"
)
cat(
...
cat(
  "  - figures/bcell_markers_dotplot.pdf  B cell marker enrichment (36 subtypes)\n"
)
...
cat(
  "  - figures/go_mf_dotplot.pdf          GO molecular function enrichment ⭐ NEW\n"
)
cat(
  "  - figures/go_cc_dotplot.pdf          GO cellular component enrichment ⭐ NEW\n"
)
