...
OUTPUT_DIR <- "/home/h2048/data/R/1218/harmony_subset_rogue_v2_6"
...
cat("========================================\n")
cat("Harmony + ROGUE Analysis (v2.6)\n")
cat("========================================\n")
...
#' CRITICAL: Pre-filter batches BEFORE NormalizeData
prefilter_batches_for_harmony <- function(obj, batch_col, celltype_name, min_cells_per_batch = 20) {
  cat("\n--- Harmony Pre-filter (BEFORE NormalizeData) ---\n")
  
  if (!batch_col %in% colnames(obj@meta.data)) {
    stop(sprintf("❌ batch_col '%s' not found (celltype=%s)", batch_col, celltype_name))
  }
  
  # Clean batch labels
  obj@meta.data[[batch_col]] <- trimws(as.character(obj@meta.data[[batch_col]]))
  bad <- is.na(obj@meta.data[[batch_col]]) | obj@meta.data[[batch_col]] == ""
  
  if (any(bad)) {
    cat(sprintf("⚠ Removing %d cells with NA/empty '%s'\n", sum(bad), batch_col))
    keep_cells <- rownames(obj@meta.data)[!bad]
    obj <- subset(obj, cells = keep_cells)
  }
  
  # Show batch distribution
  tab <- table(obj@meta.data[[batch_col]])
  cat("Batch table (post-clean):\n")
  print(tab)
  
  # Drop tiny batches
  tiny <- names(tab)[tab < min_cells_per_batch]
  if (length(tiny) > 0) {
    cat(sprintf("⚠ Dropping %d tiny batches (<%d cells): %s\n",
                length(tiny), min_cells_per_batch, paste(tiny, collapse = ", ")))
    
    # Save dropped batches for transparency
    dropped_batches <- data.frame(
      celltype = celltype_name,
      batch = tiny,
      n_cells = as.integer(tab[tiny]),
      stringsAsFactors = FALSE
    )
    
    dropped_file <- file.path(TAB_DIR, sprintf("%s_dropped_batches.csv", 
                                               gsub("[^A-Za-z0-9_-]", "_", celltype_name)))
    write.csv(dropped_batches, dropped_file, row.names = FALSE)
    cat(sprintf("  Saved dropped batches: %s\n", dropped_file))
    
    keep_cells <- rownames(obj@meta.data)[!(obj@meta.data[[batch_col]] %in% tiny)]
    obj <- subset(obj, cells = keep_cells)
  }
  
  # Check minimum batches
  nb <- length(unique(obj@meta.data[[batch_col]]))
  if (nb < 2) {
    stop(sprintf("❌ After prefilter, only %d batch left. Cannot run Harmony (celltype=%s)", 
                 nb, celltype_name))
  }
  
  cat(sprintf("✓ Prefilter kept %d cells, %d batches\n", ncol(obj), nb))
  obj
}

#' Align PCA embeddings (after RunPCA, before RunHarmony)
align_pca_embeddings <- function(obj, celltype_name) {
  cat("Aligning PCA embeddings...\n")
  
  if (!"pca" %in% names(obj@reductions)) {
    stop(sprintf("❌ PCA reduction not found (celltype=%s)", celltype_name))
  }
  ...
  obj[["pca"]]@cell.embeddings <- emb[cells, , drop = FALSE]
  
  cat(sprintf("✓ Aligned: %d cells × %d PCs\n",
              nrow(obj[["pca"]]@cell.embeddings),
              ncol(obj[["pca"]]@cell.embeddings)))
  
  obj
}

#' Strict Harmony with required params fallback
...
#' Improved clustering algorithm test
...
process_subset <- function(seurat_obj, celltype_name, celltype_filter, output_prefix, 
  ...
  # CRITICAL FIX: Pre-filter batches BEFORE NormalizeData
  seurat_sub <- prefilter_batches_for_harmony(
    seurat_sub,
    batch_col = BATCH_COL,
  ...
  # Now safe to proceed with normalization
  ...
  # Align PCA embeddings (after RunPCA, before RunHarmony)
  seurat_sub <- align_pca_embeddings(seurat_sub, celltype_name)
...
#' ROGUE with correct skipped combo export
compute_rogue_by_combo <- function(seurat_obj, dataset_col, group_col,
  ...
  # Correct skipped combo export logic
...
cat("7. Dropped batches (<20 cells) saved per celltype for transparency\n\n")
