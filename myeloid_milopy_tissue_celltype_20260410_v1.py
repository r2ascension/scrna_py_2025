...
"""Run Milo differential abundance analysis across tissues within each myeloid cell type.
...
- Runs pertpy Milo separately for each cell type, testing tissue contrasts using
  sample as the replicate unit.
- Writes both Milo's graph-aware `SpatialFDR` and standard BH-adjusted p-values.
- Also computes a global BH correction across all neighbourhood tests collected
  across cell types and contrasts.
...
LOGGER = logging.getLogger("myeloid_milo")


def patch_pertpy_rpy2_setup() -> None:
    """Patch pertpy Milo for rpy2>=3.6 where activate()/deactivate() are deprecated.

    pertpy 0.10.0 still calls numpy2ri.activate()/pandas2ri.activate(), which now
    raise a DeprecationWarning as an exception in rpy2 3.6.x. We replace the helper
    with a converter-based implementation that preserves the intended pandas/numpy
    conversions.
    """
...
@dataclass
class MiloRunConfig:
    input_h5ad: str = "/home/h2048/data/py/0402/myeloid_merge_schpl_v1_0/myeloid_reference_plus_query_schpl_v1_0.h5ad"
    output_dir: str = "/home/h2048/data/py/0410/myeloid_milopy_tissue_celltype_v1_0"
    sample_col: str = "sample"
    tissue_col: str = "tissue"
    celltype_col: str = "schpl_pred"
...
def parse_args() -> MiloRunConfig:
    ...
    parser.add_argument("--celltype-col", default=MiloRunConfig.celltype_col)
    parser.add_argument("--latent-key", default=MiloRunConfig.latent_key)
    parser.add_argument("--umap-key", default=MiloRunConfig.umap_key)
...
def lightweight_adata_from_h5ad(input_h5ad: Path, cfg: MiloRunConfig) -> ad.AnnData:
    ...
    try:
        missing_cols = [
            col for col in [cfg.sample_col, cfg.tissue_col, cfg.celltype_col] if col not in src.obs.columns
        ]
        if missing_cols:
            raise KeyError(f"Missing required obs columns: {missing_cols}")
        if cfg.latent_key not in src.obsm.keys():
            raise KeyError(f"Missing latent embedding key in obsm: {cfg.latent_key}")
        ...
        adata.obsm[cfg.latent_key] = np.asarray(src.obsm[cfg.latent_key])
        if cfg.umap_key in src.obsm.keys():
            adata.obsm[cfg.umap_key] = np.asarray(src.obsm[cfg.umap_key])
...
def clean_obs_columns(adata: ad.AnnData, cfg: MiloRunConfig) -> None:
    for col in [cfg.sample_col, cfg.tissue_col, cfg.celltype_col]:
        adata.obs[col] = adata.obs[col].astype(str).str.strip()
    adata.obs[cfg.celltype_col] = adata.obs[cfg.celltype_col].replace({"nan": "", "None": ""})
    adata.obs[cfg.tissue_col] = adata.obs[cfg.tissue_col].replace({"nan": "", "None": ""})
    adata.obs[cfg.sample_col] = adata.obs[cfg.sample_col].replace({"nan": "", "None": ""})


def summarize_celltypes(adata: ad.AnnData, cfg: MiloRunConfig) -> pd.DataFrame:
    obs = adata.obs[[cfg.celltype_col, cfg.sample_col, cfg.tissue_col]].copy()
    obs = obs.loc[
        ~obs[cfg.celltype_col].isin(cfg.exclude_celltypes)
        & (obs[cfg.sample_col] != "")
        & (obs[cfg.tissue_col] != "")
    ].copy()
    rows: list[dict] = []
    for cell_type, sub in obs.groupby(cfg.celltype_col, sort=False):
...
def prepare_celltype_subset(adata: ad.AnnData, cell_type: str, cfg: MiloRunConfig) -> tuple[ad.AnnData | None, dict]:
    subset = adata[adata.obs[cfg.celltype_col] == cell_type].copy()
...
def ensure_neighbors_and_umap(subset: ad.AnnData, cfg: MiloRunConfig) -> None:
    sc.pp.neighbors(subset, use_rep=cfg.latent_key, n_neighbors=cfg.n_neighbors, random_state=cfg.random_seed)
    if cfg.umap_key not in subset.obsm:
        sc.tl.umap(subset, random_state=cfg.random_seed)
        if "X_umap" in subset.obsm and cfg.umap_key != "X_umap":
            subset.obsm[cfg.umap_key] = subset.obsm["X_umap"].copy()
...
def compute_sample_fraction_table(
    ...
    celltype_counts = subset.obs.groupby(cfg.sample_col).size().rename("celltype_cells")
    plot_df = sample_meta.copy()
    plot_df["celltype_cells"] = plot_df[cfg.sample_col].map(celltype_counts).fillna(0).astype(int)
    ...
    plot_df["sample_fraction_pct"] = 100.0 * plot_df["celltype_cells"] / plot_df["total_lineage_cells"]
...
def draw_sample_fraction_violin(
    ...
    *,
    ...
    y_step = y_range * 0.10
    annotate_violin_significance(ax, stats_df, tissue_order, y_max=y_max, y_step=y_step, alpha=cfg.alpha)
...
def save_violin_outputs(
    ...
    plot_df = compute_sample_fraction_table(full_adata, subset, cell_type, cfg)
    ...
    if cfg.make_plots:
        ...
        draw_sample_fraction_violin(
            ...
            stats_df,
            cfg,
            title=f"{cell_type} sample-level abundance across tissues",
...
def plot_violin_overview(
    ...
    fig.suptitle("Myeloid cell-type abundance across tissues", fontsize=14, y=0.995)
    fig.tight_layout()
...
def run_da_nhoods_edger_compat(
    ...
    *,
    ...
    pertpy 0.10.0's bundled `da_nhoods()` currently breaks with modern rpy2 global
    conversion semantics. This function mirrors the pertpy logic while keeping R
    model objects as R objects and only converting pandas/numpy inputs/outputs at
    the boundaries.
    """
...
def run_milo_for_celltype(
    ...
    milo.build_nhood_graph(mdata, basis=cfg.umap_key)
    plot_df, plot_stats_df = save_violin_outputs(full_adata, subset, cell_type, cfg, celltype_dir)

    pair_summaries: list[dict] = []
    results: list[pd.DataFrame] = []
    tissue_pairs = build_tissue_pairs(subset, cfg)
    if not tissue_pairs:
        pair_summaries.append({"cell_type": cell_type, "status": "skipped", "reason": "no eligible tissue pairs"})
        ...
        if cfg.make_plots:
            try:
                plot_milo_graph(
                    ...
                    title=f"{cell_type}: {tissue_b} vs {tissue_a}",
                    alpha=cfg.alpha,
                )
...
def main() -> None:
    ...
    for cell_type in prefilter_celltypes:
        ...
        try:
            ...
            LOGGER.exception("Milo failed for %s", cell_type)
            pair_summaries.append(
                {
                    "cell_type": cell_type,
                    "status": "failed",
    ...
    if violin_tables:
        pd.concat(violin_tables, ignore_index=True).to_csv(output_dir / "sample_fraction_per_sample_all.csv", index=False)
    if violin_stats_tables:
        ...
        if cfg.make_plots:
            plot_violin_overview(violin_tables, violin_stats_tables, cfg, output_dir)

    if not all_results:
        LOGGER.warning("No Milo results were generated.")
        return

    combined = pd.concat(all_results, ignore_index=True)
    combined = apply_global_bh(combined, alpha=cfg.alpha)
    combined.to_csv(output_dir / "all_milo_results.csv", index=False)
    ...
    summary = (
        combined.groupby(["cell_type", "contrast", "tissue_case", "tissue_control"], dropna=False)
        .agg(
            n_nhoods=("nhood", "size"),
    ...
    summary.to_csv(output_dir / "contrast_level_summary.csv", index=False)

    LOGGER.info("Finished Milo run. Output directory: %s", output_dir)
    LOGGER.info("Combined results rows: %d", combined.shape[0])
    LOGGER.info("Contrasts with SpatialFDR hits: %d", int((summary["n_sig_spatialfdr"] > 0).sum()))
