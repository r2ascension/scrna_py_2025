...
seurat_obj <- GetSeurat(
  '/home/h2048/data/R/0103/ciliated/ciliated_bbknn_integrated.h5ad'
)
output_dir <- '/home/h2048/data/R/0105/ciliated'
dir.create(output_dir, recursive = TRUE)
setwd(output_dir)

# ===== Final Corrected Cell Type Annotation =====
# Date: 2025-01-05
# Changes: Clusters 3 & 7 moved to Developing Ciliated (based on UMAP)
...
# ===== Level 2: Functional Subtypes =====
annotation_level2 <- c(
  '0' = 'Immune-activated Ciliated',
  '1' = 'Developing Ciliated',
  '2' = 'Mature Ciliated',
  '3' = 'Developing Ciliated',
  '4' = 'Mature Ciliated',
  '5' = 'Developing Ciliated',
  '6' = 'Secretory-Ciliated',
  '7' = 'Developing Ciliated',
  '8' = 'Goblet',
  '9' = 'Mature Ciliated',
  '10' = 'Immune-activated Ciliated',
  '11' = 'Developing Ciliated', # ✅ CHANGED
  '12' = 'Pre-ciliated'
)

# ===== Level 3: Detailed Subtypes =====
annotation_level3 <- c(
  '0' = 'Ciliated (ISG15+/STAT1+)',
  '1' = 'Developing Ciliated (PIFO+/AGR2+)',
  '2' = 'Mature Ciliated (DNAH-high, SCGB1A1+)',
  '3' = 'Developing Ciliated (Mixed features)',
  '4' = 'Mature Ciliated (DNAH-highest, DNAAF1+)',
  '5' = 'Developing Ciliated (CD24-high)',
  '6' = 'Secretory-Ciliated (SCGB-dominant)',
  '7' = 'Developing Ciliated (TFF3+, PIFO-low)',
  '8' = 'Goblet (KRT5+/MUC5AC+)',
  '9' = 'Mature Ciliated (Multifunctional)',
  '10' = 'Ciliated (MHC-II-high)',
  '11' = 'Developing Ciliated (PIFO-high, AGR2/TFF3+)', # ✅ CHANGED
  '12' = 'Pre-ciliated (MCIDAS+/CCNO+)'
)

# ===== Apply annotations - CORRECT WAY =====
seurat_obj@meta.data$cell_type_level_2 <- annotation_level1[as.character(
  seurat_obj@meta.data$leiden_res1.0
)]
seurat_obj@meta.data$cell_type_level_3 <- annotation_level2[as.character(
  seurat_obj@meta.data$leiden_res1.0
)]
seurat_obj@meta.data$cell_type_level_4 <- annotation_level3[as.character(
  seurat_obj@meta.data$leiden_res1.0
)]
...
# Level 1 order
level1_order <- c('Ciliated', 'Goblet')

# Level 2 order (functional grouping)
level2_order <- c(
  'Pre-ciliated',
  'Developing Ciliated',
  'Mature Ciliated',
  'Secretory-Ciliated',
  'Immune-activated Ciliated',
  'Goblet'
)

# Level 3 order (detailed progression)
level3_order <- c(
  'Pre-ciliated (MCIDAS+/CCNO+)',
  'Developing Ciliated (CD24-high)',
  'Developing Ciliated (PIFO+/AGR2+)',
  'Developing Ciliated (Mixed features)',
  'Developing Ciliated (TFF3+, PIFO-low)',
  'Mature Ciliated (DNAH-high, SCGB1A1+)',
  'Mature Ciliated (DNAH-highest, DNAAF1+)',
  'Mature Ciliated (Multifunctional)',
  'Secretory-Ciliated (SCGB-dominant)',
  'Secretory-Ciliated (AGR2+/TFF3+)',
  'Ciliated (ISG15+/STAT1+)',
  'Ciliated (MHC-II-high)',
  'Goblet (KRT5+/MUC5AC+)'
)
...
seurat_obj@meta.data$cell_type_level_4 <- factor(
  seurat_obj@meta.data$cell_type_level_4,
  levels = level3_order
)
...
cat("\n========================================\n")
cat("FINAL CELL TYPE ANNOTATION SUMMARY\n")
cat("========================================\n\n")
...
summary_level2 <- seurat_obj@meta.data %>%
  ...
  summarise(
    n_cells = n(),
    clusters = paste(sort(unique(leiden_res1.0)), collapse = ', '),
    .groups = 'drop'
  ) %>%
  mutate(pct_total = n_cells / sum(n_cells) * 100) %>%
  arrange(match(cell_type_level_3, level2_order))

print(summary_level2)

cat("\n--- Cluster Composition by Subtype ---\n")
cat("Pre-ciliated: Cluster 12\n")
cat("Developing Ciliated: Clusters 1, 3, 5, 7\n")
cat("Mature Ciliated: Clusters 2, 4, 9\n")
cat("Secretory-Ciliated: Clusters 6, 11\n")
cat("Immune-activated Ciliated: Clusters 0, 10\n")
cat("Goblet: Cluster 8\n\n")

# ===== Visualization: Annotated UMAPs =====

# Level 2 UMAP
p_umap_level2 <- DimPlot(
  seurat_obj,
  ...
  scale_color_manual(
    values = c(
      'Pre-ciliated' = '#E41A1C',
      'Developing Ciliated' = '#377EB8',
      'Mature Ciliated' = '#4DAF4A',
      'Secretory-Ciliated' = '#FF7F00',
      'Immune-activated Ciliated' = '#984EA3',
      'Goblet' = '#A65628'
    )
  ) +
  ggtitle('Cell Type Annotation - Level 2 (Functional Subtypes)') +
  theme_minimal() +
  theme(
...
# Level 3 UMAP
p_umap_level3 <- DimPlot(
  seurat_obj,
  group.by = 'cell_type_level_4',
  label = TRUE,
  label.size = 3,
  repel = TRUE,
  pt.size = 0.5
) +
  ggtitle('Cell Type Annotation - Level 3 (Detailed Subtypes)') +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5, face = 'bold', size = 14),
...
# Save plots
output_dir <- 'ciliated_annotation_final'
dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)

ggsave(
  file.path(output_dir, 'umap_annotation_level2.pdf'),
  p_umap_level2,
  width = 10,
  height = 8,
  dpi = 300
...
ggsave(
  file.path(output_dir, 'umap_annotation_level3.pdf'),
  p_umap_level3,
  width = 12,
  height = 10,
  dpi = 300
...
# ===== Cluster-by-Cluster Annotation Table =====

annotation_table <- data.frame(
  Cluster = names(annotation_level1),
  Level_1 = annotation_level1,
  Level_2 = annotation_level2,
  Level_3 = annotation_level3,
  Key_Markers = c(
    '0: FOXJ1+/RSPH1+/HLA-DQA1+/ISG15+/STAT1+',
    '1: PIFO+/SNTN+/AGR2+/CD24+',
    '2: RSPH1+/DNAH5+/DNAH11+/SCGB1A1+',
    '3: FOXJ1+/RSPH1+/AGR2+/MUC5AC(low)',
    '4: DNAH5(highest)/DNAH11(highest)/DNAAF1(highest)',
    '5: CD24(highest)/FOXJ1+/PIGR+',
    '6: SCGB1A1(highest)/SCGB3A1+/BPIFA1+',
    '7: RSPH1+/TFF3(high)/PIFO(low)',
    '8: KRT5+/MUC5AC(highest)/SPDEF+',
    '9: DNAH5+/DNAH11+/SCGB1A1+/BPIFA1+',
    '10: HLA-DQA1(highest)/HLA-DQB1(highest)/FOXJ1+',
    '11: RSPH1+/AGR2(high)/TFF3+',
    '12: MCIDAS+/CCNO(high)/PCM1(high)/TOP2A+'
  ),
...
write.csv(
  annotation_table,
  file.path(output_dir, 'annotation_reference_table.csv'),
  row.names = FALSE
)

# ===== Developing Ciliated Cluster Details =====

cat("\n========================================\n")
cat("DEVELOPING CILIATED CLUSTERS (1, 3, 5, 7)\n")
cat("========================================\n\n")

developing_comparison <- data.frame(
  Feature = c('FOXJ1', 'PIFO', 'CD24', 'AGR2', 'TFF3', 'DNAH5'),
  Cluster_1 = c('88.3%', '95.3%', '99.4%', '97.8%', '16.5%', '94.8%'),
  Cluster_3 = c('61.7%', '24.0%', '90.0%', '88.2%', '24.6%', '74.1%'),
  Cluster_5 = c('44.4%', '11.8%', '74.8%↑', '49.3%', '25.2%', '69.4%'),
  Cluster_7 = c('69.3%', '1.2%↓', '91.1%', '70.5%', '74.4%↑', '83.4%'),
  stringsAsFactors = FALSE
)

print(developing_comparison)

cat("\nDeveloping Ciliated Progression:\n")
cat("Cluster 5 → Cluster 1 → Cluster 3/7 (mixed)\n")
cat("- Cluster 5: Earliest (CD24-dominant, low PIFO)\n")
cat("- Cluster 1: Active ciliogenesis (PIFO-high, AGR2+)\n")
cat("- Cluster 3: Transitional/heterogeneous\n")
cat(
  "- Cluster 7: Late developing (TFF3-high, PIFO-low, approaching maturity)\n\n"
)

# ===== Mature Ciliated Cluster Details =====

cat("========================================\n")
cat("MATURE CILIATED CLUSTERS (2, 4, 9)\n")
cat("========================================\n\n")

mature_comparison <- data.frame(
  Feature = c('DNAH5', 'DNAH11', 'DNAAF1', 'SCGB1A1', 'BPIFA1'),
  Cluster_2 = c('100%', '99.7%', '99.3%', '96.3%↑', '69.2%'),
  Cluster_4 = c('85.6%', '83.2%', '92.5%↑', '23.1%', '13.1%'),
  Cluster_9 = c('99.8%', '99.5%', '97.2%', '90.5%↑', '85.7%↑'),
  stringsAsFactors = FALSE
)

print(mature_comparison)

cat("\nMature Ciliated Characteristics:\n")
cat("- Cluster 2: Best % positive, high secretory markers\n")
cat("- Cluster 4: Highest expression levels (peak function)\n")
cat("- Cluster 9: Optimal balance (complete + multifunctional)\n\n")

# ===== Generate DotPlot with Proper Grouping =====

# Key markers organized by function
marker_dict <- list(
  'Pan-Ciliated' = c('FOXJ1', 'RSPH1', 'SNTN'),
  'Ciliogenesis' = c('PIFO', 'MCIDAS', 'CCNO'),
  'Mature Ciliated' = c('DNAH5', 'DNAH11', 'CFAP70'),
  'Development' = c('CD24', 'DNAAF1', 'PCM1'),
  'Secretory' = c('SCGB1A1', 'BPIFA1', 'TFF3', 'AGR2'),
  'Goblet' = c('MUC5AC', 'SPDEF'),
  'Basal' = c('KRT5', 'TP63'),
  'Immune' = c('HLA-DQA1', 'ISG15', 'STAT1')
)

p_dotplot_grouped <- DotPlot(
  seurat_obj,
...
print(p_dotplot_grouped)

ggsave(
  file.path(output_dir, 'dotplot_markers_grouped.pdf'),
  p_dotplot_grouped,
  width = 14,
  height = 8,
  dpi = 300
...
# ===== Stacked Bar Plot: Cluster Composition by Condition =====

if ('disease_status' %in% colnames(seurat_obj@meta.data)) {
  composition_data <- seurat_obj@meta.data %>%
    group_by(disease_status, cell_type_level_3) %>%
  ...
  p_composition <- ggplot(
    composition_data,
    aes(x = disease_status, y = pct, fill = cell_type_level_3)
  ) +
  ...
  ggsave(
    file.path(output_dir, 'composition_by_disease.pdf'),
    p_composition,
    width = 10,
    height = 7,
    dpi = 300
...
saveRDS(
  seurat_obj,
  file.path(output_dir, 'ciliated_final_20260105.rds')
)
GetH5ad(seurat_obj, 'ciliated_final_20260105.h5ad')
...
cat("========================================\n")
cat("Key Changes:\n")
cat("✓ Clusters 3 & 7 → Developing Ciliated\n")
cat("✓ All other annotations unchanged\n\n")
cat("Final Classification:\n")
cat("  Pre-ciliated: 1 cluster (12)\n")
cat("  Developing Ciliated: 4 clusters (1, 3, 5, 7)\n")
cat("  Mature Ciliated: 3 clusters (2, 4, 9)\n")
cat("  Secretory-Ciliated: 2 clusters (6, 11)\n")
cat("  Immune-activated: 2 clusters (0, 10)\n")
cat("  Goblet: 1 cluster (8)\n\n")
cat("Output files in:", output_dir, "\n")
cat("  - umap_annotation_level2.pdf\n")
cat("  - umap_annotation_level3.pdf\n")
cat("  - dotplot_markers_grouped.pdf\n")
cat("  - annotation_reference_table.csv\n")
cat("  - seurat_annotated_FINAL.rds\n")
cat("========================================\n")
...
# cluster_col <- "leiden_res1.0" # 如果你的cluster列不是这个名，改成对应列名
...
seurat_obj <- RunHarmony(
  ...
  theta = c(6), # Higher theta for more diverse clustering
...
# seurat_obj <- subset(
#   seurat_obj,
#   subset = seurat_clusters %in% c('27'),
#   invert = TRUE
# )
# saveRDS(seurat_obj, 'ciliated_20260103.rds')
...
seurat_obj$removal_reason <- 'Keep'
# Pure B cells (contamination)
seurat_obj$removal_reason[seurat_obj$leiden_res1.0 == '18'] <-
...
new_clusters <- as.character(seurat_obj$leiden_res1.0)
...
# ===== Ciliated Cell Annotation - Comprehensive Marker Visualization =====
# Purpose: Validate ciliated cell clusters using dotplot
# Author: r2end
# Date: 2025-01-05
...
# ===== 1. Define Comprehensive Marker Panel =====

# Hierarchical marker organization for ciliated cell annotation
marker_list <- list(
  # Pan-ciliated markers (should be positive in all ciliated subtypes)
  `Pan-Ciliated` = c(
    "FOXJ1", # master regulator of ciliogenesis
    "RSPH1", # radial spoke head protein 1
    "SNTN", # sentan (ciliary protein)
    "PIFO" # pifo (cilium formation)
  ),

  # Mature ciliated markers (high in cluster 4)
  `Mature Ciliated` = c(
    ...
    "RSPH10B" # radial spoke head protein 10B
  ),

  # Deuterosome/Centriole amplification markers (cluster 12)
  `Deuterosome` = c(
    "MCIDAS", # multicilin (master regulator)
    "CCNO", # cyclin O (drives centriole amplification)
    "FOXN4", # transcription factor for multiciliated cells
    "DEUP1", # deuterosome assembly protein
    "CEP152", # centrosomal protein 152
    "POC1A" # POC1 centriolar protein A
  ),

  # Ciliated developmental/assembly markers (cluster 5)
  `Ciliated Development` = c(
    "DNAAF1", # dynein axonemal assembly factor 1
    "PCM1", # pericentriolar material 1
    "CD24", # early differentiation marker
    "PIGR", # polymeric immunoglobulin receptor
    "BEST4" # bestrophin 4
  ),

  # Secretory cell markers (cluster 6)
  `Secretory` = c(
    "SCGB1A1", # secretoglobin 1A1 (club cell marker)
    "SCGB3A1", # secretoglobin 3A1
    "BPIFA1", # BPI fold containing family A1
    "BPIFB1", # BPI fold containing family B1
    "MUC5B" # mucin 5B
  ),

  # Goblet cell markers (cluster 8)
  `Goblet` = c(
    "MUC5AC", # mucin 5AC (goblet cell marker)
    "TFF3", # trefoil factor 3
    "SPDEF", # SAM pointed domain ETS factor
    "AGR2" # anterior gradient 2
  ),

  # Basal cell markers (cluster 8)
  `Basal` = c(
    "KRT5", # keratin 5
  ...
  # Proliferation markers (cluster 12)
  `Proliferation` = c(
    "MKI67", # Ki67
  ...
  # Immune activation markers (cluster 0)
  `Immune Activation` = c(
  ...
  # Epithelial general markers (quality control)
  `General Epithelial` = c(
...
# Assuming your Seurat object is named 'seurat_obj' or 'seurat_obj'
# Adjust the object name based on your actual variable
seurat_obj <- seurat_obj # or your actual object name

available_markers <- intersect(all_markers, rownames(seurat_obj))
missing_markers <- setdiff(all_markers, rownames(seurat_obj))

cat("=== Marker Availability Check ===\n")
cat(sprintf("Total markers defined: %d\n", length(all_markers)))
cat(sprintf("Available markers: %d\n", length(available_markers)))
...
# Update marker list to only include available markers
marker_list_filtered <- lapply(marker_list, function(genes) {
  intersect(genes, available_markers)
})

# Remove empty categories
marker_list_filtered <- marker_list_filtered[
  sapply(marker_list_filtered, length) > 0
]

# ===== 3. Generate DotPlot =====

# Set cluster identity (adjust column name if different)
Idents(seurat_obj) <- "leiden_res1.0" # or your actual clustering column

# Create dotplot
p_dotplot <- DotPlot(
  seurat_obj,
  features = marker_list_filtered,
  cols = c("lightgrey", "red"), # color scheme: low to high expression
  dot.scale = 8, # dot size scaling
  cluster.idents = FALSE, # keep original cluster order
  scale = TRUE, # scale expression within each gene
  col.min = -2, # minimum scaled expression
  col.max = 2 # maximum scaled expression
) +
  ...
  labs(
    title = "Ciliated Cell Marker Expression - Leiden Clustering (res=1.0)",
    x = "Marker Genes",
    y = "Cluster ID"
  ) +
  scale_y_discrete(limits = rev) # reverse y-axis for better visualization

# Display plot
print(p_dotplot)
write.csv(p_dotplot$data, 'dotplot.csv')
# ===== 4. Save High-Quality Output =====

# Create output directory
output_dir <- "ciliated_annotation_qc"
dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)

# Save PDF (vector format for publication)
ggsave(
  filename = file.path(output_dir, "ciliated_markers_dotplot.pdf"),
  plot = p_dotplot,
  width = 16,
  height = 10,
  dpi = 300,
...
# Save PNG (raster format for quick preview)
ggsave(
  filename = file.path(output_dir, "ciliated_markers_dotplot.png"),
  plot = p_dotplot,
  width = 16,
  height = 10,
  dpi = 300,
...
# ===== 5. Focus on Ciliated Clusters Only =====

# Subset to suspected ciliated clusters (0, 4, 5, 12)
ciliated_clusters <- c("0", "4", "5", "12")

seurat_ciliated <- subset(seurat_obj, idents = ciliated_clusters)

# Create focused dotplot for ciliated clusters
p_dotplot_focused <- DotPlot(
  seurat_ciliated,
  features = marker_list_filtered,
  cols = c("lightgrey", "red"),
  dot.scale = 10,
  ...
  labs(
    title = "Ciliated Clusters - Detailed Marker Profiling",
    subtitle = "Clusters: 0, 4, 5, 12",
    x = "Marker Genes",
    y = "Cluster ID"
  )

print(p_dotplot_focused)

# Save focused plot
ggsave(
  filename = file.path(output_dir, "ciliated_clusters_focused_dotplot.pdf"),
  plot = p_dotplot_focused,
  width = 16,
  height = 6,
  dpi = 300,
  device = "pdf"
...
# Calculate percentage of cells expressing key markers per cluster
key_markers <- c(
  "FOXJ1",
  "DNAH5",
  "RSPH1",
...
# Filter to available key markers
key_markers_available <- intersect(key_markers, rownames(seurat_obj))

# Extract expression data
expr_data <- FetchData(
  seurat_obj,
  vars = c(key_markers_available, "leiden_res1.0")
)

# Calculate percentage positive and mean expression per cluster
summary_stats <- expr_data %>%
  group_by(leiden_res1.0) %>%
  summarise(across(
    all_of(key_markers_available),
    list(
      pct_pos = ~ sum(. > 0) / n() * 100,
      mean_expr = ~ mean(.)
    ),
...
# Print summary for ciliated clusters
cat("\n=== Key Marker Expression Summary (Ciliated Clusters) ===\n\n")
print(summary_stats %>% filter(leiden_res1.0 %in% ciliated_clusters))

# Save summary table
write.csv(
  summary_stats,
  file = file.path(output_dir, "marker_expression_summary.csv"),
  row.names = FALSE
)

cat("\n✅ Analysis complete!\n")
cat(sprintf("Check results in: %s/\n", output_dir))

# ===== 7. Optional: Violin Plots for Top Ciliated Markers =====

# Generate violin plots for FOXJ1, DNAH5, RSPH1 across all clusters
if (all(c("FOXJ1", "DNAH5", "RSPH1") %in% rownames(seurat_obj))) {
  p_violin <- VlnPlot(
    seurat_obj,
    features = c("FOXJ1", "DNAH5", "RSPH1"),
    ncol = 3,
    pt.size = 0.1,
    cols = NULL # use default colors
  ) +
    theme(
      axis.text.x = element_text(angle = 45, hjust = 1),
  ...
  ggsave(
    filename = file.path(output_dir, "ciliated_markers_violin.pdf"),
    plot = p_violin,
    width = 14,
    height = 5,
    dpi = 300
...
# ===== 8. Optional: Feature Plot on UMAP =====

# Visualize key markers on UMAP
if (
  all(c("FOXJ1", "MCIDAS", "SCGB1A1", "HLA-DQA1") %in% rownames(seurat_obj))
) {
  p_feature <- FeaturePlot(
    seurat_obj,
    features = c("FOXJ1", "MCIDAS", "SCGB1A1", "HLA-DQA1"),
    ncol = 2,
    pt.size = 0.5,
  ...
  ggsave(
    filename = file.path(output_dir, "key_markers_umap.pdf"),
    plot = p_feature,
    width = 12,
    height = 10,
    dpi = 300
