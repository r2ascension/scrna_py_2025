#!/usr/bin/env Rscript
# ==============================================================================
# B Cell Tissue Comparison Pipeline v2.3.0 - PRODUCTION
# ==============================================================================
#
# Purpose:
...
if (!exists("PIPELINE_VERSION_LABEL"))    PIPELINE_VERSION_LABEL <- "v2.3.0"
...
if (!exists("GENERATED_BY_LABEL"))        GENERATED_BY_LABEL <- "bcell_tissue_comparison_v2_5_20260402.R"
...
if (!exists("OUTPUT_DIR")) {
  OUTPUT_DIR <- "/home/h2048/data/R/0402/bcell_tissue_comparison_v2_3_0"
}
...
# ----- 4.2 Pseudobulk aggregation (global cache; sparse until DESeq2) -----
build_pseudobulk_cache <- function(obj, group_col) {
  ...
  meta_sub$pb_group <- paste(meta_sub[[SAMPLE_COL]], meta_sub[[TISSUE_COL]], meta_sub$group_value, sep = "__")
  ...
  pb_counts <- counts_mat %*% cell_to_pb
...
aggregate_pseudobulk <- function(pseudobulk_cache, group_value,
                                 min_total_cells = 30L) {
...
build_gene_pathway_map <- function(enrich_obj, gene_fc = NULL, n_terms = 8L,
  ...
  map_df %>%
    ...
    group_by(gene) %>%
    slice_head(n = max_pathways_per_gene) %>%
    ungroup() %>%
...
run_pairwise_tissue_comparison <- function(obj, group_col, group_values, level_name,
  ...
  if (!is.null(pseudobulk_cache)) {
    cat(sprintf("[INFO] Pseudobulk cache [%s]: %d aggregated sample x tissue x group columns\n\n",
      ...
      for (dir_name in c("up", "down")) {
        ...
        if (ENABLE_LLM) {
          ...
          if (!is.null(best_er)) {
            ...
            gene_pathway_map <- build_gene_pathway_map(best_er, gene_fc = gene_fc)
            gene_pathway_map_text <- paste(format_gene_pathway_map_text(gene_pathway_map), collapse = "\n")
...
matrix_to_scipy_csr <- function(mat, scipy_sparse, np) {
  mat_t <- as(Matrix::t(as(mat, "dgCMatrix")), "dgCMatrix")
  scipy_sparse$csr_matrix(
    reticulate::tuple(
      np$array(as.numeric(mat_t@x), dtype = np$float32),
      np$array(as.integer(mat_t@i), dtype = np$int32),
      np$array(as.integer(mat_t@p), dtype = np$int32)
    ),
...
if (!ENABLE_LLM) {
  ...
  if (nrow(agent_structured_df) == 0) {
    ...
    add("- LLM emphasis: only validated top-|log2FC| gene-to-pathway hits are used for explicit mapping claims.")
...
add("- **Exploratory output labels:** `analysis_tier=exploratory_cell_level`, `statistical_unit=cell`, `replicate_model=none`")
...
cat("  */validated_gene_pathway_map_{up,down}.csv <- validated top-|log2FC| gene to pathway hits\n")
