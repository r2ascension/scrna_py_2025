...
Version: 2.5.3-MERGED-PROD-FIXED-LOADABLE (Flexible Model Loading)
Date: 2026-02-07
...
- H5AD Compatibility: String dtype conversion for older anndata versions.
- FLEXIBLE MODEL LOADING: Multiple scenarios supported.

Usage:
    # Train from scratch:
    python script.py
    
    # Load model only (verify structure):
    python script.py --mode load --model_dir /path/to/scanvi_model
    
    # Load model + view training results:
    python script.py --mode load \\
        --model_dir /path/to/scanvi_model \\
        --trained_h5ad /path/to/trained.h5ad
    
    # Load model + predict on new query:
    python script.py --mode load \\
        --model_dir /path/to/scanvi_model \\
        --query_h5ad /path/to/new_query.h5ad
...
print("=" * 80)
print("MERGED scVI+scANVI Training (v2.5.3-PROD-FIXED-LOADABLE)")
print("=" * 80)
...
parser = argparse.ArgumentParser(description="scVI+scANVI Training/Loading Pipeline")
parser.add_argument(
    "--mode",
    type=str,
    choices=["train", "load"],
    default="train",
    help="Mode: 'train' (train from scratch) or 'load' (load existing model)"
)
parser.add_argument(
    "--model_dir",
    type=str,
    default=None,
    help="Path to saved scANVI model directory (required for --mode load)"
)
parser.add_argument(
    "--trained_h5ad",
    type=str,
    default=None,
    help="Path to trained h5ad (optional, for viewing training results)"
)
parser.add_argument(
    "--query_h5ad",
    type=str,
    default=None,
    help="Path to new query h5ad (optional, for prediction/transfer learning)"
)
...
MODE = ''
LOAD_MODEL_DIR = ''
LOAD_TRAINED_H5AD = args.trained_h5ad
LOAD_QUERY_H5AD = args.query_h5ad

print(f"\nMode: {MODE.upper()}")
if MODE == "load":
    if not LOAD_MODEL_DIR:
        raise ValueError("--mode load requires --model_dir!")
    print(f"  Model directory: {LOAD_MODEL_DIR}")
    if LOAD_TRAINED_H5AD:
        print(f"  Trained h5ad: {LOAD_TRAINED_H5AD}")
    if LOAD_QUERY_H5AD:
        print(f"  Query h5ad: {LOAD_QUERY_H5AD}")
...
OUTPUT_DIR = "/home/h2048/data/py/0208/merged_scanvi_L2_prod_v1"
...
if MODE == "load":
    # ========================================================================
    # LOAD MODE: Load existing model and optionally process data
    # ========================================================================
    
    print("\n" + "=" * 80)
    print("MODE: LOAD EXISTING MODEL")
    print("=" * 80)
    
    # --- Load scANVI model (always required) ---
    print(f"\n[Step 1] Loading scANVI model from: {LOAD_MODEL_DIR}")
    
    # Create minimal adata for model loading
    # scANVI.load() needs an adata, but we can provide a dummy one if no h5ad specified
    if LOAD_TRAINED_H5AD:
        print(f"  Using trained h5ad: {LOAD_TRAINED_H5AD}")
        adata_for_model = sc.read_h5ad(LOAD_TRAINED_H5AD)
    elif LOAD_QUERY_H5AD:
        print(f"  Using query h5ad: {LOAD_QUERY_H5AD}")
        adata_for_model = sc.read_h5ad(LOAD_QUERY_H5AD)
    else:
        print(f"  No h5ad provided - loading model structure only")
        # For model-only loading, we need to create a dummy adata
        # The model will load but won't be immediately usable until given real data
        adata_for_model = None
    
    # Load model
    if adata_for_model is not None:
        scanvi_model = scvi.model.SCANVI.load(LOAD_MODEL_DIR, adata=adata_for_model)
        print("  ✅ Model loaded successfully!")
    else:
        # Model-only mode: just verify the model exists
        model_path = Path(LOAD_MODEL_DIR)
        if not model_path.exists():
            raise FileNotFoundError(f"Model directory not found: {LOAD_MODEL_DIR}")
        if not (model_path / "model.pt").exists():
            raise FileNotFoundError(f"model.pt not found in {LOAD_MODEL_DIR}")
        print("  ✅ Model structure verified (will need adata to activate)")
        scanvi_model = None
    
    # --- Load UMAP operator if exists ---
    umap_op_path = Path(LOAD_MODEL_DIR).parent / "umap_operator.joblib"
    if umap_op_path.exists():
        print(f"\n[Step 2] Loading UMAP operator: {umap_op_path}")
        umap_op = joblib.load(umap_op_path)
        print("  ✅ UMAP operator loaded!")
    else:
        print(f"\n[Step 2] UMAP operator not found at: {umap_op_path}")
        umap_op = None
    
    # --- Load training config if exists ---
    config_path = Path(LOAD_MODEL_DIR).parent / "training_config.json"
    if config_path.exists():
        print(f"\n[Step 3] Loading training config: {config_path}")
        with open(config_path, 'r') as f:
            training_config = json.load(f)
        print("  ✅ Training config loaded!")
        print(f"     Training date: {training_config.get('timestamp', 'Unknown')}")
        print(f"     HVG method: {training_config.get('hvg_method', 'Unknown')}")
        print(f"     Latent dims: {training_config.get('scvi_latent', 'Unknown')}")
    else:
    ...
    if LOAD_TRAINED_H5AD:
        ...
        # ====================================================================
        print("SCENARIO: Viewing Training Results")
        print("=" * 80)
        
        adata = adata_for_model
        
        print(f"\nTrained data shape: {adata.shape}")
        print(f"Available obsm keys: {list(adata.obsm.keys())}")
        
        # Check latent representation
        if "X_scANVI_L2" in adata.obsm:
            print(f"\n✓ Latent representation: {adata.obsm['X_scANVI_L2'].shape}")
        
        # Check UMAP
        if "X_umap_scanvi" in adata.obsm:
            print(f"✓ UMAP coordinates: {adata.obsm['X_umap_scanvi'].shape}")
        
        # Check predictions
        if "L2_scanvi_pred" in adata.obs:
            n_types = adata.obs['L2_scanvi_pred'].nunique()
            print(f"✓ Predictions: {n_types} cell types")
            print("\nPrediction distribution (top 10):")
            for ct, count in adata.obs['L2_scanvi_pred'].value_counts().head(10).items():
                pct = count / adata.n_obs * 100
                print(f"  {ct}: {count:,} ({pct:.1f}%)")
        
        # Check confidence
        if "L2_scanvi_confidence" in adata.obs:
            conf = adata.obs['L2_scanvi_confidence']
            print(f"\n✓ Confidence scores:")
        ...
        # ====================================================================
        # SCENARIO 2: Predict on new query data
        # ====================================================================
        print("SCENARIO: Predicting on New Query Data")
        print("=" * 80)
        
        adata = adata_for_model
        
        print(f"\nQuery data shape: {adata.shape}")
        print(f"Query obs columns: {list(adata.obs.columns[:10])}...")
        
        # Generate predictions
        print("\n[Prediction] Generating predictions for query data...")
        adata.obsm["X_scANVI_L2_query"] = scanvi_model.get_latent_representation(adata)
        adata.obs["L2_scanvi_pred_query"] = scanvi_model.predict(adata)
        
        # Soft probabilities
        proba = scanvi_model.predict(adata, soft=True).astype(np.float32)
        adata.obsm["proba_L2_scanvi_query"] = proba
        adata.obs["L2_scanvi_confidence_query"] = np.max(proba, axis=1)
        
        print("  ✅ Predictions generated!")
        
        # Show prediction summary
        print("\nQuery prediction distribution (top 10):")
        for ct, count in adata.obs['L2_scanvi_pred_query'].value_counts().head(10).items():
            pct = count / adata.n_obs * 100
            print(f"  {ct}: {count:,} ({pct:.1f}%)")
        
        conf = adata.obs['L2_scanvi_confidence_query']
        print(f"\nQuery confidence:")
        print(f"  Mean: {conf.mean():.3f}")
        print(f"  Median: {conf.median():.3f}")
        print(f"  High (>0.9): {(conf > 0.9).sum():,} ({(conf > 0.9).mean()*100:.1f}%)")
        
        # Project to UMAP if operator exists
        if umap_op:
            print("\n[UMAP] Projecting query to reference UMAP space...")
            adata.obsm["X_umap_scanvi_query"] = umap_op.transform(adata.obsm["X_scANVI_L2_query"])
            print("  ✅ UMAP projection complete!")
        
        # Save query results
        query_output = output_dir / "query_predictions.h5ad"
        print(f"\n[Save] Saving query results: {query_output}")
        
        # Convert string dtype for compatibility
        for col in adata.obs.columns:
            if pd.api.types.is_string_dtype(adata.obs[col]):
                adata.obs[col] = adata.obs[col].astype("object")
        
        adata.write_h5ad(query_output, compression="gzip")
        print("  ✅ Query results saved!")
        
        print("\n" + "=" * 80)
        print("Query prediction complete!")
        print("=" * 80)
        
    else:
        # ====================================================================
        # SCENARIO 3: Model-only mode (for interactive use)
        # ====================================================================
        print("SCENARIO: Model-Only Mode (Interactive)")
        print("=" * 80)
        
        print("\n⚠️  No data provided. Model structure loaded for interactive use.")
        print("\nTo use the model, provide data via one of these options:")
        print("\n1. View training results:")
        print("   python script.py --mode load --model_dir /path/to/model \\")
        print("       --trained_h5ad /path/to/trained.h5ad")
        
        print("\n2. Predict on new query:")
        print("   python script.py --mode load --model_dir /path/to/model \\")
        print("       --query_h5ad /path/to/new_query.h5ad")
        
        print("\n3. Interactive Python session:")
        print("   >>> adata = sc.read_h5ad('your_data.h5ad')")
        print("   >>> model = scvi.model.SCANVI.load('{0}', adata=adata)".format(LOAD_MODEL_DIR))
        print("   >>> predictions = model.predict(adata)")
        
        if umap_op:
            print("\n✓ UMAP operator available for projections")
        if training_config:
        ...
        print("\n" + "=" * 80)
        print("Model verified. Ready for interactive use.")
        print("=" * 80)
    ...
    if scanvi_model is not None:
        ...
        print("   latent = scanvi_model.get_latent_representation(adata)")
        
        print("\n2. Make predictions:")
        print("   predictions = scanvi_model.predict(adata)")
        print("   probabilities = scanvi_model.predict(adata, soft=True)")
        
        print("\n3. Transfer learning (requires counts layer):")
        print("   query_model = scvi.model.SCANVI.load_query_data(")
        print("       query_adata,")
        print("       reference_model=scanvi_model,")
        print("       freeze_dropout=True")
        print("   )")
        print("   query_model.train(max_epochs=100)")
        
        if umap_op:
            print("\n4. Project to UMAP:")
            print("   new_latent = scanvi_model.get_latent_representation(new_adata)")
            print("   new_umap = umap_op.transform(new_latent)")
    
    print("\n" + "=" * 80)
    print("[SUCCESS] Load operation complete!")
    print("=" * 80)
    
    # Store in global for interactive use
    if adata is not None:
        globals()['adata'] = adata
    if scanvi_model is not None:
        globals()['scanvi_model'] = scanvi_model
    if umap_op is not None:
        globals()['umap_op'] = umap_op
    if training_config is not None:
        globals()['training_config'] = training_config

else:
    # ========================================================================
    # TRAIN MODE: Train from scratch
    # ========================================================================
    ...
    # ===== CRITICAL FIX: Convert string columns to object for h5ad compatibility =====
    print("\n  -> Converting string dtype columns to object for h5ad compatibility...")
    string_cols_converted = []
    for col in adata.obs.columns:
        if pd.api.types.is_string_dtype(adata.obs[col]):
            adata.obs[col] = adata.obs[col].astype("object")
            string_cols_converted.append(col)

    if string_cols_converted:
        print(f"     Converted {len(string_cols_converted)} columns:")
        for col in string_cols_converted[:5]:
            print(f"       - {col}")
        if len(string_cols_converted) > 5:
            print(f"       ... and {len(string_cols_converted) - 5} more")

    # 3. H5AD
    out_path = output_dir / "merged_scanvi_L2_prod.h5ad"
    print(f"\n  -> Saving h5ad: {out_path}")
    adata.write_h5ad(out_path, compression="gzip")
    file_size = out_path.stat().st_size / 1024**3
    print(f"     ✅ Saved ({file_size:.2f} GB)")

    print("\n" + "=" * 80)
    print(f"[SUCCESS] Training Pipeline Finished!")
    print("=" * 80)
    print(f"\nOutput directory: {OUTPUT_DIR}")
    print(f"\nGenerated files:")
    print(f"  - scanvi_model/              (scANVI model)")
    print(f"  - umap_operator.joblib       (UMAP operator for consistency)")
    print(f"  - training_config.json       (pipeline configuration)")
    print(f"  - hvg_genes.txt              (selected HVG list)")
    print(f"  - merged_scanvi_L2_prod.h5ad ({file_size:.2f} GB)")
    print("\n" + "=" * 80)