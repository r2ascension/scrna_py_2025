#!/usr/bin/env Rscript
# ==============================================================================
# B Cell Tissue Comparison Pipeline v2.6.3 Wrapper
# ==============================================================================
#
# Review-hardened wrapper:
#   - explicit named config list instead of scattered implicit globals
#   - explicit previous-run preflight before shared-engine execution
#   - separate OUTPUT_DIR from PREVIOUS_OUTPUT_DIR by default
#   - configurable REQUIRE_LLM gate instead of unconditional hard-stop
#   - wrapper-owned env loading / summary writing with engine duplication disabled
#
# Date: 2026-04-11
# ==============================================================================
...
PIPELINE_CONFIG <- list(
  ...
  PIPELINE_VERSION_LABEL = "v2.6.3",
  PIPELINE_SUBTITLE = paste(
    "B Cell Tissue Comparison v2.6.3",
    "(review-hardened wrapper, explicit preflight/config)"
  ),
  REPORT_TITLE = "# B Cell Tissue Comparison Report (Normal Respiratory Tract)",
  GENERATED_BY_LABEL = "bcell_tissue_comparison_v2_6_3_20260411.R",
  FINAL_FILE_PREFIX = "bcell_tissue_comparison_final",
  LINEAGE_COMPLETION_BANNER = "B CELL TISSUE COMPARISON COMPLETE (v2.6.3)",
  H5AD_PATH = "/home/h2048/data/py/0203/bcell_scarches_v4_1/results/scarches_package/bcell_reference_20260203.h5ad",
  OUTPUT_DIR = "/home/h2048/data/R/0411/bcell_tissue_comparison_v2_6_3_20260411",
...
PIPELINE_CONFIG$PIPELINE_CHANGELOG_LINES <- c(
  "  [ARCH-1] Wrapper now uses an explicit named config list and validates required resume inputs up front.",
  "  [ARCH-2] Previous-run preflight is executed before sourcing the shared engine and written to reports/previous_run_summary.tsv.",
  "  [ARCH-3] OUTPUT_DIR and PREVIOUS_OUTPUT_DIR are separated by default; in-place resume requires explicit opt-in.",
  "  [ARCH-4] LLM enforcement is now controlled by REQUIRE_LLM instead of an unconditional hard-stop baked into the wrapper.",
