#!/usr/bin/env python3
"""
cNMF Batch Production Pipeline v1.1.1 FIXED
============================================

COMPREHENSIVE FIXES (based on v1.2.2):
- Enhanced distance matrix loading (dt_0_1 and dt_0.1 support)
- K-value selection metrics system (4 metrics + auto-recommendation)
- Improved visualization error handling
- Better file pattern matching
- Production-grade robustness

Multi-cell-type workflow:
- Processes T_cells, B_cells, Myeloid, Stromal_Vascular
- Main files + subclustering results
- Two-track cNMF: uncorrected + batch-aware HVG

Author: r2end
Date: 2024-12-24
Version: v1.1.1 (FIXED - Multi-Cell-Type)
"""
...
BASE_OUTPUT_DIR = Path("/home/h2048/data/py/1217/cnmf_batch_production_v1_1_1")
...
def load_gep_matrix(base_dir: Path, name: str, k: int) -> Optional[np.ndarray]:
    ...
    try:
        ...
        if not np.isfinite(mat).all():
            logger.warning(f"    ⚠️  Cleaning non-finite values...")
...
# ============================================================================
# K-VALUE METRICS (WITH AUTO-RECOMMENDATION)
# ============================================================================
...
def calculate_k_stability_metrics(
    ...
    """Calculate comprehensive K-value metrics"""
    logger.info(f"\n" + "="*70)
...
def generate_k_selection_recommendation(
    ...
    """Generate K-value recommendation"""
    logger.info(f"\n" + "="*70)
    ...
    if len(valid_k) < 3:
        ...
        recommendation["reasoning"].append("Insufficient data")
    ...
    if entropy_elbow_idx > 0:
        ...
        logger.info(f"  Entropy elbow at K={entropy_elbow_k}")
    ...
    if candidate_k:
        ...
        recommendation["recommended_k"] = recommended_k
        recommendation["confidence"] = "low"
        recommendation["reasoning"].append("Using middle K as fallback")
        logger.info(f"\n  ⚠️  Fallback K: {recommended_k}")
...
def plot_k_stability_summary(
    ...
    """Plot K-value metrics with recommendation"""
    ...
    if valid:
        ...
        if recommended_k in k_e:
            ...
            ax.plot(recommended_k, e_vals[rec_idx], 'r*', markersize=20)
        
        ax.set_xlabel('K', fontsize=10)
        ax.set_ylabel('Mean Usage Entropy', fontsize=10)
        ax.set_title('Usage Diversity', fontsize=11, weight='bold')
        ax.grid(alpha=0.3)
    ...
    if valid:
        ...
        if recommended_k in k_d:
            ...
            ax.axvline(recommended_k, color='red', linestyle='--', linewidth=2, alpha=0.7)
            ax.plot(recommended_k, d_vals[rec_idx], 'r*', markersize=20)
        
        ax.set_xlabel('K', fontsize=10)
        ax.set_ylabel('Mean Pairwise Distance', fontsize=10)
        ax.set_title('GEP Separation', fontsize=11, weight='bold')
        ax.grid(alpha=0.3)
    ...
    if valid:
        ...
        ax.plot(k_s, s_vals, 'o-', linewidth=2, markersize=8, color='green')
        ax.axhline(0.5, color='gray', linestyle=':', linewidth=1, alpha=0.5)
        
        if recommended_k in k_s:
            rec_idx = k_s.index(recommended_k)
            ax.axvline(recommended_k, color='red', linestyle='--', linewidth=2, alpha=0.7)
            ax.plot(recommended_k, s_vals[rec_idx], 'r*', markersize=20)
        
        ax.set_xlabel('K', fontsize=10)
        ax.set_ylabel('Usage Sparsity (Gini)', fontsize=10)
        ax.set_title('Usage Specificity', fontsize=11, weight='bold')
        ax.grid(alpha=0.3)
...
def plot_local_density_histogram(
    ...
    try:
        ...
        if n_neighbors < 1:
            logger.warning(f"    ⚠️  Not enough spectra")
            return False
...
def main():
    ...
    logger.info("="*70)
    logger.info("cNMF BATCH PRODUCTION PIPELINE v1.1.1 FIXED")
    logger.info("="*70)
