...
INPUT_H5AD = os.getenv(
    "CELLTYPE_SUBSET_INPUT_H5AD",
    "/home/h2048/data/py/0127/scarches_mapping_FIXED_v1_2/query_mapped_to_reference.h5ad",
)
OUTPUT_DIR = os.getenv(
    "CELLTYPE_SUBSET_OUTPUT_DIR",
...
# ===== Optional metadata-driven COVID filtering =====
FILTER_COVID_RELATED = _env_flag("CELLTYPE_SUBSET_FILTER_COVID_RELATED", False)
COVID_FILTER_PRIMARY_COLS = _env_csv("CELLTYPE_SUBSET_COVID_PRIMARY_COLS", ("disease",))
COVID_FILTER_SECONDARY_COLS = _env_csv("CELLTYPE_SUBSET_COVID_SECONDARY_COLS", ("COVID_status",))
COVID_FILTER_DATASET_COLS = _env_csv("CELLTYPE_SUBSET_COVID_DATASET_COLS", ("dataset",))
COVID_FILTER_ALLOW_DATASET_FALLBACK = _env_flag(
    "CELLTYPE_SUBSET_ALLOW_DATASET_FALLBACK",
    False,
)
COVID_FILTER_REPORT_COLUMNS = tuple(
    dict.fromkeys(COVID_FILTER_PRIMARY_COLS + COVID_FILTER_SECONDARY_COLS + COVID_FILTER_DATASET_COLS)
)
...
# ===== Minimum Cells =====
MIN_CELLS_PER_TYPE = int(os.getenv("CELLTYPE_SUBSET_MIN_CELLS_PER_TYPE", "50"))
...
config = {
    ...
    'filter_covid_related': FILTER_COVID_RELATED,
    'covid_filter_primary_cols': list(COVID_FILTER_PRIMARY_COLS),
    'covid_filter_secondary_cols': list(COVID_FILTER_SECONDARY_COLS),
    'covid_filter_dataset_cols': list(COVID_FILTER_DATASET_COLS),
    'covid_filter_allow_dataset_fallback': COVID_FILTER_ALLOW_DATASET_FALLBACK,
...
covid_filter_debug_path = output_dir / "covid_filter_debug.csv"
covid_filter_summary_path = output_dir / "covid_filter_summary.csv"

# 0. Remove COVID-related cells (optional)
if FILTER_COVID_RELATED:
    print("\n0. Removing COVID-related cells (metadata-driven)")
    covid_debug = build_covid_filter_debug_frame(
        adata.obs,
        primary_cols=COVID_FILTER_PRIMARY_COLS,
        secondary_cols=COVID_FILTER_SECONDARY_COLS,
        dataset_cols=COVID_FILTER_DATASET_COLS,
        allow_dataset_fallback=COVID_FILTER_ALLOW_DATASET_FALLBACK,
    )
    covid_mask = covid_debug["remove_covid_related"].astype(bool)
    n_covid = int(covid_mask.sum())
    covid_summary = summarize_covid_filter_debug_frame(covid_debug)

    covid_debug.to_csv(covid_filter_debug_path)
    covid_summary.to_csv(covid_filter_summary_path, index=False)

    print(f"   COVID-related cells: {n_covid:,} ({n_covid / max(n_cells_before, 1) * 100:.1f}%)")
    print(covid_summary.to_string(index=False))

    value_count_tables = build_removed_kept_value_counts(
        adata.obs,
        covid_mask,
        columns=COVID_FILTER_REPORT_COLUMNS,
    )
    for col, counts_df in value_count_tables.items():
        counts_path = output_dir / f"covid_filter_value_counts__{col}.csv"
        counts_df.to_csv(counts_path)
        print(f"   Saved value counts: {counts_path.name}")

    if n_covid > 0:
        adata = adata[~covid_mask].copy()
        print(f"   Remaining after COVID filter: {adata.n_obs:,} cells")
    else:
        print("   No COVID-related cells detected; keeping all cells")
else:
    print("\n0. COVID filtering disabled")
...
if summary_df.empty:
    if SUBSET_MAJOR:
        summary_df = pd.DataFrame(columns=['group', 'cells', 'unique_types', 'file', 'size_mb'])
    else:
        summary_df = pd.DataFrame(columns=['celltype', 'cells', 'file', 'size_mb'])
...
if FILTER_COVID_RELATED:
    report.append(f"  COVID filter debug: {covid_filter_debug_path.name}")
    report.append(f"  COVID filter summary: {covid_filter_summary_path.name}")
...
total_size_mb = float(summary_df['size_mb'].sum()) if 'size_mb' in summary_df.columns else 0.0
report.append(f"  Total size: {total_size_mb:.1f} MB")

report.append(f"\n[ Cell Distribution ]")
if len(summary_df) == 0:
    report.append("  No subset files were created after filtering and min-cell thresholds.")
else:
