...
seurat_obj <- readRDS('/home/h2048/data/R/1221/per_celltype_harmony_rogue/seurat_objects/Basal_harmony.rds')
output_dir <- '/home/h2048/data/R/1221/basal'
...
seurat_obj <- FindNeighbors(seurat_obj, reduction = "harmony", dims = 1:30,verbose = FALSE)
seurat_obj <- FindClusters(seurat_obj, res = 1, algorithm  = 4, seed = 42)

pdf( "qc_umap_harmony.pdf", width = 12, height = 9)
print(DimPlot(seurat_obj,  group.by = 'RNA_snn_res.1', label = TRUE,  raster = TRUE) +
        ggtitle(paste0("UMAP (Harmony) - ", 'RNA_snn_res.1')))
print(DimPlot(seurat_obj,  group.by = 'dataset', raster = TRUE) +
...
# # Compare with other serous subclusters
# p0 <- DotPlot(seurat_obj, features = c("SLPI", "STATH", "PRR4", "MUC5B", 
#                                   "LYZ", "LTF", "BPIFA1"),group.by = cluster_col)

# # Check disease association
# # VlnPlot(seurat_obj, features = "SLPI", split.by = "disease_status")
# p0$data
# p0

data_matrix <- GetAssayData(seurat_obj, slot = "counts")  # 使用原始 counts 数据
metadata <- seurat_obj@meta.data
# 创建一个模型并执行差异表达分析
# 使用 RNA_snn_res.0.2 作为细胞群体的标识（可以根据需要修改）
model <- zlm(~RNA_snn_res.0.2, data = metadata, method = "glmer", fitArgs = list(family = "poisson"))

# 获取差异表达结果
summary_model <- summary(model, doLRT = TRUE)
# 提取差异表达基因
de_genes <- summary_model$datatable
library(ggplot2)

# 选择显著的基因
significant_genes <- de_genes[de_genes$Pr(>Chisq) < 0.05, ]

# 火山图
ggplot(significant_genes, aes(x = logFC, y = -log10(Pr(>Chisq)))) +
  geom_point() +
  theme_minimal() +
  ggtitle("Volcano plot of DE genes from MAST") +
...
cluster_col <- "RNA_snn_res.0.2" # 如果你的cluster列不是这个名，改成对应列名
...
seurat_obj <- readRDS('/home/h2048/data/R/1217/epithelial_bbknn_raw_20251217.rds')
...
GROUPING_COL <- "RNA_snn_res.0.2"  # Change to desired metadata column
...
p1 <- ggplot(rogue_df, aes(x = reorder(cell_type, rogue_value), y = rogue_value, 
                           fill = purity_class)) +
  geom_col() +
  ...
  scale_fill_manual(values = c("High (≥0.9)" = "forestgreen",
                               ...
                               "Low (<0.7)" = "firebrick")) +
  labs(title = "ROGUE Cluster Purity by Cell Type",
...
p2 <- ggplot(rogue_df, aes(x = reorder(cell_type, rogue_value), y = rogue_value)) +
  geom_segment(aes(xend = reorder(cell_type, rogue_value), yend = 0), 
               color = "grey50") +
  geom_point(aes(size = n_cells, color = purity_class), alpha = 0.8) +
  geom_hline(yintercept = c(0.7, 0.9), linetype = "dashed", color = "red", alpha = 0.3) +
  coord_flip() +
  scale_color_manual(values = c("High (≥0.9)" = "forestgreen",
                                "Moderate (0.7-0.9)" = "orange",
                                "Low (<0.7)" = "firebrick")) +
  scale_size_continuous(range = c(3, 10), labels = scales::comma) +
  labs(title = "ROGUE Values with Cell Counts",
...
rogue_table <- rogue_df %>%
  mutate(rogue_value = sprintf("%.4f", rogue_value),
...
cat(sprintf("Successfully calculated ROGUE for %d/%d cell types\n", 
            nrow(rogue_df), length(cell_types)))
...
# Cluster 3特异性高亮
p_cluster3_highlight <- DimPlot(seurat_obj, 
...
# 祖细胞marker
p_progenitor <- FeaturePlot(seurat_obj, 
...
# 上皮TF
p_epi_tf <- FeaturePlot(seurat_obj, 
...
# 保存图片
ggsave(filename = file.path(output_dir, "Cluster3_01_location.pdf"), 
       plot = p_cluster3_highlight, width = 6, height = 5)
ggsave(filename = file.path(output_dir, "Cluster3_02_club_markers.pdf"), 
       plot = p_club, width = 8, height = 8)
ggsave(filename = file.path(output_dir, "Cluster3_03_progenitor_markers.pdf"), 
       plot = p_progenitor, width = 12, height = 8)
ggsave(filename = file.path(output_dir, "Cluster3_04_epithelial_TF.pdf"), 
       plot = p_epi_tf, width = 12, height = 8)

# ===== 1.1.3 排除性marker检查 =====

# 检查是否混入其他细胞类型
p_exclusion <- FeaturePlot(seurat_obj, 
                           features = c("TP63", "FOXJ1", "MUC5AC", "CD3D"),
                           ncol = 2,
                           pt.size = 0.5) 
  

ggsave(filename = file.path(output_dir, "Cluster3_05_exclusion_markers.pdf"), 
       plot = p_exclusion, width = 8, height = 8)

# ===== 1.1.4 VlnPlot定量对比 =====

# 准备数据
all_markers <- c(club_mature_genes, progenitor_genes[1:3], 
                 epithelial_tf_genes[1:3], basal_genes[1:2])
...
ggsave(filename = file.path(output_dir, "Cluster3_06_violin_quantitative.pdf"), 
       plot = p_violin, width = 16, height = 12)
...
# 线粒体/氧化磷酸化评分
oxidative_phos_genes <- c("ATP5A1", "ATP5C1", "NDUFA4", "NDUFB2", 
                          "UQCRQ", "COX5A", "COX5B", "COX6C",
...
# UMAP展示各个评分
score_features <- c("Club_Maturity1", "Progenitor_Score1", 
                    "Goblet_Tendency1", "Ciliated_Tendency1", "OxPhos_Score1")

p_scores_umap <- FeaturePlot(seurat_obj, 
...
ggsave(filename = file.path(output_dir, "Cluster3_07_functional_scores_umap.pdf"), 
       plot = p_scores_umap, width = 15, height = 10)

# VlnPlot对比不同cluster
p_scores_violin <- VlnPlot(seurat_obj, 
                           features = score_features,
                           group.by = "seurat_clusters",
                           pt.size = 0,
                           ncol = 3) +
...
ggsave(filename = file.path(output_dir, "Cluster3_08_functional_scores_violin.pdf"), 
       plot = p_scores_violin, width = 15, height = 10)
...
for (score in score_features) {
  ...
  score_comparison <- rbind(score_comparison, data.frame(
    Score_Type = score,
...
differentiation_data$Goblet_norm <- normalize_01(differentiation_data$Goblet_Score)
differentiation_data$Ciliated_norm <- normalize_01(differentiation_data$Ciliated_Score)

# ===== 1.3.2 散点图：Club vs Goblet vs Ciliated =====

# Club vs Goblet
p_club_goblet <- ggplot(differentiation_data, 
                        aes(x = Club_Score, y = Goblet_Score, 
                            color = Progenitor_Score)) +
  geom_point(alpha = 0.6, size = 1.5) +
  scale_color_gradient2(low = "blue", mid = "yellow", high = "red", 
                        midpoint = median(differentiation_data$Progenitor_Score)) +
  theme_minimal() +
  labs(title = "Club vs Goblet Differentiation",
...
# Club vs Ciliated
p_club_ciliated <- ggplot(differentiation_data, 
                          aes(x = Club_Score, y = Ciliated_Score, 
                              color = Progenitor_Score)) +
  geom_point(alpha = 0.6, size = 1.5) +
  scale_color_gradient2(low = "blue", mid = "yellow", high = "red", 
                        midpoint = median(differentiation_data$Progenitor_Score)) +
  theme_minimal() +
  labs(title = "Club vs Ciliated Differentiation",
       subtitle = "Color = Progenitor Score",
       x = "Club Maturity Score",
...
# Goblet vs Ciliated
p_goblet_ciliated <- ggplot(differentiation_data, 
                            aes(x = Goblet_Score, y = Ciliated_Score, 
                                color = Progenitor_Score)) +
  geom_point(alpha = 0.6, size = 1.5) +
  scale_color_gradient2(low = "blue", mid = "yellow", high = "red", 
                        midpoint = median(differentiation_data$Progenitor_Score)) +
  theme_minimal() +
  labs(title = "Goblet vs Ciliated Differentiation",
       subtitle = "Color = Progenitor Score",
...
ggsave(filename = file.path(output_dir, "Cluster3_09_differentiation_potential.pdf"), 
       plot = p_diff_combined, width = 18, height = 6)
...
differentiation_data$Differentiation_Type <- case_when(
  ...
  differentiation_data$Club_Score > median_club & 
    differentiation_data$Goblet_Score > median_goblet ~ "Club_Goblet_transition",
...
# 保存
write.csv(diff_type_summary, 
          file = file.path(output_dir, "Cluster3_differentiation_types.csv"),
          row.names = FALSE)

# 饼图可视化
p_diff_pie <- ggplot(diff_type_summary, aes(x = "", y = Percentage, fill = Differentiation_Type)) +
  geom_bar(stat = "identity", width = 1) +
  coord_polar("y", start = 0) +
  theme_void() +
  labs(title = "Cluster 3 Differentiation State Distribution",
       fill = "Differentiation Type") +
  geom_text(aes(label = paste0(round(Percentage, 1), "%")), 
            position = position_stack(vjust = 0.5))

ggsave(filename = file.path(output_dir, "Cluster3_10_differentiation_pie.pdf"), 
       plot = p_diff_pie, width = 8, height = 6)
...
# Cluster 4位置
p_cluster4_location <- DimPlot(seurat_obj, 
...
# 浆细胞marker
p_plasma_markers <- FeaturePlot(seurat_obj, 
...
# 异常marker
p_anomalous_markers <- FeaturePlot(seurat_obj, 
...
# 保存
ggsave(filename = file.path(output_dir, "Cluster4_01_location.pdf"), 
       plot = p_cluster4_location, width = 6, height = 5)
ggsave(filename = file.path(output_dir, "Cluster4_02_plasma_markers.pdf"), 
       plot = p_plasma_markers, width = 12, height = 8)
ggsave(filename = file.path(output_dir, "Cluster4_03_anomalous_markers.pdf"), 
       plot = p_anomalous_markers, width = 8, height = 8)
...
# IGKC vs FABP4
p_igkc_fabp4 <- FeatureScatter(seurat_obj, 
...
# IGKC vs VIM
p_igkc_vim <- FeatureScatter(seurat_obj, 
                             cells = cluster4_cells,
                             feature1 = "IGKC", 
                             feature2 = "VIM",
                             pt.size = 1.5) +
...
# JCHAIN vs FABP4
p_jchain_fabp4 <- FeatureScatter(seurat_obj, 
                                 cells = cluster4_cells,
                                 feature1 = "JCHAIN", 
                                 feature2 = "FABP4",
                                 pt.size = 1.5) +
...
p_vim_fabp4 <- FeatureScatter(seurat_obj, 
                              cells = cluster4_cells,
                              feature1 = "VIM", 
                              feature2 = "FABP4",
                              pt.size = 1.5) +
...
# 合并
p_coexpression <- (p_igkc_fabp4 | p_igkc_vim) / (p_jchain_fabp4 | p_vim_fabp4) +
  plot_annotation(title = "Cluster 4: Critical Co-expression Analysis")

ggsave(filename = file.path(output_dir, "Cluster4_04_coexpression_analysis.pdf"), 
       plot = p_coexpression, width = 14, height = 12)
...
for (pair in gene_pairs) {
  ...
  if (gene1 %in% rownames(cluster4_expr) & gene2 %in% rownames(cluster4_expr)) {
    cor_test <- cor.test(as.numeric(cluster4_expr[gene1, ]),
...
cluster4_subset <- RunPCA(cluster4_subset, features = VariableFeatures(cluster4_subset))

# Elbow plot
p_elbow <- ElbowPlot(cluster4_subset, ndims = 50)
ggsave(filename = file.path(output_dir, "Cluster4_07_elbow_plot.pdf"), 
       plot = p_elbow, width = 6, height = 4)
...
for (res in c(0.3, 0.5, 0.8)) {
  ...
  cat(sprintf("\n=== Resolution %.1f: %d subclusters ===\n", 
              res, length(unique(cluster4_subset@meta.data[[cluster_col]]))))
...
# 可视化子cluster
p_subcluster <- DimPlot(cluster4_subset, 
                        label = TRUE,
                        pt.size = 1.5) +
  ggtitle("Cluster 4 Sub-clustering (Resolution 0.5)")

ggsave(filename = file.path(output_dir, "Cluster4_08_subclusters.pdf"), 
       plot = p_subcluster, width = 8, height = 6)
...
p_subcluster_markers <- FeaturePlot(cluster4_subset, 
...
ggsave(filename = file.path(output_dir, "Cluster4_09_subcluster_markers.pdf"), 
       plot = p_subcluster_markers, width = 15, height = 12)

# DotPlot
p_subcluster_dot <- DotPlot(cluster4_subset, 
                            features = all_key_markers,
...
ggsave(filename = file.path(output_dir, "Cluster4_10_subcluster_dotplot.pdf"), 
       plot = p_subcluster_dot, width = 10, height = 8)

# ===== 2.3.3 找每个子cluster的DE genes =====

subcluster_markers <- FindAllMarkers(cluster4_subset, 
                                     only.pos = TRUE,
                                     min.pct = 0.25,
                                     logfc.threshold = 0.25)
...
p_subcluster_heatmap <- DoHeatmap(cluster4_subset, 
                                  features = top10_subcluster$gene,
                                  group.by = "RNA_snn_res.0.5") +
  theme(axis.text.y = element_text(size = 6))

ggsave(filename = file.path(output_dir, "Cluster4_11_subcluster_heatmap.pdf"), 
       plot = p_subcluster_heatmap, width = 12, height = 15)
...
for (sc in unique(Idents(cluster4_subset))) {
  ...
  igkc_pct <- mean(cluster4_subset[["RNA"]]@layers@data["IGKC", sc_cells] > 0) * 100
  fabp4_pct <- mean(cluster4_subset[["RNA"]]@layers@data["FABP4", sc_cells] > 0) * 100
  vim_pct <- mean(cluster4_subset[["RNA"]]@layers@adta["VIM", sc_cells] > 0) * 100
  ...
  subcluster_identity_summary <- rbind(subcluster_identity_summary, data.frame(
    Subcluster = sc,
...
write.csv(subcluster_identity_summary, 
...
# 重新进行PCA和聚类
cluster3_subset <- ScaleData(cluster3_subset, features = rownames(cluster3_subset))
cluster3_subset <- RunPCA(cluster3_subset, features = VariableFeatures(cluster3_subset))
...
# 可视化子cluster
p_subcluster_umap <- DimPlot(cluster3_subset, 
                             group.by = "seurat_clusters",
                             label = TRUE,
                             pt.size = 1) +
  ggtitle("Cluster 3 Sub-clustering")

ggsave(filename = file.path(output_dir, "Cluster3_13_subclusters.pdf"), 
       plot = p_subcluster_umap, width = 8, height = 6)

# ===== 1.5.2 子cluster的marker表达 =====

# 在子cluster上展示关键marker
p_subcluster_markers <- FeaturePlot(cluster3_subset, 
                                    features = c("ALDH1A1", "SCGB1A1", 
                                                "MUC5B", "GRHL2"),
                                    ncol = 2,
                                    pt.size = 1,
                                    order = TRUE) +
...
ggsave(filename = file.path(output_dir, "Cluster3_14_subcluster_markers.pdf"), 
       plot = p_subcluster_markers, width = 10, height = 10)

# ===== 1.5.3 找每个子cluster的marker =====

Idents(cluster3_subset) <- "seurat_clusters"
subcluster_markers <- FindAllMarkers(cluster3_subset, 
                                     only.pos = TRUE,
                                     min.pct = 0.25,
                                     logfc.threshold = 0.25)
...
write.csv(top_subcluster_markers, 
...
p_subcluster_heatmap <- DoHeatmap(cluster3_subset, 
                                  features = top10$gene,
                                  group.by = "seurat_clusters") +
  theme(axis.text.y = element_text(size = 6))

ggsave(filename = file.path(output_dir, "Cluster3_15_subcluster_heatmap.pdf"), 
       plot = p_subcluster_heatmap, width = 12, height = 15)
...
# 双染：AIF1 vs IGKC
FeatureScatter(cluster4_subset,
               feature1 = "AIF1",
               feature2 = "IGKC",
               group.by = "RNA_snn_res.0.5") +
...
# 计算IL33评分
il33_expressing_cells <- WhichCells(cluster4_subset, 
                                    expression = IL33 > 0)
...
p_corrected <- DimPlot(seurat_obj, 
...
ggsave("Cluster4_CORRECTED_annotation.pdf", 
       p_corrected, width = 12, height = 8)

# 关键marker的FeaturePlot
key_markers <- c("IL33", "AIF1", "FBN1", 
                "VIM", "IGKC", "FABP4")

p_key_markers <- FeaturePlot(seurat_obj,
...
ggsave("Cluster4_corrected_key_markers.pdf", 
       p_key_markers, width = 15, height = 10)
...
# Cluster 3 (Club细胞)
club_markers <- c("SCGB1A1", "BPIFA1", "GRHL2", "EHF", 
                  "ALDH1A1", "MSI2", "SOX2", "MUC5B")

# Cluster 4 Subcluster 0 (假设的肌成纤维细胞)
myofib_markers <- c("VIM", "FABP4", "IL33", "TAGLN", "CNN3", 
                    "CAV1", "ADIRF", "CCL14")

# Cluster 4 Subcluster 1 (假设的巨噬细胞/浆细胞)
macro_plasma_markers <- c("AIF1", "S100A4", "CD68", "FCGR3A",  # 巨噬细胞
                          "IGKC", "JCHAIN", "IGLC2", "IGHA1")   # 浆细胞

# Cluster 4 Subcluster 2 (假设的血管周成纤维细胞)
perivascular_markers <- c("FBN1", "ITGA9", "CACNA1C", "EPHA4", "RBMS3")

# 合并所有marker
all_key_markers <- c(club_markers, myofib_markers, 
                     macro_plasma_markers, perivascular_markers)
...
for (marker in markers_present) {
  for (cluster_id in sort(unique(seurat_obj$seurat_clusters))) {
    ...
    # 计算统计量
    cluster_expr_summary <- rbind(cluster_expr_summary, data.frame(
      Marker = marker,
      Cluster = cluster_id,
...
# 保存完整表格
write.csv(cluster_expr_summary, 
...
if (exists("cluster4_subset")) {
  ...
  for (marker in markers_present) {
    for (subcluster_id in sort(unique(cluster4_subset$RNA_snn_res.0.5))) {
      ...
      subcluster_cells <- WhichCells(cluster4_subset, 
                                     idents = subcluster_id)
      
      expr_values <- cluster4_expr_data[marker, subcluster_cells]
      
      cluster4_subcluster_summary <- rbind(cluster4_subcluster_summary, 
                                          data.frame(
        Marker = marker,
  ...
  write.csv(cluster4_subcluster_summary, 
...
# Club细胞markers
p_club_abs <- VlnPlot(seurat_obj, 
                      features = club_markers[club_markers %in% rownames(seurat_obj)],
                      group.by = "seurat_clusters",
                      pt.size = 0,
                      ncol = 4,
                      log = FALSE) +  # 不对Y轴log转换，显示真实值
  plot_annotation(title = "Club Cell Markers - Absolute Expression (log1p normalized)",
                  subtitle = "Higher values = higher absolute expression")

ggsave(file.path(output_dir, "Absolute_Expr_Club_markers.pdf"),
       p_club_abs, width = 16, height = 8)

# 肌成纤维细胞markers
p_myofib_abs <- VlnPlot(seurat_obj, 
...
ggsave(file.path(output_dir, "Absolute_Expr_Myofibroblast_markers.pdf"),
       p_myofib_abs, width = 16, height = 8)

# 巨噬细胞/浆细胞markers
p_macro_plasma_abs <- VlnPlot(seurat_obj, 
                              features = macro_plasma_markers[macro_plasma_markers %in% rownames(seurat_obj)],
                              group.by = "seurat_clusters",
                              pt.size = 0,
                              ncol = 4) +
  plot_annotation(title = "Macrophage/Plasma Cell Markers - Absolute Expression")

ggsave(file.path(output_dir, "Absolute_Expr_Macrophage_Plasma_markers.pdf"),
       p_macro_plasma_abs, width = 16, height = 8)
...
if (exists("cluster4_subset")) {
  ...
  key_cluster4_markers <- c("IGKC", "JCHAIN", "AIF1", "S100A4", 
                           "VIM", "FABP4", "IL33", "TAGLN",
                           "FBN1", "ITGA9")
  
  key_cluster4_markers <- key_cluster4_markers[key_cluster4_markers %in% rownames(cluster4_subset)]
  
  p_cluster4_sub_abs <- VlnPlot(cluster4_subset,
                                features = key_cluster4_markers,
                                group.by = "RNA_snn_res.0.5",
                                pt.size = 0,
                                ncol = 5) +
    plot_annotation(title = "Cluster 4 Subclusters - Absolute Expression Comparison")
  
  ggsave(file.path(output_dir, "Absolute_Expr_Cluster4_Subclusters.pdf"),
         p_cluster4_sub_abs, width = 20, height = 8)
}
...
marker_annotation <- data.frame(
  ...
  row.names = c(club_markers, myofib_markers, 
                macro_plasma_markers, perivascular_markers)
)

# 只保留存在的marker
marker_annotation <- marker_annotation[rownames(marker_annotation) %in% rownames(expr_matrix), , drop = FALSE]

pdf(file.path(output_dir, "Absolute_Expr_Heatmap_AllClusters.pdf"), 
    width = 12, height = 16)
pheatmap(expr_matrix[rownames(marker_annotation), ],
         scale = "row",  # 按行标准化以便比较
         cluster_rows = FALSE,
         cluster_cols = TRUE,
...
# 不标准化的版本（显示真实绝对值）
pdf(file.path(output_dir, "Absolute_Expr_Heatmap_AllClusters_Unscaled.pdf"), 
    width = 12, height = 16)
pheatmap(expr_matrix[rownames(marker_annotation), ],
         scale = "none",  # 不标准化
         cluster_rows = FALSE,
...
key_markers_needed <- c(
  # 间质/成纤维细胞
  "VIM", "FABP4", "IL33", "TAGLN", "CNN3", "CAV1", "CCL14",
  # 巨噬细胞
  "AIF1", "S100A4", "CD68", "FCGR3A", "CD14",
  # 浆细胞
  "IGKC", "JCHAIN", "IGLC2", "IGHA1", "CD38", "SDC1",
  # 血管周
  "FBN1", "ITGA9", "CACNA1C",
  # 上皮/Club
...
# 检查存在的marker
key_markers_present <- key_markers_needed[key_markers_needed %in% rownames(seurat_obj)]
...
if (exists("cluster4_subcluster_summary")) {
  key_cluster4_summary <- cluster4_subcluster_summary %>%
    filter(Marker %in% key_markers_present) %>%
    select(Marker, Subcluster, Mean_Expr, Median_Expr, Pct_Positive, Pct_High) %>%
...
key_markers_all_clusters <- c("VIM", "COL1A1", "COL3A1", "DCN",  # 成纤维marker
                              "EPCAM", "CDH1", "KRT8", "KRT18", "KRT19",  # 上皮marker
                              "GRHL2", "EHF",  # 上皮TF
                              "BPIFA1", "SCGB1A1", "MUC5AC", "FOXJ1", "TP63")  # 上皮亚型

# 检查哪些marker存在
key_markers_present <- key_markers_all_clusters[key_markers_all_clusters %in% rownames(seurat_obj)]
...
for (cluster in sort(unique(seurat_obj$seurat_clusters))) {
  ...
  for (marker in key_markers_present) {
    expr_values <- GetAssayData(seurat_obj, slot = "data")[marker, cluster_cells]
    
    cluster_marker_expr <- rbind(cluster_marker_expr, data.frame(
      Cluster = cluster,
      Marker = marker,
      Mean_Expr = mean(expr_values),
      Pct_Positive = 100 * mean(expr_values > 0),
...
# 保存分类结果
write.csv(cluster_classification,
...
cat("2. Which clusters are clearly Fibroblast (high VIM, low epithelial markers)?\n")
cat("3. Which are immune cells, endothelial, etc?\n\n")

# 提示用户
cat("Please run the following to see marker expression heatmap:\n")
cat("DoHeatmap(seurat_obj, features = key_markers_present, group.by = 'seurat_clusters')\n\n")
...
classification_markers <- c(
  # 成纤维细胞
  "VIM", "COL1A1", "COL3A1", "DCN", "PDGFRA", "PDGFRB",
  # 上皮细胞
  "EPCAM", "CDH1", "KRT8", "KRT18", "KRT19", 
  # 上皮TF
  "GRHL2", "EHF", "TP63",
  # 上皮亚型
  "BPIFA1", "SCGB1A1", "MUC5AC", "FOXJ1"
)

classification_markers <- classification_markers[classification_markers %in% rownames(seurat_obj)]

p_classification_dot <- DotPlot(seurat_obj,
...
ggsave(file.path(output_dir, "Classification_DotPlot.pdf"),
       p_classification_dot, width = 12, height = 10)
...
marker_annotation <- data.frame(
  Type = c(
    rep("Fibroblast", sum(c("VIM", "COL1A1", "COL3A1", "DCN", "PDGFRA", "PDGFRB") %in% rownames(heatmap_data_t))),
    rep("Epithelial_General", sum(c("EPCAM", "CDH1", "KRT8", "KRT18", "KRT19") %in% rownames(heatmap_data_t))),
    rep("Epithelial_TF", sum(c("GRHL2", "EHF", "TP63") %in% rownames(heatmap_data_t))),
    rep("Epithelial_Subtype", sum(c("BPIFA1", "SCGB1A1", "MUC5AC", "FOXJ1") %in% rownames(heatmap_data_t)))
  ),
  row.names = rownames(heatmap_data_t)
)

pdf(file.path(output_dir, "Classification_Heatmap.pdf"), width = 10, height = 12)
pheatmap(heatmap_data_t,
         scale = "row",
         cluster_rows = FALSE,
         cluster_cols = TRUE,
         annotation_row = marker_annotation,
...
p_vim_epcam <- VlnPlot(seurat_obj,
                       features = c("VIM", "EPCAM"),
                       group.by = "seurat_clusters",
                       ncol = 2,
                       pt.size = 0)

ggsave(file.path(output_dir, "VIM_vs_EPCAM_Violin.pdf"),
       p_vim_epcam, width = 12, height = 6)
...
if ("VIM" %in% colnames(cluster_avg) & "EPCAM" %in% colnames(cluster_avg)) {
  p_scatter_vim_epcam <- ggplot(cluster_avg, aes(x = VIM, y = EPCAM, label = Cluster)) +
    geom_point(size = 4, alpha = 0.7) +
    geom_text(vjust = -1, size = 3) +
    geom_vline(xintercept = 2, linetype = "dashed", color = "red") +
    geom_hline(yintercept = 1, linetype = "dashed", color = "blue") +
    annotate("text", x = 4, y = 0.5, label = "Fibroblast\n(VIM>2, EPCAM<1)", color = "red") +
    annotate("text", x = 1, y = 3, label = "Epithelial\n(EPCAM>1)", color = "blue") +
    theme_minimal() +
    labs(title = "Cluster Classification: VIM vs EPCAM",
         subtitle = "Based on mean expression")
  
  ggsave(file.path(output_dir, "Scatter_VIM_vs_EPCAM.pdf"),
         p_scatter_vim_epcam, width = 8, height = 6)
}

if ("VIM" %in% colnames(cluster_avg) & "GRHL2" %in% colnames(cluster_avg)) {
  p_scatter_vim_grhl2 <- ggplot(cluster_avg, aes(x = VIM, y = GRHL2, label = Cluster)) +
    geom_point(size = 4, alpha = 0.7) +
    geom_text(vjust = -1, size = 3) +
    geom_vline(xintercept = 2, linetype = "dashed", color = "red") +
    geom_hline(yintercept = 0.3, linetype = "dashed", color = "blue") +
    annotate("text", x = 4, y = 0.1, label = "Fibroblast\n(VIM>2, GRHL2<0.3)", color = "red") +
    annotate("text", x = 1, y = 0.8, label = "Epithelial\n(GRHL2>0.3)", color = "blue") +
    theme_minimal() +
    labs(title = "Cluster Classification: VIM vs GRHL2",
         subtitle = "Based on mean expression")
  
  ggsave(file.path(output_dir, "Scatter_VIM_vs_GRHL2.pdf"),
         p_scatter_vim_grhl2, width = 8, height = 6)
}
# dev.off()
...
if (exists("cluster4_subset") && "RNA_snn_res.0.5" %in% colnames(cluster4_subset@meta.data)) {
  ...
  # 更新Cluster_Detailed
  clusters_3_4$Cluster_Detailed[colnames(clusters_3_4) %in% names(cluster4_subcluster)] <- 
    paste0("C4_Sub", cluster4_subcluster[colnames(clusters_3_4)[colnames(clusters_3_4) %in% names(cluster4_subcluster)]])
...
all_markers <- list(
  ...
  Epithelial_TF = c("GRHL2", "GRHL1", "EHF", "ELF3", "ELF5", "OVOL2", "TFCP2L1"),
  ...
  # === 浆细胞/B细胞 ===
  Plasma_Cell = c("IGKC", "IGLC2", "IGLC3", "IGHA1", "IGHA2", "IGHG1", "JCHAIN", "SDC1", "CD38"),
...
for (category in names(all_markers)) {
  present_in_category <- all_markers[[category]][all_markers[[category]] %in% markers_present]
...
# DotPlot (不缩放)
p_dotplot_unscaled <- DotPlot(clusters_3_4,
  ...
  labs(title = "Cluster 3 & 4 Detailed Marker Expression",
...
ggsave(file.path(output_dir, "DotPlot_Cluster3_4_Detailed_Unscaled.pdf"),
       p_dotplot_unscaled, width = 50, height = 20,limitsize = FALSE)

# DotPlot (行标准化)
p_dotplot_scaled <- DotPlot(clusters_3_4,
                            features = markers_ordered,
                            group.by = "Cluster_Detailed",
                            scale = TRUE) +
  coord_flip() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        axis.text.y = element_text(size = 8)) +
  labs(title = "Cluster 3 & 4 Detailed Marker Expression (Row-scaled)",
       subtitle = "Scaled by row - Easier to compare relative differences",
       x = "Markers",
       y = "Cluster")

ggsave(file.path(output_dir, "DotPlot_Cluster3_4_Detailed_Scaled.pdf"),
       p_dotplot_scaled, width = 50, height = 20,limitsize = FALSE)
...
# 按category分面绘图
p_dotplot_faceted <- ggplot(dotplot_data, 
  ...
  labs(title = "Cluster 3 & 4: Markers by Category",
       x = "Cluster",
       y = "Markers")

ggsave(file.path(output_dir, "DotPlot_Cluster3_4_by_Category.pdf"),
       p_dotplot_faceted, width = 16, height = 24)

# ===== 2.4 保存DotPlot数据 =====

# 保存原始数据供后续分析
write.csv(dotplot_data,
...
cat("  - Cluster_ID: matching your cluster names (e.g., 'C3', 'C4_Sub0', 'C4_Sub1', etc.)\n")
cat("  - Level1: Broad cell type (e.g., 'Epithelial', 'Stromal', etc.)\n")
cat("  - Level2: Detailed cell type (e.g., 'Club_Progenitor', 'IL33_Myofibroblast', etc.)\n\n")

# 示例annotation.csv格式
example_annotation <- data.frame(
  Cluster_ID = c("C3", "C4_Sub0", "C4_Sub1", "C4_Sub2"),
  Level1 = c("Epithelial", "Stromal", "Immune", "Stromal"),
  Level2 = c("Club_Progenitor_Cells", 
             "IL33_Myofibroblasts", 
...
if (file.exists(annotation_file)) {
  ...
  if (length(missing_cols) > 0) {
    stop(sprintf("ERROR: Missing required columns in annotation.csv: %s\n", 
  ...
  if (length(missing_annotation) > 0) {
    warning(sprintf("WARNING: The following clusters are missing annotation:\n%s\n",
  ...
  if (length(extra_annotation) > 0) {
    warning(sprintf("WARNING: The following Cluster_IDs in annotation.csv don't exist in data:\n%s\n",
  ...
  write.csv(recommended_annotation,
...
if (exists("annotation")) {
  ...
  # 应用到clusters_3_4
  clusters_3_4$Cell_Type_Level1 <- cluster_to_level1[clusters_3_4$Cluster_Detailed]
  clusters_3_4$Cell_Type_Level2 <- cluster_to_level2[clusters_3_4$Cluster_Detailed]
  ...
  if (na_level1 > 0 | na_level2 > 0) {
    warning(sprintf("WARNING: %d cells have NA in Level1, %d cells have NA in Level2\n",
                   na_level1, na_level2))
  }
  ...
  seurat_obj$Cell_Type_Level1[colnames(clusters_3_4)] <- clusters_3_4$Cell_Type_Level1
  seurat_obj$Cell_Type_Level2[colnames(clusters_3_4)] <- clusters_3_4$Cell_Type_Level2
