...
# -*- coding: utf-8 -*-
"""
Epithelial Cell: scVI â†’ CellTypist â†’ scANVI Pipeline
=====================================================
...
Author: r2end
Date: 2024-12-11
Version: 3.2 - Bug fixes + memory optimization
  - Fixed: mean_conf variable overwrite bug
  - Added: scANVI early stopping support
  - Optimized: CellTypist memory usage (removed redundant copy)
  - Enhanced: Training history plotting robustness
"""
...
print(f"  Workflow: scVI â†’ CellTypist â†’ Manual merge â†’ scANVI")
print(f"\nModel Parameters:")
...
print(f"\nâš ï¸  CRITICAL: scVI requires raw count data (not log-normalized)")
print(f"âš ï¸  Using zero-copy strategy to minimize memory usage")
...
if 'counts' in adata.layers:
    counts_layer = 'counts'
    print(f"âœ“ Using adata.layers['counts'] (no copy)")
elif 'raw_counts' in adata.layers:
    counts_layer = 'raw_counts'
    print(f"âœ“ Using adata.layers['raw_counts'] (no copy)")
elif adata.raw is not None:
    print(f"âœ“ Found adata.raw - creating reference (no full copy)")
    adata.layers['counts'] = adata.raw.X
    counts_layer = 'counts'
else:
    print(f"\nâŒ No raw counts found! Checking current adata.X...")
    ...
    if max_val < 20 and has_decimals:
        raise ValueError(
            f"âŒ Data appears log-normalized (max={max_val:.2f})\n"
...
# â­ CRITICAL: Save a copy for CellTypist BEFORE HVG selection
# CellTypist works better with full genes, not HVG subset
print(f"\nCreating copy for CellTypist (full genes)...")
adata_ct = adata.copy()
print(f"âœ“ CellTypist copy created: {adata_ct.n_obs:,} cells Ã— {adata_ct.n_vars:,} genes")

# Select HVG (batch-aware, subset once) for scVI/scANVI
...
print(f"\n{'='*80}")
print(f"âœ“ scVI training complete!")
...
print(f"âœ“ X_scvi saved, shape: {adata.obsm['X_scvi'].shape}")
...
if hasattr(history, '__contains__') and 'elbo_train' in history:
    rec = history['elbo_train']
    train_elbo = getattr(rec, 'values', rec)  # More robust
if hasattr(history, '__contains__') and 'elbo_validation' in history:
    rec = history['elbo_validation']
    val_elbo = getattr(rec, 'values', rec)  # More robust
...
try:
    ...
    print(f"âœ“ Model loaded: {CELLTYPIST_MODEL}")
except:
    print(f"âš ï¸  Model not found locally, downloading...")
    models.download_models(model=CELLTYPIST_MODEL, force_update=True)
    model = models.Model.load(model=CELLTYPIST_MODEL)
    print(f"âœ“ Model downloaded and loaded")
...
elapsed = time.time() - start_time
print(f"âœ“ CellTypist prediction complete!")
print(f"  Time: {int(elapsed//60)} min {int(elapsed%60)} sec")
...
# ⚠️ CRITICAL: CellTypist confidence column name varies by version
# Robust detection of confidence score column
print(f"\nDetecting confidence score column...")
print(f"  Available columns: {list(predictions.predicted_labels.columns)}")

conf_col = None
for possible_name in ['conf_score', 'confidence', 'confidence_score', 'prob']:
    if possible_name in predictions.predicted_labels.columns:
        conf_col = possible_name
        print(f"  ✓ Using: '{possible_name}'")
        break

if conf_col is not None:
    adata.obs['celltypist_conf_score'] = predictions.predicted_labels[conf_col].values
else:
    print(f"  ⚠️  No confidence score found, setting to 1.0")
...
print(f"  3. Final label â†’ 'combined_cell_type'")
...
# â­ CRITICAL: Ensure 'Unknown' is in categories (scvi-tools 1.1+ requirement)
...
# â­ CRITICAL: Initialize scANVI from scVI (in-memory, NO re-setup)
...
print(f"  âš ï¸  Using in-memory vae object (no model reload)")
...
print(f"âœ“ scANVI model created")
...
# Add early stopping (same as scVI)
if EARLY_STOPPING:
...
elapsed = time.time() - start_time
print(f"\n{'='*80}")
print(f"âœ“ scANVI training complete!")
print(f"  Time: {int(elapsed//60)} min {int(elapsed%60)} sec")
print(f"{'='*80}")
...
if agreement_rate > 90:
    print(f"\nâœ“ Excellent: >90% agreement")
elif agreement_rate > 80:
    print(f"\nâœ“ Good: >80% agreement")
elif agreement_rate > 70:
    print(f"\nâš ï¸  Moderate: 70-80% agreement")
else:
    print(f"\nâŒ Low: <70% agreement - review disagreements")
...
if agreement_rate < 100:
    ...
    if len(disagreements) > 0:
        print(f"\nTop 5 disagreements (combined â†’ scANVI):")
        ...
        for (combined, scanvi), count in Counter(disagree_pairs).most_common(5):
            print(f"  {combined} â†’ {scanvi}: {count} cells")
...
print(f"Mean confidence: {overall_mean_conf:.3f}")
...
for cell_type, count in final_counts.head(15).items():
    pct = count / adata.n_obs * 100
    type_mean_conf = adata.obs[adata.obs[SCANVI_LABEL_KEY] == cell_type]['scanvi_confidence'].mean()
    print(f"{cell_type:<30} {count:<12,} {pct:<10.2f}% {type_mean_conf:<12.3f}")
...
print(f"âœ“ UMAP saved: 03_umap_comprehensive.png")
...
### Key Columns
- `adata.obs['{SCANVI_LABEL_KEY}']` - â­ Final cell type (USE THIS)
...
### Key Representations
- `adata.obsm['X_scanvi']` - â­ Batch-corrected latent space
...
Pipeline version: 3.0 (scVI â†’ CellTypist â†’ scANVI)
"""
...
print(f"\nðŸ’¡ Use adata.obs['{SCANVI_LABEL_KEY}'] for final annotations!")
