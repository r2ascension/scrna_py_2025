...
# -*- coding: utf-8 -*-
"""
Stromal Subcluster Results Converter v2.0 (PRODUCTION - FIXED)
===============================================================
Author: r2end
Date: 2026-01-28
...
FIXES in v2.0:
--------------
P0 Issues (Critical - Silent Failures):
  ✓ P0-1: Standardize L2/L3 annotations BEFORE concat (no mask guessing)
  ✓ P0-2: Global UMAP recomputed from HVG/PCA (no per-celltype latent)
  ✓ P0-3: Force unique obs_names with celltype prefix

P1 Issues (High Risk):
  ✓ P1-1: Preserve key metadata, document what's merged
  ✓ P1-2: No unnecessary copies in HVG/PCA workflow
  ✓ P1-3: Standardize marker tables with L2/L3 columns
  ✓ P1-4: Fixed Agg backend for headless environments

P2 Enhancements:
  ✓ Better PDF selection logic
  ✓ Consistent string types for subcluster_id
  ✓ Comprehensive validation and logging

Purpose:
--------
Convert stromal subcluster results from BATCH format (v4.1 universal pipeline)
...
│   ├── marker_genes_FDR.csv
│   └── figures/

Output Structure:
-----------------
stromal_analysis_unified/
...
print("="*80)
print("STROMAL SUBCLUSTER RESULTS CONVERTER v2.0 (PRODUCTION - FIXED)")
...
INPUT_BASE = Path("/home/h2048/data/py/0114/stromal_subcluster")

# Output: Unified format directory
OUTPUT_BASE = Path(f"/home/h2048/data/py/0120/stromal_analysis_unified")
...
adata_merged = sc.concat(
    ...
    axis=0,
    join='outer',
    merge='unique',  # More conservative than 'first'
    index_unique=None,  # Already made unique with prefix
    fill_value=0,
    label='_source_batch',  # Add source tracking
...
print(f"✓ Converted L2/L3 to categorical")
print()
...
print("   (Per-celltype latent spaces are not comparable!)")
print()

# Check if we already have a global UMAP from preprocessing
...
if 'X_umap' in adata_merged.obsm:
    ...
    if adata_merged.obsm['X_umap'].shape[0] == adata_merged.n_obs:
        print("✓ Found existing global X_umap (from preprocessing)")
        print("  Assuming this is from a unified global embedding")
...
if not has_existing_umap:
    ...
    # P1-2 FIX: No unnecessary copies, work directly on merged object
    ...
    # P1-2 FIX: No copy needed, use use_highly_variable=True
...
# 4. Key stromal marker (try several)
marker_plotted = False
if adata_merged.raw is not None:
    for marker in ['COL1A1', 'PECAM1', 'VWF', 'ACTA2', 'DCN']:
...
try:
    sc.pl.umap(
        ...
        ax=ax,
        show=False,
        title='Stromal Cell Subclusters (Level 3)',
        size=15,
        legend_loc='right margin',
        legend_fontsize=8,
        frameon=False
...
# ============================================================================
# STEP 6: Copy and Reorganize Per-Celltype Figures (P2 Enhancement)
# ============================================================================
...
for meta in celltype_metadata:
    ...
    # P2 Enhancement: Better PDF selection logic
    # Priority 1: Files matching "*subclusters*.pdf" or "*subclustering*.pdf"
    # Priority 2: First PDF file
    pdf_files = list(source_dir.glob('*.pdf'))
    
    if len(pdf_files) == 0:
        print(f"  ⚠️  No PDF files found for {celltype_safe}")
        continue
    ...
    if main_pdf is None:
        # Try without umap requirement
        for pdf in pdf_files:
            if 'subcluster' in pdf.name.lower():
                main_pdf = pdf
                break
    
    if main_pdf is None:
        main_pdf = pdf_files[0]  # Last resort
    
    # Copy with standardized name
    dest_name = f"{celltype_original.replace(' ', '_')}_subclustering.pdf"
    dest_file = FIG_DIR / dest_name
    
    try:
        shutil.copy2(main_pdf, dest_file)
        print(f"  ✓ {celltype_safe:40s} → {dest_name}")
    except Exception as e:
        print(f"  ❌ Failed to copy {main_pdf.name}: {e}")
...
for meta in celltype_metadata:
    ...
    try:
        ...
        # Try to infer L3 from subcluster column
...
# P1-1: Add comprehensive analysis metadata
adata_merged.uns['subcluster_analysis'] = {
    ...
    'batch_key': BATCH_KEY,
    'hvg_method': hvg_method if 'hvg_method' in locals() else 'N/A',
...
# Store celltype metadata for reference
...
# Final output file
output_file = OUTPUT_DIR / "adata_stromal_subclustered_FINAL_v2_20260128.h5ad"
...
===============
  ...
  - Missing L2 annotations: {adata_merged.obs['cell_type_L2'].isna().sum()}
  - Missing L3 annotations: {adata_merged.obs['cell_type_L3'].isna().sum()}
...
========================
  ✅ Check that L2 cell types match expected (no mixing)
  ✅ Verify L3 subclusters are correctly hierarchical
  ✅ Review global UMAP - cell types should be separated
  ✅ Check marker tables have L2/L3 columns
  ✅ Confirm obs_names are unique (celltype prefix)
...
=============
  ...
  4. Compare with B cell unified format for consistency
...
print("\n" + "="*80)
print("🎉 PRODUCTION-READY OUTPUT GENERATED")
print("="*80)

gc.collect()

# ============================================================================
# END OF CONVERSION SCRIPT v2.0 (PRODUCTION)
# ============================================================================
