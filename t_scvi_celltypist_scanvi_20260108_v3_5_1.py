...
# -*- coding: utf-8 -*-
"""
Epithelial Cell: scVI → CellTypist → scANVI Pipeline v3.5.1
============================================================

HOTFIX for v3.5 - Critical Bug Fixes:
- ✅ P0-FIX: scANVI now uses dedicated adata_model (labels alignment)
...
Critical Changes in v3.5.1:
1. Separate adata_model for scVI/scANVI (HVG-only + batch + labels)
2. Index-aligned result writing (pandas reindex pattern)
3. CellTypist majority voting correctly used in scANVI labels
4. No full matrix copy to .X (use layers throughout)
5. Pretrained model uses var_names for gene matching

Input: Raw epithelial data with counts layer
Output: adata_epithelial_FINAL.h5ad + count statistics

Author: r2end
Date: 2024-12-14
Version: 3.5.1 - Critical hotfix for training logic
"""
...
# Input/Output paths
INPUT_FILE = "/home/h2048/data/py/1217/cnmf_batch_production_v1_1_1/T_cells/batch_aware/cnmf_analysis_k40_1/T_cells_with_cnmf_k40.h5ad"
OUTPUT_DIR = "/home/h2048/data/py/0108/tcell_scanvi_v3_5_1_hotfix"
...
# ⭐ Rare type filtering
MIN_CELLS_PER_TYPE = 50  # Types with <200 cells → Unknown

# ============================================================================
# INSERT THIS AFTER LINE 92 (after MIN_CELLS_PER_TYPE definition)
# ============================================================================

# ⭐ T CELL CORE MARKERS - Force include in HVG
# These markers are essential for T cell subtype identification
...
for category, genes in TCELL_CORE_MARKERS.items():
    ALL_TCELL_MARKERS.extend(genes)
ALL_TCELL_MARKERS = list(set(ALL_TCELL_MARKERS))  # Remove duplicates

print(f"  T cell markers: {len(ALL_TCELL_MARKERS)} unique genes")
...
print("="*80)
print("EPITHELIAL PIPELINE v3.5.1 (CRITICAL HOTFIX)")
print("="*80)
print(f"\nConfiguration:")
...
# ============================================================================
# INSERT THIS AFTER LINE 393 (after "print(f'✓ Gene list saved...')")
# BEFORE LINE 397 (before "# STAGE 2: CellTypist ANNOTATION")
# ============================================================================

# ⭐ FORCE INCLUDE T CELL MARKERS IN HVG
print(f"\n{'='*80}")
...
print(f"Total markers to check: {len(ALL_TCELL_MARKERS)}")
...
print(f"\n{'='*60}")
print(f"SUMMARY:")
print(f"  Total markers: {len(ALL_TCELL_MARKERS)}")
...
critical_markers = ['CD3D', 'CD4', 'CD8A', 'CD8B']
print(f"\n{'='*60}")
...
if not all_present:
    ...
    print(f"   CD4/CD8 classification may fail in downstream analysis")
...
output_file = OUTPUT_DIR / "adata_epithelial_FINAL.h5ad"
...
print(f"\n{'='*80}")
print("🎉 PIPELINE COMPLETE - v3.5.1 HOTFIX")
print("="*80)
