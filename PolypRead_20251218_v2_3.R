...
OUTPUT_DIR <- "/home/h2048/data/R/1218/Polyp"
QC_PLOTS_DIR <- "/home/h2048/data/R/1218/Polyp/qc_plots"
...
cat("Gene standardization:", ifelse(STANDARDIZE_GENE_IDS, "ENABLED", "DISABLED"), "\n")
...
is_ensembl_id <- function(gene_ids) {
  if (length(gene_ids) == 0) return(FALSE)
...
convert_ensg_to_symbol <- function(
  ...
  if (!is.null(provided_symbols) && length(provided_symbols) == length(ensg_ids)) {
    ...
    cat("      Mapped:", n_mapped, "/", length(ensg_ids), 
        "genes (", round(mapping_rate * 100, 1), "%)\n")
    
    if (mapping_rate < MIN_MAPPING_RATE) {
      cat("      ⚠️ Warning: Low mapping rate (<", MIN_MAPPING_RATE * 100, "%)\n")
    }
...
deduplicate_symbols <- function(counts, gene_mapping, method = "sum") {
  ...
  if (method == "sum") {
    ...
    mapping_dedup$n_duplicates <- as.integer(table(symbols)[mapping_dedup$symbol])
...
extract_10x_prefix <- function(gsm_dir) {
  ...
  if (is.na(barcode_file)) return(NULL)
  ...
  tryCatch(
    {
      ...
      if (was_standardized) {
        cat("    Post-standardization:", nrow(counts_std), "genes x", 
            ncol(counts_std), "cells\n")
      }
  ...
  tryCatch(
    {
      ...
      log_entry <- jsonlite::toJSON(list(
        ...
        data = list(gsm_id = gsm_id, e_out_class = class(e.out), e_out_names = names(e.out), 
                   has_fdr = "FDR" %in% names(e.out), fdr_na_count = if("FDR" %in% names(e.out)) sum(is.na(e.out$FDR)) else NA),
      ...
      log_entry <- jsonlite::toJSON(list(
        ...
        data = list(gsm_id = gsm_id, n_cells_before = length(is.cell), n_cells_after = sum(is.cell), 
                   n_na_fdr = if("FDR" %in% names(e.out)) sum(is.na(e.out$FDR)) else NA),
        timestamp = as.numeric(Sys.time()) * 1000
      ), auto_unbox = TRUE)
      cat(log_entry, "\n", file = "/home/h2048/.cursor/debug.log", append = TRUE)
      # #endregion
      ...
      cat(
        ...
        " (",
        if (cells_before > 0) round(cells_removed / cells_before * 100, 1) else 0,
        "%)\n",
        sep = ""
      )
  ...
  combined <- p1 +
    ...
    plot_annotation(
      ...
      subtitle = paste0(
        ...
        " | Median nFeature: ",
        format(round(median(qc_data$nFeature_RNA, na.rm = TRUE)), big.mark = ","),
        " | Median nCount: ",
  ...
  for (k in names(dt)) {
    ...
    if (is.character(v) || is.factor(v)) {
      ...
      log_entry <- jsonlite::toJSON(list(
        ...
        data = list(column = k, match_rate = match_rate, n_na = sum(is.na(v_char)), n_total = length(v_char)),
        timestamp = as.numeric(Sys.time()) * 1000
      ), auto_unbox = TRUE)
      cat(log_entry, "\n", file = "/home/h2048/.cursor/debug.log", append = TRUE)
      # #endregion
...
for (dataset_id in DATASET_IDS) {
  ...
  for (sample_dir in sample_dirs) {
    ...
    if (ncol(counts) == 0 || nrow(counts) == 0) {
      ...
      bio_sample <- if ("gsm_id" %in% names(metadata) && gsm_id %in% metadata$gsm_id) {
    ...
    if ("gsm_id" %in% names(metadata) && gsm_id %in% metadata$gsm_id) {
      ...
      log_entry <- jsonlite::toJSON(list(
        ...
        data = list(gsm_id = gsm_id, meta_row_cols = names(meta_row), n_rows = nrow(meta_row)),
        timestamp = as.numeric(Sys.time()) * 1000
      ), auto_unbox = TRUE)
      cat(log_entry, "\n", file = "/home/h2048/.cursor/debug.log", append = TRUE)
      # #endregion
      
      required_cols <- c("sample", "GEO", "tissue", "tissue_level_2", "disease_level_1", 
                        "disease_level_2", "condition", "tissue_sampling_method", 
                        "filtered_or_raw", "target_cell", "frozen_or_fresh", "sex", 
                        "age", "platform", "reference_genome", "assay", "tissue_dissociation_protocol")
      ...
      log_entry <- jsonlite::toJSON(list(
        ...
        data = list(gsm_id = gsm_id, missing_cols = missing_cols, required_cols = required_cols),
        timestamp = as.numeric(Sys.time()) * 1000
      ), auto_unbox = TRUE)
      cat(log_entry, "\n", file = "/home/h2048/.cursor/debug.log", append = TRUE)
      # #endregion
      ...
      if ("sample" %in% names(meta_row)) seurat_obj$sample <- meta_row$sample
      if ("GEO" %in% names(meta_row)) seurat_obj$GEO <- meta_row$GEO
      if ("tissue" %in% names(meta_row)) seurat_obj$tissue <- meta_row$tissue
      if ("tissue_level_2" %in% names(meta_row)) seurat_obj$tissue_level_2 <- meta_row$tissue_level_2
      if ("disease_level_1" %in% names(meta_row)) seurat_obj$disease_level_1 <- meta_row$disease_level_1
      if ("disease_level_2" %in% names(meta_row)) seurat_obj$disease_level_2 <- meta_row$disease_level_2
      if ("condition" %in% names(meta_row)) seurat_obj$condition <- meta_row$condition
      if ("tissue_sampling_method" %in% names(meta_row)) seurat_obj$tissue_sampling_method <- meta_row$tissue_sampling_method
      if ("filtered_or_raw" %in% names(meta_row)) seurat_obj$filtered_or_raw <- meta_row$filtered_or_raw
      if ("target_cell" %in% names(meta_row)) seurat_obj$target_cell <- meta_row$target_cell
      if ("frozen_or_fresh" %in% names(meta_row)) seurat_obj$frozen_or_fresh <- meta_row$frozen_or_fresh
      if ("sex" %in% names(meta_row)) seurat_obj$sex <- meta_row$sex
      if ("age" %in% names(meta_row)) seurat_obj$age <- meta_row$age
      if ("platform" %in% names(meta_row)) seurat_obj$platform <- meta_row$platform
      if ("reference_genome" %in% names(meta_row)) seurat_obj$reference_genome <- meta_row$reference_genome
      if ("assay" %in% names(meta_row)) seurat_obj$assay <- meta_row$assay
      if ("tissue_dissociation_protocol" %in% names(meta_row)) seurat_obj$tissue_dissociation_protocol <- meta_row$tissue_dissociation_protocol
      ...
      log_entry <- jsonlite::toJSON(list(
        ...
        data = list(gsm_id = gsm_id, assigned_cols = intersect(required_cols, names(meta_row))),
        timestamp = as.numeric(Sys.time()) * 1000
      ), auto_unbox = TRUE)
      cat(log_entry, "\n", file = "/home/h2048/.cursor/debug.log", append = TRUE)
      # #endregion
      ...
      log_entry <- jsonlite::toJSON(list(
        ...
        data = list(gsm_id = gsm_id, has_gsm_id_col = "gsm_id" %in% names(metadata), gsm_id_in_metadata = if("gsm_id" %in% names(metadata)) gsm_id %in% metadata$gsm_id else FALSE),
        timestamp = as.numeric(Sys.time()) * 1000
      ), auto_unbox = TRUE)
      cat(log_entry, "\n", file = "/home/h2048/.cursor/debug.log", append = TRUE)
      # #endregion
...
if (STANDARDIZE_GENE_IDS) {
  n_standardized <- sum(
    processing_summary$Gene_Standardized[processing_summary$Status == "success"],
    na.rm = TRUE
  )
  cat("  Samples with gene standardization:", n_standardized, "/", 
      total_processed, "\n")
  
  if (!KEEP_UNMAPPED_GENES) {
    total_unmapped_removed <- sum(
      processing_summary$Genes_Unmapped_Removed[processing_summary$Status == "success"],
      na.rm = TRUE
    )
    cat("  Total unmapped genes removed:", format(total_unmapped_removed, big.mark = ","), "\n")
  }
  ...
  if (total_dup_symbols > 0) {
    cat("  Total duplicate symbols deduplicated:", format(total_dup_symbols, big.mark = ","), "\n")
  }
}
  ...
  .(
    ...
    Pct_Removed = as.numeric(
      ifelse(
        ...
        round((sum(Cells_Raw, na.rm = TRUE) - sum(Cells_Filtered, na.rm = TRUE)) / 
              sum(Cells_Raw, na.rm = TRUE) * 100, 1),
...
mapping_count <- length(list.files(OUTPUT_DIR, pattern = "_gene_mapping\\.csv\\.gz$"))
