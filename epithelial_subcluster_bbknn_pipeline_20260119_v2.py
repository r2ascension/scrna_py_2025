...
# **Author:** r2end  
# **Date:** 2025-01-18  
# **Version:** 1.1 - Simplified (no redundant batch correction)
# 
# **Key Features:**
...
print("="*80)
print("Epithelial Subcluster Analysis Pipeline (Post-Integration)")
print("="*80)
...
if 'dataset' in adata.obs.columns:
    ...
    print(f"✓ Found 'dataset' column: {n_batches} batches")
    print(f"  (Already integrated - no correction needed)")
    batch_counts = adata.obs['dataset'].value_counts()
    print(f"  Batch distribution:")
    for batch, count in batch_counts.items():
        print(f"    {batch}: {count:,} cells")
else:
    print(f"ℹ️  No 'dataset' column found (batch info not available)")
...
for idx, celltype in enumerate(celltypes, 1):
    ...
    # Check batches in subset (for information only)
    if 'dataset' in adata_sub.obs.columns:
        n_batches_sub = adata_sub.obs['dataset'].nunique()
        print(f"  Batches: {n_batches_sub} (already integrated globally)")
        batch_sizes = adata_sub.obs['dataset'].value_counts()
        print(f"  Batch distribution:")
    ...
    # Select HVG for memory optimization
    # Note: batch-aware HVG not needed since data already integrated
    try:
        ...
        print(f"✓ HVG selected (seurat_v3)")
    except Exception as e:
        print(f"⚠️  HVG with counts layer failed: {e}")
        print(f"  → Trying without counts layer")
        sc.pp.highly_variable_genes(
            adata_sub,
            n_top_genes=2000,
            flavor='seurat_v3',
    ...
    # 6. ⭐ SIMPLIFIED: Rebuild neighbors graph on subset
    print(f"\n🔗 Building neighbors graph for subset...")
    print(f"  ℹ️  Note: Global BBKNN already integrated batches")
    print(f"  ℹ️  Only rebuilding graph structure for subset")
    
    # Simply rebuild neighbors on the already-integrated data
    # No need for PCA, no need for BBKNN - data is already integrated!
    sc.pp.neighbors(adata_sub, n_neighbors=15)
    print(f"✓ Neighbors graph built ({n_cells:,} cells)")
    
    # 7. Calculate dynamic resolution
    resolution = calculate_resolution(n_cells, RESOLUTION_BASE)
    print(f"\n🎯 Leiden clustering with dynamic resolution:")
    print(f"  Cell count: {n_cells:,}")
    print(f"  Resolution: {resolution:.2f}")
    print(f"  (Higher cell count → higher resolution → finer subclusters)")
    ...
    # 9. Create combined labels
    adata_sub.obs['subcluster'] = celltype + "_" + adata_sub.obs['leiden_sub'].astype(str)
    
    # 10. ⭐ Marker gene identification (use .raw for full genes)
    ...
    # 11. Compute UMAP for this subset (optional, for QC visualization)
    ...
    # 12. ⭐ Write results back to main adata
    ...
    # 13. Save intermediate result for this cell type
