...
# 基本设置
DATASET_ID = "Stromal_Vascular"              # 你的数据集ID
TRACK = "batch_aware"                # 推荐使用batch_aware
K_VALUE = 50                        # 你选定的K值

# 路径设置
BASE_DIR = Path("/home/h2048/data/py/1217/cnmf_batch_production_v1_1_1")
...
# 原始数据路径（包含细胞类型注释）
ORIGINAL_H5AD = Path('/home/h2048/data/py/1217/stromal_vascular/stromal_vascular_with_bbknn.h5ad')

# 输出目录
OUTPUT_BASE = BASE_DIR / DATASET_ID / TRACK / f"cnmf_analysis_k{K_VALUE}_1"

# 列名设置（根据你的数据调整）
CELLTYPE_COL = 'scanvi_predictions'           # 细胞类型列名
DISEASE_COL = 'disease_status'       # 疾病状态列名（如果有）
BATCH_COL = 'sample'                 # 批次列名
CLUSTER_COL = 'leiden_bbknn_res1.0'  # cluster列名，会自动检测
