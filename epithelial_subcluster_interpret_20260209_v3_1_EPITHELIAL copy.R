#!/usr/bin/env Rscript
# ==============================================================================
# Epithelial Cell Subcluster Interpretation Pipeline - v3.1-EPITHELIAL
# ==============================================================================
#
# Version: v3.1-EPITHELIAL (2026-02-09)
# Status: Production-ready Epithelial specialized version
# Base: v3.0-HOTFIX + Multi-ssGSEA + Load existing enrichment + Epithelial v2.8
#
# Key Features:
#   ✅ Epithelial expert context (Basal/Goblet/Ciliated/Secretory differentiation)
...
#   ✅ Cell cycle gene filtering (epithelial specific)
#   ✅ All v2.8 bug fixes + v3.1 improvements
#
# New in v3.1:
#   • Multi-method ssGSEA in single run
#   • Optional loading of pre-computed enrichment
#   • interpret_agent with DeepSeek-Reasoner
#   • Multisession parallel (fork-safe)
#   • Enhanced epithelial differentiation context
#
# Epithelial-specific:
#   - 14 major cell types with 48 total subclusters
#   - Custom epithelial marker database (Basal/Goblet/Ciliated/etc)
#   - Differentiation trajectory context
#   - Cell cycle gene filtering
#
# ==============================================================================
...
OUTPUT_DIR <- "/home/h2048/data/R/0209/epithelial_interpret_v3_1"
...
CELLTYPE_SPECIFIC <- FALSE  # Analyze all cells
...
# ⭐ NEW: Load pre-computed enrichment results
...
INTERPRET_AGENT_ADD_PPI <- FALSE  # ⚠️ CRITICAL: Epithelial - PPI disabled
...
# ==============================================================================
# Thread Limiting (v3.1 improvement)
# ==============================================================================
...
# v3.1: Use multisession (spawn) instead of multicore (fork)
...
# v3.1: Direct API testing without fanyi dependency
...
# v3.1: Fail-fast validation for required columns
...
if (has_celltype_column) {
  ...
  cat(sprintf("[WARN] Cell type column '%s' not found\n", CELLTYPE_COLUMN))
}

cat(sprintf("\nSubcluster distribution ('%s'):\n", CLUSTER_COLUMN))
print(table(seurat_obj@meta.data[[CLUSTER_COLUMN]], useNA = "ifany"))
...
# ⭐ EPITHELIAL SPECIFIC: Extended filtering including cell cycle genes
genes_to_filter <- c(
  ...
  # Cell cycle (G2/M and S phase) - CRITICAL for epithelial with 48 subclusters
...
# ==============================================================================
# Custom Epithelial Marker Database (v2.8 component)
# ==============================================================================

cat("\n=== Creating Custom Epithelial Marker Database ===\n")

# ⭐ EPITHELIAL SPECIFIC: 13 epithelial subtypes
epithelial_markers_db <- data.frame(
  subtype = c(
    # Basal cells (stem/progenitor)
  ...
  markers = c(
    # Basal
    "TP63,KRT5,KRT14,ITGA6,ITGB4,COL17A1,MKI67,TOP2A",  # Proliferative
    "TP63,KRT5,KRT14,ITGA6,ITGB4,COL17A1",  # Quiescent
    "TP63,KRT5,KRT14,KRT6A,KRT16,S100A2,SPRR3",  # Squamous
    # Secretory
    "MUC5AC,MUC5B,TFF3,SPDEF,AGR2,WFDC2",  # Goblet
    "SCGB1A1,SCGB3A1,CC10,CYP2F1,BPIFA1",  # Club
    "LTF,LCN2,MUC7,PRB3,PRB4",  # Serous
    # Ciliated
    "FOXJ1,DNAI1,DNAI2,RSPH1,TUBA1A,TUBB4B,PIFO",  # Mature
    "DEUP1,CCNO,CDC20B,FOXN4,PLK4",  # Deuterosomal
    # Specialized
    "FOXI1,CFTR,ATP6V1B1,ATP6V0D2,ASCL3",  # Ionocyte
    "POU2F3,TRPM5,GFI1B,AVIL,IL25",  # Tuft
    "CHGA,CHGB,SYP,INSM1,ASCL1,CALCA",  # Neuroendocrine
    # Transitional
    "KRT13,KRT4,CLDN4,DSP,PKP1",  # Suprabasal
    "KRT8,KRT18,KRT19,MUC1,EPCAM"  # Intermediate
  ),
...
if (LOAD_EXISTING_ENRICHMENT && dir.exists(enrich_dir)) {
  ...
  # ============================================================================
  # Load CellMarker Database (Epithelial specific filtering)
  # ============================================================================
  ...
  if (!is.null(cellmarker_db)) {
    ...
    # ⭐ EPITHELIAL SPECIFIC: Filter for epithelial-related entries
  ...
  # Custom Epithelial Markers
  epithelial_markers_enrich <- tryCatch(
    {
      compareCluster(
        gene ~ cluster,
        data = top_markers,
    ...
    error = function(e) {
      cat("[WARN] Epithelial Markers enrichment failed:", conditionMessage(e), "\n")
      return(NULL)
    }
  )
  
  if (!is.null(epithelial_markers_enrich)) {
    ccr <- epithelial_markers_enrich@compareClusterResult
    n_sig <- sum(ccr$p.adjust < 0.05, na.rm = TRUE)
    cat(sprintf("[OK] Epithelial Markers: %d significant terms\n", n_sig))
    saveRDS(epithelial_markers_enrich, 
            file.path(OUTPUT_DIR, "reports", "epithelial_markers_enrich.rds"))
  }
  ...
  if (!is.null(panglaodb_term2gene)) {
    ...
    if (!is.null(panglaodb_enrich)) {
      ...
      saveRDS(panglaodb_enrich, 
              file.path(OUTPUT_DIR, "reports", "panglaodb_enrich.rds"))
    }
  }
  
  # GO BP/MF/CC (GMT-based, zero gene loss)
...
if (RUN_SSGSEA) {
  ...
  run_ssgsea_method <- function(method_name, term2gene, sample_avg) {
    ...
    # ⭐ CRITICAL FIX: Use param instead of BPPARAM
    # Option 1: Parallel (spawn-based, fork-safe)
    bpparam <- BiocParallel::SnowParam(workers = N_CORES, progressbar = FALSE)
    
    # Option 2: Serial (safer, slower)
    ...
    scores <- tryCatch({
      gsva(
        ...
        max.sz = 500,
        verbose = FALSE,
        param = bpparam  # ⭐ NEW: Use param instead of BPPARAM
      )
    }, error = function(e) {
  ...
  # Memory-optimized pseudobulk aggregation
...
if (RUN_INTERPRET_AGENT) {
  ...
  # Ensure scalar strings (handle nested structures)
...
source('/home/h2048/script/R/interpret_agent_hotfix.R')
  # ============================================================================
  # ⭐ EPITHELIAL SPECIFIC: Expert Context for interpret_agent
  # ============================================================================
  
  epithelial_context <- paste(
    "Respiratory epithelial cells from nasal cavity, sinus, bronchi, and lung.",
    "These are HEALTHY controls from normal respiratory tract.",
    "\nEpithelial cell types include:",
    ...
    "- Homeostatic barrier maintenance",
    "- Balanced cell turnover (low proliferation)",
    "- Proper differentiation gradients",
    "- Functional mucociliary apparatus",
    "- Minimal inflammatory signaling"
  )
  
  cat("[INFO] Epithelial expert context prepared\n")
  ...
  cat(sprintf("[INFO] Prepared fold change data for %d markers\n", nrow(gene_fc)))
  ...
  if (RUN_ENRICHMENT && !is.null(go_bp_enrich)) {
    ...
    for (cid in unique(top_markers$cluster)) {
      ...
      # Filter GO BP enrichment to this cluster
...
tryCatch({
  ...
  cat("# Epithelial Cell Subcluster Interpretation Report v3.1-EPITHELIAL\n\n")
  cat("**Generated:** ", format(Sys.time()), "\n\n")
  cat("**Pipeline:** Epithelial cell specialized v3.1 (ssGSEA + LLM + interpret_agent)\n\n")
  cat("---\n\n")
  ...
  cat("- `reports/msigdb_kegg_enrich.rds` - KEGG pathways\n")
  cat("\n")
...
cat("\n")
cat("================================================================================\n")
cat("EPITHELIAL ANALYSIS COMPLETE - v3.1-EPITHELIAL\n")
cat("================================================================================\n\n")
...
cat("Key Features (v3.1-EPITHELIAL):\n")
...
cat("  ✅ Cell cycle gene filtering (48 subclusters optimized)\n")
cat("  ✅ All v2.8 bug fixes + v3.1 improvements\n")
...
cat("  • Enhanced epithelial differentiation context\n")
cat("  • Smarter normalization detection\n")
cat("  • Robust API validation\n")
cat("\n")

cat("Improvements over v2.8:\n")
...
cat("  • Thread limiting for stability\n")
cat("  • Better memory management\n")
cat("\n")

cat("Epithelial-specific features:\n")
cat("  • 13 epithelial subtypes in custom database\n")
cat("  • Differentiation trajectory context (Basal → Secretory/Ciliated)\n")
cat("  • Cell cycle gene filtering (G2/M, S phase markers)\n")
cat("  • Respiratory tract-specific marker filtering\n")
cat("  • Epithelial-focused CellMarker subset\n")
cat("\n")
