options(stringsAsFactors = FALSE, future.globals.maxSize = +Inf)
Sys.setenv(RETICULATE_PYTHON = "E:/ProgramData/anaconda3/python.exe")

pkgs <- c(
  "Seurat","dplyr","ggplot2","Matrix","rsvd","harmony","presto","reticulate",
  "DoubletFinder","DirichletReg","speckle","tidyverse","MAST","cowplot","edgeR",
  "reshape2","pheatmap","tidyr","patchwork","org.Hs.eg.db","clusterProfiler",
  "enrichplot","DOSE","CellChat","NMF","ggalluvial","ComplexHeatmap","DropletUtils",
  "data.table","celda","viridis"
)
pkgs <- unique(pkgs)
...
source("/home/h2048/script/R/enrichment_functions.R")
source("/home/h2048/script/R/scMASC.R")
source("/home/h2048/script/R/CellMarker.R")
# rm(results)
...
diagnose_barcode_format <- function(seurat_obj, obj_name = "Seurat Object") {
  cat("=", rep("=", 60), "=\n", sep = "")
  cat(sprintf("诊断对象 (Diagnosing): %s\n", obj_name))
  cat("=", rep("=", 60), "=\n", sep = "")
  ...
  cat(sprintf("\n【基本信息 Basic Info】\n"))
  cat(sprintf("  总细胞数 (Total cells): %d\n", n_cells))

  # 2. 展示前10个barcode示例
  cat(sprintf("\n【Barcode示例 Examples】\n"))
  cat("  前10个barcode:\n")
  for (i in 1:min(10, n_cells)) {
    cat(sprintf("    [%2d] %s\n", i, barcodes[i]))
  }

  # 3. Barcode长度分析
  barcode_lengths <- nchar(barcodes)
  cat(sprintf("\n【Barcode长度分析 Length Analysis】\n"))
  cat(sprintf("  最小长度 (Min): %d\n", min(barcode_lengths)))
  cat(sprintf("  最大长度 (Max): %d\n", max(barcode_lengths)))
  cat(sprintf("  中位数长度 (Median): %d\n", median(barcode_lengths)))

  length_table <- table(barcode_lengths)
  if (length(length_table) > 1) {
    cat("  ⚠️  警告：发现不同长度的barcode！\n")
    cat("  长度分布:\n")
    print(length_table)
  } else {
    cat("  ✓ 所有barcode长度一致\n")
  ...
  # 4. 检查是否包含样本后缀 (sample suffix)
  has_suffix <- grepl("-[0-9]+$", barcodes)
  n_with_suffix <- sum(has_suffix)

  cat(sprintf("\n【后缀检测 Suffix Detection】\n"))
  if (n_with_suffix > 0) {
    cat(sprintf(
      "  ✓ 检测到样本后缀 (Detected suffix): %d/%d (%.1f%%)\n",
      n_with_suffix,
      n_cells,
      100 * n_with_suffix / n_cells
    ))

    # 提取后缀
    suffixes <- gsub(".*-([0-9]+)$", "\\1", barcodes[has_suffix])
    unique_suffixes <- unique(suffixes)

    cat(sprintf(
      "  唯一后缀数 (Unique suffixes): %d\n",
      length(unique_suffixes)
    ))
    cat("  后缀列表:\n")
    for (suf in sort(unique_suffixes)) {
      n_suf <- sum(suffixes == suf)
      cat(sprintf(
        "    -%s: %d cells (%.1f%%)\n",
        suf,
        n_suf,
    ...
    for (i in 1:min(5, n_cells)) {
      original <- barcodes[i]
      cleaned <- gsub("-[0-9]+$", "", original)
      cat(sprintf("    %s  →  %s\n", original, cleaned))
    }
  } else {
    cat("  ✗ 未检测到样本后缀 (No suffix detected)\n")
  }

  # 5. 检查meta.data中的样本信息
  cat(sprintf("\n【Meta.data中的样本信息 Sample Info in Meta.data】\n"))
  if ("orig.ident" %in% colnames(seurat_obj@meta.data)) {
    orig_idents <- unique(seurat_obj@meta.data$orig.ident)
    cat(sprintf("  orig.ident列: %d个唯一值\n", length(orig_idents)))
    cat("  值列表:\n")
    for (ident in orig_idents) {
      n_ident <- sum(seurat_obj@meta.data$orig.ident == ident)
      cat(sprintf("    %s: %d cells\n", ident, n_ident))
    }
  } else {
    cat("  ⚠️  未找到orig.ident列\n")
  }

  # 6. 检查是否有重复barcode
  cat(sprintf("\n【重复性检查 Duplication Check】\n"))
  duplicated_barcodes <- barcodes[duplicated(barcodes)]
  if (length(duplicated_barcodes) > 0) {
    cat(sprintf("  ⚠️  发现重复barcode: %d个\n", length(duplicated_barcodes)))
    cat("  示例:\n")
    for (i in 1:min(5, length(duplicated_barcodes))) {
      cat(sprintf("    %s\n", duplicated_barcodes[i]))
    }
  } else {
    cat("  ✓ 无重复barcode\n")
  }
  ...
  return(invisible(list(
    ...
    has_suffix = any(has_suffix),
    suffix_percentage = 100 * n_with_suffix / n_cells,
    unique_suffixes = if (n_with_suffix > 0) unique(suffixes) else NULL,
    has_duplicates = length(duplicated_barcodes) > 0
  )))
}
...
# result1 <- diagnose_barcode_format(seurat_obj2, "Object 1")
...
metadata_transfer_simple <- function(
  merged_seurat,
  ...
  cat("\n")
  cat("╔", rep("═", 70), "╗\n", sep = "")
  cat("║", "  Metadata转移工具 - 直接匹配版", rep(" ", 39), "║\n", sep = "")
  cat("╚", rep("═", 70), "╝\n", sep = "")

  # ========== 步骤1: 读取原始metadata ==========
  cat("\n")
  cat("【步骤 1/5】读取原始Metadata\n")
  cat("─", rep("─", 70), "\n", sep = "")

  cat(sprintf("文件路径: %s\n", original_metadata_file))

  # 使用fread读取
  original_metadata <- fread(original_metadata_file, data.table = FALSE)

  cat(sprintf("✓ 成功读取\n"))
  cat(sprintf("  行数（细胞数）: %d\n", nrow(original_metadata)))
  cat(sprintf("  列数: %d\n", ncol(original_metadata)))

  # 显示列名
  cat("\n列名 (前15列):\n")
  col_names <- names(original_metadata)
  for (i in 1:min(15, length(col_names))) {
    cat(sprintf("  [%2d] %s\n", i, col_names[i]))
  }
  ...
  # 确定原始barcode列
  if (barcode_col %in% col_names) {
    original_barcodes <- original_metadata[[barcode_col]]
    cat(sprintf("\n✓ 使用barcode列: %s\n", barcode_col))
  } else if ("original_barcode" %in% col_names) {
    barcode_col <- "original_barcode"
    original_barcodes <- original_metadata[[barcode_col]]
    cat(sprintf("\n✓ 使用barcode列: %s\n", barcode_col))
  } else {
    barcode_col <- col_names[1]
    original_barcodes <- original_metadata[[barcode_col]]
    cat(sprintf("\n⚠️  未找到标准barcode列，使用第一列: %s\n", barcode_col))
  }

  cat("\n原始metadata的barcode示例:\n")
  for (i in 1:min(5, length(original_barcodes))) {
    cat(sprintf("  [%d] %s\n", i, original_barcodes[i]))
  }

  # 检查是否有重复的barcode
  n_unique_original <- length(unique(original_barcodes))
  if (n_unique_original < length(original_barcodes)) {
    cat(sprintf("\n⚠️  警告：原始metadata中有重复的barcode\n"))
    cat(sprintf("  总barcode数: %d\n", length(original_barcodes)))
    cat(sprintf("  唯一barcode数: %d\n", n_unique_original))
    cat(sprintf(
      "  重复数: %d\n",
      length(original_barcodes) - n_unique_original
    ))
  }

  # ========== 步骤2: 分析merged对象的barcode结构 ==========
  cat("\n")
  cat("【步骤 2/5】分析Merged对象的Barcode结构\n")
  cat("─", rep("─", 70), "\n", sep = "")

  merged_barcodes <- colnames(merged_seurat)
  n_merged_cells <- length(merged_barcodes)

  cat(sprintf("Merged对象细胞数: %d\n", n_merged_cells))

  cat("\nMerged对象的barcode示例:\n")
  for (i in 1:min(5, n_merged_cells)) {
    cat(sprintf("  [%d] %s\n", i, merged_barcodes[i]))
  }

  # 提取前缀（最后一个下划线之前的内容）
  merged_prefixes <- gsub("_[^_]+$", "", merged_barcodes)
  unique_prefixes <- unique(merged_prefixes)

  # 使用table统计（避免循环）
  prefix_counts <- table(merged_prefixes)

  cat(sprintf("\n检测到 %d 个唯一前缀:\n", length(unique_prefixes)))

  # 按数量排序显示
  prefix_counts_sorted <- sort(prefix_counts, decreasing = TRUE)

  for (i in 1:length(prefix_counts_sorted)) {
    prefix_name <- names(prefix_counts_sorted)[i]
    prefix_count <- prefix_counts_sorted[i]
    prefix_pct <- round(100 * prefix_count / n_merged_cells, 2)
    ...
    # 找示例（使用which避免全向量比较）
    example_indices <- which(merged_prefixes == prefix_name)[1:2]
    example_indices <- example_indices[!is.na(example_indices)]
  ...
  # 如果用户指定了要处理的前缀
  if (!is.null(merge_prefixes)) {
    cat("\n用户指定的前缀:\n")
    for (prefix in merge_prefixes) {
      cat(sprintf("  - %s\n", prefix))
    }

    missing_prefixes <- setdiff(merge_prefixes, unique_prefixes)
    if (length(missing_prefixes) > 0) {
      cat("\n⚠️  警告：以下指定的前缀在merged对象中未找到:\n")
      for (prefix in missing_prefixes) {
        cat(sprintf("  - %s\n", prefix))
      }
    }

    prefixes_to_process <- intersect(merge_prefixes, unique_prefixes)
  } else {
    prefixes_to_process <- unique_prefixes
  }

  cat(sprintf("\n将处理 %d 个前缀\n", length(prefixes_to_process)))

  # ========== 步骤3: 去除前缀，提取原始barcode ==========
  cat("\n")
  cat("【步骤 3/5】去除前缀，提取原始Barcode\n")
  cat("─", rep("─", 70), "\n", sep = "")

  # 去除前缀，得到原始barcode
  original_barcode_from_merged <- gsub("^.*_", "", merged_barcodes)

  cat("\nBarcode转换示例（随机10个）:\n")
  sample_indices <- sample(
    length(merged_barcodes),
    min(10, length(merged_barcodes))
  )
  ...
  for (idx in sample_indices) {
    ...
    cat(sprintf("  前缀:     %s\n", merged_prefixes[idx]))
    cat(sprintf("  去除前缀: %s\n", original_barcode_from_merged[idx]))
  }
  ...
  for (prefix in prefixes_to_process) {
    prefix_mask <- merged_prefixes == prefix
    n_cells <- sum(prefix_mask)

    cat(sprintf("前缀 '%s': %d cells\n", prefix, n_cells))

    # 显示该前缀下的原始barcode示例
    examples <- head(original_barcode_from_merged[prefix_mask], 3)
    cat("  去除前缀后的barcode示例:\n")
    for (ex in examples) {
      cat(sprintf("    %s\n", ex))
    }
  ...
  # ========== 步骤4: 直接匹配barcode ==========
  cat("\n")
  cat("【步骤 4/5】直接匹配Barcode\n")
  cat("─", rep("─", 70), "\n", sep = "")

  cat("匹配策略: 使用去除前缀后的barcode直接在原始metadata中查找\n")

  # 使用match函数直接查找匹配的索引
  # match(x, table) 返回x中每个元素在table中第一次出现的位置
  match_indices <- match(original_barcode_from_merged, original_barcodes)

  # 统计匹配结果
  matched_mask <- !is.na(match_indices)
  n_matched <- sum(matched_mask)
  match_rate <- 100 * n_matched / n_merged_cells

  cat("\n【匹配结果统计】\n\n")
  cat(sprintf("Merged对象总细胞数: %d\n", n_merged_cells))
  cat(sprintf("原始Metadata细胞数: %d\n", length(original_barcodes)))
  cat(sprintf("成功匹配: %d cells (%.2f%%)\n", n_matched, match_rate))

  # 未匹配的细胞
  if (n_matched < n_merged_cells) {
    unmatched_indices <- which(!matched_mask)

    cat(sprintf(
      "\n⚠️  有 %d 个细胞未匹配到metadata (%.2f%%)\n",
      length(unmatched_indices),
      100 * length(unmatched_indices) / n_merged_cells
    ))

    cat("\n未匹配细胞示例:\n")
    show_n <- min(5, length(unmatched_indices))
    for (i in 1:show_n) {
  ...
  for (prefix in prefixes_to_process) {
    prefix_mask <- merged_prefixes == prefix
    n_total <- sum(prefix_mask)
    n_matched_prefix <- sum(prefix_mask & matched_mask)
    prefix_match_rate <- 100 * n_matched_prefix / n_total
    ...
    if (n_matched_prefix < n_total) {
      n_unmatched <- n_total - n_matched_prefix
      cat(sprintf(
        "  未匹配: %d (%.2f%%)\n",
        n_unmatched,
  ...
  # 检查原始metadata中哪些barcode被使用了
  used_indices <- unique(match_indices[matched_mask])
  unused_indices <- setdiff(1:length(original_barcodes), used_indices)

  if (length(unused_indices) > 0) {
    cat(sprintf(
      "ℹ️  原始metadata中有 %d 个barcode在merged对象中未找到\n",
      length(unused_indices)
    ))
    cat("这些barcode可能在merge过程中被过滤掉了\n")
    cat("\n示例:\n")
    for (i in 1:min(5, length(unused_indices))) {
      cat(sprintf("  %s\n", original_barcodes[unused_indices[i]]))
    }
    cat("\n")
  }
  ...
  cols_to_transfer <- setdiff(names(original_metadata), cols_to_exclude)

  cat(sprintf("\n将转移 %d 列metadata:\n\n", length(cols_to_transfer)))

  for (i in seq_along(cols_to_transfer)) {
    col <- cols_to_transfer[i]
    # 统计匹配细胞中非NA的数量
    matched_values <- original_metadata[[col]][match_indices[matched_mask]]
    n_non_na <- sum(!is.na(matched_values))

    cat(sprintf("  [%2d] %-30s (非NA: %d cells)\n", i, col, n_non_na))
  }

  # 显示匹配成功的细胞数据预览
  cat("\n")
  cat("─", rep("─", 70), "\n", sep = "")
  cat("【数据预览 - 随机抽取5个匹配的细胞】\n\n")

  if (n_matched > 0) {
    matched_indices_list <- which(matched_mask)
    sample_matched <- sample(
      matched_indices_list,
      min(5, length(matched_indices_list))
    )

    for (i in seq_along(sample_matched)) {
      idx <- sample_matched[i]
      meta_idx <- match_indices[idx]

      cat(sprintf("Cell %d:\n", i))
      cat(sprintf("  Merged barcode: %s\n", merged_barcodes[idx]))
      cat(sprintf("  去除前缀后:     %s\n", original_barcode_from_merged[idx]))
      cat(sprintf("  前缀:           %s\n", merged_prefixes[idx]))
      cat(sprintf("  匹配到原始metadata的第 %d 行\n", meta_idx))

      cat("  Metadata:\n")
      show_cols <- head(cols_to_transfer, 5)
      for (col in show_cols) {
        val <- original_metadata[[col]][meta_idx]
        cat(sprintf("    %-25s: %s\n", col, val))
      }
      cat("\n")
    }
  ...
  cat("统计摘要:\n")
  cat(sprintf("  - Merged对象细胞数: %d\n", n_merged_cells))
  cat(sprintf("  - 成功匹配细胞数: %d (%.2f%%)\n", n_matched, match_rate))
  cat(sprintf("  - 将转移列数: %d\n", length(cols_to_transfer)))

  if (match_rate >= 95) {
    cat("\n✓ 匹配率优秀 (≥95%%)，建议执行转移\n")
  } else if (match_rate >= 80) {
    cat("\n⚠️  匹配率良好 (80-95%%)，可以执行转移\n")
    cat("   建议检查未匹配的细胞是否符合预期\n")
  } else {
    cat("\n⚠️  匹配率较低 (<80%%)，建议检查:\n")
    cat("   1. 原始metadata的barcode列是否正确\n")
    cat("   2. 去除前缀后的barcode格式是否正确\n")
    cat("   3. barcode格式是否一致\n")
  }
  ...
  if (execute_transfer) {
    cat("\n")
    cat("╔", rep("═", 70), "╗\n", sep = "")
    cat("║", "  执行Metadata转移", rep(" ", 53), "║\n", sep = "")
    cat("╚", rep("═", 70), "╝\n", sep = "")

    cat("\n开始转移metadata到Seurat对象...\n")

    # 直接使用match_indices进行转移
    for (col in cols_to_transfer) {
      # 检查列是否已存在
      if (col %in% colnames(merged_seurat@meta.data)) {
        cat(sprintf("  ⚠️  列 '%s' 已存在，将被覆盖\n", col))
      }

      # 初始化为NA
      new_col <- rep(NA, n_merged_cells)

      # 对匹配成功的细胞，转移数据
      new_col[matched_mask] <- original_metadata[[col]][match_indices[
        matched_mask
      ]]

      # 添加到Seurat对象
      merged_seurat@meta.data[[col]] <- new_col
    }
    ...
    verify_indices <- sample(which(matched_mask), min(3, sum(matched_mask)))

    for (idx in verify_indices) {
      cell_barcode <- merged_barcodes[idx]
      meta_idx <- match_indices[idx]

      cat(sprintf("Cell: %s\n", cell_barcode))

      show_cols <- head(cols_to_transfer, 3)
      for (col in show_cols) {
        original_val <- original_metadata[[col]][meta_idx]
        transferred_val <- merged_seurat@meta.data[cell_barcode, col]
        match_status <- ifelse(
          identical(original_val, transferred_val),
          "✓",
          "✗"
        )
        cat(sprintf("  %s %-25s: %s\n", match_status, col, transferred_val))
      }
      cat("\n")
    }
    ...
    return(list(
      success = TRUE,
      updated_seurat = merged_seurat,
      match_rate = match_rate,
      n_matched = n_matched,
      match_indices = match_indices,
      matched_mask = matched_mask,
      cols_transferred = cols_to_transfer
    ))
  } else {
    cat("\n⚠️  execute_transfer = FALSE，仅预览，未执行转移\n")
    cat("\n如果确认上述匹配结果正确，请设置:\n")
    cat("  execute_transfer = TRUE\n")
    cat("然后重新运行函数\n\n")
    ...
    return(list(
      ...
      match_indices = match_indices,
      matched_mask = matched_mask,
      cols_to_transfer = cols_to_transfer,
      merged_barcodes = merged_barcodes,
      original_barcode_from_merged = original_barcode_from_merged,
...
cat("\n")
cat("═", rep("═", 70), "═\n", sep = "")
cat("开始运行Metadata转移流程（预览模式）\n")
cat("═", rep("═", 70), "═\n\n", sep = "")

result <- metadata_transfer_simple(
  merged_seurat = seurat_obj_main,
  original_metadata_file = "E:/R/1012/complete_cell_metadata.csv",
  merge_prefixes = c("seurat_raw_1012_Batch1", "brushCTRL"),
  barcode_col = "cell_barcode",
  execute_transfer = FALSE # 先预览
)
...
#   merge_prefixes = c("seurat_raw_1012_Batch1", "brushCTRL"),
#   barcode_col = "cell_barcode",
#   execute_transfer = TRUE  # 执行转移
# )
#
# # 保存更新后的对象
# seurat_obj_main <- result$updated_seurat
# saveRDS(seurat_obj_main, "seurat_obj_main_with_metadata.rds")


#----- 完整导出：Barcode解析 + 所有Metadata -----
...
export_complete_metadata <- function(
  seurat_obj,
  output_file = "complete_cell_metadata.csv"
) {
  cat("\n")
  cat("╔", rep("═", 68), "╗\n", sep = "")
  cat(
    "║  完整Metadata导出工具 (Barcode + All Metadata)",
    rep(" ", 21),
    "║\n",
    sep = ""
  )
  ...
  # 解析前缀（最后一个下划线之前）
  barcode_info$prefix <- gsub("_[^_]+$", "", barcodes)

  # 解析原始barcode（最后一个下划线之后）
  barcode_info$original_barcode <- gsub("^.*_", "", barcodes)

  # 检查是否有数字后缀（如 -1, -27）
  has_numeric_suffix <- grepl("-[0-9]+", barcode_info$original_barcode)
  barcode_info$has_numeric_suffix <- has_numeric_suffix

  # 提取核心barcode（去除所有后缀）
  barcode_info$core_barcode <- gsub(
    "-[0-9]+.*$",
    "",
    barcode_info$original_barcode
  )

  # 如果有数字后缀，提取最后一个数字后缀
  barcode_info$numeric_suffix <- ifelse(
    has_numeric_suffix,
    gsub(".*-([0-9]+)$", "\\1", barcode_info$original_barcode),
    NA
  )

  cat(sprintf("  检测到 %d 个唯一前缀\n", length(unique(barcode_info$prefix))))
  cat(sprintf(
    "  有数字后缀的细胞: %d (%.1f%%)\n",
    sum(has_numeric_suffix),
    100 * sum(has_numeric_suffix) / n_cells
  ))
  ...
  # 4. 合并barcode信息和metadata
  cat("\n[4/5] 合并数据...\n")

  # 确保行名一致
  rownames(barcode_info) <- barcodes
  rownames(metadata) <- rownames(metadata) # 保持原有行名

  # 合并：barcode信息 + metadata
  complete_data <- cbind(barcode_info, metadata)

  cat(sprintf(
    "  合并后数据维度: %d cells × %d columns\n",
    nrow(complete_data),
    ncol(complete_data)
  ))
  ...
  # 使用data.table的fwrite，更快速
  fwrite(complete_data, output_file, row.names = FALSE, sep = ",", quote = TRUE)
  ...
  # 显示前几列
  preview_cols <- colnames(complete_data)[1:min(10, ncol(complete_data))]
  print(head(complete_data[, preview_cols], 3))

  if (ncol(complete_data) > 10) {
    cat(sprintf("\n... 还有 %d 列未显示\n", ncol(complete_data) - 10))
  }
  ...
  # 前缀分布
  prefix_table <- table(complete_data$prefix)
  prefix_df <- data.frame(
    prefix = names(prefix_table),
    n_cells = as.vector(prefix_table),
    percentage = round(100 * as.vector(prefix_table) / n_cells, 2)
  )
  prefix_df <- prefix_df[order(-prefix_df$n_cells), ]
  ...
  return(invisible(complete_data))
}

# ========== 使用方法 ==========

# 假设您的Seurat对象已读取，名为 seurat_obj
# seurat_obj <- readRDS("your_seurat.rds")

# 导出完整数据
complete_data <- export_complete_metadata(
  seurat_obj = seurat_obj_main,
  output_file = "complete_cell_metadata_seurat_obj_main.csv"
)

# 导出完成！文件包含：
# - cell_barcode: 完整的细胞barcode
# - prefix: 提取的前缀
# - original_barcode: 去除前缀后的原始barcode
# - has_numeric_suffix: 是否有数字后缀
# - core_barcode: 核心barcode（去除所有后缀）
# - numeric_suffix: 数字后缀值
# - [所有原始metadata列...]
...
# ==============================================================================
cat("读取CSV和Seurat对象...\n")
updates <- fread("metadata.csv")
# seurat_obj_main <- readRDS("your_seurat_obj_mainect.rds")  # 修改为你的文件路径

cat(sprintf("CSV规则数: %d\n", nrow(updates)))
...
# 获取需要更新的列名
update_cols <- setdiff(colnames(updates), c("study", "tissue_sampling_method"))

# 逐行处理每个规则
for (i in 1:nrow(updates)) {
  rule <- updates[i, ]

  # 找出符合条件的细胞（同时满足study和tissue_sampling_method）
  cells_to_update <- which(
    seurat_obj_main$study == rule$study &
      seurat_obj_main$tissue_sampling_method == rule$tissue_sampling_method
  )

  n_cells <- length(cells_to_update)

  if (n_cells > 0) {
    cat(sprintf(
      "规则%d: study='%s', method='%s' -> 更新%d个细胞\n",
      i,
      rule$study,
      rule$tissue_sampling_method,
      n_cells
    ))
    ...
    for (col in update_cols) {
      ...
      if (!is.na(value) && value != "") {
        seurat_obj_main@meta.data[cells_to_update, col] <- value
      }
    }
  } else {
    cat(sprintf("警告: 规则%d未匹配到细胞\n", i))
  }
}
...
# ==============================================================================
cat("\n保存更新后的对象...\n")
# saveRDS(seurat_obj_main, "seurat_obj_main_updated.rds")
cat("完成!\n")

# 4. 简单验证
# ==============================================================================
cat("\n验证更新结果:\n")
for (i in 1:min(3, nrow(updates))) {
  # 验证前3条规则
  rule <- updates[i, ]
  cells <- which(
    seurat_obj_main$study == rule$study &
  ...
  if (length(cells) > 0) {
    ...
    cat(sprintf(
      "  assay: %s\n",
      unique(seurat_obj_main@meta.data[cells, "assay"])
    ))
  }
}
###########################################################################################################
...
#!/usr/bin/env Rscript
# ============================================================================
# Metadata Cleaning Script for Seurat Object
# 清理Seurat对象中的临时和中间计算metadata列
# ============================================================================
...
#' @return 包含待删除列名的列表，按类别分组
identify_columns_to_remove <- function(seurat_obj) {
  # 获取所有metadata列名
  all_cols <- colnames(seurat_obj@meta.data)

  # 初始化结果列表
  cols_to_remove <- list(
    doubletfinder = character(),
    cell_type_scores = character(),
    doublet_detection = character()
  )

  # ==========================================================================
  # 第1类: DoubletFinder产生的临时列
  # ==========================================================================
  # Pattern: pANN_* 和 DF.classifications_*
  cols_to_remove$doubletfinder <- c(
    grep("^pANN_", all_cols, value = TRUE),
    grep("^DF\\.classifications_", all_cols, value = TRUE)
  )

  # ==========================================================================
  # 第2类: 细胞类型评分的临时列
  # ==========================================================================
  # Pattern: *_score, *_count, *_ratio
  # 但要排除一些可能的保留列（如doublet_score会在第3类处理）

  score_cols <- grep("_score$", all_cols, value = TRUE)
  count_cols <- grep("_count$", all_cols, value = TRUE)
  ratio_cols <- grep("_ratio$", all_cols, value = TRUE)

  # 常见的细胞类型前缀
  cell_type_prefixes <- c(
    "Epithelial",
    "Myeloid",
    "Fibroblast",
    "Endothelial",
  ...
  score_pattern <- paste0(
    "^(",
    paste(cell_type_prefixes, collapse = "|"),
    ")_score$"
  )
  ...
  ratio_pattern <- paste0(
    "^(",
    paste(cell_type_prefixes, collapse = "|"),
    ")_ratio$"
  )

  cols_to_remove$cell_type_scores <- c(
    grep(score_pattern, all_cols, value = TRUE),
    grep(count_pattern, all_cols, value = TRUE),
    grep(ratio_pattern, all_cols, value = TRUE)
  )
  ...
  # 只保留实际存在的列
  cols_to_remove$doublet_detection <- intersect(
    doublet_intermediate_cols,
    all_cols
  )
...
#' 打印待删除列的摘要信息
#'
#' @param cols_list 待删除列的分类列表
print_removal_summary <- function(cols_list) {
  cat("\n")
  cat("=", rep("=", 70), "\n", sep = "")
  cat("  Metadata Cleaning Summary | 元数据清理汇总\n")
  cat("=", rep("=", 70), "\n", sep = "")

  # 第1类
  cat("\n【Category 1】DoubletFinder Temporary Columns | DoubletFinder临时列\n")
  cat(rep("-", 72), "\n", sep = "")
  if (length(cols_list$doubletfinder) > 0) {
    cat(sprintf("Found %d columns:\n", length(cols_list$doubletfinder)))
    for (col in cols_list$doubletfinder) {
      cat(sprintf("  - %s\n", col))
    }
  } else {
    cat("  No columns found in this category.\n")
  }

  # 第2类
  cat("\n【Category 2】Cell Type Score Columns | 细胞类型评分列\n")
  cat(rep("-", 72), "\n", sep = "")
  if (length(cols_list$cell_type_scores) > 0) {
    cat(sprintf("Found %d columns:\n", length(cols_list$cell_type_scores)))

    # 按后缀分组显示
    score_cols <- grep("_score$", cols_list$cell_type_scores, value = TRUE)
    count_cols <- grep("_count$", cols_list$cell_type_scores, value = TRUE)
    ratio_cols <- grep("_ratio$", cols_list$cell_type_scores, value = TRUE)
  ...
  # 第3类
  cat(
    "\n【Category 3】Doublet Detection Intermediate Columns | 双胞体检测中间列\n"
  )
  cat(rep("-", 72), "\n", sep = "")
  if (length(cols_list$doublet_detection) > 0) {
    cat(sprintf("Found %d columns:\n", length(cols_list$doublet_detection)))
    for (col in cols_list$doublet_detection) {
      cat(sprintf("  - %s\n", col))
    }
  } else {
    cat("  No columns found in this category.\n")
  ...
  # 总计
  total_cols <- sum(sapply(cols_list, length))
  cat("\n")
  cat(rep("=", 72), "\n", sep = "")
  cat(sprintf(
    "【Total】Will remove %d columns in total | 总共将删除 %d 列\n",
    total_cols,
    total_cols
  ))
  cat(rep("=", 72), "\n", sep = "")
...
#'
#' @param seurat_obj Seurat对象
#' @param cols_to_remove 待删除列的列表
#' @param backup 是否备份原始对象（默认TRUE）
#' @return 清理后的Seurat对象
clean_metadata <- function(seurat_obj, cols_to_remove, backup = TRUE) {
  # 合并所有要删除的列
  all_cols_to_remove <- unique(unlist(cols_to_remove))

  if (length(all_cols_to_remove) == 0) {
    cat("\nNo columns to remove. Metadata is already clean!\n")
    cat("无需删除任何列，metadata已经是干净的！\n\n")
    return(seurat_obj)
  }

  # 可选：备份原始对象
  if (backup) {
    cat("\nCreating backup of metadata...\n")
    cat("正在创建metadata备份...\n")
    backup_metadata <- seurat_obj@meta.data
    backup_file <- paste0(
      "metadata_backup_",
      format(Sys.time(), "%Y%m%d_%H%M%S"),
      ".rds"
    )
    saveRDS(backup_metadata, backup_file)
    cat(sprintf("Backup saved to: %s\n", backup_file))
    cat(sprintf("备份已保存至: %s\n\n", backup_file))
  }

  # 记录删除前的列数
  cols_before <- ncol(seurat_obj@meta.data)

  # 删除指定列
  cat("Removing columns from metadata...\n")
  cat("正在从metadata中删除列...\n")

  # 验证所有列都存在
  existing_cols <- intersect(all_cols_to_remove, colnames(seurat_obj@meta.data))
  non_existing_cols <- setdiff(
    all_cols_to_remove,
    colnames(seurat_obj@meta.data)
  )

  if (length(non_existing_cols) > 0) {
    cat("\nWarning: The following columns do not exist and will be skipped:\n")
    cat("警告：以下列不存在，将被跳过：\n")
  ...
  seurat_obj@meta.data <- seurat_obj@meta.data[,
    !(colnames(seurat_obj@meta.data) %in% existing_cols),
    drop = FALSE
  ]

  # 记录删除后的列数
  cols_after <- ncol(seurat_obj@meta.data)

  cat("\n")
  cat(rep("=", 72), "\n", sep = "")
  cat("  Cleaning Completed | 清理完成\n")
  cat(rep("=", 72), "\n", sep = "")
  cat(sprintf(
    "Columns before cleaning: %d | 清理前列数: %d\n",
    cols_before,
    cols_before
  ))
  cat(sprintf(
    "Columns removed: %d | 已删除列数: %d\n",
    length(existing_cols),
    length(existing_cols)
  ))
  cat(sprintf(
    "Columns after cleaning: %d | 清理后列数: %d\n",
    cols_after,
    cols_after
  ))
  cat(rep("=", 72), "\n", sep = "")
...
# 主函数：交互式清理流程
# ============================================================================
#' 交互式清理Seurat对象的metadata
#'
#' @param seurat_obj Seurat对象
#' @param interactive 是否使用交互式确认（默认TRUE）
#' @param backup 是否备份metadata（默认TRUE）
#' @return 清理后的Seurat对象
clean_seurat_metadata <- function(
  seurat_obj,
  interactive = TRUE,
  backup = TRUE
) {
  ...
  cat(
    "#  Seurat Metadata Cleaning Tool | Seurat元数据清理工具              #\n"
  )
  cat(
    "#                                                                     #\n"
  )
  ...
  # 步骤1: 识别待删除的列
  cat("\nStep 1/3: Identifying columns to remove...\n")
  cat("步骤 1/3: 正在识别需要删除的列...\n")

  cols_to_remove <- identify_columns_to_remove(seurat_obj)

  # 步骤2: 显示摘要并等待确认
  print_removal_summary(cols_to_remove)
  ...
  if (interactive) {
    ...
    cat("步骤 2/3: 等待用户确认...\n\n")
    cat(
      "Do you want to proceed with removing these columns? | 是否继续删除这些列？\n"
    )
    cat("  [Y/y] Yes, remove all listed columns | 是，删除所有列出的列\n")
    cat("  [N/n] No, cancel the operation | 否，取消操作\n")
    cat("\nYour choice | 您的选择: ")
  ...
  # 步骤3: 执行清理
  cat("\nStep 3/3: Cleaning metadata...\n")
  cat("步骤 3/3: 正在清理metadata...\n")

  cleaned_obj <- clean_metadata(seurat_obj, cols_to_remove, backup = backup)

  cat("✅ Metadata cleaning completed successfully!\n")
  cat("✅ Metadata清理成功完成！\n\n")
...
#
# 用法1：交互式清理（推荐）
seurat_obj_main <- clean_seurat_metadata(
  seurat_obj_main,
  interactive = TRUE,
  backup = TRUE
)
#
# 用法2：非交互式清理（自动执行）
# seurat_obj_main <- clean_seurat_metadata(seurat_obj_main,
#                                           interactive = FALSE,
#                                           backup = TRUE)
#
# 用法3：仅查看待删除的列，不执行删除
# cols_info <- identify_columns_to_remove(seurat_obj_main)
# print_removal_summary(cols_info)
#
# ============================================================================

# 如果直接运行此脚本，假设对象名为 seurat_obj_main
if (sys.nframe() == 0) {
  # 检查对象是否存在
  if (!exists("seurat_obj_main")) {
    cat("\n❌ Error: 'seurat_obj_main' object not found in workspace!\n")
    cat("❌ 错误：在工作空间中未找到 'seurat_obj_main' 对象！\n\n")
    cat("Please load your Seurat object first:\n")
    cat("请先加载您的Seurat对象：\n")
    cat("  seurat_obj_main <- readRDS('your_seurat_object.rds')\n\n")
  } else {
    # 执行清理
    seurat_obj_main <- clean_seurat_metadata(
      seurat_obj_main,
...
# =============================================================================
# 元数据Sample级别字段一致性验证 (Sample-level Metadata Consistency Check)
# =============================================================================
...
# 提取元数据为data.table格式（高效处理）
metadata <- as.data.table(seurat_obj2@meta.data, keep.rownames = "cell_barcode")

cat(sprintf("总细胞数 (Total cells): %d\n", nrow(metadata)))
cat(sprintf("独立样本数 (Unique samples): %d\n", uniqueN(metadata$sample)))
cat("\n")

# -----------------------------------------------------------------------------
# 定义应该是sample级别的字段
# -----------------------------------------------------------------------------
sample_level_fields <- c(
  # Tier 1: 核心标识 (Core identifiers)
  "sequencing_platform",
  "organism",
  "tissue",
  "tissue_type",
  ...
  # Tier 2: 批次与供体信息 (Batch and donor info)
  "donor_id",
  "sex",
  ...
  # Tier 3: 取样与处理方法 (Sampling and processing)
  ...
  # 额外重要字段 (Additional important fields)
  "BMI",
  "subject_type",
  "cause_of_death",
...
# 验证向量是否正确构建
cat("定义的字段总数:", length(sample_level_fields), "\n")
cat("检查 'assay' 是否在列表中:", "assay" %in% sample_level_fields, "\n\n")

# 检查这些字段是否存在
existing_fields <- intersect(sample_level_fields, colnames(metadata))
missing_fields <- setdiff(sample_level_fields, colnames(metadata))
...
cat(sprintf("开始检查 %d 个字段的一致性...\n\n", length(existing_fields)))

# 特别检查 assay 字段
if ("assay" %in% colnames(metadata)) {
  cat("✅ 'assay' 字段存在于元数据中\n")
  cat(sprintf("   唯一值数量: %d\n", uniqueN(metadata$assay)))
  cat("   唯一值:\n")
  print(unique(metadata$assay))
  cat("\n")
} else {
  cat("❌ 'assay' 字段不存在于元数据中！\n\n")
}

# -----------------------------------------------------------------------------
# 方法1: 快速检查 - 每个sample内唯一值数量
# -----------------------------------------------------------------------------
cat("=", rep("=", 78), "\n", sep = "")
cat("方法1: 快速一致性检查 (Quick Consistency Check)\n")
cat("=", rep("=", 78), "\n", sep = "")

consistency_check <- lapply(existing_fields, function(field) {
  # 计算每个sample内该字段的唯一值数量
  check <- metadata[,
    .(n_unique = uniqueN(get(field), na.rm = TRUE)),
    by = sample
  ]

  # 统计有多少sample存在不一致
  inconsistent_samples <- check[n_unique > 1, .N]
  max_unique <- max(check$n_unique)

  data.table(
    field = field,
    consistent = (inconsistent_samples == 0),
    inconsistent_samples = inconsistent_samples,
    max_unique_values = max_unique
  )
})

consistency_summary <- rbindlist(consistency_check)
...
# 特别突出显示 assay 的检查结果
if ("assay" %in% consistency_summary$field) {
  assay_result <- consistency_summary[field == "assay"]
  cat("\n")
  cat("━", rep("━", 78), "\n", sep = "")
  cat("🔍 ASSAY 字段检查结果:\n")
  cat("━", rep("━", 78), "\n", sep = "")
  print(assay_result, row.names = FALSE)
  if (assay_result$consistent) {
    cat("✅ assay 字段在每个sample内一致，可以安全提取\n")
  } else {
    cat("⚠️  assay 字段在某些sample内不一致，需要进一步检查\n")
  }
  cat("━", rep("━", 78), "\n", sep = "")
}
...
cat("\n\n")
cat("=", rep("=", 78), "\n", sep = "")
cat("方法2: 详细不一致案例检查 (Detailed Inconsistency Report)\n")
cat("=", rep("=", 78), "\n", sep = "")

# 只检查发现不一致的字段
inconsistent_fields <- consistency_summary[consistent == FALSE, field]

if (length(inconsistent_fields) > 0) {
  cat(sprintf("\n发现 %d 个字段存在不一致!\n\n", length(inconsistent_fields)))
  ...
  for (field in inconsistent_fields) {
    ...
    # 找出不一致的sample
    inconsistent_samples <- metadata[,
      .(
        n_unique = uniqueN(get(field), na.rm = TRUE),
        values = paste(unique(get(field)), collapse = " | "),
        n_cells = .N
      ),
      by = sample
    ][n_unique > 1]

    cat(sprintf("不一致的样本数: %d\n\n", nrow(inconsistent_samples)))

    # 显示前5个不一致的案例
    if (nrow(inconsistent_samples) > 0) {
      cat("示例案例:\n")
      print(head(inconsistent_samples, 5), row.names = FALSE)
      cat("\n")
    }
  }
} else {
  cat("\n✅ 所有字段都完全一致! 可以安全按sample复制。\n\n")
}

# -----------------------------------------------------------------------------
# 方法3: 交叉验证 - 检查关键字段组合的一致性
# -----------------------------------------------------------------------------
cat("\n")
cat("=", rep("=", 78), "\n", sep = "")
cat("方法3: 关键字段组合验证 (Key Field Combination Check)\n")
cat("=", rep("=", 78), "\n", sep = "")

# 检查sample-donor-dataset的对应关系
if (all(c("sample", "donor_id", "dataset") %in% colnames(metadata))) {
  sample_donor_check <- metadata[,
    .(
      n_donors = uniqueN(donor_id),
      n_datasets = uniqueN(dataset),
      donors = paste(unique(donor_id), collapse = ","),
      datasets = paste(unique(dataset), collapse = ",")
    ),
    by = sample
  ]

  cat("\n检查1: Sample-Donor-Dataset 对应关系\n")
  cat("期望: 每个sample应该只对应1个donor和1个dataset\n\n")

  multi_donor <- sample_donor_check[n_donors > 1, .N]
  multi_dataset <- sample_donor_check[n_datasets > 1, .N]

  cat(sprintf("- 对应多个donor的sample数: %d\n", multi_donor))
  cat(sprintf("- 对应多个dataset的sample数: %d\n", multi_dataset))

  if (multi_donor > 0) {
    cat("\n⚠️  异常: 发现sample对应多个donor的情况:\n")
    print(sample_donor_check[n_donors > 1], row.names = FALSE)
  }

  if (multi_dataset > 0) {
    cat("\n⚠️  异常: 发现sample对应多个dataset的情况:\n")
    print(sample_donor_check[n_datasets > 1], row.names = FALSE)
  }
}

# 检查生物学一致性
if (all(c("sample", "sex", "disease") %in% colnames(metadata))) {
  cat("\n\n检查2: 生物学字段一致性 (sex, disease)\n")

  bio_check <- metadata[,
    .(
      n_sex = uniqueN(sex),
      n_disease = uniqueN(disease),
      sex_values = paste(unique(sex), collapse = ","),
      disease_values = paste(unique(disease), collapse = ",")
    ),
    by = sample
  ]

  bio_inconsistent <- bio_check[n_sex > 1 | n_disease > 1]

  if (nrow(bio_inconsistent) > 0) {
    cat("\n⚠️  发现生物学字段不一致:\n")
    print(bio_inconsistent, row.names = FALSE)
  } else {
    cat("✅ 所有样本的生物学字段一致\n")
  }
}
...
if (consistency_summary[, all(consistent)]) {
  # 所有字段一致，可以安全提取
  sample_metadata <- metadata[,
    .SD[1],
    by = sample,
    .SDcols = c("sample", existing_fields)
  ]

  cat(sprintf("\n✅ 成功提取 %d 个样本的元数据\n", nrow(sample_metadata)))
  cat(sprintf("包含 %d 个字段\n\n", ncol(sample_metadata)))

  # 验证 assay 是否被正确提取
  if ("assay" %in% colnames(sample_metadata)) {
    cat("✅ 'assay' 字段已成功提取\n")
    cat(sprintf("   样本-assay分布:\n"))
    assay_dist <- sample_metadata[, .N, by = assay]
    print(assay_dist)
    cat("\n")
  } else {
    cat("❌ 'assay' 字段未被提取！\n\n")
  ...
  # 保存
  output_file <- "sample_level_metadata.csv"
  fwrite(sample_metadata, output_file)
  cat(sprintf("\n✅ 已保存到: %s\n", output_file))
} else {
  cat("\n⚠️  发现不一致字段，建议先解决数据质量问题\n")
  cat("不一致的字段:\n")
  print(consistency_summary[
    consistent == FALSE,
    .(field, inconsistent_samples)
  ])

  cat("\n解决方案:\n")
  cat("1. 检查原始数据注释是否正确\n")
  cat("2. 对不一致字段使用众数或第一个值\n")
  cat("3. 标记这些样本供后续质控\n")
}
...
cat(sprintf("总样本数: %d\n", uniqueN(metadata$sample)))
cat(sprintf("检查字段数: %d\n", length(existing_fields)))
cat(sprintf("一致字段数: %d\n", consistency_summary[consistent == TRUE, .N]))
cat(sprintf("不一致字段数: %d\n", consistency_summary[consistent == FALSE, .N]))

# 特别报告 assay 状态
if ("assay" %in% existing_fields) {
  cat(sprintf("\n✅ 'assay' 字段状态: 已检查并包含在提取字段中\n"))
} else if ("assay" %in% missing_fields) {
  cat(sprintf("\n⚠️  'assay' 字段状态: 在元数据中不存在\n"))
} else {
  cat(sprintf("\n❓ 'assay' 字段状态: 未知\n"))
}

if (consistency_summary[, all(consistent)]) {
  cat("\n✅ 结论: 所有字段都是真正的sample级别字段，可以安全按sample复制!\n")
} else {
  cat("\n⚠️  结论: 存在不一致字段，需要进一步检查或清洗数据\n")
}
...
if (exists("sample_metadata")) {
  # 关键字段的分布统计
  if ("donor_id" %in% colnames(sample_metadata)) {
    cat(sprintf(
      "\n供体数 (Unique donors): %d\n",
      uniqueN(sample_metadata$donor_id)
    ))
  }

  if ("assay" %in% colnames(sample_metadata)) {
    cat("\nAssay类型分布 (Assay distribution):\n")
    print(sample_metadata[, .N, by = assay][order(-N)])
  }

  if ("dataset" %in% colnames(sample_metadata)) {
    cat("\n数据集分布 (Dataset distribution):\n")
    print(sample_metadata[, .N, by = dataset][order(-N)])
  }

  if ("sequencing_platform" %in% colnames(sample_metadata)) {
    cat("\n测序平台分布 (Sequencing platform distribution):\n")
    print(sample_metadata[, .N, by = sequencing_platform][order(-N)])
  }

  if ("disease" %in% colnames(sample_metadata)) {
    cat("\n疾病状态分布 (Disease status distribution):\n")
    print(sample_metadata[, .N, by = disease][order(-N)])
  }

  if ("tissue_sampling_method" %in% colnames(sample_metadata)) {
    cat("\n取样方法分布 (Sampling method distribution):\n")
    print(sample_metadata[, .N, by = tissue_sampling_method][order(-N)])
  }
}
###########################################################################################################
###########################################################################################################
...
# 读取sample级别元数据
sample_metadata <- fread("sample_level_metadata.csv")

cat(sprintf("主对象细胞数: %d\n", ncol(seurat_obj_main)))
cat(sprintf("Sample元数据记录数: %d\n", nrow(sample_metadata)))

# 验证主对象是否有sample列
if (!"sample" %in% colnames(seurat_obj_main@meta.data)) {
  stop("❌ 主Seurat对象的metadata中没有'sample'列！请先确认sample标识字段。")
}

# 获取主对象当前的sample信息
main_samples <- unique(seurat_obj_main@meta.data$sample)
meta_samples <- sample_metadata$sample

cat(sprintf("\n主对象中的样本数: %d\n", length(main_samples)))
cat(sprintf("元数据CSV中的样本数: %d\n", length(meta_samples)))

# 检查sample是否匹配
matched_samples <- intersect(main_samples, meta_samples)
missing_in_csv <- setdiff(main_samples, meta_samples)
extra_in_csv <- setdiff(meta_samples, main_samples)

cat(sprintf("匹配的样本数: %d\n", length(matched_samples)))

if (length(missing_in_csv) > 0) {
  cat(sprintf(
    "\n⚠️  主对象中有 %d 个样本在CSV中缺失:\n",
    length(missing_in_csv)
  ))
  cat(paste("  ", head(missing_in_csv, 10), collapse = "\n"), "\n")
  if (length(missing_in_csv) > 10) cat("  ...\n")
}

if (length(extra_in_csv) > 0) {
  cat(sprintf(
    "\n⚠️  CSV中有 %d 个样本在主对象中不存在:\n",
    length(extra_in_csv)
  ))
  cat(paste("  ", head(extra_in_csv, 10), collapse = "\n"), "\n")
  if (length(extra_in_csv) > 10) cat("  ...\n")
}

# 准备映射：将sample_metadata转换为data.frame并设置行名为sample
sample_meta_df <- as.data.frame(sample_metadata)
rownames(sample_meta_df) <- sample_meta_df$sample

# 要添加的字段（排除sample本身，因为已经存在）
fields_to_add <- setdiff(colnames(sample_metadata), "sample")

cat("\n")
cat("=", rep("=", 78), "\n", sep = "")
cat("开始更新元数据字段\n")
cat("=", rep("=", 78), "\n", sep = "")
cat(sprintf("将添加/更新 %d 个字段\n\n", length(fields_to_add)))

# 逐字段添加/更新
for (field in fields_to_add) {
  # 根据每个细胞的sample值，从sample_meta_df中查找对应的字段值
  sample_vector <- seurat_obj_main@meta.data$sample
  new_values <- sample_meta_df[sample_vector, field]

  # 检查是否字段已存在
  if (field %in% colnames(seurat_obj_main@meta.data)) {
    action <- "更新"
  } else {
    action <- "添加"
  }
  ...
  # 添加到metadata
  seurat_obj_main@meta.data[[field]] <- new_values

  # 统计有效值数量
  n_valid <- sum(!is.na(new_values))
  cat(sprintf(
    "[%s] %s: %d/%d 细胞有有效值 (%.1f%%)\n",
    action,
    field,
...
# 随机抽取几个sample验证
set.seed(123)
test_samples <- sample(matched_samples, min(3, length(matched_samples)))
...
for (test_sample in test_samples) {
  ...
  # 从主对象中获取该sample的所有细胞的前3个新字段值（应该都一样）
  sample_cells <- seurat_obj_main@meta.data$sample == test_sample
  test_fields <- head(fields_to_add, 3)

  for (field in test_fields) {
    unique_vals <- unique(seurat_obj_main@meta.data[sample_cells, field])
    cat(sprintf("  %s: %s", field, paste(unique_vals, collapse = ", ")))

    if (length(unique_vals) == 1) {
      cat(" ✅\n")
    } else {
      cat(sprintf(" ⚠️  (有 %d 个不同值!)\n", length(unique_vals)))
    }
  }
  cat("\n")
}
...
cat(sprintf(
  "元数据字段数: %d (原始) → %d (更新后)\n",
  ncol(seurat_obj_main@meta.data) - length(fields_to_add),
  ncol(seurat_obj_main@meta.data)
))
cat(sprintf("新增/更新字段数: %d\n", length(fields_to_add)))

# 保存更新后的对象
output_rds <- "seurat_obj_main_updated.rds"
cat(sprintf("\n正在保存更新后的Seurat对象到: %s\n", output_rds))
# saveRDS(seurat_obj_main, output_rds)
cat("✅ 保存完成!\n")
...
################################################################################
# Ambient RNA去除方法1: DecontX (推荐)
# 优点: 不需要原始矩阵，基于贝叶斯模型，对严重污染效果好
# 适用于: 已过滤的Seurat对象或SingleCellExperiment对象
# 官方文档: https://rdrr.io/bioc/celda/man/decontX.html
################################################################################
################################################################################
# DecontX Ambient RNA去除 - Seurat对象优化版本
# 方法: 直接提取counts矩阵,避免对象转换,节省内存
# 作者: [Your Name]
# 日期: 2025-10-20
################################################################################
...
run_decontX_from_matrix <- function(
  seurat_obj,
  assay = "RNA",
  slot = "counts",
  batch_column = NULL,
  ...
  if (verbose) {
    cat("==========================================\n")
    cat("DecontX Ambient RNA去污染分析\n")
    cat("==========================================\n\n")
  }

  # 1. 提取counts矩阵 (genes x cells)
  if (verbose) {
    cat("步骤1: 提取counts矩阵...\n")
  }
  ...
  if (!is.null(batch_column)) {
    if (batch_column %in% colnames(seurat_obj@meta.data)) {
      ...
      warning(sprintf("警告: 未找到列 '%s',将不使用批次信息", batch_column))
    }
  }
  ...
  # 4. 提取结果
  if (verbose) {
    cat("步骤4: 提取contamination结果...\n")
  }

  # contamination是一个向量,长度等于细胞数
  contamination_vector <- decontX_results$contamination

  # 确保顺序与Seurat对象一致
  if (length(contamination_vector) != ncol(seurat_obj)) {
    stop("错误: contamination向量长度与Seurat对象细胞数不匹配!")
  }

  # 5. 添加到Seurat对象metadata
  seurat_obj$decontX_contamination <- contamination_vector

  # 也保存DecontX自动识别的clusters
  ...
  if (verbose) {
    cat("\n==========================================\n")
    cat("Contamination统计摘要\n")
    cat("==========================================\n")
    cat(sprintf("平均污染率: %.2f%%\n", mean(contamination_vector) * 100))
    cat(sprintf("中位污染率: %.2f%%\n", median(contamination_vector) * 100))
    cat(sprintf(
      "污染率范围: %.2f%% - %.2f%%\n",
      min(contamination_vector) * 100,
      max(contamination_vector) * 100
    ))
    cat(sprintf("标准差: %.2f%%\n", sd(contamination_vector) * 100))

    # 按阈值统计
    cat("\n按contamination阈值统计细胞数:\n")
    thresholds <- c(0.1, 0.2, 0.3, 0.5)
    for (thresh in thresholds) {
      n_cells <- sum(contamination_vector < thresh)
      pct <- n_cells / length(contamination_vector) * 100
      cat(sprintf("  < %.1f: %d cells (%.1f%%)\n", thresh, n_cells, pct))
    }
  }

  # 7. 可选: 保存校正后的矩阵
  if (verbose) {
    cat("\n步骤5: 保存校正后的counts矩阵到新assay 'decontXcounts'...\n")
  }
  ...
  # Round到整数(Seurat要求)
  decontX_counts_rounded <- round(decontX_counts)

  # 创建新assay
  seurat_obj[["decontXcounts"]] <- CreateAssayObject(
    counts = decontX_counts_rounded
  )
  ...
  if (verbose) {
    ...
    cat("==========================================\n")
    cat("结果已添加到Seurat对象:\n")
    cat("  - seurat_obj$decontX_contamination: 污染率\n")
    cat("  - seurat_obj$decontX_clusters: DecontX聚类\n")
    cat("  - seurat_obj@assays$decontXcounts: 校正后矩阵\n\n")
  }
...
#===============================================================================
# 可视化函数: Contamination分布
#===============================================================================

plot_contamination_overview <- function(
  seurat_obj,
  group_by = NULL,
  reduction = "umap",
  pt_size = 0.1
  ...
  p1 <- FeaturePlot(
    ...
    ggtitle("Contamination Distribution") +
    theme_bw() +
    theme(
  ...
  plot_list[["contamination_umap"]] <- p1
  ...
  if (!is.null(group_by) && group_by %in% colnames(seurat_obj@meta.data)) {
    p2 <- VlnPlot(
      ...
      ggtitle("Contamination by Cell Type") +
      ylab("Contamination") +
      theme_bw() +
      theme(
    ...
    plot_list[["contamination_violin"]] <- p2
    ...
    p3 <- ggplot(
      ...
      scale_y_continuous(labels = scales::percent) +
      ggtitle("Contamination Statistics by Cell Type") +
      xlab("Cell Type") +
      ylab("Contamination") +
      theme_bw() +
      theme(
    ...
    plot_list[["contamination_boxplot"]] <- p3
  }

  # 4. Contamination直方图
  p4 <- ggplot(seurat_obj@meta.data, aes(x = decontX_contamination)) +
    geom_histogram(
      bins = 50,
      fill = "steelblue",
      alpha = 0.7,
  ...
  plot_list[["contamination_histogram"]] <- p4
...
plot_ambient_genes <- function(
  seurat_obj,
  ambient_genes = c("BPIFA1", "STATH", "ZG16B", "MUC5AC"),
  group_by = NULL,
  reduction = "umap"
) {
  # 检查基因是否存在
  genes_present <- ambient_genes[ambient_genes %in% rownames(seurat_obj)]
  if (length(genes_present) == 0) {
    warning("警告: 提供的ambient基因均不在数据集中")
    return(NULL)
  }

  if (length(genes_present) < length(ambient_genes)) {
    missing_genes <- setdiff(ambient_genes, genes_present)
    cat(sprintf(
      "注意: 以下基因不在数据集中: %s\n",
  ...
  plot_list[["before_featureplot"]] <- p1

  # 2. 校正后: FeaturePlot
  if ("decontXcounts" %in% names(seurat_obj@assays)) {
    ...
    plot_list[["after_featureplot"]] <- p2
  }
  ...
  if (!is.null(group_by) && group_by %in% colnames(seurat_obj@meta.data)) {
    # 提取数据
    DefaultAssay(seurat_obj) <- "RNA"
    before_data <- FetchData(seurat_obj, vars = c(genes_present, group_by))
    before_data$status <- "Before DecontX"

    if ("decontXcounts" %in% names(seurat_obj@assays)) {
      DefaultAssay(seurat_obj) <- "decontXcounts"
      after_data <- FetchData(seurat_obj, vars = c(genes_present, group_by))
      after_data$status <- "After DecontX"
      ...
      p3 <- ggplot(
        combined_long,
        aes(x = !!sym(group_by), y = expression, fill = status)
      ) +
        geom_boxplot(outlier.size = 0.3, alpha = 0.7) +
        facet_wrap(~gene, scales = "free_y", ncol = 2) +
        scale_fill_manual(
          values = c("Before DecontX" = "#E64B35", "After DecontX" = "#4DBBD5")
        ) +
        ggtitle("Ambient Gene Expression: Before vs After DecontX") +
        xlab("Cell Type") +
      ...
      plot_list[["comparison_boxplot"]] <- p3
    }
  }
...
filter_contaminated_cells <- function(
  seurat_obj,
  contamination_threshold = 0.2,
  verbose = TRUE
) {
  if (!"decontX_contamination" %in% colnames(seurat_obj@meta.data)) {
    stop("错误: 请先运行 run_decontX_from_matrix()")
  }
  ...
  seurat_filtered <- subset(
    seurat_obj,
    subset = decontX_contamination < contamination_threshold
  )
...
#===============================================================================
# 辅助函数: 生成contamination报告
#===============================================================================

generate_contamination_report <- function(
  seurat_obj,
  output_file = "decontX_report.txt",
  group_by = NULL
) {
  if (!"decontX_contamination" %in% colnames(seurat_obj@meta.data)) {
    stop("错误: 请先运行 run_decontX_from_matrix()")
  ...
  cat("====================================================================\n")
  cat("DecontX Ambient RNA Contamination分析报告\n")
  cat("====================================================================\n")
  cat(sprintf("分析日期: %s\n", Sys.Date()))
  cat(sprintf("细胞总数: %d\n", ncol(seurat_obj)))
  cat(sprintf("基因总数: %d\n", nrow(seurat_obj)))
  ...
  # 全局统计
  cat("--------------------------------------------------------------------\n")
  cat("Contamination全局统计\n")
  cat("--------------------------------------------------------------------\n")
  cont <- seurat_obj$decontX_contamination
  cat(sprintf("平均值: %.4f (%.2f%%)\n", mean(cont), mean(cont) * 100))
  cat(sprintf("中位数: %.4f (%.2f%%)\n", median(cont), median(cont) * 100))
  cat(sprintf("标准差: %.4f (%.2f%%)\n", sd(cont), sd(cont) * 100))
  cat(sprintf("最小值: %.4f (%.2f%%)\n", min(cont), min(cont) * 100))
  cat(sprintf("最大值: %.4f (%.2f%%)\n", max(cont), max(cont) * 100))
  cat(sprintf(
    "25th percentile: %.4f (%.2f%%)\n",
    quantile(cont, 0.25),
    quantile(cont, 0.25) * 100
  ))
  cat(sprintf(
    "75th percentile: %.4f (%.2f%%)\n",
    quantile(cont, 0.75),
    quantile(cont, 0.75) * 100
  ))
  cat("\n")

  # 按阈值统计
  cat("--------------------------------------------------------------------\n")
  cat("按contamination阈值统计\n")
  cat("--------------------------------------------------------------------\n")
  thresholds <- c(0.05, 0.1, 0.15, 0.2, 0.3, 0.5)
  for (thresh in thresholds) {
    n <- sum(cont < thresh)
    pct <- n / length(cont) * 100
    cat(sprintf("< %.2f: %6d cells (%5.2f%%)\n", thresh, n, pct))
  }
  cat("\n")
  ...
  if (!is.null(group_by) && group_by %in% colnames(seurat_obj@meta.data)) {
    ...
    summary_stats <- seurat_obj@meta.data %>%
      ...
      summarise(
        n_cells = n(),
        mean_cont = mean(decontX_contamination),
        median_cont = median(decontX_contamination),
        sd_cont = sd(decontX_contamination),
        .groups = "drop"
      ) %>%
...
# # 假设您的Seurat对象名为 seurat_obj
#
# # 1. 运行DecontX分析
# seurat_obj <- run_decontX_from_matrix(
#   seurat_obj = seurat_obj,
#   assay = "RNA",
#   batch_column = NULL,  # 如有批次效应,设置为批次列名,如 "sample_id"
#   verbose = TRUE
# )
#
#
...
# 步骤1: 运行DecontX
seurat_obj_main <- run_decontX_from_matrix(
  seurat_obj_main
  # ,  batch_column = "sample"     # ⚠️ 如有多样本
  # z = seurat_obj$celltype         # ⚠️ 如已有注释
)

# 步骤2: 生成报告
generate_contamination_report(
  seurat_obj_main,
  output_file = "contamination_report.txt",
  group_by = "Annotation"
)

# 2. 可视化contamination分布
plots_overview <- plot_contamination_overview(
  seurat_obj = seurat_obj_main,
  group_by = "Annotation", # 替换为您的细胞类型列名
  reduction = "umap"
)
...
#   group_by = "Annotation"
# )
pdf('contamination.pdf')
# 查看各个图
(plots_overview$contamination_umap)
(plots_overview$contamination_violin)
(plots_overview$contamination_boxplot)
(plots_overview$contamination_histogram)
dev.off()

#
# # 3. 可视化ambient基因
# plots_genes <- plot_ambient_genes(
#   seurat_obj = seurat_obj,
#   ambient_genes = c("BPIFA1", "STATH", "ZG16B", "MUC5AC"),
#   group_by = "celltype",
#   reduction = "umap"
# )
#
# print(plots_genes$before_featureplot)
# print(plots_genes$after_featureplot)
# print(plots_genes$comparison_boxplot)
#
# # 4. 生成报告
# generate_contamination_report(
#   seurat_obj = seurat_obj,
#   output_file = "/mnt/user-data/outputs/decontX_report.txt",
#   group_by = "celltype"
# )
#
# 5. 过滤高污染细胞
seurat_obj_main <- filter_contaminated_cells(
  seurat_obj = seurat_obj_main,
...
#
# # 6. 保存结果
# saveRDS(seurat_filtered, "/mnt/user-data/outputs/seurat_decontX_filtered.rds")
...
# seurat_obj2 <- readRDS("E:/R/0216/LungMap.rds")
...
seurat_obj_main <- subset(
  seurat_obj_main,
  subset = nFeature_RNA <= 6000 & nFeature_RNA >= 200
)
...
# seurat_obj_main$percent.mt <- ifelse(
#   is.na(seurat_obj_main$percent.mito),
#   seurat_obj_main$percent.mt,
#   seurat_obj_main$percent.mito*100
# )
seurat_obj_main$percent.rp1 <- ifelse(
  is.na(seurat_obj_main$percent.ribo),
  seurat_obj_main$percent.rp,
  seurat_obj_main$percent.ribo
)
seurat_obj_main$percent.rp <- seurat_obj_main$percent.rp1
seurat_obj_main$percent.rp1 <- NULL
# summary(seurat_obj_main$percent.mito*100)
# seurat_obj_main@meta.data$tissue[seurat_obj_main@meta.data$tissue == 'Nose'] <- 'nose'
# seurat_obj_main <- subset(
#   seurat_obj_main,
#   subset = doublet_status == 'Doublet',
#   invert = TRUE
# )
# 直接复制，保留所有值（包括 NA）
seurat_obj_main$percent.mt <- seurat_obj_main$percent.mito
# =====================================================================
# 方法1: 完全重建Assay（最稳定）
# =====================================================================
# seurat_obj_main <- results[["merged_object"]]
...
process_sample_with_conversion <- function(
  seurat_obj,
  sample_name,
  sample_col = "sample"
) {
  ...
  # 提取样本
  cells <- WhichCells(seurat_obj, expression = get(sample_col) == sample_name)
...
# === 方法1: 完全重建（最推荐） ===
seurat_obj_main <- safe_convert_assay(seurat_obj_main)
# print(Layers(seurat_obj_main, assay = "RNA"))
# # 1. 查看现有的降维结果
...
for (sample_name in sample_names) {
  ...
  if (length(sample_cells) > 0) {
    ...
    # 保存质控图
    pdf(
      file.path("qaqc_plots", paste0(sample_name, "_qc.pdf")),
      width = 15,
      height = 10
    )
    print(p1)
    ...
    cat(sprintf(
      "Generated QC plots for sample: %s (Cells: %d)\n",
      sample_name,
      ncol(sample_obj)
    ))
  } else {
...
# ============================================================
# 整合单细胞质控流程 - 逐样本处理版本
# Integrated scRNA-seq Quality Control Pipeline
# ============================================================
#
# 流程顺序 Pipeline Order:
# 1. EmptyDrops过滤 (Empty droplet filtering)
# 2. 基础QC指标 (Basic QC metrics)
# 3. 回归异常值检测 (Regression-based outlier detection)
# 4. DoubletFinder双细胞检测 (Doublet detection)
#
# ============================================================

# ============================================================================================
# 函数1: EmptyDrops处理 (EmptyDrops Processing)
# ============================================================================================
process_emptydrops <- function(
  seurat_obj,
  lower = 100,
  fdr_threshold = 0.01,
  niters = 10000
  ...
  #'
  #' 生物学原理 Biological Rationale:
  #' EmptyDrops基于ambient RNA背景模型,区分真实细胞和空液滴
  #' 真实细胞的基因表达谱显著偏离环境RNA的多项分布
  #'
  #' @param seurat_obj Seurat对象 Seurat object
  #' @param lower 最低UMI阈值,低于此值的液滴用于构建背景模型
  #'              Minimum UMI threshold for background model
  #' @param fdr_threshold FDR阈值,控制假阳性率 FDR threshold
  #' @param niters Monte Carlo模拟迭代次数 MC iterations
  #' @return 通过过滤的细胞索引 Indices of cells passing filter
  ...
  cat(sprintf("Input: %d barcodes × %d genes\n", ncol(mat), nrow(mat)))
  cat(sprintf(
    "Parameters: lower=%d, FDR<%.3f, niters=%d\n",
    lower,
    fdr_threshold,
    niters
  ))
  ...
  cat(sprintf(
    "  ✓ Real cells: %d (%.1f%%)\n",
    n_cells,
    n_cells / ncol(mat) * 100
  ))
  cat(sprintf(
    "  ✗ Empty droplets: %d (%.1f%%)\n",
    n_empty,
    n_empty / ncol(mat) * 100
  ))
  ...
  if (sum(is.cell) > 0) {
    ...
    p <- ggplot(
      plot_df,
      aes(x = Total_UMI, y = -log10(FDR + 1e-100), color = IsCell)
    ) +
      ...
      scale_x_log10() +
      scale_color_manual(values = c("TRUE" = "#2ECC71", "FALSE" = "gray70")) +
      labs(
        title = "EmptyDrops Cell Calling",
        x = "Total UMI counts",
        y = "-log10(FDR)",
        color = "Real Cell"
      ) +
    ...
    return(list(
      cell_indices = which(is.cell),
      plot = p,
      stats = data.frame(
        Total = ncol(mat),
        RealCells = n_cells,
        EmptyDroplets = n_empty,
        CellRate = n_cells / ncol(mat) * 100
      )
    ))
  }
...
# ============================================================================================
# 函数2: 回归异常值检测 (Regression-based Outlier Detection)
# ============================================================================================
detect_regression_outliers <- function(seurat_obj, outlier_threshold = 0.999) {
  #' 基于回归模型的异常细胞检测
  #' Regression-based outlier cell detection
  #'
  #' 生物学原理 Biological Rationale:
  #' nFeature与nCount呈对数线性关系
  #' 偏离此关系的细胞可能是:
  #' - 高复杂度细胞(活跃转录)或多细胞黏连
  #' - 低复杂度细胞(破损/凋亡)或背景污染
  #'
  #' @param seurat_obj Seurat对象 Seurat object
  #' @param outlier_threshold 预测区间置信水平 Prediction interval confidence
  #' @return 通过过滤的细胞名称 Names of cells passing filter
  ...
  cat(sprintf("Regression model: log10(nFeature) ~ log10(nCount)\n"))
  ...
  # 分类异常值类型
  upper_outliers <- log_features > pred[, "upr"]
  lower_outliers <- log_features < pred[, "lwr"]

  cat(sprintf("    - High complexity: %d\n", sum(upper_outliers)))
  cat(sprintf("    - Low complexity: %d\n", sum(lower_outliers)))
  ...
  plot_df <- data.frame(
    ...
    outlier = outliers,
    outlier_type = factor(
      ifelse(!outliers, "Normal", ifelse(upper_outliers, "High", "Low")),
  ...
  p <- ggplot(
    plot_df,
    aes(x = log_counts, y = log_features, color = outlier_type)
  ) +
    geom_point(size = 0.8, alpha = 0.6) +
    geom_line(aes(y = pred[, "fit"]), color = "blue", size = 1) +
    geom_line(aes(y = pred[, "lwr"]), color = "blue", linetype = "dashed") +
    geom_line(aes(y = pred[, "upr"]), color = "blue", linetype = "dashed") +
    scale_color_manual(
      values = c("Normal" = "gray70", "High" = "#E74C3C", "Low" = "#F39C12")
    ) +
    labs(
      title = "Regression-based Outlier Detection",
      subtitle = sprintf("Confidence level: %.1f%%", outlier_threshold * 100),
      x = "log10(nCount_RNA)",
  ...
  return(list(
    cell_names = cells.keep,
    outlier_indices = which(outliers),
    plot = p,
    stats = data.frame(
      Total = length(outliers),
      Normal = sum(!outliers),
      HighComplexity = sum(upper_outliers),
      LowComplexity = sum(lower_outliers),
      OutlierRate = sum(outliers) / length(outliers) * 100
    )
  ))
}

# ============================================================================================
# 函数3: DoubletFinder检测 (DoubletFinder Detection)
# ============================================================================================
run_doubletfinder_sample <- function(
  seurat_obj,
  sample_name,
  doublet_rate = 0.06,
  n_pcs = 30
  ...
  #' 单样本DoubletFinder分析
  #' Single-sample DoubletFinder analysis
  #'
  #' @param seurat_obj 样本子集Seurat对象 Sample subset Seurat object
  #' @param sample_name 样本名称 Sample name
  #' @param doublet_rate 预期双细胞率 Expected doublet rate
  #' @param n_pcs PCA主成分数 Number of PCs
  #' @return 包含检测结果的列表 List with detection results

  n_cells <- ncol(seurat_obj)
  cat(sprintf(
    "\n  [DoubletFinder] Sample: %s (%d cells)\n",
    sample_name,
    n_cells
  ))
  ...
  # 检查细胞数是否充足
  if (n_cells < 50) {
    cat("  ⚠️  Insufficient cells (<50), skipping...\n")
    return(list(
  ...
  tryCatch(
    {
      # 数据预处理
      cat("  [1/4] Preprocessing...\n")
      if (
        is.null(seurat_obj@assays$RNA@data) ||
          nrow(seurat_obj@assays$RNA@data) == 0
      ) {
      ...
      # PCA降维
      cat("  [2/4] Running PCA...\n")
      n_pcs_use <- min(n_pcs, ncol(seurat_obj) - 1, 30)
      n_pcs_use <- max(n_pcs_use, 10)
      seurat_obj <- RunPCA(seurat_obj, npcs = n_pcs_use, verbose = FALSE)

      # pK参数优化
      cat("  [3/4] Optimizing pK...\n")
      sweep.res <- paramSweep(seurat_obj, PCs = 1:n_pcs_use, sct = FALSE)
      ...
      if (is.na(pK) || pK < 0.01 || pK > 0.3) {
        pK <- 0.09
        cat(sprintf("    Using default pK: %.3f\n", pK))
      } else {
      ...
      df_score_col <- grep("pANN", colnames(seurat_obj@meta.data), value = TRUE)
      ...
      if (length(df_class_col) > 0) {
        ...
        # 提取结果
        status_values <- seurat_obj@meta.data[[df_class_col]]
        score_values <- if (!is.null(df_score_col)) {
          seurat_obj@meta.data[[df_score_col]]
        } else {
          rep(NA, n_cells)
        }
        ...
        return(list(
          ...
          results = list(
            ...
            doublet_status = status_values,
            doublet_score = score_values,
            doublet_class = ifelse(
              status_values == "Singlet",
              "Singlet",
              "Doublet"
            ),
        ...
        return(list(status = "Failed", results = NULL))
      }
    },
    error = function(e) {
      cat(sprintf("  ✗ ERROR: %s\n", e$message))
      return(list(status = "Error", results = NULL))
    }
  )
}

# ============================================================================================
# 主流程: 整合质控 (Main Pipeline: Integrated QC)
# ============================================================================================
integrated_qc_pipeline <- function(
  seurat_obj,
  sample_column = "sample",
  output_dir = "integrated_qc_results",
  # EmptyDrops参数
  run_emptydrops = TRUE,
  ...
  cat("\n============================================================\n")
  cat("   Integrated Single-cell QC Pipeline\n")
  cat("   整合单细胞质控流程\n")
  cat("============================================================\n\n")
  ...
  for (i in 1:n_samples) {
    ...
    # 提取样本子集
    cell_idx <- seurat_obj@meta.data[[sample_column]] == sample_name
    cell_names <- colnames(seurat_obj)[cell_idx]
    obj_sample <- subset(seurat_obj, cells = cell_names)

    n_initial <- ncol(obj_sample)
    ...
    # ========== Stage 1: EmptyDrops ==========
    if (run_emptydrops && n_initial >= 50) {
      cat("\n--- Stage 1: EmptyDrops Filtering ---\n")

      emptydrops_result <- process_emptydrops(
        obj_sample,
        lower = emptydrops_lower,
        fdr_threshold = emptydrops_fdr
      )

      # 保存图形
      if (!is.null(emptydrops_result$plot)) {
        ggsave(
          file.path(qc_plots_dir, sprintf("emptydrops_%s.pdf", sample_name)),
          emptydrops_result$plot,
          width = 8,
          height = 6
        )
      }
      ...
      obj_sample <- subset(
        obj_sample,
        cells = colnames(obj_sample)[emptydrops_result$cell_indices]
      )

      sample_qc$stages$emptydrops <- emptydrops_result$stats

      cat(sprintf("Remaining cells: %d\n", ncol(obj_sample)))
    }

    # ========== Stage 2: 基础QC指标 ==========
    cat("\n--- Stage 2: Basic QC Metrics ---\n")

    # 计算线粒体比例
    if (!"percent.mt" %in% colnames(obj_sample@meta.data)) {
      obj_sample[["percent.mt"]] <- PercentageFeatureSet(
        obj_sample,
        pattern = "^MT-"
      )
    }
    ...
    # 应用基础过滤
    n_before_basic <- ncol(obj_sample)
    obj_sample <- subset(
      obj_sample,
    ...
    n_after_basic <- ncol(obj_sample)
    n_removed_basic <- n_before_basic - n_after_basic

    cat(sprintf(
      "Basic QC filter removed: %d cells (%.1f%%)\n",
      n_removed_basic,
      n_removed_basic / n_before_basic * 100
    ))
    cat(sprintf("Remaining cells: %d\n", n_after_basic))

    sample_qc$stages$basic_qc <- data.frame(
      Before = n_before_basic,
      After = n_after_basic,
      Removed = n_removed_basic,
      RemovalRate = n_removed_basic / n_before_basic * 100
    )

    # ========== Stage 3: 回归异常值检测 ==========
    if (run_regression && ncol(obj_sample) >= 50) {
      cat("\n--- Stage 3: Regression Outlier Detection ---\n")

      regression_result <- detect_regression_outliers(
        obj_sample,
        outlier_threshold = regression_threshold
      )
      ...
      ggsave(
        file.path(
          qc_plots_dir,
          sprintf("regression_outliers_%s.pdf", sample_name)
        ),
      ...
      # 过滤异常值
      obj_sample <- subset(obj_sample, cells = regression_result$cell_names)

      sample_qc$stages$regression <- regression_result$stats

      cat(sprintf("Remaining cells: %d\n", ncol(obj_sample)))
    }

    # ========== Stage 4: DoubletFinder ==========
    if (run_doubletfinder && ncol(obj_sample) >= 50) {
      cat("\n--- Stage 4: DoubletFinder Detection ---\n")

      doubletfinder_result <- run_doubletfinder_sample(
        obj_sample,
        sample_name = sample_name,
        doublet_rate = doublet_rate
      )

      if (doubletfinder_result$status == "Success") {
        results <- doubletfinder_result$results

        # 更新主对象metadata
        cells_to_update <- match(results$cell_names, colnames(seurat_obj))
        seurat_obj$doublet_status[cells_to_update] <- results$doublet_status
        seurat_obj$doublet_score[cells_to_update] <- results$doublet_score
        seurat_obj$doublet_class[cells_to_update] <- results$doublet_class

        sample_qc$stages$doubletfinder <- data.frame(
          Total = length(results$cell_names),
          Singlets = results$n_singlets,
          Doublets = results$n_doublets,
          DoubletRate = results$n_doublets / length(results$cell_names) * 100,
          Status = "Success"
        )
      } else {
        # 标记为特殊状态
        seurat_obj$doublet_status[cell_idx] <- doubletfinder_result$status
        seurat_obj$doublet_class[cell_idx] <- doubletfinder_result$status

        sample_qc$stages$doubletfinder <- data.frame(
          Status = doubletfinder_result$status
        )
      }
    }
    ...
    sample_qc$processing_time <- as.numeric(
      difftime(sample_end_time, sample_start_time, units = "secs")
    )

    # 保存样本QC记录
    global_stats[[sample_name]] <- sample_qc
    checkpoint$samples_processed <- c(checkpoint$samples_processed, sample_name)
    checkpoint$qc_stages[[sample_name]] <- sample_qc

    # 保存checkpoint
    saveRDS(checkpoint, file.path(output_dir, "qc_checkpoint.rds"))

    cat(sprintf(
      "\n✓ Sample %s completed (%.1fs)\n",
      sample_name,
      sample_qc$processing_time
    ))
  ...
  summary_df <- do.call(
    rbind,
    lapply(names(global_stats), function(sname) {
      s <- global_stats[[sname]]

      data.frame(
        Sample = sname,
        Initial_Cells = s$n_initial,
        After_EmptyDrops = if (!is.null(s$stages$emptydrops)) {
          s$stages$emptydrops$RealCells
        } else {
          s$n_initial
        },
        After_BasicQC = if (!is.null(s$stages$basic_qc)) {
          s$stages$basic_qc$After
        } else {
          NA
        },
        After_Regression = if (!is.null(s$stages$regression)) {
          s$stages$regression$Normal
        } else {
          NA
        },
        Singlets = if (!is.null(s$stages$doubletfinder)) {
          if ("Singlets" %in% names(s$stages$doubletfinder)) {
            s$stages$doubletfinder$Singlets
          } else {
            NA
          }
        } else {
        ...
        Doublets = if (!is.null(s$stages$doubletfinder)) {
          if ("Doublets" %in% names(s$stages$doubletfinder)) {
            s$stages$doubletfinder$Doublets
          } else {
            0
          }
        } else {
        ...
        Processing_Time_sec = round(s$processing_time, 1)
      )
    })
  )
  ...
  # 整体统计
  total_initial <- sum(summary_df$Initial_Cells)
  total_final <- sum(summary_df$Singlets, na.rm = TRUE)
  total_removed <- total_initial - total_final

  cat("Overall QC Results:\n")
  cat(sprintf("  Initial cells:     %d\n", total_initial))
  cat(sprintf(
    "  Final cells:       %d (%.1f%%)\n",
    total_final,
    total_final / total_initial * 100
  ))
  cat(sprintf(
    "  Total removed:     %d (%.1f%%)\n",
    total_removed,
    total_removed / total_initial * 100
  ))
  ...
  p1 <- ggplot(
    summary_long,
    aes(x = Stage, y = Cells, group = Sample, color = Sample)
  ) +
    geom_line(size = 1) +
    geom_point(size = 3) +
    scale_y_continuous(labels = scales::comma) +
    labs(
      title = "QC Pipeline Cell Retention",
      subtitle = "Cell count through progressive filtering stages",
      x = "QC Stage",
      y = "Number of Cells"
    ) +
    theme_classic(base_size = 12) +
  ...
  ggsave(
    file.path(output_dir, "qc_pipeline_overview.pdf"),
    p1,
    width = 12,
    height = 7
  )
  ...
  if (run_doubletfinder) {
    doublet_df <- summary_df %>%
      select(Sample, Singlets, Doublets) %>%
      filter(!is.na(Singlets)) %>%
      tidyr::pivot_longer(
        cols = c(Singlets, Doublets),
        names_to = "Type",
        values_to = "Count"
      )

    p2 <- ggplot(doublet_df, aes(x = Sample, y = Count, fill = Type)) +
      geom_bar(stat = "identity", position = "stack", color = "black") +
      scale_fill_manual(
        values = c("Singlets" = "#2ECC71", "Doublets" = "#E74C3C")
      ) +
      labs(
        title = "Doublet Detection Results",
        x = "Sample",
        y = "Cell Count",
        fill = "Cell Type"
      ) +
    ...
    ggsave(
      file.path(output_dir, "doublet_detection_summary.pdf"),
      p2,
      width = 10,
      height = 6
    )
  ...
  saveRDS(seurat_obj, file.path(output_dir, "seurat_qc_complete.rds"))

  # ========== 完成 ==========
  cat("\n============================================================\n")
  cat("✓ QC PIPELINE COMPLETED SUCCESSFULLY\n")
  cat("============================================================\n\n")

  cat("Output files:\n")
  cat(sprintf("  📁 Directory: %s/\n", output_dir))
  cat("  📊 qc_summary_statistics.csv\n")
  cat("  💾 seurat_qc_complete.rds\n")
  cat("  📋 qc_checkpoint.rds\n")
  cat("  📈 qc_pipeline_overview.pdf\n")
  cat("  📈 doublet_detection_summary.pdf\n")
  cat(sprintf("  📁 qc_plots/ (%d sample-specific plots)\n", n_samples * 2))

  cat("\nNext steps:\n")
  cat("  1. Review qc_summary_statistics.csv\n")
  cat(
    "  2. Filter doublets: seurat_clean <- subset(seurat_qc, doublet_class == 'Singlet')\n"
  )
  cat("  3. Proceed with normalization and integration\n\n")
...
seurat_obj_main$doublet_status <- NA
seurat_obj_main$doublet_score <- NA
seurat_obj_main$doublet_class <- NA
# 运行完整QC流程
qc_results <- integrated_qc_pipeline(
  seurat_obj = seurat_obj_main,
  sample_column = "sample",
  output_dir = "integrated_qc_results",
...
# 提取清洁数据
seurat_obj_main <- subset(qc_results$seurat_obj, doublet_class == "Singlet")

# 查看统计
View(qc_results$summary)
###########################################################################################################
###########################################################################################################
###########################################################################################################
library(Seurat)
...
# ============================================================
# 双细胞检测流程 - 直接更新主对象版本
# Single-cell Doublet Detection - Direct Metadata Update
# ============================================================
library(Seurat)
library(DoubletFinder)
library(ggplot2)
...
# ============================================================
# 双细胞检测流程 - 轻量级Checkpoint版本
# Single-cell Doublet Detection - Lightweight Checkpoint
# ============================================================
# seurat_obj_main <- readRDS("E:/R/0301/seurat_raw_with_doublets_1003.rds")

# 设置参数
doublet_rate <- 0.06 # 预期双细胞率 Expected doublet rate
sample_column <- "sample" # 样本列名 Sample column name
...
cat("\n============================================================\n")
cat("   Doublet Detection Pipeline - Lightweight Checkpoint\n")
cat("============================================================\n\n")

# ========== 步骤0: 清理主对象中的旧DoubletFinder列 ==========
cat("Step 0: Cleaning old DoubletFinder columns from main object...\n")

old_df_cols <- grep(
  "DF.classifications|pANN",
  colnames(seurat_obj_main@meta.data),
  value = TRUE
)
if (length(old_df_cols) > 0) {
  cat(sprintf("  Found %d old DoubletFinder columns:\n", length(old_df_cols)))
  for (col in head(old_df_cols, 10)) {
    # 只显示前10个
    cat(sprintf("    - %s\n", col))
  }
  if (length(old_df_cols) > 10) {
    cat(sprintf("    ... and %d more\n", length(old_df_cols) - 10))
  }
  cat("  Removing them...\n")
  seurat_obj_main@meta.data[, old_df_cols] <- NULL
  cat("  ✓ Cleanup complete\n\n")
} else {
  cat("  No old columns found\n\n")
}

# ========== 步骤1: 初始化轻量级Checkpoint矩阵 ==========
cat("Step 1: Initializing lightweight checkpoint matrix...\n")
...
cat(sprintf(
  "  Created checkpoint matrix: %d cells × %d columns\n",
  nrow(checkpoint_df),
  ncol(checkpoint_df)
))
cat(sprintf("  Memory size: %.2f MB\n\n", object.size(checkpoint_df) / 1024^2))
...
print(table(seurat_obj_main@meta.data[[sample_column]]))
cat("\n")
...
for (i in 1:n_samples) {
  ...
  if (n_cells < 50) {
    ...
    checkpoint_df[cell_names, "processing_time"] <- format(
      Sys.time(),
      "%Y-%m-%d %H:%M:%S"
    )
  ...
  # 3.3.1 清理子集中的旧DoubletFinder列
  old_cols_in_subset <- grep(
    "DF.classifications|pANN",
  ...
  if (length(old_cols_in_subset) > 0) {
    cat(sprintf(
      "  Removing %d old columns from subset...\n",
      length(old_cols_in_subset)
    ))
  ...
  tryCatch(
    {
      ...
      df_score_col <- grep("pANN", colnames(obj_sample@meta.data), value = TRUE)
      ...
      if (length(df_class_col) > 0) {
        ...
        cat(sprintf(
          "    NA count: %d / %d (%.1f%%)\n",
          na_count,
          length(status_values),
          na_count / length(status_values) * 100
        ))

        if (all(is.na(status_values))) {
          stop("ERROR: All classification values are NA!")
        }

        # 3.8 同时更新主对象和checkpoint矩阵
        cat("    Writing to main object and checkpoint...\n")
        ...
        # 3.9 保存轻量级checkpoint（每个样本完成后立即保存）
        cat(sprintf(
          "\n  💾 Saving checkpoint (%.2f MB)...\n",
          object.size(checkpoint_df) / 1024^2
        ))
        ...
        cat("  ✗ Detection failed - no results generated\n")

        # 更新为Failed状态
        seurat_obj_main$doublet_status[cell_idx] <- "Failed"
        seurat_obj_main$doublet_score[cell_idx] <- NA
        seurat_obj_main$doublet_class[cell_idx] <- "Failed"

        checkpoint_df[cell_names, "doublet_status"] <- "Failed"
        checkpoint_df[cell_names, "doublet_score"] <- NA
        checkpoint_df[cell_names, "doublet_class"] <- "Failed"
        checkpoint_df[cell_names, "processing_time"] <- format(
          Sys.time(),
          "%Y-%m-%d %H:%M:%S"
        )
...
# ========== 步骤4: 验证checkpoint与主对象一致性 ==========
cat("\n\n============================================================\n")
cat("VERIFICATION: Comparing Checkpoint vs Main Object\n")
cat("============================================================\n\n")

# 4.1 加载最终checkpoint
final_checkpoint <- readRDS(file.path(output_dir, "checkpoint_matrix.rds"))

# 4.2 比对细胞ID
cat("Checking cell IDs...\n")
checkpoint_cells <- rownames(final_checkpoint)
main_cells <- colnames(seurat_obj_main)

if (identical(checkpoint_cells, main_cells)) {
  cat("  ✓ Cell IDs match perfectly\n\n")
} else {
  cat("  ⚠️ WARNING: Cell ID mismatch detected!\n")
  cat(sprintf("    Checkpoint: %d cells\n", length(checkpoint_cells)))
  cat(sprintf("    Main object: %d cells\n", length(main_cells)))
  cat(sprintf(
    "    Intersection: %d cells\n\n",
    length(intersect(checkpoint_cells, main_cells))
  ))
}

# 4.3 比对doublet_class
cat("Comparing doublet classifications...\n")
common_cells <- intersect(checkpoint_cells, main_cells)
checkpoint_class <- final_checkpoint[common_cells, "doublet_class"]
main_class <- seurat_obj_main$doublet_class[common_cells]

mismatch_count <- sum(checkpoint_class != main_class, na.rm = TRUE)
na_diff <- sum(is.na(checkpoint_class) != is.na(main_class))

cat(sprintf("  Total cells compared: %d\n", length(common_cells)))
cat(sprintf("  Mismatches: %d\n", mismatch_count))
cat(sprintf("  NA differences: %d\n", na_diff))

if (mismatch_count == 0 && na_diff == 0) {
  cat("  ✓ Classifications match perfectly!\n\n")
} else {
  cat("  ⚠️ WARNING: Discrepancies detected!\n")

  # 显示不一致的样本
  if (mismatch_count > 0) {
    mismatch_idx <- which(
      checkpoint_class != main_class &
        !is.na(checkpoint_class) &
        !is.na(main_class)
    )
    if (length(mismatch_idx) > 0) {
      cat("\n  First 10 mismatches:\n")
      mismatch_df <- data.frame(
        Cell = common_cells[head(mismatch_idx, 10)],
        Checkpoint = checkpoint_class[head(mismatch_idx, 10)],
        MainObject = main_class[head(mismatch_idx, 10)]
      )
      print(mismatch_df)
    }
  }
...
# 4.4 比对doublet_score
cat("Comparing doublet scores...\n")
checkpoint_score <- final_checkpoint[common_cells, "doublet_score"]
main_score <- seurat_obj_main$doublet_score[common_cells]

# 计算相关系数（排除NA）
valid_idx <- !is.na(checkpoint_score) & !is.na(main_score)
if (sum(valid_idx) > 0) {
  correlation <- cor(checkpoint_score[valid_idx], main_score[valid_idx])
  max_diff <- max(abs(checkpoint_score[valid_idx] - main_score[valid_idx]))

  cat(sprintf("  Correlation: %.6f\n", correlation))
  cat(sprintf("  Max difference: %.6e\n", max_diff))

  if (correlation > 0.9999 && max_diff < 1e-10) {
    cat("  ✓ Scores match perfectly!\n\n")
  } else {
    cat("  ⚠️ WARNING: Score discrepancies detected!\n\n")
  }
} else {
  cat("  No valid scores to compare\n\n")
}
...
total_other <- total_cells - total_singlets - total_doublets
...
cat(sprintf(
  "  Singlets:       %d (%.1f%%)\n",
  total_singlets,
  total_singlets / total_cells * 100
))
cat(sprintf(
  "  Doublets:       %d (%.1f%%)\n",
  total_doublets,
  total_doublets / total_cells * 100
))
cat(sprintf(
  "  Other/Failed:   %d (%.1f%%)\n",
  total_other,
  total_other / total_cells * 100
))

# 总处理时间
if ("Time_sec" %in% colnames(stats)) {
  total_time <- sum(stats$Time_sec, na.rm = TRUE)
...
saveRDS(seurat_obj_main, file.path(output_dir, "seurat_with_doublets_1011.rds"))
saveRDS(
  seurat_obj_main,
  "seurat_without_partial_dual_contamination_doublets_2_1028.rds"
)
# ========== 步骤7: 生成可视化 ==========
...
p1 <- ggplot(stats, aes(x = Sample, y = Rate, fill = Status)) +
  ...
  labs(
    title = "Doublet Detection Rate by Sample",
    subtitle = sprintf(
      "Red dashed line = Expected rate (%.1f%%)",
      doublet_rate * 100
    ),
    x = "Sample",
    y = "Doublet Rate (%)"
...
ggsave(
  file.path(output_dir, "doublet_rate_barplot.pdf"),
  p1,
  width = max(10, n_samples * 0.4),
  height = 6
)
...
p2 <- ggplot(stats_long, aes(x = Sample, y = Count, fill = Type)) +
  ...
  labs(
    title = "Cell Distribution: Singlets vs Doublets",
    x = "Sample",
    y = "Cell Count",
    fill = "Cell Type"
  ) +
...
ggsave(
  file.path(output_dir, "cell_distribution_barplot.pdf"),
  p2,
  width = max(10, n_samples * 0.4),
  height = 6
)
...
# 图3: Score分布密度图
valid_data <- seurat_obj_main@meta.data[
  !is.na(seurat_obj_main$doublet_score) &
    seurat_obj_main$doublet_class %in% c("Singlet", "Doublet"),
]
...
if (nrow(valid_data) > 0) {
  ...
  ggsave(
    file.path(output_dir, "score_distribution.pdf"),
    p3,
    width = 8,
    height = 6
  )
...
cat("  💾 seurat_with_doublets_final.rds (final version only)\n")
cat(
  "  📋 checkpoint_matrix.rds (lightweight, %.2f MB)\n",
  object.size(checkpoint_df) / 1024^2
)
cat("  📋 checkpoint_matrix.csv (human-readable)\n")
cat("  📈 doublet_rate_barplot.pdf\n")
cat("  📈 cell_distribution_barplot.pdf\n")
cat("  📈 score_distribution.pdf\n\n")

cat("Checkpoint matrix columns:\n")
cat("  - cell_id: Unique cell barcode\n")
cat("  - sample: Sample name\n")
cat("  - doublet_status: Raw DoubletFinder classification\n")
cat("  - doublet_score: pANN score\n")
cat("  - doublet_class: Simplified classification (Singlet/Doublet)\n")
cat("  - processing_time: Timestamp of processing\n\n")

cat("Next steps:\n")
cat("  1. Review summary_statistics.csv\n")
cat("  2. Check checkpoint_matrix.csv for per-cell results\n")
cat(
  "  3. Filter doublets: seurat_clean <- subset(seurat_obj_main, doublet_class == 'Singlet')\n"
)
cat("  4. Proceed with downstream analysis\n\n")
...
run_dim_reduction <- function(seurat_obj, harmony = TRUE) {
  ...
  if (harmony) {
    ...
    if ("sample" %in% colnames(seurat_obj@meta.data)) {
      batch_vars <- c(batch_vars, "sample")
    }
    if ("samples" %in% colnames(seurat_obj@meta.data)) {
      batch_vars <- c(batch_vars, "sample")
    }
    if ("study" %in% colnames(seurat_obj@meta.data)) {
      batch_vars <- c(batch_vars, "study")
    }
    if ("Tissue" %in% colnames(seurat_obj@meta.data)) {
      batch_vars <- c(batch_vars, "tissue")
    }
    if ("tissue" %in% colnames(seurat_obj@meta.data)) {
      batch_vars <- c(batch_vars, "tissue")
    }
    ...
    if (length(batch_vars) == 0) {
      warning(
        "No batch variables found. Using harmony without batch correction."
      )
      batch_vars <- c("orig.ident")
    }

    cat(paste0(
      "Running Harmony with batch variables: ",
      ...
      tryCatch(
        {
          ...
          if (
            "ann_level_2" %in%
              colnames(metadata) &&
              sum(!is.na(metadata$ann_level_2)) > 0
          ) {
          ...
          if (length(valid_columns) > 0) {
            ...
            if (sum(!embeddings_valid) > 0) {
              cat(sprintf(
                "Removed %d cells with NA/NaN/Inf values\n",
              ...
              tryCatch(
                {
                  ...
                  cat(sprintf(
                    "Iteration %d completed: iLISI = %.3f, cLISI = %.3f\n",
                    i,
                    ilisi_scores[i],
                    clisi_scores[i]
                  ))
            ...
            if (length(ilisi_scores) > 0) {
              ...
              cat("\n--------------------------")
              cat(sprintf(
                "\nNumber of cells used: %d (after removing NA annotations)",
                nrow(embeddings)
              ))
              cat(sprintf(
                "\niLISI Score: %.3f ± %.3f",
...
run_analysis <- function(seurat_obj, Markers = NULL, output_prefix = "") {
  ...
  if (!is.null(Markers)) {
    ...
    if (length(valid_markers) < length(Markers)) {
      ...
      cat(
        "Warning: The following markers were not found in the data:",
    ...
    if (length(valid_markers) > 0) {
      ...
      write.csv(
        results_df,
        paste0(output_prefix, "cluster_marker_stats.csv"),
        row.names = FALSE
      )
    }
  }
  ...
  visualize_group_distribution <- function(group_var) {
    if (group_var %in% colnames(seurat_obj@meta.data)) {
      ...
      seurat_obj@meta.data[[var_name]][is.na(seurat_obj@meta.data[[
        var_name
      ]])] <- "Unknown"

      # 绘制整体分布
      pdf(
        paste0(output_prefix, group_var, "_umap.pdf"),
        width = 15,
        height = 10
      )
      print(
      ...
      pdf(
        paste0(output_prefix, group_var, "_umap_split.pdf"),
        width = 12,
        height = 12
      )
  ...
  marker_batches <- split(
    marker_genes,
    ceiling(seq_along(marker_genes) / batch_size)
  )

  # 生成热图
  pdf(
    paste0(output_prefix, "marker_heatmap_batch.pdf"),
    width = 32,
    height = 18
  )
  for (i in seq_along(marker_batches)) {
    ...
    print(
      DoHeatmap(seurat_obj, features = marker_batches[[i]], size = 24) +
        NoLegend() +
...
run_subset_analysis <- function(
  seurat_obj,
  cell_type = NULL,
  cell_type_column = NULL,
  cell_type_value = NULL,
  markers = NULL,
  output_dir = ".",
  normalize = TRUE
) {
  ...
  if (
    !is.null(cell_type) &&
      (is.null(cell_type_column) || is.null(cell_type_value))
  ) {
    # 自动检测细胞类型列
    ...
    stop(
      "Error: Either provide 'cell_type' or both 'cell_type_column' and 'cell_type_value'"
    )
  }
  ...
  subset_obj <- subset(
    seurat_obj,
    cells = rownames(seurat_obj@meta.data)[subset_cells]
  )
...
#' @return 处理后的Seurat对象
run_optimized_analysis <- function(
  seurat_obj,
  markers = NULL,
  output_prefix = "",
  dot_plot_cols = c("lightgrey", "red"),
  find_markers_params = list(
    only.pos = TRUE,
    min.pct = 0.25,
    logfc.threshold = 0.25
  ),
  ...
  if (!is.null(markers)) {
    ...
    if (length(valid_markers) > 0) {
      ...
      tryCatch(
        {
          feature_plot_width <- min(
            10,
            5 * ceiling(sqrt(length(valid_markers)))
          )
          feature_plot_height <- min(
            8,
            4 *
              ceiling(
                length(valid_markers) / ceiling(sqrt(length(valid_markers)))
              )
          )

          pdf(
            paste0(output_prefix, "umap_FeaturePlot.pdf"),
      ...
      tryCatch(
        {
          ...
          pdf(
            paste0(output_prefix, "markers_dotplot.pdf"),
      ...
      tryCatch(
        {
          ...
          for (cluster in unique(Idents(seurat_obj))) {
            ...
            for (gene in valid_markers) {
              ...
              mean_expr_nonzero <- if (sum(expr_data > 0) > 0) {
                mean(expr_data[expr_data > 0])
              } else {
                0
              }
  ...
  if (!file.exists(cluster_markers_file)) {
    ...
    message(paste0(
      "Loading existing marker genes from: ",
      cluster_markers_file
    ))
  ...
  # 生成热图 - 仅在热图可视化时进行抽样
  if (
    file.exists(cluster_markers_file) && file.size(cluster_markers_file) > 0
  ) {
    if (!exists("markers")) {
      markers <- read.csv(cluster_markers_file)
    }
          ...
          if (length(marker_genes) > 0) {
            ...
            if (cell_count > max_cells_for_heatmap) {
              message(paste0(
                ...
                cell_count,
                " cells). Creating sampled object with ",
                max_cells_for_heatmap,
                " cells for heatmap visualization only."
              ))
              ...
              cells_per_cluster <- lapply(cells_by_cluster, function(cells) {
                n_sample <- ceiling(
                  length(cells) / cell_count * max_cells_for_heatmap
                )
                if (length(cells) <= n_sample) {
                  return(cells)
                }
              ...
              message(paste0(
                "Created temporary object with ",
                ncol(heatmap_obj),
                " cells for heatmap visualization"
              ))
            } else {
            ...
            for (i in seq_along(marker_batches)) {
              ...
              if (length(batch_genes) > 0) {
                ...
                tryCatch(
                  ...
                  error = function(e) {
                    message(paste0(
                      "Error generating heatmap batch ",
                      i,
                      ": ",
                      e$message
                    ))
...
for (i in seq_along(merged_list)) {
  ...
  if (length(missing_genes) > 0) {
    cat(sprintf(
      "Batch %d: Adding %d missing genes\n",
      i,
      length(missing_genes)
    ))

    # 创建填充矩阵
    fill_mat <- Matrix::Matrix(
      0,
...
seurat_obj_main <- subset(
  seurat_obj_main,
  subset = tissue_sampling_method != 'brush' & study != 'Jain_Misharin_2021'
)
# 8. 清理中间对象节省内存
  ...
  tryCatch(
    {
      ...
      # 获取classification列名
      DF.name <- colnames(current_obj@meta.data)[grep(
        "DF.classifications",
    ...
    error = function(e) {
      warning(sprintf(
        "DoubletFinder failed for sample %s: %s",
        sample_name,
        e$message
      ))
      current_obj$doublet_status <- "Unknown"
...
for (sample_name in names(sample_results)) {
  cells_in_sample <- WhichCells(
    seurat_obj_main,
    expression = sample == sample_name
  )
  seurat_obj_main$doublet_status[cells_in_sample] <- sample_results[[
    sample_name
  ]]
}
...
#' @return 整合后的结果
integrate_sample_results <- function(
  sample_results,
  seurat_obj,
  verbose = TRUE
) {
  ...
  # 转换样本统计为数据框
  sample_stats_df <- do.call(
    rbind,
    lapply(sample_stats, function(x) {
      data.frame(
        sample = x$sample,
...
calculate_cluster_stats <- function(seurat_obj, verbose = TRUE) {
  ...
  doublet_factor <- factor(
    seurat_obj$is_marker_doublet,
    levels = c(FALSE, TRUE),
  ...
  if (length(high_doublet_clusters) > 0 && verbose) {
    ...
    for (cl in high_doublet_clusters) {
      cat(paste(
        "  聚类",
        names(cluster_doublet_percent)[cl],
        ": ",
...
detect_marker_doublets_by_sample <- function(
  seurat_obj,
  marker_genes,
  sample_col = "sample",
  ...
  results$summary <- list(
    ...
    doublet_percent = sum(seurat_obj$is_marker_doublet, na.rm = TRUE) /
      ncol(seurat_obj) *
      100,
...
prepare_markers <- function(marker_genes, seurat_obj, verbose = TRUE) {
  ...
  for (cell_type in names(marker_genes)) {
    genes_in_data <- marker_genes[[cell_type]][
      marker_genes[[cell_type]] %in% rownames(seurat_obj)
    ]
...
#' @param verbose 是否输出详细信息
#' @return 双胞体模式列表
define_doublet_patterns <- function(
  valid_cell_types,
  marker_genes,
  min_gene_threshold,
  verbose = TRUE
...
#' @param verbose 是否输出详细信息
#' @return 细胞类型得分数据框
calculate_cell_type_scores <- function(
  seurat_obj,
  marker_genes,
  valid_cell_types,
  expression_threshold,
...
add_cell_type_scores <- function(seurat_obj, cell_type_scores, verbose = TRUE) {
  ...
  common_cells <- intersect(
    rownames(cell_type_scores_subset),
    colnames(seurat_obj)
  )
  ...
  if (length(ratio_cols) > 0) {
    # 获取每个细胞的主导细胞类型
    seurat_obj$dominant_cell_type <- apply(
      seurat_obj@meta.data[, ratio_cols, drop = FALSE],
      1,
      function(x) {
        if (all(is.na(x))) {
          return(NA)
    ...
    # 计算主导细胞类型的表达率
    seurat_obj$dominant_ratio <- apply(
      seurat_obj@meta.data[, ratio_cols, drop = FALSE],
      1,
      function(x) {
        if (all(is.na(x))) {
    ...
    # 计算次要细胞类型的表达率
    seurat_obj$secondary_ratio <- apply(
      seurat_obj@meta.data[, ratio_cols, drop = FALSE],
      1,
      function(x) {
    ...
    seurat_obj$marker_mixing_score <- rowSums(
      seurat_obj@meta.data[, ratio_cols, drop = FALSE] >= threshold,
      na.rm = TRUE
    )
...
process_samples <- function(
  ...
  process_one_sample <- function(current_sample) {
    ...
    # 获取当前样本的细胞索引
    sample_cells_idx <- which(
      seurat_obj@meta.data[[sample_col]] == current_sample
    )
    ...
    for (pattern_name in names(doublet_patterns)) {
      ...
      group1_expr <- pattern_expr[[pattern_name]]$group1[,
        sample_cells,
        drop = FALSE
      ]
      group2_expr <- pattern_expr[[pattern_name]]$group2[,
        sample_cells,
        drop = FALSE
      ]
      ...
      cells_express_group1 <- colnames(group1_expr)[
        Matrix::colSums(group1_expr > expression_threshold) >=
          pattern$min_group1
      ]

      # 找出表达足够多第二组标记基因的细胞
      cells_express_group2 <- colnames(group2_expr)[
        Matrix::colSums(group2_expr > expression_threshold) >=
          pattern$min_group2
      ]

      # 找出同时满足两个条件的细胞
      potential_doublets <- intersect(
        cells_express_group1,
        cells_express_group2
      )
    ...
    if (
      all(
        c("Epithelial_ratio", "Fibroblast_ratio") %in%
          colnames(seurat_obj@meta.data)
      )
    ) {
      # 获取潜在EMT细胞
    ...
    sample_doublet_percent <- (sample_doublet_count / length(sample_cells)) *
      100
    ...
    if (verbose) {
      cat(paste(
        ...
        current_sample,
        "中检测到",
        sample_doublet_count,
        "个双胞体 (",
        round(sample_doublet_percent, 2),
        "%)\n"
      ))
    }
...
message(paste0(
  "Removed ",
  doublet_results$summary$doublet_cells,
...
# 检查基因是否存在于数据集中
genes_present <- all_marker_genes[
  all_marker_genes %in% rownames(seurat_obj_main)
]
genes_missing <- all_marker_genes[
  !all_marker_genes %in% rownames(seurat_obj_main)
]
...
genes_to_scale <- genes_present[
  !genes_present %in% rownames(seurat_obj_main[["RNA"]]@scale.data)
]
...
if (length(genes_present) > 0) {
  ...
  for (gene in key_genes_present) {
    ...
    p <- FeaturePlot(
      ...
      pt.size = 0.5,
      cells = sample(
        colnames(seurat_obj_main),
        min(20000, ncol(seurat_obj_main))
      )
    ) +
  ...
  if (length(t_present) > 0 && length(myeloid_present) > 0) {
    ...
    for (t_gene in t_present) {
      for (m_gene in myeloid_present) {
        ...
        set.seed(42)
        sample_idx <- sample(
          1:ncol(seurat_obj_main),
          min(20000, ncol(seurat_obj_main))
        )

        # T细胞标记图
        plot(
          umap_coords[sample_idx, 1],
          umap_coords[sample_idx, 2],
          pch = 16,
          cex = 0.5,
          col = colorRampPalette(c("grey", "blue"))(10)[cut(
            t_expr[sample_idx],
            breaks = 10
          )],
          main = paste("Expression of", t_gene),
          xlab = "UMAP1",
          ylab = "UMAP2"
        )
        ...
        plot(
          ...
          pch = 16,
          cex = 0.5,
          col = colorRampPalette(c("grey", "red"))(10)[cut(
            m_expr[sample_idx],
            breaks = 10
          )],
          main = paste("Expression of", m_gene),
          xlab = "UMAP1",
          ylab = "UMAP2"
        )
  ...
  for (pair in key_pairs) {
    if (all(pair %in% genes_present)) {
      ...
      # 获取表达数据和聚类信息
      expr_data <- GetAssayData(seurat_obj_main, layer = "data")[
        c(gene1, gene2),
      ]
      ...
      sample_idx <- sample(
        1:ncol(seurat_obj_main),
        min(10000, ncol(seurat_obj_main))
      )
      ...
      # 绘制散点图
      plot(
        expr_data[1, sample_idx],
        expr_data[2, sample_idx],
        pch = 16,
        cex = 0.5,
    ...
    for (m_marker in myeloid_markers) {
      if (m_marker %in% rownames(seurat_obj_main)) {
        ...
        print(
          p +
            plot_annotation(
              title = paste(t_marker, "vs", m_marker, "Co-expression")
            )
        )
      }
    }
...
cat(
  "所有blend共表达图表已保存到marker_gene_plots/t_myeloid_blend_plots.pdf文件\n"
)
...
# 定义函数：识别并提取双表型细胞
identify_dual_phenotype_cells <- function(
  seurat_obj,
  t_marker,
  m_marker,
  threshold = 0.3,
  ...
  # 检查标记是否存在
  if (
    !(t_marker %in% rownames(seurat_obj) && m_marker %in% rownames(seurat_obj))
  ) {
  ...
  # 识别双表型细胞
  dual_cells <- colnames(expr_data)[
    expr_data[t_marker, ] > threshold &
  ...
  if (visualize && length(dual_cells) > 0) {
    ...
    expr_data_sample$cell_type[
      rownames(expr_data_sample) %in% dual_cells_in_sample
    ] <- "Dual Phenotype"

    p2 <- ggplot(expr_data_sample, aes_string(x = t_marker, y = m_marker)) +
      geom_point(aes(color = cell_type), alpha = 0.7, size = 1) +
      scale_color_manual(
        values = c("Dual Phenotype" = "purple", "Other" = "grey")
      ) +
      theme_classic() +
...
for (t_marker in t_cell_markers) {
  for (m_marker in myeloid_markers) {
    ...
    # 识别双表型细胞
    dual_cells <- identify_dual_phenotype_cells(
      seurat_obj_main,
      t_marker,
      m_marker,
...
for (combo_name in names(all_dual_cells)) {
  ...
  if (!is.null(cells) && length(cells) > 0) {
    seurat_obj_main$dual_phenotype[
      colnames(seurat_obj_main) %in% cells
    ] <- combo_name
  }
}

# 保存标记了双表型细胞的Seurat对象(可选)
saveRDS(seurat_obj_main, "seurat_obj_with_doublets.rds")
...
if (length(all_dual_cells_combined) > 0) {
  p <- DimPlot(
    ...
    cols = c(
      "None" = "grey",
      setNames(rainbow(length(names(all_dual_cells))), names(all_dual_cells))
    )
  ) +
...
# saveRDS(seurat_obj_main, "final_analyzed_seurat_obj_main_1009.rds")

regression_outliers_by_sample <- function(
  seurat_obj,
  outliers_threshold = 0.999
) {
  # 创建输出目录 [^1]
  ...
  for (sample_name in unique(seurat_obj$sample)) {
    ...
    lines(
      sort(log_counts),
      pred[order(log_counts), "lwr"],
      col = "blue",
      lty = 2
    )
    lines(
      sort(log_counts),
      pred[order(log_counts), "upr"],
      col = "blue",
      lty = 2
    )
    lines(
      sort(log_counts),
      pred[order(log_counts), "fit"],
      col = "blue",
      lwd = 2
    )
    ...
    # 输出每个样本的结果
    cat(sprintf(
      "Sample %s: Detected %d outliers (%.1f%%)\n",
      sample_name,
      sum(outliers),
      mean(outliers) * 100
    ))
...
# seurat_obj_main <- JoinLayers(seurat_obj_main)
# seurat_obj_main$For_MASC[seurat_obj_main$tissue_sampling_method %in% c('brush','scraping')] <- FALSE
# table(seurat_obj_main$For_MASC)
s.genes <- cc.genes$s.genes
g2m.genes <- cc.genes$g2m.genes

seurat_obj_main <- CellCycleScoring(
  seurat_obj_main,
  s.features = s.genes,
...
seurat_obj_main[["percent.mt"]] <- PercentageFeatureSet(
  seurat_obj_main,
  pattern = "^MT-"
)
...
# #
# # 核糖体基因分析
seurat_obj_main[["percent.ribo"]] <- PercentageFeatureSet(
  seurat_obj_main,
  pattern = "^RP[SL]"
)
...
ggplot(seurat_obj_main@meta.data, aes(x = percent.rb1)) +
  geom_histogram(bins = 100, fill = "#4ECDC4", color = "white", alpha = 0.8) +
  labs(
    title = "Ribosomal Gene Percentage Distribution",
    x = "Ribosomal Ratio (%)",
    y = "Cell Count"
  ) +
  theme_classic() +
...
seurat_obj_main <- subset(seurat_obj_main, subset = percent.rb1 <= 40)
...
# ===== 步骤1：识别线粒体和核糖体基因（不删除）=====
identify_mt_ribo_genes_seurat <- function(
    seurat_obj,
    species = "human",
    assay = "RNA"
) {
  ...
  # 获取基因名
  all_genes <- rownames(seurat_obj[[assay]])
  ...
  if (species == "human") {
    ...
    stop("species参数必须是 'human' 或 'mouse'")
  }
  
  # 识别基因
  mt_genes <- grep(mt_pattern, all_genes, value = TRUE)
  ribo_genes <- grep(ribo_pattern, all_genes, value = TRUE)
  genes_to_exclude <- union(mt_genes, ribo_genes)
  ...
  cat(sprintf(
    "线粒体基因数: %d (%.2f%%)\n",
    length(mt_genes),
    length(mt_genes) / length(all_genes) * 100
  ))
  cat(sprintf(
    "核糖体基因数: %d (%.2f%%)\n",
    length(ribo_genes),
    length(ribo_genes) / length(all_genes) * 100
  ))
  cat(sprintf(
    "需要排除的基因数: %d (%.2f%%)\n\n",
    length(genes_to_exclude),
    length(genes_to_exclude) / length(all_genes) * 100
  ))
  
  # 显示部分基因
  if (length(mt_genes) > 0) {
    cat("线粒体基因示例:\n")
    print(head(mt_genes, 10))
    cat("\n")
  }
  
  if (length(ribo_genes) > 0) {
    cat("核糖体基因示例:\n")
    print(head(ribo_genes, 10))
    cat("\n")
  }
...
# ===== 步骤2：按数据集统计基因表达（排除MT/Ribo）=====
calculate_gene_expression_across_datasets <- function(
    seurat_obj,
    genes_to_exclude = NULL,
    dataset_col = "dataset",
    min_cell_pct = 0.03,
    min_mean_umi = 0.1,
    assay = "RNA",
  ...
  # 检查dataset列是否存在
  if (!dataset_col %in% colnames(metadata)) {
    stop(sprintf("metadata中未找到列: %s", dataset_col))
  }
  
  # 排除MT/Ribo基因
  all_genes <- rownames(expr_mat)
  if (!is.null(genes_to_exclude)) {
    genes_to_analyze <- setdiff(all_genes, genes_to_exclude)
    cat(sprintf(
      "排除 %d 个MT/Ribo基因后，分析 %d 个基因\n\n",
      length(genes_to_exclude),
      length(genes_to_analyze)
    ))
    expr_mat <- expr_mat[genes_to_analyze, ]
  } else {
    genes_to_analyze <- all_genes
  }
  
  # 获取所有数据集名称
  datasets <- unique(metadata[[dataset_col]])
  n_datasets <- length(datasets)
  
  cat(sprintf("检测到 %d 个数据集\n", n_datasets))
  cat(sprintf("总细胞数: %d\n", ncol(expr_mat)))
  cat(sprintf("分析基因数: %d\n", nrow(expr_mat)))
  cat(sprintf("\n表达标准阈值:\n"))
  cat(sprintf("  min_cell_pct = %.3f (%.1f%%)\n", min_cell_pct, min_cell_pct * 100))
  cat(sprintf("  min_mean_umi = %.3f\n\n", min_mean_umi))
  ...
  for (i in seq_along(datasets)) {
    ...
    # 获取当前数据集的细胞索引
    cells_idx <- which(metadata[[dataset_col]] == dataset_name)
    n_cells <- length(cells_idx)
    
    cat(sprintf(
      "处理数据集 %d/%d: %s (n=%d cells)\n",
      i,
      n_datasets,
      dataset_name,
      n_cells
    ...
    # 提取该数据集的表达矩阵
    mat_subset <- expr_mat[, cells_idx, drop = FALSE]
    
    # 计算统计量
    cell_pct <- rowSums(mat_subset > 0) / n_cells
    mean_expr <- Matrix::rowMeans(mat_subset)
    
    # 判断基因是否满足表达标准
    expressed <- (cell_pct >= min_cell_pct) & (mean_expr >= min_mean_umi)
    
    gene_stats_list[[dataset_name]] <- data.table(
    ...
    cat(sprintf(
      ...
      sum(expressed),
      length(expressed),
      sum(expressed) / length(expressed) * 100
    ))
  }
  
  # 合并所有数据集的统计
  gene_stats_all <- rbindlist(gene_stats_list)
  
  # 计算每个基因在多少个数据集中表达
  gene_dataset_summary <- gene_stats_all[
    expressed == TRUE,
    .(
      n_datasets_expressed = .N,
      mean_cell_pct = mean(cell_pct),
      mean_expression = mean(mean_expr),
      sd_expression = sd(mean_expr)
    ),
    by = gene
  ]
  ...
  cat(sprintf(
    "在所有%d个数据集中都表达的基因: %d\n",
    n_datasets,
    sum(gene_dataset_summary$n_datasets_expressed == n_datasets)
  ))
  cat(sprintf(
    "在至少50%%数据集中表达的基因: %d\n",
    sum(gene_dataset_summary$n_datasets_expressed >= ceiling(n_datasets * 0.5))
  ))
  cat(sprintf(
    "在至少40%%数据集中表达的基因: %d\n",
    sum(gene_dataset_summary$n_datasets_expressed >= ceiling(n_datasets * 0.4))
  ))
  cat(sprintf(
    "在至少30%%数据集中表达的基因: %d\n\n",
    sum(gene_dataset_summary$n_datasets_expressed >= ceiling(n_datasets * 0.3))
  ))
  
  return(list(
    gene_stats_all = gene_stats_all,
    gene_dataset_summary = gene_dataset_summary,
...
# ===== 步骤3：根据数据集表达阈值筛选基因 =====
select_genes_for_pca <- function(
    gene_dataset_summary,
    n_datasets,
    min_dataset_pct = 0.2
) {
  cat("========================================\n")
  ...
  min_datasets <- ceiling(n_datasets * min_dataset_pct)
  ...
  # 筛选基因
  genes_for_pca <- gene_dataset_summary[
    n_datasets_expressed >= min_datasets,
    gene
  ]
  
  cat(sprintf("\n筛选出用于PCA的基因数: %d\n", length(genes_for_pca)))
  cat(sprintf(
    "未被选中的基因数: %d\n",
    nrow(gene_dataset_summary) - length(genes_for_pca)
  ))
  cat(sprintf(
    "选中比例: %.1f%%\n\n",
    length(genes_for_pca) / nrow(gene_dataset_summary) * 100
  ))
  
  # 选中基因的统计
  genes_selected_summary <- gene_dataset_summary[gene %in% genes_for_pca]
  
  cat("=== 选中基因的表达特征 ===\n")
  cat(sprintf(
    "平均在 %.1f 个数据集中表达 (范围: %d-%d)\n",
    mean(genes_selected_summary$n_datasets_expressed),
    min(genes_selected_summary$n_datasets_expressed),
    max(genes_selected_summary$n_datasets_expressed)
  ))
  cat(sprintf(
    "平均细胞表达比例: %.2f%%\n",
    mean(genes_selected_summary$mean_cell_pct) * 100
  ))
  cat(sprintf(
    "平均表达量: %.3f\n\n",
    mean(genes_selected_summary$mean_expression)
  ))
  
  return(list(
    genes_for_pca = genes_for_pca,
    genes_selected_summary = genes_selected_summary,
    min_datasets = min_datasets  # 返回计算好的阈值,避免重复计算
  ))
}


# ===== 步骤4：可视化基因筛选结果 =====
visualize_gene_selection <- function(
    gene_dataset_summary,
    min_datasets,
    n_total_datasets,
    genes_for_pca
  ...
  # 添加筛选标记
  gene_dataset_summary[, selected_for_pca := gene %in% genes_for_pca]
  
  # 图1：基因在数据集中的分布
  p1 <- ggplot(
    gene_dataset_summary,
    aes(x = n_datasets_expressed, fill = selected_for_pca)
  ) +
    geom_histogram(
      binwidth = 1,
      alpha = 0.7,
    ...
    geom_vline(
      xintercept = min_datasets,
      linetype = "dashed",
      color = "red",
      linewidth = 1.2
    ) +
    scale_fill_manual(
      values = c("FALSE" = "#95a5a6", "TRUE" = "#3498db"),
      labels = c("FALSE" = "Excluded", "TRUE" = "Selected for PCA"),
      name = ""
    ) +
    annotate(
      "text",
    ...
    labs(
      ...
      subtitle = sprintf("Total: %d datasets", n_total_datasets),
      x = "Number of Datasets with Expression",
      y = "Number of Genes"
    ) +
  ...
  # 图2：选中基因的表达特征
  genes_selected <- gene_dataset_summary[selected_for_pca == TRUE]
  
  p2 <- ggplot(
    genes_selected,
    aes(x = mean_cell_pct * 100, y = mean_expression)
  ) +
    geom_point(alpha = 0.4, color = "#2ecc71", size = 1.5) +
    geom_density_2d(color = "#27ae60", alpha = 0.5) +
    scale_x_log10() +
    scale_y_log10() +
    labs(
      title = "Expression Profile of Selected Genes",
      subtitle = sprintf("n = %d genes for PCA", length(genes_for_pca)),
      x = "Mean Cell Percentage (%, log10)",
      y = "Mean Expression Level (log10)"
    ) +
    theme_minimal(base_size = 13) +
    theme(
      plot.title = element_text(face = "bold", size = 15, hjust = 0.5),
  ...
  # 图3：选中 vs 未选中基因的比较
  comparison_data <- gene_dataset_summary[,
                                          .(
                                            mean_datasets = mean(n_datasets_expressed),
                                            median_datasets = median(n_datasets_expressed),
  ...
  p3 <- ggplot(
    gene_dataset_summary,
    aes(x = selected_for_pca, y = n_datasets_expressed, fill = selected_for_pca)
  ) +
    geom_violin(alpha = 0.6) +
    geom_boxplot(width = 0.2, alpha = 0.8, outlier.alpha = 0.3) +
    scale_fill_manual(
      values = c("FALSE" = "#e74c3c", "TRUE" = "#2ecc71"),
      labels = c("FALSE" = "Excluded", "TRUE" = "Selected"),
      name = ""
    ) +
    labs(
      title = "Dataset Coverage: Selected vs Excluded Genes",
      x = "",
      y = "Number of Datasets with Expression"
    ) +
    scale_x_discrete(
      labels = c("FALSE" = "Excluded\nGenes", "TRUE" = "Selected\nfor PCA")
    ) +
    theme_minimal(base_size = 13) +
    theme(
      plot.title = element_text(face = "bold", size = 15, hjust = 0.5),
  ...
  return(list(
    distribution = p1,
    expression_profile = p2,
    comparison = p3
  ))
}


# ===== 主函数：完整的基因筛选流程（不修改Seurat对象）=====
select_genes_for_pca_analysis <- function(
    seurat_obj,
    dataset_col = "dataset",
    species = "human",
    min_dataset_pct = 0.3,
  ...
  cat("\n")
  cat("########################################\n")
  cat("# 基因筛选流程（标记用于PCA的基因）\n")
  cat("########################################\n\n")
  
  cat(sprintf("输入Seurat对象信息:\n"))
  cat(sprintf("  细胞数: %d\n", ncol(seurat_obj)))
  cat(sprintf("  基因数: %d\n", nrow(seurat_obj)))
  cat(sprintf("  数据集列名: %s\n", dataset_col))
  cat(sprintf("  物种: %s\n\n", species))
  ...
  cat(sprintf("筛选参数设置:\n"))
  cat(sprintf("  min_dataset_pct = %.2f (基因需在至少%.0f%%的数据集中表达)\n", 
              min_dataset_pct, min_dataset_pct * 100))
  cat(sprintf("  min_cell_pct = %.4f (每个数据集中至少%.2f%%的细胞表达)\n", 
              min_cell_pct, min_cell_pct * 100))
  cat(sprintf("  min_mean_umi = %.3f (平均UMI阈值)\n\n", min_mean_umi))
  
  # 步骤1：识别线粒体和核糖体基因
  cat("\n【步骤1/4】识别线粒体和核糖体基因\n")
  mt_ribo_result <- identify_mt_ribo_genes_seurat(
    seurat_obj = seurat_obj,
    species = species,
    assay = assay
  )
  
  # 步骤2：计算基因在各数据集中的表达（排除MT/Ribo）
  cat("\n【步骤2/4】计算基因在各数据集中的表达\n")
  expression_result <- calculate_gene_expression_across_datasets(
    seurat_obj = seurat_obj,
    genes_to_exclude = mt_ribo_result$genes_to_exclude,
    dataset_col = dataset_col,
    min_cell_pct = min_cell_pct,
    min_mean_umi = min_mean_umi,
    assay = assay,
    slot = slot
  )
  ...
  # 步骤3：根据数据集阈值筛选基因
  cat("\n【步骤3/4】根据数据集阈值筛选基因\n")
  selection_result <- select_genes_for_pca(
    gene_dataset_summary = expression_result$gene_dataset_summary,
    n_datasets = expression_result$n_datasets,
    min_dataset_pct = min_dataset_pct
  )
  
  genes_for_pca <- selection_result$genes_for_pca
  min_datasets <- selection_result$min_datasets  # 直接使用返回的计算结果
  
  # 步骤4：可视化
  cat("\n【步骤4/4】生成可视化结果\n")
  plots <- visualize_gene_selection(
    gene_dataset_summary = expression_result$gene_dataset_summary,
    min_datasets = min_datasets,
    n_total_datasets = expression_result$n_datasets,
    genes_for_pca = genes_for_pca
  )
  ...
  cat("########################################\n\n")
  cat("=== 总结 ===\n")
  cat(sprintf("Seurat对象总基因数: %d\n", nrow(seurat_obj)))
  cat(sprintf("线粒体基因: %d\n", length(mt_ribo_result$mt_genes)))
  cat(sprintf("核糖体基因: %d\n", length(mt_ribo_result$ribo_genes)))
  cat(sprintf(
    "排除MT/Ribo后分析的基因: %d\n",
    nrow(seurat_obj) - length(mt_ribo_result$genes_to_exclude)
  ))
  cat(sprintf("最终选中用于PCA的基因: %d\n", length(genes_for_pca)))
  cat(sprintf(
    "\n选中比例: %.1f%% (相对于总基因)\n",
    length(genes_for_pca) / nrow(seurat_obj) * 100
  ))
  cat(sprintf(
    "选中比例: %.1f%% (相对于非MT/Ribo基因)\n",
    length(genes_for_pca) /
      (nrow(seurat_obj) - length(mt_ribo_result$genes_to_exclude)) *
      100
  ))
  
  cat("\n提示: Seurat对象保持不变，所有基因都被保留\n")
  cat("      请使用返回的 genes_for_pca 进行后续PCA分析\n")
  
  return(list(
    genes_for_pca = genes_for_pca,
    mt_genes = mt_ribo_result$mt_genes,
    ribo_genes = mt_ribo_result$ribo_genes,
    genes_excluded_mt_ribo = mt_ribo_result$genes_to_exclude,
    gene_stats_all = expression_result$gene_stats_all,
    gene_dataset_summary = expression_result$gene_dataset_summary,
    genes_selected_summary = selection_result$genes_selected_summary,
    plots = plots,
...
# 执行基因筛选流程（不修改原始Seurat对象）
result <- select_genes_for_pca_analysis(
  seurat_obj = seurat_obj_main,
  dataset_col = "dataset", # metadata中标识数据集的列名
  species = "human", # 或 "mouse"
  min_dataset_pct = 0.4, # 40%的数据集（20个中的8个）
  min_cell_pct = 0.03, # 每个数据集中至少3%的细胞表达
  min_mean_umi = 0.1, # 平均UMI阈值
  assay = "RNA",
  slot = "counts"
)

# 查看可视化结果
print(result$plots$distribution)
print(result$plots$expression_profile)
print(result$plots$comparison)

# 获取用于PCA的基因列表
genes_for_pca <- result$genes_for_pca
cat(sprintf("用于PCA的基因数: %d\n", length(genes_for_pca)))

# Restrict universe to selected genes
genes_use <- intersect(unique(genes_for_pca), rownames(seurat_obj_main))
stopifnot(length(genes_use) > 50)
genes_use <- unique(c(genes_use,Markers))

view(genes_use)
# Create filtered assay
counts_filt <- GetAssayData(seurat_obj_main, assay = "RNA", slot = "counts")[
  genes_use,
  ,
  drop = FALSE
]
seurat_obj_main[["RNA_filt"]] <- CreateAssayObject(counts = counts_filt)
DefaultAssay(seurat_obj_main) <- "RNA_filt"

# SCTransform with cell cycle and MT regression
seurat_obj_main <- SCTransform(
  seurat_obj_main,
  assay = "RNA_filt",
  new.assay.name = "SCT",
...
seurat_obj_main <- RunPCA(
  seurat_obj_main,
  assay = "SCT",
  features = VariableFeatures(seurat_obj_main, assay = "SCT"),
  npcs = 50,
  verbose = TRUE
)

gc()
# Optional: switch back after PCA
DefaultAssay(seurat_obj_main) <- "RNA"

# 查看PCA结果
DimPlot(seurat_obj_main, reduction = "pca", group.by = "dataset")
ElbowPlot(seurat_obj_main, ndims = 50)

# ===== gene filtering ended =====#

# ===== downsampled =====#
# Downsample, then preprocess (Normalize, HVG, Scale, PCA, Harmony optional, UMAP)
set.seed(42)

# 1) Stratified downsampling (by sample). Adjust caps as needed.
meta <- seurat_obj_main@meta.data
cells_all <- rownames(meta)
per_sample_cap <- 5000
global_cap <- 60000

samples <- na.omit(unique(meta$dataset))
picked <- character(0)
for (s in samples) {
  idx <- which(meta$dataset == s)
  if (length(idx) == 0) {
    next
  }
  take <- min(length(idx), per_sample_cap)
  picked <- c(picked, cells_all[sample(idx, take)])
}
if (length(picked) > global_cap) {
  picked <- sample(picked, global_cap)
}

cat(sprintf(
  "Downsampled to %d cells across %d samples\n",
  length(picked),
  length(samples)
))

# 2) Create downsampled object
seurat_obj_ds <- subset(seurat_obj_main, cells = picked)

# 4) Normalize, HVG, Scale
seurat_obj_ds <- NormalizeData(
  seurat_obj_ds,
  normalization.method = "LogNormalize",
  scale.factor = 10000
)
seurat_obj_ds <- FindVariableFeatures(
  seurat_obj_ds,
  selection.method = "vst",
  nfeatures = 3000
)
seurat_obj_ds <- ScaleData(
  seurat_obj_ds,
  features = VariableFeatures(seurat_obj_ds),
  vars.to.regress = c("S.Score", "G2M.Score", "percent.mt")
)

# 5) PCA
seurat_obj_ds <- RunPCA(
  seurat_obj_ds,
  npcs = 50,
  features = VariableFeatures(seurat_obj_ds),
  verbose = TRUE
...
# 6) Optional: Harmony on the subset (adjust params to taste)
seurat_obj_ds <- RunHarmony(
  seurat_obj_ds,
...
# Choose reduction for UMAP/neighbors
red_use <- if ("harmony" %in% names(seurat_obj_ds@reductions)) {
  "harmony"
} else {
  "pca"
}

# 7) UMAP/neighbors/clusters on subset
dims_to_use <- 1:min(40, ncol(Embeddings(seurat_obj_ds, red_use)))
n_nb <- min(200, max(50, round(ncol(seurat_obj_ds) * 0.03)))

seurat_obj_ds <- RunUMAP(
  seurat_obj_ds,
  reduction = red_use,
  dims = dims_to_use,
  n.neighbors = n_nb,
...
seurat_obj_ds <- FindNeighbors(
  seurat_obj_ds,
  reduction = red_use,
  dims = dims_to_use,
  k.param = min(50, max(15, round(n_nb * 0.5))),
  n.trees = 50
)
seurat_obj_ds <- FindClusters(
  seurat_obj_ds,
  algorithm = 4,
...
# 8) Quick plots
if (!dir.exists("qc_plots_ds")) {
  dir.create("qc_plots_ds")
}
pdf("qc_plots_ds/umap_clusters_ds.pdf", width = 10, height = 8)
Idents(seurat_obj_ds) <- seurat_obj_ds$seurat_clusters
DimPlot(
  seurat_obj_ds,
  reduction = "umap",
  label = TRUE,
...
# 9) Save
saveRDS(seurat_obj_ds, "seurat_downsampled_preprocessed.rds")

# seurat_obj_ds
# --------------------- downsampled ended---------------------#


# Enhanced normalization for smoother embeddings
seurat_obj_main <- NormalizeData(
  seurat_obj_main,
  normalization.method = "LogNormalize",
  scale.factor = 10000
)

# More variable features for smoother transitions
seurat_obj_main <- FindVariableFeatures(
  seurat_obj_main,
  selection.method = "vst",
  nfeatures = 3000 # Increased from 2000 for smoother manifold
)

# Scale with all variable features
seurat_obj_main <- ScaleData(
  seurat_obj_main,
  vars.to.regress = c("S.Score", "G2M.Score", 'percent.mt')
)

# More PCs for smoother embedding
seurat_obj_main <- RunPCA(
  seurat_obj_main,
  npcs = 50, # Increased from 40
  verbose = TRUE
)

# Visualize PCA
pdf('dimmap.pdf', width = 15, height = 10)
print(ElbowPlot(seurat_obj_main, ndims = 50))
print(DimHeatmap(seurat_obj_main, dims = 1:20, cells = 100, balanced = TRUE))
dev.off()
# #
...
seurat_obj_main <- RunHarmony(
  ...
  group.by.vars = c("dataset"), #, , "tissue_sampling_method"
  theta = c(0.5), # Higher theta for smoother mixing  , 0.5,1
  lambda = c(8), # Moderate lambda   , 3,8
  sigma = 0.025, # Lower sigma for tighter, smoother clusters
  nclust = 50,
  reduction.use = "pca",
  max_iter = 20, # More iterations for better convergence
  early_stop = TRUE, # Don't stop early
  dims = 1:30 # Use more dimensions (was 1:33)
)

# seurat_obj_main <- RunHarmony(
#   object = seurat_obj_main,
#   group.by.vars = c("sample"),   #,'sequencing_platform','tissue_dissociation_protocol','assay'
#   theta = c(1),      # Higher theta for more diverse clustering           ,1,1,1
#   lambda = c(6),   # Higher lambda to reduce overcorrection              ,1,1,1
#   sigma = 0.1,           # Lower sigma for tighter clusters
#   nclust = 40,            # Increased number of clusters
#   reduction.use = "pca",
#   max_iter = 70,
#   # cool_down = 10,
#   # epsilon_harmony = 0.0001,
...
#   # epsilon_cluster = 0.00001,
#   early_stop = TRUE,
#   dims = 1:33           # More iterations for better convergence
# )
#
print("Calculating integration quality metrics...")
...
# 
# print(
#   ggplot(data.frame(rank = umi_rank, UMI = total_umi), aes(x = rank, y = UMI)) +
...
#     theme_bw() +
#     labs(
#       title = "Post-integration UMI Knee Plot",
#       x = "Cell rank",
#       y = "Total UMIs"
#     )
# )
...
# print(
#   ggplot(
#     data.frame(rank = gene_rank, Genes = genes_per_cell),
#     aes(x = rank, y = Genes)
#   ) +
#     geom_line(color = "blue") +
#     geom_point(size = 0.5, alpha = 0.5, color = "blue") +
...
#     theme_bw() +
#     labs(
#       title = "Post-integration Gene Detection Knee Plot",
#       x = "Cell rank",
...
# 
# # 3. PC肘部图
# pct_var <- seurat_obj_main[["pca"]]@stdev /
#   sum(seurat_obj_main[["pca"]]@stdev) *
#   100
...
# harmony_scores_ordered <- sort(
#   colMeans(as.matrix(harmony_dist)),
#   decreasing = TRUE
# )
# 
# print(
#   ggplot(
#     data.frame(
#       rank = 1:length(harmony_scores_ordered),
...
#     theme_bw() +
#     labs(
#       title = "Harmony Scores Distribution (10k cells sampled)",
#       x = "Cell rank",
...
# 
# seurat_obj_main <- readRDS(
#   "/home/h2048/data/R/1029/seurat_without_partial_dual_contamination_doublets_2_1028.rds"
# )
seurat_obj_main <- readRDS(
  "seurat_without_partial_dual_contamination_doublets_2_1028.rds"
)
seurat_obj_main <- RunUMAP(
  seurat_obj_main,
  reduction = "harmony",
  dims = 1:30, # More dimensions from Harmony
  n.neighbors = 400, # Higher for smoother transitions
  min.dist = 0.55, # Moderate min.dist
  # spread = 1.5,
  # learning.rate = 0.2,
  # n.epochs = 1000, # More epochs for convergence
  # repulsion.strength = 1.0,
  metric = "correlation", # Try cosine instead of correlation
  # n.components = 2,
  verbose = TRUE
)
gc()
# Find neighbors
seurat_obj_main <- FindNeighbors(
  seurat_obj_main,
  reduction = "harmony",
  dims = 1:30,
  # n.trees = 50,
...
# summary(seurat_obj_main$nCount_RNA)
# VlnPlot(seurat_obj_main, 
#         features = c("nCount_RNA", "nFeature_RNA", "percent.mt"),
#         pt.size = 0, ncol = 3)
# # 计算阈值
# median_count <- median(seurat_obj_main$nCount_RNA)
# mad_count <- mad(seurat_obj_main$nCount_RNA)
# 
# # 下限：median - 3*MAD（log空间）
# lower_threshold <- 10^(log10(median_count) - 3 * mad(log10(seurat_obj_main$nCount_RNA)))
# 
# # 上限：median + 3*MAD（log空间）
# upper_threshold <- 10^(log10(median_count) + 3 * mad(log10(seurat_obj_main$nCount_RNA)))
# 
# cat("推荐下限:", round(lower_threshold), "UMI\n")
# cat("推荐上限:", round(upper_threshold), "UMI\n")


# saveRDS(seurat_obj_main, "seurat_1104processed.rds")
# seurat_obj_main <- subset(seurat_obj_main, subset = seurat_clusters == '53',invert = TRUE) #High KRT8-PTPRC doublets
# seurat_obj_main <- subset(seurat_obj_main, subset = nCount_RNA<=upper_threshold &nCount_RNA>=lower_threshold)
table(seurat_obj_main$seurat_clusters)
...
seurat_obj_main$ann_level_2_no_na[is.na(
  seurat_obj_main$ann_level_2_no_na
)] <- "Unknown"
...
for (i in seq_along(marker_batches)) {
  ...
  # 绘制热图
  print(
    DoHeatmap(seurat_obj_main, features = marker_batches[[i]], size = 24) +
      NoLegend() +
      ggtitle(paste("Marker Genes Batch", i)) +
      theme(axis.text.y = element_text(size = 36))
  )
...
new_idents <- annotations$Annotation[match(
  current_clusters,
  annotations$Cluster
)]
...
Undefined_obj <- FindVariableFeatures(
  Undefined_obj,
  selection.method = "vst",
  nfeatures = 3000
)
...
Undefined_obj <- ScaleData(
  Undefined_obj,
  features = VariableFeatures(Undefined_obj)
)
...
if (requireNamespace("lisi", quietly = TRUE)) {
  ...
  write.csv(
    integration_metrics,
    "Undefined_integration_metrics.csv",
    row.names = FALSE
  )
} else {
...
pdf("Undefined_umap_by_metadata.pdf", width = 12, height = 10)
p1 <- DimPlot(
  Undefined_obj,
  reduction = "umap",
  group.by = "sample",
...
for (sample_name in sample_names) {
  ...
  if (length(sample_cells) > 0) {
    ...
    stats <- data.frame(
      ...
      Median_Mt_Percent = round(
        median(sample_qc_data$percent.mt, na.rm = TRUE),
        2
      ),
...
cat(sprintf(
  "Plot layout: %d rows × %d cols (total %d samples)\n\n",
  n_rows,
  n_cols,
  n_samples
))
...
all_qc_data_valid <- do.call(
  rbind,
  sample_data_list[sapply(sample_data_list, nrow) > 0]
)
...
if (nrow(all_qc_data_valid) > 0) {
  ...
  percent_mt_range <- c(
    max(0, percent_mt_range[1] - 1),
    percent_mt_range[2] + 1
  )
...
create_violin_plot <- function(sample_name, feature, ylabel, y_range) {
  if (
    sample_name %in%
      ...
      nrow(sample_data_list[[sample_name]]) > 0
  ) {
...
create_scatter_plot <- function(
  ...
  if (
    ...
    p <- ggplot(data, aes_string(x = x_feature, y = y_feature)) +
      ...
      xlim(x_range) +
      ylim(y_range) + # Apply consistent axis ranges
      ggtitle(paste("Sample:", sample_name)) +
      theme_classic() +
      theme(plot.title = element_text(size = 12, hjust = 0.5)) +
      xlab(xlabel) +
    ...
    return(
      ggplot() +
        ggtitle(paste("Sample:", sample_name, "(No cells)")) +
        xlim(x_range) +
        ylim(y_range) + # Keep consistent axes
        theme_void() +
        theme(plot.title = element_text(size = 12, hjust = 0.5))
    )
  }
...
scatter_plots_umi_genes <- lapply(
  sample_names,
  create_scatter_plot,
...
umi_genes_combined <- wrap_plots(
  scatter_plots_umi_genes,
  ncol = n_cols,
  nrow = n_rows
)
...
umi_mt_combined <- wrap_plots(
  scatter_plots_umi_mt,
  ncol = n_cols,
  nrow = n_rows
)
...
ggsave(
  "qc_plots_by_metric/nFeature_RNA_all_samples.pdf",
  plot = genes_combined,
  width = fig_width,
  height = fig_height,
  units = "in",
...
ggsave(
  "qc_plots_by_metric/nCount_RNA_all_samples.pdf",
...
ggsave(
  "qc_plots_by_metric/percent_mt_all_samples.pdf",
  plot = mt_combined,
  width = fig_width,
  height = fig_height,
  units = "in",
...
ggsave(
  "qc_plots_by_metric/scatter_UMI_vs_Genes_all_samples.pdf",
...
ggsave(
  "qc_plots_by_metric/scatter_UMI_vs_Mt_all_samples.pdf",
...
write.csv(
  all_stats,
  "qc_plots_by_metric/sample_qc_metrics_summary.csv",
  row.names = FALSE
)
...
write.csv(
  all_qc_data,
  "qc_plots_by_metric/all_samples_qc_raw_data.csv",
  row.names = FALSE
)
...
p <- ggplot(
  ...
  aes(
    y = cell_count,
    axis1 = sample_ordered,
    axis2 = study,
    axis3 = tissue,
    axis4 = tissue_sampling_method
  )
) +
  # 流线 - 半透明，按tissue着色
  ...
  geom_text(
    stat = "stratum",
    aes(
      label = paste0(after_stat(stratum), " (", comma(after_stat(count)), ")")
    ),
    size = 2.3,
    hjust = 1,
    nudge_x = -0.2, # 增大向左偏移
  ...
  # 坐标轴设置
  scale_x_discrete(
    limits = c("Sample", "Study", "Tissue", "Sampling Method"),
    labels = c("Sample\n(by Study)", "Study", "Tissue", "Sampling Method")
  ) + # 增加左侧空间
...
# 计算每千细胞的通讯数
stats_data$Int_Per_1k_Cells <- stats_data$Int_Count /
  (stats_data$Cell_Count / 1000)
...
# 保存比较图
ggsave(
  file.path(output_dir, "interaction_comparison.pdf"),
  p_combined,
  width = 12,
  height = 7
)
...
# 各组织网络图比较
message("生成各组织网络图...")
weight_max_count <- getMaxWeight(
  cellchat_list,
  attribute = c("idents", "count")
)
weight_max_weight <- getMaxWeight(
  cellchat_list,
...
pdf(
  file.path(diff_dir, "tissue_weight_comparison.pdf"),
  width = 14,
  height = 12
)
par(mfrow = c(ceiling(length(cellchat_list) / 2), 2), xpd = TRUE)
...
for (tissue in names(merged_cellchat@netP)) {
  if ("pathways" %in% names(merged_cellchat@netP[[tissue]])) {
    pathways_all[[tissue]] <- names(merged_cellchat@netP[[tissue]][[
      "pathways"
    ]])
  }
}
  ...
  tryCatch(
    {
      ...
      p <- gg1 + gg2
      ggsave(
        file.path(pair_dir, paste0("pathway_comparison_", pair_name, ".pdf")),
        p,
        width = 12,
        height = 8
      )
...
if (length(common_pathways) > 0) {
  ...
  if (requireNamespace("pheatmap", quietly = TRUE)) {
    ...
    # 绘制热图并保存
    pdf(
      file.path(output_dir, "pathway_strength_heatmap.pdf"),
      width = 12,
      height = 10
    )
    pheatmap::pheatmap(
    ...
    pdf(
      file.path(output_dir, "pathway_strength_heatmap_scaled.pdf"),
      width = 12,
      height = 10
    )
    pheatmap::pheatmap(
  ...
  if (
    ...
    plot_data_long$Pathway <- factor(
      ...
      levels = rev(top_15_pathways)
    )

    # 创建条形图
    bar_plot <- ggplot2::ggplot(
      plot_data_long,
      ggplot2::aes(x = Pathway, y = Strength, fill = Tissue)
    ) +
    ...
    # 保存条形图
    ggplot2::ggsave(
      file.path(output_dir, "top_pathways_barplot.pdf"),
      bar_plot,
      width = 12,
      height = 10
    )
    ...
    if (
      ...
      # 创建雷达图
      pdf(
        file.path(output_dir, "pathway_radar_chart.pdf"),
        width = 10,
        height = 10
      )
      ...
      fmsb::radarchart(
        ...
        pfcol = sapply(rainbow(ncol(radar_data)), function(x) {
          adjustcolor(x, alpha.f = 0.3)
        }),
        plwd = 2,
        cglcol = "grey",
        cglty = 1,
  ...
  if (is.matrix(presence_matrix)) {
    ...
    if (requireNamespace("pheatmap", quietly = TRUE)) {
      pdf(
        file.path(output_dir, "pathway_presence_heatmap.pdf"),
        width = 12,
        height = 10
      )
      pheatmap::pheatmap(
        t(presence_matrix)[sorted_pathways, ],
        cluster_rows = FALSE,
        cluster_cols = TRUE,
    ...
    if (requireNamespace("ggplot2", quietly = TRUE)) {
      ...
      # 设置因子水平以控制绘图顺序
      freq_data$Pathway <- factor(
        freq_data$Pathway,
        levels = rev(freq_data$Pathway)
      )

      # 创建条形图
      bar_plot <- ggplot2::ggplot(
        freq_data,
        ggplot2::aes(x = Pathway, y = Frequency)
      ) +
      ...
      # 保存条形图
      ggplot2::ggsave(
        file.path(output_dir, "pathway_frequency_barplot.pdf"),
        bar_plot,
        width = 12,
        height = 10
      )
...
for (pattern in c("outgoing", "incoming", "all")) {
  ...
  for (i in seq_along(cellchat_list)) {
    ...
    file_path <- file.path(
      pattern_dir,
      paste0("signaling_pattern_", pattern, "_", safe_tissue_name, ".pdf")
    )
    ...
    tryCatch(
      ...
      error = function(e) {
        ...
        message(paste0(
          "Error generating heatmap for tissue ",
          tissue,
          ": ",
          e$message
        ))
  ...
  # 也创建一个汇总文件
  file_path_all <- file.path(
    pattern_dir,
    paste0("signaling_pattern_", pattern, "_all_tissues.pdf")
  )
...
cat(paste0(
  "Valid tissues with pathways: ",
  paste(valid_tissues, collapse = ", "),
  "\n\n"
))
...
if (length(valid_tissues) >= 2) {
  ...
  cat(paste0(
    "Pathways present in at least 2 tissues: ",
  ...
  if (length(shared_pathways) > 0 && length(shared_pathways) <= 20) {
    ...
    cat(paste0(
      "  Top 20: ",
      paste(shared_pathways[1:20], collapse = ", "),
      "...\n"
    ))
  }
  cat("\n")
  ...
  for (tissue in valid_tissues) {
    unique_paths <- setdiff(
      pathways_by_tissue[[tissue]],
    ...
    if (length(unique_paths) > 0 && length(unique_paths) <= 10) {
      ...
      cat(paste0(
        "    Top 10: ",
        paste(unique_paths[1:10], collapse = ", "),
        "...\n"
      ))
    }
  }
...
if (length(valid_tissues) >= 2) {
  ...
  if (length(valid_tissues) < length(cellchat_list)) {
    ...
    merged_subset <- mergeCellChat(
      cellchat_subset,
      add.names = names(cellchat_subset)
    )
  } else {
  ...
  tryCatch(
    {
      ...
      merged_subset <- computeNetSimilarityPairwise(
        merged_subset,
        type = "structural"
      )
      ...
      tryCatch(
        ...
        error = function(e) {
          message(
            "Error in netEmbedding with default method. Trying with alternative method..."
          )
          # Try with PCA instead
          merged_subset <- netEmbedding(
            merged_subset,
            type = "structural",
      ...
      if (!is.null(merged_subset@net$structural.similarity)) {
        ...
        # Visualization
        pdf(
          file.path(similarity_dir, "standard_structural_similarity.pdf"),
          width = 12,
          height = 10
        )
    ...
    error = function(e) {
      message(paste0(
        "Standard structural similarity analysis failed: ",
        e$message
      ))
    }
  )
  ...
  if (same_celltypes) {
    ...
    tryCatch(
      {
        ...
        if (!is.null(merged_subset@net$functional.similarity)) {
          ...
          pdf(
            file.path(similarity_dir, "standard_functional_similarity.pdf"),
            width = 12,
            height = 10
          )
      ...
      error = function(e) {
        message(paste0(
          "Standard functional similarity analysis failed: ",
          e$message
        ))
      }
    )
    ...
    message(
      "Tissues have different cell type compositions, skipping functional similarity analysis"
    )
  }
}
    ...
    if (length(cellchat_list[[tissue]]@netP$pathways) > 0) {
      for (pathway in all_pathways) {
        if (pathway %in% names(cellchat_list[[tissue]]@netP$pathways)) {
          ...
          pathway_strength[i, pathway] <- sum(cellchat_list[[
            tissue
          ]]@netP$prob[, pathway])
        }
      }
...
if (!is.null(jaccard_sim) && requireNamespace("pheatmap", quietly = TRUE)) {
  ...
  write.csv(
    jaccard_sim,
    file = file.path(similarity_dir, "jaccard_similarity_matrix.csv")
  )

  # Create heatmap
  pdf(
    file.path(similarity_dir, "jaccard_similarity_heatmap.pdf"),
    width = 10,
    height = 8
  )
  pheatmap::pheatmap(
  ...
  pdf(
    file.path(similarity_dir, "jaccard_similarity_dendrogram.pdf"),
    width = 10,
    height = 8
  )
  ...
  plot(
    hclust_result,
    main = "Hierarchical Clustering of Tissues by Pathway Overlap",
    xlab = "",
    sub = ""
  )
  dev.off()
...
if (
  !is.null(strength_sim_result) && requireNamespace("pheatmap", quietly = TRUE)
) {
  ...
  write.csv(
    strength_sim,
    file = file.path(similarity_dir, "strength_correlation_matrix.csv")
  )
  write.csv(
    pathway_strength,
    file = file.path(similarity_dir, "pathway_strength_by_tissue.csv")
  )

  # Create correlation heatmap
  pdf(
    file.path(similarity_dir, "strength_correlation_heatmap.pdf"),
    width = 10,
    height = 8
  )
  pheatmap::pheatmap(
  ...
  # Create hierarchical clustering visualization
  pdf(
    file.path(similarity_dir, "strength_correlation_dendrogram.pdf"),
    width = 10,
    height = 8
  )
  # First create a distance matrix (1 - similarity)
  ...
  plot(
    hclust_result,
    main = "Hierarchical Clustering of Tissues by Pathway Strength",
    xlab = "",
    sub = ""
  )
  dev.off()
  ...
  top_pathways <- names(sort(pathway_totals, decreasing = TRUE))[
    1:min(30, length(pathway_totals))
  ]

  # Create heatmap of top pathways
  pdf(
    file.path(similarity_dir, "pathway_strength_heatmap.pdf"),
    width = 12,
    height = 10
  )
  pheatmap::pheatmap(
    pathway_strength[, top_pathways],
...
for (pair in tissues_pairs) {
  ...
  # 差异表达分析
  merged_cellchat <- identifyOverExpressedGenes(
    merged_cellchat,
  ...
  if (nrow(net1.up) > 0) {
    write.csv(
      net1.up,
      file = file.path(pair_dir, paste0(pair[1], "_upregulated_LR.csv"))
    )
  }
  if (nrow(net1.down) > 0) {
    write.csv(
      net1.down,
      file = file.path(pair_dir, paste0(pair[2], "_upregulated_LR.csv"))
    )
  }
  ...
  if (nrow(net1.up) > 0 || nrow(net1.down) > 0) {
    pdf(
      file.path(pair_dir, "differential_lr_bubble.pdf"),
      width = 12,
      height = 10
    )
  ...
  # 3. 富集分析
  pdf(
    file.path(pair_dir, "enriched_ligands_wordcloud.pdf"),
    width = 10,
    height = 8
  )
  if (nrow(net1.up) > 0) {
    enriched.up <- computeEnrichmentScore(
      net1.up,
      species = 'human',
      variable.both = TRUE
    )
  ...
  if (nrow(net1.down) > 0) {
    enriched.down <- computeEnrichmentScore(
      net1.down,
      species = 'human',
      variable.both = TRUE
    )
...
if (length(common_pathways) > 0) {
  ...
  # 选择前N个强度最高的通路
  selected_pathways <- names(sort(pathway_strengths, decreasing = TRUE)[
    1:min(top_n_pathways, length(pathway_strengths))
  ])
} else {
  message("未找到所有组织共有的信号通路，将使用出现频率最高的通路")

  # 获取并集和出现频率
  all_pathways <- unlist(lapply(cellchat_list, function(x) {
    names(x@netP$pathways)
  }))
  pathway_counts <- table(all_pathways)

  # 选择出现频率最高的通路
  selected_pathways <- names(sort(pathway_counts, decreasing = TRUE)[
    1:min(top_n_pathways, length(pathway_counts))
  ])
}
...
for (pathway in selected_pathways) {
  ...
  for (layout in c("circle", "chord", "hierarchy")) {
    pdf(
      file.path(pathway_dir_specific, paste0(pathway, "_", layout, ".pdf")),
      width = 12,
      height = 10
    )
  ...
  # 热图比较
  pdf(
    file.path(pathway_dir_specific, paste0(pathway, "_heatmap.pdf")),
    width = 12,
    height = 10
  )
  ht_list <- list()
  ...
  # 配体-受体对贡献分析
  pdf(
    file.path(pathway_dir_specific, paste0(pathway, "_contribution.pdf")),
    width = 12,
    height = 10
  )
  ...
  # 表达情况分析
  pdf(
    file.path(pathway_dir_specific, paste0(pathway, "_expression.pdf")),
    width = 12,
    height = 10
  )

  # 设置数据集顺序
  merged_cellchat@meta$datasets <- factor(
    merged_cellchat@meta$datasets,
...
common_celltypes <- Reduce(
  intersect,
  lapply(cellchat_list, function(x) levels(x@idents))
)
...
if (length(common_celltypes) > 0) {
  ...
  for (cell_type in common_celltypes) {
    ...
    # 创建PDF文件
    pdf(
      file.path(cell_dir, paste0(cell_type, "_across_tissues.pdf")),
      width = 12,
      height = 10
    )
    ...
    for (i in seq_along(tissues_with_celltype)) {
      ...
      if (length(outgoing_paths) > 0) {
        top_out_paths <- names(sort(outgoing_paths, decreasing = TRUE)[
          1:min(3, length(outgoing_paths))
        ])
        tissue_stats$top_outgoing_pathways[
          tissue_stats$tissue == tissue
        ] <- paste(top_out_paths, collapse = ", ")
      }
      ...
      if (length(incoming_paths) > 0) {
        top_in_paths <- names(sort(incoming_paths, decreasing = TRUE)[
          1:min(3, length(incoming_paths))
        ])
        tissue_stats$top_incoming_pathways[
          tissue_stats$tissue == tissue
        ] <- paste(top_in_paths, collapse = ", ")
      }
      ...
      if (nrow(mat) > 1) {
        ...
        if (length(out_weights) > 0) {
          top_targets <- names(sort(out_weights, decreasing = TRUE)[
            1:min(3, length(out_weights))
          ])
          tissue_stats$top_outgoing_targets[
            tissue_stats$tissue == tissue
          ] <- paste(top_targets, collapse = ", ")
        }
        ...
        if (length(in_weights) > 0) {
          top_sources <- names(sort(in_weights, decreasing = TRUE)[
            1:min(3, length(in_weights))
          ])
          tissue_stats$top_incoming_sources[
            tissue_stats$tissue == tissue
    ...
    if (length(pathway_list) > 0) {
      pathway_data <- do.call(
        rbind,
        lapply(names(pathway_list), function(tissue) {
      ...
      if (nrow(pathway_data) > 0) {
        ...
        p <- ggplot(plot_data, aes(x = pathway, y = strength, fill = tissue)) +
          ...
          theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
          labs(
            title = paste0(cell_type, " 在不同组织中的信号通路强度"),
            x = "通路",
            y = "通讯强度"
          )
  ...
  if (length(common_celltypes) > 1) {
    ...
    if (nrow(all_stats) > 0) {
      # 计算额外指标
      all_stats$total_weight <- all_stats$outgoing_weight +
        all_stats$incoming_weight
      all_stats$send_receive_ratio <- all_stats$outgoing_weight /
        (all_stats$incoming_weight + 0.1)

      # 创建PDF报告
      pdf(
        file.path(celltype_dir, "CellType_Comparison_Report.pdf"),
        width = 12,
        height = 10
      )
      ...
      # 绘制热图
      if (
        requireNamespace("pheatmap", quietly = TRUE) && nrow(heatmap_data) > 1
      ) {
        pheatmap::pheatmap(
          heatmap_data_norm,
          cluster_rows = TRUE,
      ...
      ratio_plot <- ggplot(
        all_stats,
        aes(x = cell_type, y = send_receive_ratio, fill = tissue)
      ) +
        geom_bar(stat = "identity", position = "dodge") +
        scale_y_log10() +
        theme_bw() +
      ...
      diversity_plot <- ggplot(
        all_stats,
        aes(x = cell_type, y = outgoing_count + incoming_count, fill = tissue)
      ) +
        geom_bar(stat = "identity", position = "dodge") +
        theme_bw() +
        theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
        labs(title = "细胞类型的通讯多样性", x = "细胞类型", y = "总互作数量")
      ...
      summary_table <- all_stats %>%
        ...
        dplyr::summarise(
          ...
          var_across_tissues = sd(total_weight, na.rm = TRUE) /
            mean(total_weight, na.rm = TRUE)
        ) %>%
  ...
  if (nrow(cellchat_obj@net$weight) > 0) {
    ...
    if (length(cellchat_obj@netP$pathways) > 0) {
      ...
      summary_data <- data.frame(
        Tissue = tissue,
        Active_Senders = paste(
          names(active_senders)[1:min(3, length(active_senders))],
          collapse = ", "
        ),
        Active_Receivers = paste(
          names(active_receivers)[1:min(3, length(active_receivers))],
          collapse = ", "
        ),
        Top_Pathways = paste(
          names(top_pathways)[1:min(5, length(top_pathways))],
          collapse = ", "
        )
      )

      # 打印表格（使用 gridExtra::tableGrob 安全绘制，若未安装提示用户）
      if (requireNamespace("gridExtra", quietly = TRUE)) {
        gt <- gridExtra::tableGrob(summary_data, rows = NULL)
        grid::grid.newpage()
        grid::grid.draw(gt)
      } else {
        grid::grid.newpage()
        grid::grid.text(
          "gridExtra package is not installed. Install with install.packages('gridExtra')",
...
# 5. 对比热图
# 合并所有通路
pathway.union <- unique(unlist(lapply(cellchat_list, function(x) {
  names(x@netP$pathways)
})))
...
if (length(common_pathways) > 0) {
  ...
  cat(paste0(
    "- 前5个共有通路: ",
    paste(common_pathways[1:min(5, length(common_pathways))], collapse = ", "),
    "\n"
  ))
} else {
  cat("- 未找到所有组织共有的通路\n")
  ...
  if (!is.null(gg4$data)) {
    ...
    if (nrow(gg4_data) > 0) {
      ...
      pathway_diff <- gg4_data %>%
        ...
        dplyr::summarise(
          max_diff = max(interaction.strength) - min(interaction.strength)
        ) %>%
      ...
      for (i in 1:min(5, nrow(pathway_diff))) {
        ...
        cat(paste0(
          ...
          pathway,
          ": 差异值 = ",
          round(pathway_diff$max_diff[i], 2),
          "\n"
        ))
      }
    }
        ...
        if ("information" %in% names(cellchat_obj@netP$centrality)) {
          ...
          if (length(info_centrality) > 0) {
            ...
            cat(
              "- 关键中介细胞: ",
              paste(
                names(top_mediators)[1:min(3, length(top_mediators))],
                collapse = ", "
              ),
              "\n"
            )
...
if (exists("stats_data") && nrow(stats_data) > 0) {
  ...
  interaction_plot <- ggplot(stats_data, aes(x = Tissue)) +
    geom_bar(
      aes(y = Int_Count, fill = "互作数量"),
      stat = "identity",
      position = "dodge",
      alpha = 0.7
    ) +
    geom_bar(
      aes(y = Int_Per_1k_Cells * 10, fill = "每千细胞互作(×10)"),
      stat = "identity",
      position = "dodge",
      alpha = 0.7
    ) +
    scale_fill_manual(
      values = c("互作数量" = "darkblue", "每千细胞互作(×10)" = "orangered")
    ) +
    theme_bw() +
    theme(
      axis.text.x = element_text(angle = 45, hjust = 1),
    ...
    labs(title = "组织间互作数量比较", x = "组织", y = "互作数量")

  # 保存图形
  ggsave(
    file.path(vis_dir, "interaction_counts_comparison.pdf"),
    interaction_plot,
    width = 10,
    height = 6
  )
  ...
  pathway_plot <- ggplot(
    stats_data,
    aes(x = Tissue, y = Unique_Pathways, fill = Tissue)
  ) +
    ...
    labs(title = "组织间激活通路数量比较", x = "组织", y = "激活通路数量")

  # 保存图形
  ggsave(
    file.path(vis_dir, "pathway_counts_comparison.pdf"),
    pathway_plot,
    width = 10,
    height = 6
  )
...
if (exists("heatmap_data") && ncol(heatmap_data) > 0) {
  # 保存通路热图
  pdf(
    file.path(vis_dir, "pathway_heatmap_summary.pdf"),
    width = 12,
    height = 10
  )
...
if (exists("all_stats") && nrow(all_stats) > 0) {
  ...
  celltype_plot <- ggplot(plot_data, aes(x = cell_type)) +
    geom_bar(
      aes(y = outgoing_weight, fill = "发送强度"),
      stat = "identity",
      position = "stack"
    ) +
    geom_bar(
      aes(y = -incoming_weight, fill = "接收强度"),
      stat = "identity",
      position = "stack"
    ) +
    scale_fill_manual(
      values = c("发送强度" = "firebrick", "接收强度" = "steelblue")
    ) +
    facet_wrap(~tissue) +
    theme_bw() +
    theme(
  ...
  # 保存图形
  ggsave(
    file.path(vis_dir, "celltype_communication_summary.pdf"),
    celltype_plot,
    width = 12,
    height = 8
  )
...
# 4. 创建组织差异通路网络图摘要
# 选择一张差异最显著的组织对比图
gg_diff_summary <- netVisual_diffInteraction(
  merged_cellchat,
  weight.scale = TRUE,
  measure = "weight",
  top = 0.25
...
roe_to_symbol <- function(roe_value, thresholds = c(0.5, 0.8, 1.2, 2.0)) {
  ...
  if (!is.numeric(roe_value)) {
    return("-")
  }

  # 按照STARTRAC标准分类
  if (roe_value >= thresholds[4]) {
    # >= 2.0
    return("+++")
  } else if (roe_value >= thresholds[3]) {
    # >= 1.2
    return("+")
  } else if (roe_value > thresholds[2]) {
    # > 0.8
    return("+/-")
  } else if (roe_value >= thresholds[1]) {
    # >= 0.5
    return("-")
  } else {
    # < 0.5
...
clean_roe_matrix <- function(roe_matrix) {
  ...
  if (
    is.null(roe_matrix) ||
      (!is.matrix(roe_matrix) && !is.data.frame(roe_matrix))
  ) {
  ...
  message(sprintf(
    "  Matrix dimensions: %d cell types × %d tissues",
    nrow(roe_matrix),
    ncol(roe_matrix)
  ))
  message(sprintf(
    "  Valid values: %d/%d (%.1f%%)",
    sum(is.finite(roe_matrix)),
    length(roe_matrix),
...
create_startrac_dist_plot <- function(
  roe_matrix,
  ...
  if (method == "auto") {
    # 根据数据大小和复杂性选择方法
    if (
      n_tissues * n_celltypes > 500 || any(nchar(c(tissues, celltypes)) > 20)
    ) {
  ...
  tryCatch(
    {
      ...
      pdf(
        output_path,
        width = max(12, n_tissues * 1.5),
        height = max(8, n_celltypes * 0.4)
      )
      ...
      get_colors <- function(roe_val) {
        if (!is.finite(roe_val)) {
          return(list(bg = "#F0F0F0", text = "black"))
        }

        if (roe_val >= 2.0) {
          return(list(bg = "#8B0000", text = "white"))
        }
        if (roe_val >= 1.2) {
          return(list(bg = "#DC143C", text = "white"))
        }
        if (roe_val > 0.8) {
          return(list(bg = "#FFFACD", text = "black"))
        }
        if (roe_val >= 0.5) {
          return(list(bg = "#87CEEB", text = "black"))
        }
  ...
  tryCatch(
    {
      ...
      pdf(
        output_path,
        width = max(12, n_tissues * 1.2),
        height = max(8, n_celltypes * 0.35)
      )
      ...
      plot(
        ...
        1,
        type = "n",
        xlim = c(0.5, n_tissues + 0.5),
        ylim = c(0.5, n_celltypes + 0.5),
        xlab = "",
        ylab = "",
        main = "STARTRAC Symbols",
        axes = FALSE
      )
...
add_title_and_legend <- function(title) {
  ...
  grid.text(
    "Method: STARTRAC-dist uses Ro/e = Observed/Expected from chi-square test",
    x = 0.5,
    y = 0.02,
    just = "center",
...
integrated_startrac_analysis <- function(
  ...
  analysis_data <- analysis_data[
    analysis_data$majorCluster != "" &
      ...
      !analysis_data$majorCluster %in%
        c("NA", "Unknown", "unknown", "Unassigned") &
      !analysis_data$loc %in% c("NA", "Unknown", "unknown"),
  ]

  cells_after_cleaning <- nrow(analysis_data)
  removal_rate <- round(
    (original_cells - cells_after_cleaning) / original_cells * 100,
    2
  )

  message(sprintf(
    "Data cleaning: %d → %d cells (%.1f%% removed)",
    original_cells,
    cells_after_cleaning,
    removal_rate
  ))

  # 移除低频细胞类型
  celltype_counts <- table(analysis_data$majorCluster)
  low_count_celltypes <- names(celltype_counts)[
    celltype_counts < max(5, min_cells / 10)
  ]
  if (length(low_count_celltypes) > 0) {
    message(
      "Removing low-count cell types: ",
      paste(head(low_count_celltypes, 3), collapse = ", ")
    )
    analysis_data <- analysis_data[
      !analysis_data$majorCluster %in% low_count_celltypes,
    ]
  }
  ...
  if (length(unique_tissues) < 2) {
    stop(
      "Need at least 2 tissue locations. Found: ",
      paste(unique_tissues, collapse = ", ")
    )
  }

  message(sprintf(
    "Final dataset: %d cells, %d cell types, %d tissues, %d samples",
    nrow(analysis_data),
  ...
  global_chisq_result <- tryCatch(
    {
      ...
      if (
        min_expected >= 1 &&
          sum(expected_from_margins < 5) / length(expected_from_margins) <= 0.2
      ) {
  ...
  if (!is.null(global_chisq_result)) {
    ...
    message(sprintf(
      "Global chi-square test: χ² = %.3f, p = %.6f (%s)",
    ...
    if (global_significant) {
      ...
      chi_contributions <- ((observed_matrix - expected_matrix)^2) /
        expected_matrix
      celltype_chi_contrib <- rowSums(chi_contributions)
      std_residuals <- (observed_matrix - expected_matrix) /
        sqrt(expected_matrix)
      ...
      significance_results <- data.frame(
        ...
        Significant = celltype_chi_contrib >
          qchisq(0.95, df = ncol(contingency_table) - 1),
        stringsAsFactors = FALSE
      )
      ...
      significance_results$Significance_Level <- ifelse(
        ...
        "***",
        ifelse(
          significance_results$Max_Std_Residual > qnorm(0.995),
          "**",
          ifelse(
            significance_results$Max_Std_Residual > qnorm(0.975),
            "*",
            "ns"
          )
        )
      ...
      write.csv(
        ...
        file.path(output_dir, "significance_results.csv"),
        row.names = FALSE
      )

      n_significant <- sum(significance_results$Significant)
      message(sprintf(
        "Significant cell types: %d/%d",
        n_significant,
        nrow(significance_results)
      ))
    }
  }
...
#' @param output_dir 输出目录
create_enhanced_roe_heatmap <- function(
  roe_result,
  significance_results,
  output_dir
) {
  ...
  tryCatch(
    {
      ...
      celltype_groups <- sapply(rownames(roe_result), function(x) {
        x_lower <- tolower(x)
        if (grepl("^t[_ ]|^cd[48]|tcr|th[0-9]|treg|cytotoxic", x_lower)) {
          return("T_cells")
        }
        if (grepl("^b[_ ]|plasma|^cd19|^cd20|antibody", x_lower)) {
          return("B_cells")
        }
        if (
          grepl("macro|monocyte|^m[0-9]|dendritic|dc[_ ]|^cd14|^cd16", x_lower)
        ) {
          return("Myeloid")
        }
        if (grepl("fibroblast|smc|smooth.*muscle|pericyte|myofibro", x_lower)) {
          return("Stromal")
        }
        if (grepl("epithelial|endothelial|acinar|basal|ciliated", x_lower)) {
          return("Epithelial")
        }
        if (grepl("neutrophil|eosinophil|basophil|mast", x_lower)) {
          return("Granulocytes")
        }
        return("Other")
      })
      ...
      if (!is.null(significance_results)) {
        ...
        for (i in 1:nrow(significance_results)) {
          ...
          if (celltype %in% names(sig_annotation)) {
            sig_annotation[celltype] <- significance_results$Significance_Level[
              i
            ]
          }
        }
        ...
        row_annotation <- rowAnnotation(
          ...
          col = list(
            `Cell Group` = group_colors[
              names(group_colors) %in% unique(celltype_groups)
            ],
      ...
      # 创建PDF文件
      pdf(
        file.path(output_dir, "enhanced_roe_heatmap_pairwise.pdf"),
        width = 14,
        height = max(8, nrow(roe_result) * 0.4)
      )
      ...
      ht <- Heatmap(
        ...
        heatmap_legend_param = list(
          title = "Ro/e Index",
          at = c(
            min(roe_result, na.rm = TRUE),
            1,
            max(roe_result, na.rm = TRUE)
          ),
      ...
      # 添加说明文字
      grid.text(
        "Red/Blue dots: Extreme enrichment/depletion (Ro/e ≥2.0 or ≤0.5)",
        x = 0.5,
        y = 0.05,
...
create_alternative_heatmap <- function(
  roe_result,
  significance_results,
  output_dir
) {
          ...
          for (i in 1:nrow(significance_results)) {
            ...
            if (celltype %in% rownames(annotation_row)) {
              annotation_row[
                celltype,
                "Significance"
              ] <- significance_results$Significance_Level[i]
            }
          }
...
#' 创建补充可视化图像
create_supplementary_plots <- function(
  roe_result,
  analysis_data,
  output_dir,
  significance_results = NULL
  ...
  tryCatch(
    {
      ...
      p_bar <- ggplot(roe_df, aes(x = CellType, y = Roe_Index, fill = Tissue)) +
        ...
        labs(
          title = "Cell Type Distribution Across Tissues (Ro/e Index)",
          subtitle = "Values > 1: Enriched, Values < 1: Depleted",
          x = "Cell Type",
          y = "Ro/e Index",
          fill = "Tissue Location"
        ) +
        theme(axis.text.x = element_text(angle = 45, hjust = 1))

      ggsave(
        file.path(output_dir, "roe_barplot.pdf"),
        p_bar,
        width = 12,
        height = 8
      )
    ...
    error = function(e) {
      message("Warning: Failed to create bar plot - ", e$message)
    }
  )
  ...
  tryCatch(
    {
      ...
      p_stack <- ggplot(
        prop_data,
        aes(x = loc, y = proportion, fill = majorCluster)
      ) +
        geom_col() +
        scale_fill_viridis_d() +
        labs(
          title = "Cell Type Composition by Tissue Location",
          x = "Tissue Location",
          y = "Proportion",
          fill = "Cell Type"
        ) +
      ...
      ggsave(
        file.path(output_dir, "cell_composition_stacked.pdf"),
        p_stack,
        width = 10,
        height = 6
      )
    ...
    error = function(e) {
      message("Warning: Failed to create stacked plot - ", e$message)
    }
  )
    ...
    tryCatch(
      {
        ...
        if (nrow(valid_roe_data) >= 2) {
          ...
          pdf(
            file.path(output_dir, "celltype_correlation.pdf"),
            width = 10,
            height = 10
          )
          corrplot::corrplot(
            ...
            tl.col = "black",
            tl.srt = 45,
            title = "Cell Type Distribution Pattern Correlation",
            mar = c(0, 0, 2, 0)
          )
          dev.off()
        }
      ...
      error = function(e) {
        message("Warning: Failed to create correlation plot - ", e$message)
      }
    )
  }
}
...
generate_analysis_report <- function(
  ...
  cat(
    "- Interpretation: >1 enriched, <1 depleted, =1 expected distribution\n\n"
  )
  ...
  if (!is.null(significance_results)) {
    ...
    if (sig_count > 0) {
      ...
      top_sig <- head(
        significance_results[significance_results$Significant, ],
        5
      )
...
quick_startrac_analysis <- function(
  seurat_obj,
  celltype_col,
  tissue_col,
  patient_col,
...
interpret_results_summary <- function(results) {
  ...
  if (!is.null(results$significance_results)) {
    ...
    message(
      ...
      n_sig,
      "/",
      nrow(results$significance_results),
      " cell types show significant tissue distribution patterns"
    )
  }
}
...
#' @param output_prefix 输出文件前缀
#' @return ggplot对象列表
create_multiCondition_dimplot <- function(
  seurat_obj,
  ...
  for (cond in conditions) {
    # 使用逻辑索引提取该条件的细胞
    condition_cells <- rownames(seurat_obj@meta.data)[
      seurat_obj@meta.data[[condition_col]] == cond
    ]
  ...
  if (save_pdf) {
    ...
    ggsave(
      paste0(output_prefix, "_combined.pdf"),
      final_plot,
      width = pdf_width,
      height = pdf_height,
      dpi = 300
    ...
    for (plot_name in names(plot_list)) {
      ggsave(
        paste0(output_prefix, "_", plot_name, ".pdf"),
        plot_list[[plot_name]],
        width = 6,
        height = 5,
        dpi = 300
...
#' @param condition_col 条件列名称
#' @param cell_type_col 细胞类型列名称
check_seurat_structure <- function(
  seurat_obj,
  condition_col = "tissue",
  cell_type_col = "Annotation"
) {
  ...
  if (condition_col %in% meta_cols) {
    ...
    cat("     请检查列名或使用以下建议:\n")
    potential_cols <- meta_cols[grepl(
      "tissue|group|condition|sample",
      meta_cols,
      ignore.case = TRUE
    )]
  ...
  if (cell_type_col %in% meta_cols) {
    ...
    cat(paste(
      "     细胞类型列表:",
      paste(head(cell_types, 10), collapse = ", ")
    ))
    if (length(cell_types) > 10) {
      cat(" ...")
    }
    cat("\n")
  } else {
    ...
    cat("     请检查列名或使用以下建议:\n")
    potential_cols <- meta_cols[grepl(
      "annotation|cell_type|cluster|ident",
      meta_cols,
      ignore.case = TRUE
    )]
    if (length(potential_cols) > 0) {
...
message(
  "DimPlot-based multi-condition rasterized visualization functions loaded!"
)
message("现在使用Seurat原生的raster支持，无需额外包依赖")
message(
  "Now using Seurat's native raster support, no additional package dependencies required"
)
message("已修复WhichCells错误，现在可以正常运行")
message("Fixed WhichCells error, should work properly now")
message(
  "使用 check_seurat_structure(seurat_obj, 'tissue', 'Annotation') 检查数据"
)
message(
  "Use check_seurat_structure(seurat_obj, 'tissue', 'Annotation') to check your data"
)
...
# 或者使用table + unique组合
unique_combinations <- unique(seurat_obj_main@meta.data[, c(
  "tissue",
  "sample"
)])
...
create_comprehensive_expression_heatmap <- function(
  ...
  message(
    "开始创建大型表达热图 / Starting comprehensive expression heatmap creation..."
  )
    ...
    tryCatch(
      {
        ...
        top_markers <- markers %>%
          group_by(cluster) %>%
          top_n(
            n = min(15, max_genes %/% length(cell_types)),
            wt = avg_log2FC
          ) %>%
          pull(gene) %>%
          unique()
  ...
  for (ct in cell_types) {
    ct_cells <- colnames(seurat_obj)[
      seurat_obj@meta.data[[cell_type_col]] == ct
    ]
  ...
  if (is.null(cell_type_colors)) {
    if (length(cell_types) <= 12) {
      cell_type_colors <- brewer.pal(
        min(12, max(3, length(cell_types))),
        "Set3"
      )
    } else {
  ...
  png(
    png_file,
    width = figure_width * 300,
    height = figure_height * 300,
    res = 300,
    bg = "white"
  )
  draw(
...
quick_preview_heatmap <- function(
  seurat_obj,
  cell_type_col = "Annotation",
  ...
  result <- create_comprehensive_expression_heatmap(
    seurat_obj = seurat_obj,
    cell_type_col = cell_type_col,
    max_cells_per_type = max_cells_total %/%
      length(unique(seurat_obj@meta.data[[cell_type_col]])),
    max_genes = max_genes,
    output_prefix = "preview",
    figure_width = 12,
    figure_height = 8,
...
diagnose_seurat_gene_quality <- function(
  ...
  missing_cols <- required_cols[
    !required_cols %in% colnames(seurat_obj@meta.data)
  ]
  if (length(missing_cols) > 0) {
  ...
  message(sprintf(
    "  细胞类型数: %d",
    length(unique(seurat_obj@meta.data[[celltype_col]]))
  ))
  message(sprintf(
    "  组织类型数: %d",
    length(unique(seurat_obj@meta.data[[tissue_col]]))
  ))
  message(sprintf(
    "  样本数: %d\n",
  ...
  for (tissue in tissues) {
    tissue_cells <- colnames(seurat_obj)[
  ...
  prop_hist <- hist(
    gene_stats$nonzero_prop[gene_stats$nonzero_prop < 0.5],
    breaks = prop_breaks,
    plot = FALSE
  )
  ...
  tryCatch(
    {
      ...
      # 图2: 表达量分布（log scale）
      hist(
        log10(gene_stats$total_count + 1),
        breaks = 50,
        col = "coral",
        border = "white",
...
seurat_obj_main$T_high <- 0
seurat_obj_main$B_high <- 0
seurat_obj_main$Myeloid_high <- 0
seurat_obj_main$Stromal_high <- 0
# 定义三个判定条件 (Criteria)
# 条件1: 10个基因中至少2个高表达 (≥2 genes with expression > 1)
condition1 <- rowSums(expr[, 1:3] > 2) >= 1
...
seurat_obj_main$T_high <- as.numeric(rowSums(expr[, 4:5] > 2) >= 1)
seurat_obj_main$B_high <- as.numeric(rowSums(expr[, 6:7] > 2) >= 1)
seurat_obj_main$Myeloid_high <- as.numeric(rowSums(expr[, 8:9] > 2) >= 1)
seurat_obj_main$Stromal_high <- as.numeric(rowSums(expr[, 10:12] > 2) >= 1)
...
# 统计非上皮细胞中的阳性标记分布
# (Distribution of positive cells in non-epithelial populations)
seurat_obj_main$doublets_manual <- 0
seurat_obj_main$doublets_manual[
  (seurat_obj_main$secretory_epi_high +
    seurat_obj_main$T_high +
    seurat_obj_main$B_high +
    seurat_obj_main$Myeloid_high +
    seurat_obj_main$Stromal_high) >=
    2
] <- 1
table(seurat_obj_main$sample[seurat_obj_main$doublets_manual == 1])
# table(seurat_obj_main$sample[seurat_obj_main$doublets_manual == 0])
seurat_obj_main <- subset(
  seurat_obj_main,
  subset = doublets_manual == 1,
  invert = TRUE
)
...
seurat_obj_main <- subset(
  seurat_obj_main,
  subset = T_NK_high == 1 &
    !(seurat_obj_main$Annotation %in% c('T', 'B', 'Myeloid')),
  invert = TRUE
)
...
# 检查哪些基因在数据中存在
available_b_markers <- all_b_markers[
  all_b_markers %in% rownames(seurat_obj_main)
]
...
if (length(available_core) >= 2) {
  condition1 <- rowSums(
    expr[, available_core, drop = FALSE] > expression_threshold
  ) >=
    2
...
if (length(available_plasma) > 0) {
  ...
  if ("JCHAIN" %in% available_plasma) {
    ...
    condition2 <- rowSums(
      expr[, available_plasma, drop = FALSE] > expression_threshold
    ) >=
      1
  }
} else {
...
if (length(available_ig) >= 3) {
  condition3 <- rowSums(
    expr[, available_ig, drop = FALSE] > expression_threshold
  ) >=
    3
...
table(
  seurat_obj_main$Annotation[
    seurat_obj_main$b_marker_positive == 1 &
      !(seurat_obj_main$Annotation %in% c('B'))
  ],
  seurat_obj_main$sample[
    seurat_obj_main$b_marker_positive == 1 &
      !(seurat_obj_main$Annotation %in% c('B'))
  ]
)
...
backup <- readRDS('metadata_backup_20251019_183726.rds')
######################################################################################################
######################################################################################################
######################################################################################################
# ============================================================================
# 优化版互斥配对定义 - 排除EMT和生物学转化状态
# Optimized Mutually Exclusive Pairings (Excluding EMT & Biological Transitions)
# ============================================================================
...
# 创建输出目录
output_dir <- "Doublet_QC_Report"
plot_dir <- file.path(output_dir, "Marker_Correlation_Plots")
...
marker_groups <- list(
  ...
  NK_cell = c("NCAM1", "NKG7"),
  Endothelial = c("PECAM1", "VWF"),
  Myeloid = c("CD14", "CD68"),
  Fibroblast = c("COL1A2", "DCN"),
  Ciliated = c("FOXJ1", "RSPH1")
)

# 检查可用基因
all_markers <- unique(unlist(marker_groups))
available_markers <- intersect(all_markers, rownames(seurat_obj_main))
missing_markers <- setdiff(all_markers, available_markers)

if (length(missing_markers) > 0) {
  cat("注意: 以下标记基因不存在，已跳过:\n")
  cat(paste(missing_markers, collapse = ", "), "\n\n")
}

# 更新基因组
marker_groups <- lapply(marker_groups, function(genes) {
  intersect(genes, available_markers)
})
marker_groups <- marker_groups[sapply(marker_groups, length) > 0]

# 提取表达数据
expr_all <- FetchData(seurat_obj_main, vars = available_markers)

# 计算基因组平均表达
for (group_name in names(marker_groups)) {
  genes <- marker_groups[[group_name]]
  if (length(genes) > 1) {
    expr_all[[paste0(group_name, "_Mean")]] <- rowMeans(expr_all[,
      genes,
      drop = FALSE
    ])
  } else {
    expr_all[[paste0(group_name, "_Mean")]] <- expr_all[[genes]]
  }
}
...
cat("排除的配对类型（可能具有生物学意义）:\n\n")

cat("1. 上皮-间质配对 (EMT相关) ❌ 已排除\n")
cat("   - Glandular vs Fibroblast\n")
cat("   - Epithelial vs Fibroblast\n")
cat("   - Ciliated vs Fibroblast\n")
cat("   理由: EMT是真实的生物学过程，不应视为doublets\n\n")

cat("2. 同上皮谱系配对 ❌ 已排除\n")
cat("   - Glandular vs Ciliated\n")
cat("   - Glandular vs Epithelial\n")
cat("   理由: 同属上皮细胞，可能是分化状态或亚型差异\n\n")

cat("3. 上皮-髓系配对 ❌ 已排除 (可选)\n")
cat("   - Glandular vs Myeloid\n")
cat("   - Epithelial vs Myeloid\n")
cat("   理由: 某些组织中巨噬细胞可能吞噬上皮细胞碎片\n")
cat("        或存在上皮-免疫细胞相互作用\n\n")

cat("保留的配对类型（明确的谱系互斥）:\n\n")

cat("1. 上皮-淋巴细胞 ✓ 保留\n")
cat("   - Glandular/Epithelial vs T/B/NK cells\n")
cat("   理由: 上皮与淋巴细胞谱系绝对互斥\n\n")

cat("2. 上皮-内皮 ✓ 保留\n")
cat("   - Glandular/Epithelial vs Endothelial\n")
cat("   理由: 上皮与内皮谱系互斥（除极少数过渡状态）\n\n")

cat("3. 淋巴细胞间互斥 ✓ 保留\n")
cat("   - T vs B cells\n")
cat("   理由: 成熟淋巴细胞谱系互斥\n\n")

cat("4. 内皮-成纤维细胞 ✓ 保留 (可选)\n")
cat("   - Endothelial vs Fibroblast\n")
cat("   理由: 虽然EndMT存在，但较罕见\n")
cat("   建议: 如研究血管生成/纤维化，可考虑排除此配对\n\n")
...
mutually_exclusive_pairs <- list(
  ...
  list(
    ...
    group2 = "T_cell",
    risk = "High",
    rationale = "Epithelial-Lymphoid lineage exclusivity"
  ),
  ...
  list(
    ...
    group2 = "Endothelial",
    risk = "Medium",
    rationale = "Epithelial-Endothelial exclusivity"
  ),
  ...
  list(
    ...
    group2 = "B_cell",
    risk = "High",
    rationale = "Mature lymphocyte lineage exclusivity"
  ),
  ...
  list(
    ...
    group2 = "NK_cell",
    risk = "Medium",
    rationale = "Lymphoid lineage exclusivity (some overlap possible)"
  ),
  ...
  # 第四类: 内皮-间质（低置信度，可选）
  # ═══════════════════════════════════════════════════════════════
  # ⚠️  如果您的研究涉及血管生成、EndMT或纤维化，请注释掉此配对

  # list(group1 = "Endothelial", group2 = "Fibroblast", risk = "Low",
  #      rationale = "EndMT is rare but possible - use with caution")
)

cat("\n最终采用配对数量: ", length(mutually_exclusive_pairs), "\n")
cat("排除配对数量: 7 (EMT相关3个 + 同上皮2个 + 上皮-髓系2个)\n\n")

# ============================================================================
# 使用之前的绘图函数（保持不变）
# ============================================================================

# 函数1: 单基因 vs 单基因 - 多阈值 + 零表达过滤版本
plot_gene_vs_gene_multithreshold <- function(
  data,
  gene_x,
  gene_y,
  annotation,
  thresholds = c(0.2, 0.5, 1.0),
  title = NULL
) {
  # 准备完整数据（用于统计）
  full_df <- data.frame(
  ...
  # ★★★ 关键修改：仅保留两个基因都有表达的细胞用于绘图 ★★★
  plot_df <- full_df[full_df$x > 0 & full_df$y > 0, ]

  # 计算每个阈值下的双阳性细胞（基于全部细胞）
  threshold_stats <- data.frame(
    Threshold = thresholds,
    N_Double = sapply(thresholds, function(t) {
      sum(full_df$x > t & full_df$y > t)
    }),
    Pct_Double = sapply(thresholds, function(t) {
      100 * mean(full_df$x > t & full_df$y > t)
    })
  )

  # 使用最高阈值标记双阳性细胞（仅在绘图数据中）
  highest_thresh <- max(thresholds)
  plot_df$Doublet <- (plot_df$x > highest_thresh & plot_df$y > highest_thresh)

  # 统计信息：绘图细胞数 vs 总细胞数
  n_plotted <- nrow(plot_df)
  n_total <- nrow(full_df)
  pct_plotted <- 100 * n_plotted / n_total

  if (is.null(title)) {
    title <- sprintf("%s vs %s", gene_x, gene_y)
  }
  ...
  p_main <- ggplot(plot_df, aes(x = x, y = y)) +
    ...
    geom_hline(
      yintercept = thresholds[3],
      linetype = "solid",
      color = "#DC0000",
      linewidth = 0.6
    ) +
    geom_vline(
      xintercept = thresholds[3],
      linetype = "solid",
      color = "#DC0000",
      linewidth = 0.6
    ) +
    ...
    annotation_custom(
      grob = tableGrob(
        data.frame(
          Threshold = sprintf("%.1f", threshold_stats$Threshold),
          Cells = sprintf("%d", threshold_stats$N_Double),
          Percent = sprintf("%.2f%%", threshold_stats$Pct_Double)
        ),
        rows = NULL,
        theme = ttheme_minimal(
          core = list(
            ...
            bg_params = list(
              fill = c("white", "#FFF8DC", "#FFE4E1"),
              alpha = 0.8
            )
          ),
          colhead = list(
            fg_params = list(fontsize = 10, fontface = "bold"),
            bg_params = list(fill = "grey90", alpha = 0.9)
          )
        )
      ),
      xmin = -Inf,
    ...
    # 坐标轴和标题
    labs(
      x = paste0(gene_x, " Expression"),
      y = paste0(gene_y, " Expression"),
      title = title,
      subtitle = "Multi-threshold doublet detection (Non-zero expression only)"
    ) +
...
# 函数2: 基因组平均 vs 基因组平均 - 多阈值 + 零表达过滤版本
plot_group_vs_group_multithreshold <- function(
  data,
  group1_name,
  group2_name,
  annotation,
  ...
  mean_col1 <- paste0(group1_name, "_Mean")
  mean_col2 <- paste0(group2_name, "_Mean")
  ...
  title <- sprintf("%s vs %s (Group Average)", group1_name, group2_name)
  ...
  p_main <- ggplot(plot_df, aes(x = x, y = y)) +
    ...
    # 标题
    labs(
      x = paste0(group1_name, " Mean Expression"),
      y = paste0(group2_name, " Mean Expression"),
      title = title,
      subtitle = sprintf(
        "N genes: %s=%d, %s=%d | Non-zero expression only",
        group1_name,
        length(marker_groups[[group1_name]]),
        group2_name,
        length(marker_groups[[group2_name]])
      )
    ) +
...
group_plot_counter <- 1
for (pair in mutually_exclusive_pairs) {
  group1 <- pair$group1
  group2 <- pair$group2
  risk <- pair$risk

  if (!group1 %in% names(marker_groups) || !group2 %in% names(marker_groups)) {
    next
  }

  pdf_name <- sprintf(
    "GroupAvg_%02d_%s_vs_%s_[%s_Risk].pdf",
    group_plot_counter,
    group1,
    group2,
    risk
  ...
  p <- plot_group_vs_group_multithreshold(
    data = expr_all,
    group1_name = group1,
    group2_name = group2,
    annotation = seurat_obj_main$Annotation,
    thresholds = thresholds,
    risk_level = risk
  )

  pdf(pdf_path, width = 9, height = 7)
  print(p)
  dev.off()
  ...
  group_plot_counter <- group_plot_counter + 1
}
...
for (pair in mutually_exclusive_pairs) {
  ...
  genes1 <- marker_groups[[group1]]
  genes2 <- marker_groups[[group2]]
  ...
  for (gene1 in genes1) {
    for (gene2 in genes2) {
      pdf_name <- sprintf(
        "Single_%03d_%s_%s_vs_%s_%s.pdf",
        single_plot_counter,
        group1,
        gene1,
        group2,
      ...
      p <- plot_gene_vs_gene_multithreshold(
        data = expr_all,
        gene_x = gene1,
        gene_y = gene2,
        annotation = seurat_obj_main$Annotation,
      ...
      single_plot_counter <- single_plot_counter + 1
    }
  }

  cat(sprintf("  完成 %d 个单基因配对图\n", length(genes1) * length(genes2)))
}
...
pairing_rationale_file <- file.path(
  output_dir,
  "Pairing_Strategy_Rationale.txt"
)
sink(pairing_rationale_file)

cat("╔════════════════════════════════════════════════════════════════╗\n")
cat("║          配对策略优化说明文档                                  ║\n")
cat("║       Pairing Strategy Optimization Rationale                 ║\n")
cat("╚════════════════════════════════════════════════════════════════╝\n\n")

cat(sprintf("生成时间: %s\n", Sys.time()))
cat(sprintf("优化后配对数量: %d\n", length(mutually_exclusive_pairs)))
cat(sprintf("排除配对数量: 7\n\n"))

cat(rep("=", 70), "\n", sep = "")
cat("一、排除配对的生物学依据\n")
cat(rep("=", 70), "\n\n", sep = "")

cat("1. 上皮-间质转化 (Epithelial-Mesenchymal Transition, EMT)\n\n")
cat("排除的配对:\n")
cat("  - Glandular vs Fibroblast\n")
cat("  - Epithelial vs Fibroblast\n")
cat("  - Ciliated vs Fibroblast\n\n")
...
cat("生物学原理:\n")
cat("  EMT是细胞在特定生理或病理条件下发生的可逆性转化过程。\n")
cat("  在此过程中,细胞会同时表达上皮标记和间质标记,这是正常的\n")
cat("  生物学状态,而非技术假象。\n\n")

cat("关键证据:\n")
cat("  - 发育生物学: 胚胎发育中的原肠胚形成、神经嵴迁移\n")
cat("  - 组织修复: 伤口愈合过程中的上皮细胞迁移\n")
cat("  - 肿瘤转移: 癌细胞侵袭和远处转移的关键步骤\n")
cat("  - 器官纤维化: 慢性炎症导致的组织重塑\n\n")

cat("相关标记基因:\n")
cat("  上皮标记: EPCAM, KRT8, 腺体标记(SCGB, MUC5B等)\n")
cat("  间质标记: COL1A2, DCN, VIM, FN1\n")
cat("  EMT转录因子: SNAI1, SNAI2, TWIST1, ZEB1/2\n\n")

cat("文献支持:\n")
cat("  - Nieto MA, et al. Science 2016; EMT: 2016\n")
cat("  - Kalluri R, Weinberg RA. J Clin Invest 2009; 119(6):1420-8\n")
cat("  - Dongre A, Weinberg RA. Nat Rev Mol Cell Biol 2019; 20(2):69-84\n\n")
...
cat("排除的配对:\n")
cat("  - Glandular vs Ciliated\n")
cat("  - Glandular vs Epithelial (通用上皮标记)\n\n")

cat("生物学原理:\n")
cat("  气道上皮包含多种分化亚型,包括:\n")
cat("  - 基底细胞 (Basal cells) - 干/祖细胞\n")
cat("  - 纤毛细胞 (Ciliated cells) - 黏液清除\n")
cat("  - 杯状/腺细胞 (Goblet/Glandular cells) - 黏液分泌\n")
cat("  - 俱乐部细胞 (Club cells) - 分泌和修复\n\n")

cat("  这些细胞:\n")
cat("  1. 均表达通用上皮标记(如EPCAM)\n")
cat("  2. 存在分化过渡状态\n")
cat("  3. 可能在损伤修复时相互转化\n\n")

cat("  因此,观察到腺细胞标记与纤毛标记的共表达,可能代表:\n")
cat("  - 分化中间态细胞\n")
cat("  - 转分化(Transdifferentiation)过程\n")
cat("  - 细胞可塑性(Plasticity)\n\n")
...
cat("生物学原理:\n")
cat("  上皮细胞与巨噬细胞之间存在复杂的相互作用:\n\n")

cat("  机制1: 胞葬作用 (Efferocytosis)\n")
cat("    - 巨噬细胞吞噬凋亡上皮细胞\n")
cat("    - 可能导致上皮mRNA被检测到\n\n")

cat("  机制2: 胞外囊泡交换\n")
cat("    - 上皮细胞释放外泌体\n")
cat("    - 巨噬细胞摄取含上皮mRNA的囊泡\n\n")

cat("  机制3: 紧密接触\n")
cat("    - 组织驻留巨噬细胞与上皮细胞紧密相邻\n")
cat("    - 单细胞解离可能导致细胞团聚\n\n")

cat("  注意: 此配对是否排除取决于具体研究:\n")
cat("    - 如研究免疫-上皮互作 → 不应排除\n")
cat("    - 如关注纯粹上皮生物学 → 可以排除\n\n")
...
for (i in seq_along(mutually_exclusive_pairs)) {
  pair <- mutually_exclusive_pairs[[i]]
  cat(sprintf(
    "[%d] %s vs %s [%s Risk]\n",
    i,
    pair$group1,
...
cat("1. 根据研究目的定制配对:\n\n")
cat("   如果您的研究聚焦于:\n")
cat("   - EMT机制 → 不要过滤上皮-间质配对\n")
cat("   - 免疫-上皮互作 → 不要过滤上皮-髓系配对\n")
cat("   - 血管生成/EndMT → 不要过滤内皮-间质配对\n")
cat("   - 上皮分化 → 不要过滤腺-纤毛配对\n\n")

cat("2. 人工验证高检出率配对:\n\n")
cat("   对于任何配对,如果双阳性率>5%,建议:\n")
cat("   - 查看散点图中细胞的空间分布\n")
cat("   - 检查这些细胞的其他标记表达\n")
cat("   - 结合组织学背景判断生物学合理性\n\n")
...
cat("3. 结合降维可视化:\n\n")
cat("   建议步骤:\n")
cat("   a. 在UMAP/tSNE上标记双阳性细胞\n")
cat("   b. 观察它们是否形成独立cluster\n")
cat("   c. 如果分散在边界区域 → 可能是过渡状态\n")
cat("   d. 如果随机分布 → 更可能是doublets\n\n")

cat("4. 文献验证:\n\n")
cat("   搜索关键词:\n")
cat("   - 您的组织类型 + EMT\n")
cat("   - 您的疾病模型 + plasticity\n")
cat("   - 细胞类型 + transdifferentiation\n\n")

cat(rep("=", 70), "\n", sep = "")
cat("四、推荐的质控流程\n")
cat(rep("=", 70), "\n\n", sep = "")

cat("第一阶段: 保守过滤 (仅移除高置信度doublets)\n")
cat("  - 使用优化后的配对列表\n")
cat("  - 仅过滤High Risk配对中阈值≥1.0的细胞\n")
cat("  - 保留所有可能的生物学状态\n\n")
...
cat("第三阶段: 有针对性的二次过滤 (可选)\n")
cat("  - 如果某些cluster明确为doublets\n")
cat("  - 使用更严格的标准二次过滤\n")
cat("  - 记录所有过滤决策供审稿人参考\n\n")
...
cat("关键改进:\n")
cat("  ✓ 排除EMT相关配对,保留生物学转化状态\n")
cat("  ✓ 排除同上皮谱系配对,保留分化过渡细胞\n")
cat("  ✓ 仅保留明确谱系互斥的配对\n")
cat("  ✓ 提供详细的生物学依据文档\n\n")

cat("优化结果:\n")
cat(sprintf("  - 原配对数量: 17\n"))
cat(sprintf("  - 优化后配对数量: %d\n", length(mutually_exclusive_pairs)))
cat(sprintf("  - 排除配对数量: %d\n", 17 - length(mutually_exclusive_pairs)))
cat(sprintf("  - 基因组平均图: %d 个\n", group_plot_counter - 1))
cat(sprintf("  - 单基因配对图: %d 个\n\n", single_plot_counter - 1))

cat("重要文档:\n")
cat(sprintf("  1. 配对策略说明: %s\n", basename(pairing_rationale_file)))
cat("  2. 散点图文件夹: %s\n\n", basename(plot_dir))

cat("下一步建议:\n")
cat("  1. 阅读配对策略说明文档了解生物学依据\n")
cat("  2. 查看优化后的散点图\n")
cat("  3. 根据您的研究目的进一步定制配对\n")
cat("  4. 使用保守的阈值(1.0)进行初步过滤\n")
cat("  5. 结合UMAP可视化验证过滤结果\n\n")
######################################################################################################
######################################################################################################
######################################################################################################
...
#!/usr/bin/env Rscript
# ============================================================================
# 从RDS备份文件填充Seurat对象的NA值
# Fill NA Values from RDS Backup File
# ============================================================================

library(Seurat)

cat("\n开始从RDS文件填充NA值...\n")
cat("Starting NA filling from RDS file...\n\n")
...
# 修改为您的RDS文件路径
# backup <- readRDS("/path/to/your/backup_metadata.rds")

# 检查是否是dataframe
if (!is.data.frame(backup)) {
  stop("错误：RDS文件不是dataframe格式！")
}

cat(sprintf("   备份: %d 细胞, %d 列\n", nrow(backup), ncol(backup)))

# 检查行名是否已设置
if (all(rownames(backup) == as.character(1:nrow(backup)))) {
  cat("   ⚠ 警告：行名未设置，将使用第一列作为行名\n")
...
# 步骤2: 获取当前metadata
# ----------------------------------------------------------------------------
cat("\n2. 获取当前Seurat对象metadata...\n")
current_meta <- seurat_obj_main@meta.data
cat(sprintf(
  "   当前: %d 细胞, %d 列\n",
  nrow(current_meta),
  ncol(current_meta)
...
# ----------------------------------------------------------------------------
cat("\n3. 识别共同的列和细胞...\n")
common_cols <- intersect(colnames(current_meta), colnames(backup))
common_cells <- intersect(rownames(current_meta), rownames(backup))
cat(sprintf("   共同列: %d\n", length(common_cols)))
cat(sprintf(
  "   共同细胞: %d (%.1f%%)\n",
  length(common_cells),
  100 * length(common_cells) / nrow(current_meta)
))
...
for (col in common_cols) {
  # 找到当前列中的NA（仅在共同细胞中）
  na_cells <- common_cells[is.na(current_meta[common_cells, col])]

  if (length(na_cells) > 0) {
    # 从备份中获取值（排除备份中也是NA的情况）
    backup_values <- backup[na_cells, col]
    fillable_cells <- na_cells[!is.na(backup_values)]

    if (length(fillable_cells) > 0) {
      # 填充NA
      seurat_obj_main@meta.data[fillable_cells, col] <- backup[
        fillable_cells,
        col
      ]
      total_filled <- total_filled + length(fillable_cells)
      cat(sprintf("   ✓ %-30s: %d 个NA被填充\n", col, length(fillable_cells)))
    }
  }
}
...
cat("\n5. 完成!\n")
cat("   ========================================\n")
cat(sprintf("   ✓ 总共填充: %d 个NA值\n", total_filled))
cat("   ========================================\n\n")

# 验证
cat("剩余NA值统计:\n")
na_counts <- sapply(colnames(seurat_obj_main@meta.data), function(col) {
  sum(is.na(seurat_obj_main@meta.data[, col]))
})
print(head(na_counts[na_counts > 0], 10))

table(seurat_obj_main$study)
table(seurat_obj_main$assay)

# cat("saveRDS(seurat_obj_main, 'seurat_obj_main_filled.rds')\n\n")
