...
# -*- coding: utf-8 -*-
"""
Myeloid Cell: scVI → CellTypist → scANVI Pipeline v3.5.1
=========================================================
...
Myeloid-Specific Features:
1. Dedicated myeloid cell marker panel (monocytes, macrophages, DCs, mast cells)
2. M1/M2 polarization markers included
3. Antigen presentation markers (HLA-DR)
4. Optimized for Type 2 inflammation myeloid populations

Input: Raw myeloid data with counts layer
Output: adata_myeloid_FINAL.h5ad + count statistics

Author: r2end
Date: 2025-01-11
Version: 3.5.1 - Myeloid adaptation from T cell pipeline
"""
...
# Input/Output paths
INPUT_FILE = "/home/h2048/data/py/1217/cnmf_batch_production_v1_1_1/Myeloid/batch_aware/cnmf_analysis_k40_1/Myeloid_with_cnmf_k40.h5ad"  # ⭐ UPDATE THIS
OUTPUT_DIR = (f"/home/h2048/data/py/{datetime.now().strftime('%m%d')}/celltypist_myeloid")
...
# CellTypist
CELLTYPIST_MODEL = '/home/h2048/data/source/reference/celltypist_models/Immune_All_Low.pkl'  # Standard immune model covers myeloid
...
# ============================================================================
# MYELOID CELL CORE MARKERS
# ============================================================================

# ⭐ MYELOID CORE MARKERS - Force include in HVG
# These markers are essential for myeloid subtype identification
# Will be automatically added to HVG if not selected by variance
MYELOID_CORE_MARKERS = {
    # Pan-myeloid
    'Pan_Myeloid': ['PTPRC', 'ITGAM', 'LYZ', 'CSF1R'],
    
    # Monocytes
    'Classical_Mono': ['CD14', 'S100A8', 'S100A9', 'S100A12', 'FCN1', 'VCAN'],
    'Non_Classical_Mono': ['FCGR3A', 'MS4A7', 'CDKN1C', 'CX3CR1'],
    'Intermediate_Mono': ['CD14', 'FCGR3A', 'HLA-DRA'],
    
    # Macrophages
    'Macrophages': ['CD68', 'CD163', 'MSR1', 'MRC1', 'C1QA', 'C1QB', 'C1QC', 'MARCO'],
    'Alveolar_Mac': ['FABP4', 'MARCO', 'MCEMP1'],  # If lung/airway samples
    
    # M1/M2 Polarization
    'M1_Polarization': ['IL1B', 'TNF', 'CXCL9', 'CXCL10', 'CXCL11', 'NOS2'],
    'M2_Polarization': ['CD163', 'MRC1', 'CD209', 'ARG1', 'ALOX15', 'CCL18', 'CCL22'],
    
    # Dendritic Cells
    'cDC1': ['CLEC9A', 'XCR1', 'BATF3', 'IRF8'],
    'cDC2': ['CD1C', 'FCER1A', 'CLEC10A'],
    'pDC': ['CLEC4C', 'IL3RA', 'IRF7', 'TCF4'],
    'Mature_DC': ['CD83', 'CCR7', 'LAMP3', 'FSCN1'],
    
    # Mast Cells
    'Mast_Cells': ['TPSAB1', 'TPSB2', 'CPA3', 'KIT', 'HDC', 'MS4A2'],
    
    # Antigen Presentation
    'Antigen_Presentation': ['HLA-DRA', 'HLA-DRB1', 'HLA-DPA1', 'HLA-DPB1', 'CD74'],
    
    # Chemokine Receptors
    'Chemokine_Receptors': ['CCR2', 'CCR5', 'CCR7', 'CX3CR1', 'CXCR4'],
    
    # Type 2 Inflammation (relevant for CRSwNP)
    'Type2_Response': ['IL4R', 'IL13RA1', 'STAT6', 'CCL17', 'CCL22', 'CCL24', 'TGFB1'],
    
    # Activation/Migration
    'Activation': ['CD40', 'CD80', 'CD86', 'ICAM1'],
    'Proliferation': ['MKI67', 'TOP2A', 'STMN1'],
}

# Flatten to list
ALL_MYELOID_MARKERS = []
for category, genes in MYELOID_CORE_MARKERS.items():
    ALL_MYELOID_MARKERS.extend(genes)
ALL_MYELOID_MARKERS = list(set(ALL_MYELOID_MARKERS))  # Remove duplicates

print(f"  Myeloid markers: {len(ALL_MYELOID_MARKERS)} unique genes")
...
print("="*80)
print("MYELOID CELL PIPELINE v3.5.1")
print("="*80)
print(f"\nConfiguration:")
...
print(f"  Myeloid markers: {len(ALL_MYELOID_MARKERS)} genes")
...
# ============================================================================
# FORCE INCLUDE MYELOID MARKERS IN HVG
# ============================================================================

print(f"\n{'='*80}")
print("FORCE INCLUDE MYELOID MARKERS IN HVG")
print("="*80)
...
print(f"Total markers to check: {len(ALL_MYELOID_MARKERS)}")
...
# Check each marker
print(f"\n{'='*60}")
print("Checking myeloid markers:")
print("="*60)
...
print(f"\n{'='*60}")
print(f"SUMMARY:")
print(f"  Total markers: {len(ALL_MYELOID_MARKERS)}")
...
critical_markers = ['CD14', 'FCGR3A', 'CD68', 'CD163', 'HLA-DRA']
print(f"\n{'='*60}")
...
if not all_present:
    ...
    print(f"   Monocyte/Macrophage classification may fail in downstream analysis")

print("="*60)
print(f"\n✅ Myeloid marker check complete")
...
output_file = OUTPUT_DIR / "adata_myeloid_FINAL.h5ad"

# Add metadata
adata.uns['pipeline_info'] = {
    'version': '3.5.1-MYELOID',
    'date': '2025-01-11',
    'cell_type': 'myeloid',
    ...
    'myeloid_markers_count': len(ALL_MYELOID_MARKERS),
    ...
    'myeloid_marker_categories': list(MYELOID_CORE_MARKERS.keys())
}
...
print(f"\n{'='*80}")
print("🎉 MYELOID PIPELINE COMPLETE - v3.5.1")
print("="*80)

print(f"\n📊 Analysis Summary:")
print(f"   Cell type: MYELOID")
print(f"   Cells: {adata.n_obs:,}")
...
print(f"   Myeloid markers forced: {len(ALL_MYELOID_MARKERS)}")
...
print(f"\n🔬 Myeloid-Specific Features:")
print(f"   ✅ {len(MYELOID_CORE_MARKERS)} marker categories")
print(f"   ✅ Pan-myeloid, Monocytes, Macrophages, DCs, Mast cells")
print(f"   ✅ M1/M2 polarization markers")
print(f"   ✅ Antigen presentation markers (HLA-DR)")
...
print("2. Validate myeloid markers (CD14, FCGR3A, CD68, HLA-DRA)")
print("3. Check M1/M2 polarization markers if relevant")
print("4. Run downstream DE analysis by cell type")
print("5. Investigate monocyte-to-macrophage trajectories")
print("="*80)
