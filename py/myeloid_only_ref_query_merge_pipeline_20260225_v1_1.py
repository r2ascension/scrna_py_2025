...
"""
==============================================================================
Myeloid ONLY Reference-Query Merge Pipeline v1.1
==============================================================================

Changelog v1.1 (Bug Fixes over v1.0):
---------------------------------------
NEW-BUG-A [P0] FIXED: REFERENCE_H5AD and QUERY_H5AD were copy-pasted from
      the T cell pipeline and still pointed to T cell data paths
      ("tnk_subclustered_FINAL" / "t_cells.h5ad"). Running v1.0 would silently
      train a myeloid model on T cell data. Paths are now replaced with
      myeloid-specific placeholders that must be set before running.

NEW-BUG-B [P1] FIXED: "CD16" in NONCLASSICAL_MONO_MARKERS is the protein
      surface marker name, not the HGNC gene symbol. The actual gene is
      FCGR3A (already present in the list). "CD16" will never appear in
      var_names and is silently dropped, reducing the NonClassical Mono score
      to 4 genes instead of 5. Removed the duplicate/invalid entry.

NEW-BUG-C [P2] FIXED: compute_monocyte_subtype_scores used a Python for-loop
      over all cells (for i in range(len(adata))). For merged datasets of
      100k+ cells this takes several minutes. Replaced with vectorized
      np.select() which is O(n) in numpy and ~100x faster.

All fixes from v1.2 T cell pipeline are inherited:
  - BUG-6: sparse .X sampling uses .toarray().flatten()
  - BUG-1: module score uses direct mean-expression (no score_genes)
  - BUG-2: CellTypist Model.load uses local-path check before download_models
  - BUG-5: predict(soft=True) wrapped in np.asarray()
...
print("=" * 80)
print("Myeloid ONLY Ref-Query Merge Pipeline v1.1")
print("=" * 80)
...
# NEW-BUG-A FIX: v1.0 had T cell paths copy-pasted here.
# Replace with actual myeloid reference and query h5ad paths.
REFERENCE_H5AD = "/home/h2048/data/py/0128/myeloid_analysis_unified/results/subcluster_unified_v2_20260128/adata_myeloid_subclustered_FINAL_v2_20260128.h5ad"   # UPDATE THIS
QUERY_H5AD     = "/home/h2048/data/py/0127/scarches_mapping_FIXED_v1_2/subsets/myeloid_cells.h5ad"       # UPDATE THIS
...
# NEW-BUG-B FIX: "CD16" is the protein surface name, not an HGNC gene symbol.
# The gene is FCGR3A, which is already listed. Removed "CD16" to avoid silent
# gene-not-found failure that reduces the score gene count from 5 to 4.
NONCLASSICAL_MONO_MARKERS = ["FCGR3A", "FCER1G", "PICALM", "RHOC"]         # CD14+ CD16++
...
def ensure_counts_layer(adata, counts_layer="counts"):
    ...
    if counts_layer not in (adata.layers or {}):
        ...
        if hasattr(adata, 'X') and adata.X is not None:
            # Sparse matrix slice returns a sub-sparse-matrix; must call
            # .toarray().flatten() before passing to np.asarray().
            if issparse(adata.X):
...
def compute_module_score_efficient(adata, gene_list, score_name, counts_layer="counts"):
    ...
    Does NOT use sc.tl.score_genes (requires background genes not present
    when adata_tmp contains only the marker subset).
    """
...
def compute_monocyte_subtype_scores(adata):
    ...
    NEW-BUG-C FIX: v1.0 used a Python for-loop (for i in range(len(adata)))
    to classify each cell. For merged datasets of 100k+ cells this takes
    several minutes. Replaced with np.select() vectorized operations (~100x
    faster for typical dataset sizes).
    """
    ...
    # NEW-BUG-C FIX: vectorized classification with np.select
    # Old (slow): for i in range(len(adata)): ... .iloc[i] ...
    conditions = [
...
def prepare_covariates(adata):
    ...
    if not all(k in adata.obs.columns for k in ["S_score", "G2M_score"]):
        ...
        if len(s_in) >= 5 and len(g_in) >= 5:
            ...
            sc.pp.log1p(ad_tmp)
            # score_genes_cell_cycle is safe here: ad_tmp contains BOTH s_in and
            # g2m_in genes, so each set has background genes from the other set.
...
def run_query_only_leiden(adata_merged, resolution=QUERY_LEIDEN_RESOLUTION):
    ...
    Uses query_mask.values to safely index numpy array (BUG-4 fix).
    """
    ...
    # query_mask.values: convert pandas bool Series to numpy array for safe
    # boolean indexing of numpy obsm array (avoids pandas-index ambiguity).
...
def main():
    ...
    # Fail early if paths are still placeholders
    if "EDIT_THIS" in REFERENCE_H5AD or "EDIT_THIS" in QUERY_H5AD:
        raise ValueError(
            "REFERENCE_H5AD and/or QUERY_H5AD still contain placeholder paths.\n"
            "Please update them at the top of the script before running."
        )
    ...
    print("  -> Getting scVI latent representation (index-aligned)...")
    # pandas reindex pattern for safe obsm writeback (BUG-3 fix)
    ...
    config = {
        ...
        "bug_fixes_v1_1": {
            ...
            "NEW-BUG-B_P1": "CD16 removed from NONCLASSICAL_MONO_MARKERS (not an HGNC symbol)",
    ...
    sc.pl.umap(adata_merged, basis="X_umap_scVI",   color="data_source", ax=axes2[0, 0], show=False, title="Data Source (scVI UMAP)", s=10)
    sc.pl.umap(adata_merged, basis="X_umap_scVI",   color="_ref_coarse", ax=axes2[0, 1], show=False, title="Reference Labels (scVI)", legend_loc="on data", s=10)
    sc.pl.umap(adata_merged, basis="X_umap_scVI",   color="scanvi_pred", ax=axes2[0, 2], show=False, title="scANVI Predictions (scVI)", legend_loc="on data", s=10)
    sc.pl.umap(adata_merged, basis="X_umap_scANVI", color="data_source", ax=axes2[1, 0], show=False, title="Data Source (scANVI UMAP)", s=10)
    sc.pl.umap(adata_merged, basis="X_umap_scANVI", color="_ref_coarse", ax=axes2[1, 1], show=False, title="Reference Labels (scANVI)", legend_loc="on data", s=10)
    sc.pl.umap(adata_merged, basis="X_umap_scANVI", color="scanvi_pred", ax=axes2[1, 2], show=False, title="scANVI Predictions (scANVI)", legend_loc="on data", s=10)
    ...
    # --- Summary ---
    print("\n" + "=" * 80)
    print("MYELOID PIPELINE COMPLETE (v1.1)")
    print("=" * 80)
    print(f"\nOutput: {output_dir}")
    ...
    print("\nBug Fixes Applied in v1.1:")
    print("  NEW-BUG-A [P0] Input paths corrected (T cell paths removed)")
    print("  NEW-BUG-B [P1] CD16 removed from NONCLASSICAL_MONO_MARKERS")
    print("  NEW-BUG-C [P2] compute_monocyte_subtype_scores vectorized with np.select")
    print("=" * 80)
