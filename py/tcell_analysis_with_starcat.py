...
sc.pl.pca_variance_ratio(adata_hvg, log=True, n_pcs=50, ax=ax, show=False)
plt.tight_layout()
...
if gep_of_interest in usage.columns:
    ...
    if 'leiden_bbknn' in adata_hvg.obs.columns:
        ...
        sc.pl.violin(adata_hvg, keys=f"GEP_{gep_of_interest}", 
                     groupby='leiden_bbknn', ax=axes[0], show=False)
        axes[0].set_title(f'GEP Usage: {gep_of_interest}')
        
        # UMAP
        sc.pl.umap(adata_hvg, color=f"GEP_{gep_of_interest}", 
                   vmax='p99', ax=axes[1], show=False)
...
if 'ASA' in adata_hvg.obs.columns and 'leiden_bbknn' in adata_hvg.obs.columns:
    ...
    sc.pl.violin(adata_hvg, keys='ASA', groupby='leiden_bbknn', 
                 ax=axes[0], show=False)
    axes[0].set_title('ASA Score Distribution by Cluster')
    
    sc.pl.umap(adata_hvg, color='ASA', vmax='p99', ax=axes[1], show=False)
    axes[1].set_title('ASA Score on UMAP')
