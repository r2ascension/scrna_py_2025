...
"""
==============================================================================
All-Cells scVI/scANVI Dual Pipeline v2.5 PRODUCTION
==============================================================================
...
CRITICAL IMPROVEMENTS in v2.5:
================================
1. Fixed HVG workflow (correct execution order)
2. Simplified model control (clear retrain flags)
3. Integrated best practices from QUICK_REFERENCE_MEMORY v2.11-12
4. Complete English documentation (UTF-8 compatible)
5. Safety mechanisms (HVG check, error handling, status reporting)
...
SCVI_DROPOUT_RATE = 0.2      # Dropout rate
SCVI_MAX_EPOCHS = 400        # Maximum training epochs
SCVI_BATCH_SIZE = 256        # Batch size (reduce if GPU OOM)
SCVI_LEARNING_RATE = 1e-3    # Learning rate
SCVI_EARLY_STOPPING = True   # Enable early stopping
SCVI_EARLY_STOPPING_PATIENCE = 45  # Early stopping patience
SCVI_ENCODE_COVARIATES = True      # Encode covariates in latent space
SCVI_USE_LAYER_NORM = "both"       # Layer normalization
SCVI_USE_BATCH_NORM = "none"       # Batch normalization

# ===== scANVI Hyperparameters =====
SCANVI_MAX_EPOCHS = 200        # Maximum training epochs
SCANVI_BATCH_SIZE = 256        # Batch size (smaller than scVI)
SCANVI_LEARNING_RATE = 5e-4    # Learning rate
SCANVI_EARLY_STOPPING_PATIENCE = 30  # Early stopping patience
...
# ===== UMAP Parameters =====
UMAP_N_NEIGHBORS = 30     # Number of neighbors
UMAP_MIN_DIST = 0.5       # Minimum distance
...
def force_include_markers_in_hvg(adata, n_top_genes, forced_markers, 
    ...
    Strategy:
    1. Identify forced markers present in data
    2. Check if already in HVG
    3. For missing ones: swap out lowest-variance HVG genes
    4. Maintain target HVG count (n_top_genes)
    ...
    if case_insensitive:
        ...
        var_symbols = adata.var[symbol_col].str.upper().values
    else:
