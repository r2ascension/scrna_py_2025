...
scVI/scANVI Complete Pipeline - VERSION 1.3
...
Date: 2024-12-10
Version: v1.3 - COMPLETE with scANVI + Code Review Fixes
...
SCVI_N_LATENT = 50  # Latent dimensions (30-50 for stromal cells)
SCVI_N_LAYERS = 2
SCVI_N_HIDDEN = 128
...
SCANVI_N_LAYERS = 2  # Use same as scVI
SCANVI_N_LATENT = 50  # Inherits from scVI
...
print("\n" + "="*80)
print("STROMAL/VASCULAR CELLS scVI/scANVI PIPELINE - v1.3 COMPLETE")
print("="*80)
...
# ⭐ CRITICAL FIX: Compute UMAP and save with specific name
...
# ⭐ CRITICAL FIX: Use basis='umap_scvi' instead of obsm parameter
...
sc.pl.umap(adata, color=BATCH_KEY, ax=axes[0, 0], show=False, 
           title='Batch Distribution', size=UMAP_SIZE, frameon=False,
           basis='umap_scvi')  # ⭐ FIXED: use basis instead of obsm

# Plot 2: UMAP by cell type
print(f"   Plotting UMAP colored by cell type...")
sc.pl.umap(adata, color=MAJOR_CELLTYPE_KEY, ax=axes[0, 1], show=False,
           title='Cell Type', size=UMAP_SIZE, frameon=False, legend_loc='right margin',
           basis='umap_scvi')
...
sc.pl.umap(adata, color='leiden_scvi', ax=axes[0, 2], show=False,
           title='Leiden Clustering', size=UMAP_SIZE, frameon=False, legend_loc='right margin',
           basis='umap_scvi')
...
for idx, marker in enumerate(pan_markers):
    if marker in adata.var_names:
        sc.pl.umap(adata, color=marker, ax=axes[1, idx], show=False,
                   title=f'{marker} Expression', size=UMAP_SIZE, frameon=False, 
...
sc.pl.umap(adata, color=BATCH_KEY, ax=axes[0, 0], show=False,
           title='scVI: Batch', size=UMAP_SIZE, frameon=False,
           basis='umap_scvi')

sc.pl.umap(adata, color=MAJOR_CELLTYPE_KEY, ax=axes[0, 1], show=False,
           title='scVI: Original Labels', size=UMAP_SIZE, frameon=False,
           legend_loc='right margin', basis='umap_scvi')

sc.pl.umap(adata, color='leiden_scvi', ax=axes[0, 2], show=False,
           title='scVI: Leiden Clustering', size=UMAP_SIZE, frameon=False,
           legend_loc='right margin', basis='umap_scvi')

# Row 2: scANVI results
print(f"   Plotting scANVI results...")
sc.pl.umap(adata, color=BATCH_KEY, ax=axes[1, 0], show=False,
           title='scANVI: Batch', size=UMAP_SIZE, frameon=False,
           basis='umap_scanvi')

sc.pl.umap(adata, color='cell_type_scanvi', ax=axes[1, 1], show=False,
           title='scANVI: Predicted Labels', size=UMAP_SIZE, frameon=False,
           legend_loc='right margin', basis='umap_scanvi')

sc.pl.umap(adata, color='scanvi_confidence', ax=axes[1, 2], show=False,
           title='scANVI: Confidence', size=UMAP_SIZE, frameon=False,
           cmap='viridis', basis='umap_scanvi')
...
if len(available_markers) > 0:
    ...
    for idx, marker in enumerate(available_markers):
        sc.pl.umap(
            ...
            frameon=False,
            size=UMAP_SIZE,
            basis='umap_scanvi'  # ⭐ Use scANVI UMAP
        )
...
print(f"\n{'='*80}")
print("🎉 COMPLETE PIPELINE FINISHED - v1.3")
print("="*80)
...
print(f"   ✅ Fixed obsm parameter error (use basis='umap_scvi')")
print(f"   ✅ Unified variable naming (SCVI_/SCANVI_ prefixes)")
print(f"   ✅ Use cell_type as scANVI labels")
print(f"   ✅ All code review suggestions implemented")
