...
- âœ… P0 Fixes (stats alignment, n_latent, Unknown, HVG save, expanded markers)
- âœ… Covariates System (mt%, stress, cell cycle as continuous covariates)
- âœ… Unknown Cleaning (neighborhood purity + quality gating)
- âœ… Whitelist Filtering (from downstream h5ad)
- âœ… scArches-ready parameters
- âœ… Class imbalance handling (n_samples_per_label)
...
print("âœ… Complete Allcells v2.3.1 Architecture")
print("="*80)
...
FORCE_RETRAIN_SCVI = False
FORCE_RETRAIN_SCANVI_MAJOR = False
FORCE_RETRAIN_SCANVI_FINE = False
...
print(f"âœ“ HVG method: {hvg_method}")
...
# â­ Setup with covariates (from allcells line 902-908)
...
print(f"âœ“ Model saved: {scvi_model_path}")
...
print(f"âœ“ HVG list saved: {hvg_file_path}")

adata.obsm['X_scvi'] = scvi_model.get_latent_representation()
print(f"âœ“ Latent: {adata.obsm['X_scvi'].shape}")
...
try:
    ...
    print("âœ“ Initialized from scVI")
    
except RuntimeError as e:
    if "size mismatch" in str(e):
        print("âš ï¸ Using fallback initialization")
        ...
        print("âœ“ Fallback initialization")
    else:
        raise

# â­ Train with class balancing (from allcells line 228)
...
scanvi_major_model.train(
    ...
    early_stopping=EARLY_STOPPING,
    train_size=0.9,
    n_samples_per_label=SCANVI_N_SAMPLES_PER_LABEL,  # â­ Class balancing
    plan_kwargs={'lr': 1e-3}
)
...
print(f"âœ“ Model saved: {scanvi_major_model_path}")
...
print("âœ“ Predictions extracted")
...
print(f"âœ“ Model saved: {scanvi_fine_model_path}")
