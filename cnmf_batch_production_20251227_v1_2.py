#!/usr/bin/env python3
"""
cNMF Legacy Batch Orchestrator v1.2.3
=====================================

This script originally contained a full in-file cNMF implementation. It now
acts as a helper-backed legacy entrypoint for lineage-specific runs, while
retaining the historical discovery pipeline behind an explicit opt-in switch.

Default behavior:
- route through lineage registry (currently `bcell`)
- support `smoke` / `full` run types
- default to single-track `batch_aware`
- delegate actual execution to `cnmf_helper_20260419_v1_1.run_cnmf_full()`

Compatibility behavior:
- the old discovery-based batch pipeline is still available via
    `--legacy-discovery`

Author: r2end
Date: 2026-05-20
Version: v1.2.3 (helper-backed orchestrator default)
"""
...
import cnmf_helper_20260419_v1_1 as CNMF_HELPER
...
try:
    ...
    print("="*70)
    print("WARN: cNMF not installed for legacy in-file execution")
    print("Helper-backed orchestrator can still be imported; install with: pip install cnmf")
    print("="*70)
...
INPUT_DIR = Path("/home/h2048/data/R/1223/per_celltype_harmony_rogue/h5ad_objects/1228")
...
LEGACY_HELPER_OUTPUT_ROOT = Path("/home/h2048/data/py/0423/cnmf_batch_production_legacy_helper")
PIPELINE_VERSION = '1.2.3'
DEFAULT_LINEAGE_MODE = 'bcell'
DEFAULT_RUN_TYPE = 'full'
TRACK_CHOICES = ('batch_aware', 'uncorrected')
DEFAULT_TRACKS = ['batch_aware']
SMOKE_SUBSET_N_CELLS = 1500
...
LINEAGE_REGISTRY = {
    'bcell': {
        ...
        'output_root': LEGACY_HELPER_OUTPUT_ROOT / 'bcell',
        'description': 'B-cell latest 0415 tissue comparison final h5ad',
        'celltype_col_candidates': ['cell_type_L3', 'cell_type_L2', *COLUMN_CANDIDATES['celltype']],
        'batch_col_candidates': ['sample', *COLUMN_CANDIDATES['batch']],
    },
}
...
def get_lineage_config(mode: str) -> Dict[str, Any]:
    """Return registry configuration for a supported lineage mode."""
    if mode not in LINEAGE_REGISTRY:
        raise KeyError(f"Unsupported lineage mode: {mode}")
    cfg = dict(LINEAGE_REGISTRY[mode])
    cfg['mode'] = mode
    return cfg


def normalize_tracks(tracks: Optional[Union[str, List[str], Tuple[str, ...]]]) -> List[str]:
    """Normalize track selection while preserving explicit order."""
    if tracks is None:
        raw = list(DEFAULT_TRACKS)
    elif isinstance(tracks, str):
        raw = [part.strip() for part in tracks.split(',')]
    else:
        raw = [str(part).strip() for part in tracks]
    ...
    if not out:
        raise ValueError("No valid cNMF tracks selected")
...
def build_run_settings(
    ...
    """Build helper-facing settings for smoke/full lineage runs."""
    lineage_cfg = get_lineage_config(mode)

    if run_type not in {'smoke', 'full'}:
        raise ValueError(f"Unsupported run_type: {run_type}")

    track_list = normalize_tracks(tracks)
    cnmf_config = dict(CNMF_HELPER.DEFAULT_CNMF_CONFIG)
    viz_config = dict(CNMF_HELPER.DEFAULT_VIZ_CONFIG)
    ...
    if run_type == 'smoke':
        ...
        subset_n_cells = SMOKE_SUBSET_N_CELLS
        k_range = [3, 4]
    else:
        subset_n_cells = None
        k_range = list(CNMF_HELPER.K_RANGE_MEDIUM)

    resolved_output_root = Path(output_root) if output_root is not None else lineage_cfg['output_root']

    return {
        'mode': mode,
        'run_type': run_type,
        'input_h5ad': lineage_cfg['input_h5ad'],
        'output_dir': resolved_output_root / run_type,
        'tracks': track_list,
        'subset_n_cells': subset_n_cells,
        'random_seed': CNMF_HELPER.DEFAULT_CNMF_CONFIG['seed'],
        'k_range': k_range,
        'cnmf_config': cnmf_config,
        'viz_config': viz_config,
        'celltype_col_candidates': list(lineage_cfg['celltype_col_candidates']),
        'batch_col_candidates': list(lineage_cfg['batch_col_candidates']),
    }


def execute_helper_track(
    adata: sc.AnnData,
    dataset_id: str,
    track_name: str,
    ...
    """Delegate a single lineage/track run to the helper module."""
    run_name = safe_name(f"{dataset_id}_{track_name}_{run_settings['run_type']}")
    use_batch_hvg = (track_name == 'batch_aware')

    return CNMF_HELPER.run_cnmf_full(
        adata=adata,
        output_dir=Path(output_dir),
        run_name=run_name,
        k_range=list(run_settings['k_range']),
        celltype_col=celltype_col,
        batch_col=batch_col,
        use_batch_hvg=use_batch_hvg,
        cnmf_config=dict(run_settings['cnmf_config']),
        viz_config=dict(run_settings['viz_config']),
    )


def subset_adata_for_smoke(
    adata: sc.AnnData,
    subset_n_cells: Optional[int],
    random_seed: int,
) -> sc.AnnData:
    """Return a deterministic smoke-test subset of AnnData."""
    if subset_n_cells is None or subset_n_cells >= adata.n_obs:
        return adata.copy()

    rng = np.random.default_rng(random_seed)
    selected = np.sort(rng.choice(adata.n_obs, size=subset_n_cells, replace=False))
    return adata[selected].copy()


def resolve_preferred_obs_column(
    adata: sc.AnnData,
    metadata_column: Optional[str],
    candidates: List[str],
) -> Optional[str]:
    """Prefer lineage-specific candidate columns over generic metadata hints."""
    preferred = infer_column_name(adata, candidates)
    if preferred:
        return preferred
...
def run_lineage_pipeline(
    ...
    """Run a helper-backed lineage cNMF job for smoke/full execution."""
    run_settings = build_run_settings(mode=mode, run_type=run_type, tracks=tracks, output_root=output_root)
    output_dir = Path(run_settings['output_dir'])
    if not safe_mkdir(output_dir, f"{mode} {run_type} output directory"):
        return {
            'mode': mode,
            'run_type': run_type,
            'input_h5ad': str(run_settings['input_h5ad']),
    ...
    adata, metadata = load_and_validate_dataset(Path(run_settings['input_h5ad']))
    if adata is None:
        result['error'] = 'Failed to load input h5ad'
        with open(output_dir / 'processing_result.json', 'w') as f:
    ...
    adata_run = subset_adata_for_smoke(
        adata,
        subset_n_cells=run_settings['subset_n_cells'],
        random_seed=run_settings['random_seed'],
    )

    celltype_col = resolve_preferred_obs_column(
        adata_run,
        metadata.get('celltype_col'),
        run_settings['celltype_col_candidates'],
    )
    batch_col = resolve_preferred_obs_column(
        adata_run,
        metadata.get('batch_col'),
        run_settings['batch_col_candidates'],
    )

    dataset_id = safe_name(f"{mode}_latest")
    result['n_cells_original'] = int(adata.n_obs)
    result['n_cells_analyzed'] = int(adata_run.n_obs)
    result['celltype_col'] = celltype_col
    result['batch_col'] = batch_col

    for track_name in run_settings['tracks']:
        track_output_dir = output_dir / track_name
        safe_mkdir(track_output_dir, f"{track_name} output directory")
        track_result = execute_helper_track(
            adata=adata_run,
        ...
        result['tracks'][track_name] = track_result

    result['success'] = any(track.get('success', False) for track in result['tracks'].values())
    if not result['success']:
        result['error'] = 'All requested tracks failed'
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    """Parse CLI args for helper-backed lineage runs."""
    parser = argparse.ArgumentParser(
        description=(
            'Legacy cNMF batch entrypoint. Defaults to helper-backed lineage orchestration '
            '(currently bcell/full/batch_aware). Use --legacy-discovery to run the historical '
            'input-directory batch pipeline.'
        )
    )
    parser.add_argument(
        '--mode',
        default=DEFAULT_LINEAGE_MODE,
        choices=sorted(LINEAGE_REGISTRY.keys()),
        help='Lineage registry entry to run (default: bcell).',
    )
    parser.add_argument(
        '--run-type',
        default=DEFAULT_RUN_TYPE,
        choices=['smoke', 'full'],
        help='Run profile to use (default: full).',
    )
    parser.add_argument(
        '--tracks',
        default=','.join(DEFAULT_TRACKS),
        help='Comma-separated cNMF tracks, e.g. batch_aware or uncorrected,batch_aware.',
    )
    parser.add_argument(
        '--output-root',
        default=None,
        type=Path,
        help='Optional lineage-specific output root. Results are written under <output-root>/<run-type>.',
    )
    parser.add_argument(
        '--legacy-discovery',
        action='store_true',
        help='Opt into the historical discovery-based INPUT_DIR batch pipeline instead of helper-backed lineage routing.',
    )
    return parser.parse_args(argv)


def run_helper_orchestrator(
    mode: str,
    run_type: str,
    output_root: Optional[Path] = None,
    tracks: Optional[Union[str, List[str], Tuple[str, ...]]] = None,
) -> Dict[str, Any]:
    """Run the helper-backed orchestrator and emit a pipeline summary."""
    run_settings = build_run_settings(mode=mode, run_type=run_type, tracks=tracks, output_root=output_root)
    output_dir = Path(run_settings['output_dir'])
    if not safe_mkdir(output_dir, f"{mode} {run_type} output directory"):
        return {
            'version': PIPELINE_VERSION,
            'execution_mode': 'helper_orchestrator',
            'mode': mode,
            'run_type': run_type,
            'tracks': list(run_settings['tracks']),
            'results': [],
            'success': False,
            'error': 'Failed to create output directory',
        }
    ...
    logger.info("=" * 70)
    logger.info("cNMF LEGACY HELPER-BACKED ORCHESTRATOR")
    logger.info("=" * 70)
    logger.info(f"Mode      : {mode}")
    logger.info(f"Run type  : {run_type}")
    logger.info(f"Tracks    : {run_settings['tracks']}")
    logger.info(f"Input h5ad: {run_settings['input_h5ad']}")
    logger.info(f"Output dir: {output_dir}")

    started_at = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    pipeline_start = time.time()
    result = run_lineage_pipeline(mode=mode, run_type=run_type, output_root=output_root, tracks=run_settings['tracks'])
    ...
    logger.info("=" * 70)
    logger.info("HELPER ORCHESTRATOR COMPLETE")
    logger.info("=" * 70)
    logger.info(f"Success   : {summary['success']}")
    logger.info(f"Time      : {pipeline_time / 60:.1f} min")
    logger.info(f"Summary   : {summary_file}")

    return summary


def run_legacy_discovery_pipeline(output_dir: Optional[Path] = None) -> Dict[str, Any]:
    """Run the historical discovery-based batch pipeline (explicit fallback only)."""
    legacy_output_dir = Path(output_dir) if output_dir is not None else OUTPUT_DIR

    if not safe_mkdir(legacy_output_dir, "base output directory"):
        return {
            'version': PIPELINE_VERSION,
            'execution_mode': 'legacy_discovery',
            'results': [],
            'success': False,
            'error': 'Cannot create base output directory',
        }

    log_file = legacy_output_dir / f"pipeline_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"

    global logger
    logger = setup_logging(log_file)

    logger.info("=" * 70)
    logger.info("cNMF BATCH PRODUCTION PIPELINE v1.2.3 (LEGACY DISCOVERY FALLBACK)")
    logger.info("=" * 70)
    ...
    for h5ad_path in h5ad_files:
        result = process_single_dataset(h5ad_path, legacy_output_dir)
    ...
    summary = {
        ...
        'total_time_seconds': pipeline_time,
        'results': all_results,
        'success': bool(n_total > 0 and n_success == n_total),
    }

    summary_file = legacy_output_dir / 'pipeline_summary.json'
    with open(summary_file, 'w') as f:
        json.dump(summary, f, indent=2)
...
def main(argv: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    Default path is helper-backed lineage orchestration; the historical discovery
    pipeline is available via `--legacy-discovery`.
    """
    args = parse_args(argv)
    tracks = normalize_tracks(args.tracks)

    if args.legacy_discovery:
        return run_legacy_discovery_pipeline(output_dir=args.output_root)

    return run_helper_orchestrator(
        mode=args.mode,
...
if __name__ == "__main__":
    summary = main()
    raise SystemExit(0 if summary.get('success', False) else 1)
