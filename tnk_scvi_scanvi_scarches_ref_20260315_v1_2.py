...
# Sanitize categorical covariate dtypes -- use object-backed Categorical
# NOT astype('string') which creates nullable-string backend in categories
# and later crashes write_h5ad at /obs/<col>/categories
for col in cat_covariates + [BATCH_KEY]:
    s = adata.obs[col].astype(object)          # cast to plain Python str/None
    s = s.where(pd.notna(s), 'Unknown')        # fill NaN with 'Unknown'
    adata.obs[col] = pd.Categorical(s.astype(str))  # object-backed category
...
chosen_count_layer = chosen   # expose for uns['pipeline_params']['counts_source_layer']
...
# %% [markdown]
# ## 14. Pre-Write Cleanup + Save Output h5ad
#
# Two write-blocking issues fixed here before write_h5ad:
#   [BUG-W1] StringDtype (pd.StringDtype) columns crash anndata writer
#            (allow_write_nullable_strings=False by default).
#            Fix: convert to object dtype.  Annotation cols then re-cast to category.
#   [BUG-W2] Column named '_index' is reserved by anndata h5ad format.
#            Fix: rename to 'orig_index'.
#   [CLEAN]  Remove stale obsm keys inherited from source object.
#            Only X_scvi, X_scanvi, X_umap are produced by this pipeline.

# %%
# ---- BUG-W1: StringDtype -> object (then re-cast annotations to category) ----
# ---- BUG-W1 FIXED: pandas 3.0 StringDtype + anndata write ----
# In pandas 3.0+, ALL string data is stored as pd.StringDtype (StringArray).
# There is no reliable way to reconstruct object-backed categoricals because
# pd.Index / pd.Categorical always infers StringDtype for string values.
#
# anndata added allow_write_nullable_strings=True specifically for this:
# it tells the writer to accept StringArray categories (anndata >= 0.10.x).
# This is the correct and intended fix for pandas 3.0 compatibility.
import anndata as _anndata
_anndata.settings.allow_write_nullable_strings = True
print("allow_write_nullable_strings=True (pandas 3.0 + anndata compatibility)")

# ---- BUG-W2: rename reserved '_index' column if present ----
for attr in ('obs', 'var'):
    df = getattr(adata_hvg, attr)
    if '_index' in df.columns:
        setattr(adata_hvg, attr, df.rename(columns={'_index': 'orig_index'}))
        print(f"Renamed {attr}['_index'] -> 'orig_index'  (reserved name)")

# ---- CLEAN: drop stale obsm keys not produced by this pipeline ----
OBSM_KEEP = {'X_scvi', 'X_scanvi', 'X_umap'}
stale_obsm = [k for k in list(adata_hvg.obsm.keys()) if k not in OBSM_KEEP]
for k in stale_obsm:
    del adata_hvg.obsm[k]
if stale_obsm:
    print(f"Removed stale obsm keys: {stale_obsm}")
print(f"obsm kept: {list(adata_hvg.obsm.keys())}")

# %%
out_h5ad = output_dir / 'adata_tnk_scanvi_ref_20260315_v1_2.h5ad'
print(f"\nWriting {out_h5ad.name}...")
