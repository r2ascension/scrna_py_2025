...
Markers <- c('FXYD3', 'EPCAM', 'ELF3', 'SERPINF1', 'TSPAN1',
             ...
             'MXRA8', 'LEPR',
             'MYH11', 'TINAGL1', 'PLN', 'DES', 'ACTA2', 'CNN1', 'TAGLN', #Fibroblast & Endothelial
...
Marker_Endothelial <- c('TP63','KRT5',
...
setwd("E:/R/0301/Endothelial_analysis/")#设置工作路径
Endothelial_object <- readRDS("Endothelial_analyzed_20250625.rds")#读取数据
Endothelial_object <- NormalizeData(Endothelial_object)#归一化
Endothelial_object <- FindVariableFeatures(Endothelial_object, selection.method = "vst", nfeatures = 2000)#寻找变异基因
Endothelial_object <- ScaleData(Endothelial_object)#标准化
Endothelial_object <- RunPCA(Endothelial_object, npcs = 40)#PCA
Endothelial_object <- RunHarmony(
    object = Endothelial_object,           
...
# 获取降维结果和元数据
embeddings <- Embeddings(Endothelial_object, "harmony")
metadata <- Endothelial_object@meta.data
...
# # 1. UMI knee plot
# counts <- GetAssayData(Endothelial_object, layer = "counts")
...
# # 3. PC肘部图
# pct_var <- Endothelial_object[["pca"]]@stdev / sum(Endothelial_object[["pca"]]@stdev) * 100
...
# set.seed(42)
# n_sample <- min(10000, ncol(Endothelial_object))
# sample_idx <- sample(seq_len(ncol(Endothelial_object)), n_sample)

# harmony_scores <- Endothelial_object[["harmony"]]@cell.embeddings[sample_idx, 1:20]
...
Endothelial_object <- FindNeighbors(
    Endothelial_object, 
    reduction = "harmony", 
    dims = 1:30,
    n.trees = 500,
...
Endothelial_object <- FindClusters(
    Endothelial_object, 
    resolution = 3, 
    algorithm = 2
    )
...
Idents(Endothelial_object) <- Endothelial_object$seurat_clusters
pdf("umap_clusters.pdf", width = 10, height = 8)
DimPlot(Endothelial_object, reduction = "umap", label = TRUE)
dev.off()
# saveRDS(Endothelial_object, "final_analyzed_Endothelial_object.rds")
# 9. 保存分析结果
print("Saving analysis results...")

# Endothelial_object <- readRDS("E:/R/0224/final_analyzed_Endothelial_object.rds")
...
for(marker in Marker_Endothelial) {
  if(marker %in% rownames(Endothelial_object)) {
    print(paste("Processing marker:", marker))
    print(FeaturePlot(Endothelial_object, features = marker, raster = TRUE))
...
p1 <- DimPlot(Endothelial_object, reduction = "umap", group.by = "study", raster = TRUE, 
              pt.size = 0.5) + ggtitle("Batches")
p2 <- DimPlot(Endothelial_object, reduction = "umap", group.by = "sample", raster = TRUE, 
              pt.size = 0.5) + ggtitle("sample")
p3 <- DimPlot(Endothelial_object, reduction = "umap", group.by = "ann_level_3", raster = TRUE, 
              pt.size = 0.5) + ggtitle("ann_level_3")
p4 <- DimPlot(Endothelial_object, reduction = "umap", group.by = "tissue", raster = TRUE, 
              pt.size = 0.5) + ggtitle("tissue")
p5 <- DimPlot(Endothelial_object, reduction = "umap", group.by = "ann_finest_level", raster = TRUE, 
              pt.size = 0.5) + ggtitle("ann_finest_level")
# p6 <- DimPlot(Endothelial_object, reduction = "umap", group.by = "Annotation", raster = TRUE, 
...
tissues <- unique(Endothelial_object$tissue)
pdf("tissue_umap_split.pdf", width = 12, height = 12)
Idents(Endothelial_object) <- "tissue"  # 先设置identity
for(tissue_name in tissues) {
  print(DimPlot(Endothelial_object, 
                reduction = "umap",
                cells.highlight = CellsByIdentities(object = Endothelial_object, 
                                                    idents = tissue_name),
                cols = "grey",
                pt.size = 0.5,
                label = FALSE) +
...
samples <- unique(Endothelial_object$sample)
pdf("sample_umap_split.pdf", width = 12, height = 12)
Idents(Endothelial_object) <- "sample"  # 先设置identity
...
studys <- unique(Endothelial_object$study)
pdf("study_umap_split.pdf", width = 12, height = 12)
Idents(Endothelial_object) <- "study"  # 先设置identity
...
Endothelial_object$ann_finest_level_no_na <- Endothelial_object$ann_finest_level
Endothelial_object$ann_finest_level_no_na[is.na(Endothelial_object$ann_finest_level_no_na)] <- "Unknown"
Idents(Endothelial_object) <- "ann_finest_level_no_na"

pdf("ann_level_3_umap_split.pdf", width = 12, height = 12)
# 获取唯一的细胞类型（不包括NA和Unknown）
ann_finest_level_no_na <- unique(Endothelial_object$ann_finest_level_no_na)
...
for(anno in ann_finest_level_no_na) {
  print(DimPlot(Endothelial_object, 
                reduction = "umap",
                cells.highlight = CellsByIdentities(Endothelial_object, 
                                                    idents = anno),
                cols = "grey",
                pt.size = 0.5,
                raster = TRUE,
...
# 2. 如果发现有重复，可以移除重复项
Marker_Endothelial <- unique(Marker_Endothelial)

# 1. 生成点状图
print("Generating DotPlot...")
pdf("markers_dotplot.pdf", width = 32, height = 18)
dot_plot <- DotPlot(Endothelial_object, 
                    features = Marker_Endothelial, 
...
dev.off()
getwd()
saveRDS(Endothelial_object, "Endothelial_analyzed_20250722.rds")
...
current_clusters <- Endothelial_object$seurat_clusters  # 获取当前的cluster标识
new_idents <- annotations$Annotation[match(current_clusters, annotations$Cluster)]
names(new_idents) <- names(current_clusters)

# 3. 添加到meta.data并设置标识
Endothelial_object$Annotation <- new_idents
Endothelial_object <- SetIdent(Endothelial_object, value = new_idents)

# 呼吸道细胞类型配色方案
cell_colors <- c(
  "Capillary_Endothelial" = "#E31A1C",          # 鲜红色 - 肺泡上皮祖细胞
  "Lymphatic_Endothelial" = "#FF7F00",          # 橙色 - I型肺泡上皮细胞
  "Venous_Endothelial" = "#1F78B4",          # 蓝色 - II型肺泡上皮细胞
  "Arterial_Endothelial" = "#33A02C"
)
Idents(Endothelial_object) <- "Annotation"  # 先设置identity
# 5. 绘制UMAP图
pdf("cell_types_umap.pdf", width = 8, height = 8)
DimPlot(Endothelial_object, 
        reduction = "umap",
        raster = TRUE,
        label = TRUE,
        pt.size = 0.5,
...
# 获取所有细胞类型
cell_types <- unique(Endothelial_object$Annotation)
...
for(cell_type in cell_types) {
  ...
  print(DimPlot(Endothelial_object, 
                reduction = "umap",
                cells.highlight = WhichCells(Endothelial_object, idents = cell_type),
...
Undefined_obj <- subset(Endothelial_object, subset = Annotation == "Undefined")
...
# 检查这些细胞是否都存在于主对象中
cells_in_main <- cells_to_update %in% rownames(Endothelial_object@meta.data)
valid_cells_to_update <- cells_to_update[cells_in_main]

# 只更新主对象中存在的细胞的Annotation
Endothelial_object@meta.data[valid_cells_to_update, "Annotation"] <- 
  Undefined_obj@meta.data[valid_cells_to_update, "Annotation"]

# 验证更新
table(Endothelial_object@meta.data$Annotation)

Idents(Endothelial_object) <- "Annotation"
# 5. 绘制UMAP图
pdf("cell_types_umap.pdf", width = 8, height = 8)
DimPlot(Endothelial_object, 
        reduction = "umap",
...
# Endothelial_object$Annotation[Endothelial_object@meta.data[["Annotation"]] == "Ciliated secretory cell"] <- "Ciliated cell"
rm(Undefined_obj)
gc()

table(Endothelial_object$Annotation)

original_annotations <- Endothelial_object@meta.data$Annotation
Endothelial_object@meta.data$Annotation_clean <- gsub(" ", "_", original_annotations)

# 使用优化版scPairwiseMASCAnalysis函数
results <- scPairwiseMASCAnalysis(
  seurat_obj = Endothelial_object,
...
Idents(Endothelial_object) <- Endothelial_object$seurat_clusters
markers <- FindAllMarkers(Endothelial_object, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
write.csv(markers, "cluster_markers.csv")

# table(subset(Endothelial_object, subset = seurat_clusters =='85')@meta.data[["cell_type"]])
...
Endothelial_object <- ScaleData(Endothelial_object, features = marker_genes)
...
for(i in seq_along(marker_batches)) {
  ...
  # 绘制热图
  print(DoHeatmap(Endothelial_object, 
