#!/usr/bin/env python3
"""
All Cells: Integrate scANVI Annotations → scVI → scANVI Pipeline v1.0
=======================================================================

Purpose:
--------
1. Extract scANVI annotations from cell-type-specific analyses
...
5. Train scVI for batch correction
...
- B cell annotations: /home/h2048/data/py/1208/b_scvi_celltypist_scanvi_v1_1/adata_bcell_scvi_celltypist_scanvi_final_v2.h5ad
- Myeloid annotations: /home/h2048/data/py/1208/myeloid_scvi_celltypist_scanvi_v1_1/adata_myeloid_scvi_celltypist_scanvi_final_v2.h5ad
- T cell annotations: /home/h2048/data/py/1206/tcell_scvi_celltypist_scanvi_v2/adata_tcell_scvi_celltypist_scanvi_final_v2.h5ad
- Stromal annotations: /home/h2048/data/py/1209/stromal_vascular_scvi_analysis/adata_scvi_scanvi_final.h5ad
...
UNIFIED_ANNOTATION_COL = 'scanvi_annotation'
UNIFIED_CONFIDENCE_COL = 'scanvi_confidence'
UNIFIED_LINEAGE_COL = 'cell_lineage'  # Track which lineage the annotation came from
...
# =============================================================================
# Step 1: Extract scANVI Annotations
# =============================================================================

log_print("="*80)
log_print("STEP 1: EXTRACT scANVI ANNOTATIONS FROM CELL-TYPE-SPECIFIC ANALYSES")
log_print("="*80)
...
for lineage, config in SCANVI_RESULTS.items():
    ...
    try:
        # Read the file
        adata_subset = sc.read_h5ad(config['path'])
        log_print(f"  ✓ Loaded: {adata_subset.shape[0]} cells × {adata_subset.shape[1]} genes")

        # Extract annotations and confidence
        annotation_col = config['annotation_col']
        confidence_col = config['confidence_col']

        if annotation_col not in adata_subset.obs.columns:
            log_print(f"  ✗ ERROR: Column '{annotation_col}' not found!")
            log_print(f"    Available columns: {list(adata_subset.obs.columns[:10])}")
            continue

        if confidence_col not in adata_subset.obs.columns:
            log_print(f"  ✗ WARNING: Column '{confidence_col}' not found, using default confidence=1.0")
            confidence = pd.Series(1.0, index=adata_subset.obs.index)
        else:
            confidence = adata_subset.obs[confidence_col]

        # Create annotation DataFrame with cell barcode as index
        annotations = pd.DataFrame({
            UNIFIED_ANNOTATION_COL: adata_subset.obs[annotation_col].astype(str),
            UNIFIED_CONFIDENCE_COL: confidence,
            UNIFIED_LINEAGE_COL: lineage
        ...
        # Summary
        n_unique_types = annotations[UNIFIED_ANNOTATION_COL].nunique()
        mean_conf = annotations[UNIFIED_CONFIDENCE_COL].mean()
        log_print(f"  ✓ Extracted {len(annotations)} cells")
        log_print(f"    Unique cell types: {n_unique_types}")
        log_print(f"    Mean confidence: {mean_conf:.3f}")
        log_print(f"    Top 3 types: {list(annotations[UNIFIED_ANNOTATION_COL].value_counts().head(3).index)}")
...
log_print(f"  Unique cell types: {all_annotations[UNIFIED_ANNOTATION_COL].nunique()}")
...
annotations_file = OUTPUT_DIR / "extracted_scanvi_annotations.csv"
...
log_print(f"  Obs columns (first 10): {list(adata.obs.columns[:10])}")
...
adata.obs[UNIFIED_ANNOTATION_COL] = UNLABELED_CATEGORY
adata.obs[UNIFIED_CONFIDENCE_COL] = 0.0
...
valid_mask = ~matched_annotations[UNIFIED_ANNOTATION_COL].isna()

adata.obs.loc[valid_mask, UNIFIED_ANNOTATION_COL] = matched_annotations.loc[valid_mask, UNIFIED_ANNOTATION_COL]
adata.obs.loc[valid_mask, UNIFIED_CONFIDENCE_COL] = matched_annotations.loc[valid_mask, UNIFIED_CONFIDENCE_COL]
adata.obs.loc[valid_mask, UNIFIED_LINEAGE_COL] = matched_annotations.loc[valid_mask, UNIFIED_LINEAGE_COL]

log_print(f"\n✓ Merged annotations")
log_print(f"  Unique cell types (including Unknown): {adata.obs[UNIFIED_ANNOTATION_COL].nunique()}")
log_print(f"  Mean confidence (excluding Unknown): {adata.obs[adata.obs[UNIFIED_ANNOTATION_COL] != UNLABELED_CATEGORY][UNIFIED_CONFIDENCE_COL].mean():.3f}")
...
adata.obs['labels_for_scanvi'] = adata.obs[UNIFIED_ANNOTATION_COL].copy()
...
low_conf_mask = (adata.obs[UNIFIED_CONFIDENCE_COL] < MIN_CONFIDENCE) & \
...
sc.pl.umap(adata, color=UNIFIED_ANNOTATION_COL, ax=axes[0, 0], show=False,
...
sc.pl.umap(adata, color=UNIFIED_CONFIDENCE_COL, ax=axes[1, 0], show=False,
...
final_file = OUTPUT_DIR / "adata_allcells_scvi_scanvi_integrated_v1.h5ad"
adata.write_h5ad(final_file)
log_print(f"\n✓ Final integrated data saved: {final_file.name}")
...
summary_lines = [
    ...
    "ANNOTATION SUMMARY",
    "="*80,
    f"  Cells with original annotations: {(adata.obs[UNIFIED_ANNOTATION_COL] != UNLABELED_CATEGORY).sum():,}",
    f"  Unique cell types (original): {adata.obs[adata.obs[UNIFIED_ANNOTATION_COL] != UNLABELED_CATEGORY][UNIFIED_ANNOTATION_COL].nunique()}",
    f"  Mean confidence (original): {adata.obs[adata.obs[UNIFIED_ANNOTATION_COL] != UNLABELED_CATEGORY][UNIFIED_CONFIDENCE_COL].mean():.3f}",
    "",
...
summary_lines.extend([
    ...
    "="*80,
    f"  {UNIFIED_ANNOTATION_COL}: Original annotations from cell-type-specific analyses",
    f"  {UNIFIED_CONFIDENCE_COL}: Original annotation confidence",
    f"  {UNIFIED_LINEAGE_COL}: Cell lineage source (B_cell, Myeloid, T_cell, Stromal)",
...
# Save summary report
summary_file = OUTPUT_DIR / "analysis_summary_v1.txt"
