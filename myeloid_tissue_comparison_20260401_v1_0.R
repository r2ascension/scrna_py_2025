#!/usr/bin/env Rscript
# ==============================================================================
# Myeloid Tissue Comparison Pipeline v1.0.0
# ==============================================================================
#
# Purpose:
#   Adapt the validated tissue-comparison workflow used for B cells to the
#   myeloid reference object.
#
...
#     the dataset (for example neutrophils nested under Classical monocytes,
#     cDC2 split across DC/DC2, and interstitial macrophages partly under
#     Intestinal macrophages)
#
# Strategy:
#   - use cell_type_L3_refined as the standardized L3 source
#   - rebuild a cleaner lineage-level L2 from refined L3 labels
#   - reuse the shared tissue-comparison engine from the B-cell script by
...
BIOLOGICAL_QUESTION_FRAGMENT <- paste(
  ...
  "antigen presentation, interferon tone, or inflammatory recruitment"
)

# ----- Versioning / outputs -----
PIPELINE_VERSION_LABEL <- "v1.0.0"
PIPELINE_SUBTITLE <- sprintf(
  "%s Tissue Comparison %s (pseudobulk DESeq2, multi-database enrichment)",
  LINEAGE_DISPLAY, PIPELINE_VERSION_LABEL
)
REPORT_TITLE <- "# Myeloid Tissue Comparison Report (Normal Respiratory Tract)"
GENERATED_BY_LABEL <- "myeloid_tissue_comparison_20260401_v1_0.R"
...
OUTPUT_DIR <- "/home/h2048/data/R/0401/myeloid_tissue_comparison_v1_0"
...
PIPELINE_CHANGELOG_LINES <- c(
  ...
  "  [MYELOID-2] Rebuilds cleaner L2 labels from refined L3 to fix inconsistencies in the legacy `cell_type_L2` column.",
  "  [MYELOID-3] Adds myeloid-specific marker panels, enrichment filters, and custom marker database.",
  "  [MYELOID-4] Reuses the validated shared tissue-comparison engine for DE, enrichment, ssGSEA, CHOIR, OFA, and LLM interpretation."
)
PIPELINE_INHERITED_FIXES_TITLE <- "Shared engine fixes inherited from the validated template:"
PIPELINE_INHERITED_FIX_LINES <- c(
  "  [ENGINE-1] Sparse-friendly pseudobulk aggregation (no dense pre-allocation).",
  ...
  "  [ENGINE-5] Pseudobulk DE, ssGSEA, CHOIR, and OFA run at both coarse and fine annotation levels where applicable."
)
