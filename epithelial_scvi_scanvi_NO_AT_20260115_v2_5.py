...
"""
Epithelial scVI-scANVI DUAL Pipeline v2.7 - PRODUCTION
======================================================

Complete Implementation of Allcells v2.3.1 Best Practices:
...
Based on: epithelial_v2_6_P0_HOTFIX.py

Author: r2end
Date: 2025-01-15
Version: 2.7-PRODUCTION - Complete allcells architecture
"""
...
print("="*80)
print("Epithelial scVI-scANVI Dual Training Pipeline (v2.7-PRODUCTION)")
print("✅ Complete Allcells v2.3.1 Architecture")
print("="*80)
...
FORCE_RETRAIN_SCVI = True
FORCE_RETRAIN_SCANVI_MAJOR = True
FORCE_RETRAIN_SCANVI_FINE = True
...
# ⭐ NEW: Stress signature genes (from allcells line 165-179)
...
# ⭐ NEW: Cell cycle genes (from allcells line 182-197)
S_GENES = [
...
# ⭐ NEW: Unknown cleaning thresholds (from allcells line 203-206)
...
# ⭐ NEW: scArches-ready parameters (from allcells line 218-221)
...
# ⭐ NEW: Class imbalance handling (from allcells line 228)
...
scanvi_major_model_path = f"{OUTPUT_DIR}/scanvi_major_model"
scanvi_fine_model_path = f"{OUTPUT_DIR}/scanvi_fine_model"
hvg_file_path = f"{scvi_model_path}/hvg_genes.txt"
...
if ENABLE_WHITELIST_FILTERING:
    ...
    if cell_id_sets:
        if CELLID_SET_MODE == "union":
            ...
            print(f"\n✓ Whitelist (union): {len(whitelist_cells)} cells")
        elif CELLID_SET_MODE == "intersection":
            whitelist_cells = set.intersection(*cell_id_sets)
            print(f"\n✓ Whitelist (intersection): {len(whitelist_cells)} cells")
        else:
            raise ValueError(f"Invalid CELLID_SET_MODE: {CELLID_SET_MODE}")
    else:
        print("\n⚠️  No cell IDs collected, skipping whitelist filtering")
        ENABLE_WHITELIST_FILTERING = False
else:
    print("\n⚠️  Whitelist filtering disabled")
...
if 'counts' not in adata.layers:
    raise ValueError("❌ 'counts' layer required")
...
if n_at_cells > 0:
    ...
    print(f"✓ Removed {n_at_cells} cells, remaining: {adata.n_obs}")
...
# ⭐ 2.3 MT% recalculation (from allcells line 664-696)
print("\n⭐ 2.3 MT% calculation...")
...
if 'pct_counts_mt' in adata.obs.columns:
    ...
    if zero_mt_pct > 0.1:
        print("  ⚠️  High zero rate, recalculating...")
        adata.var['mt'] = adata.var_names.str.startswith('MT-')
        sc.pp.calculate_qc_metrics(adata, qc_vars=['mt'], layer='counts', 
                                   percent_top=None, inplace=True)
        print(f"  ✓ Recalculated, mean: {adata.obs['pct_counts_mt'].mean():.2f}%")
else:
    print("  Computing pct_counts_mt...")
    adata.var['mt'] = adata.var_names.str.startswith('MT-')
    sc.pp.calculate_qc_metrics(adata, qc_vars=['mt'], layer='counts',
                               percent_top=None, inplace=True)
    print(f"  ✓ Computed, mean: {adata.obs['pct_counts_mt'].mean():.2f}%")

# ⭐ 2.4 Stress score (from allcells line 708-734)
print("\n⭐ 2.4 Calculating stress_score...")
...
if len(stress_genes_in_data) > 10:
    ...
    print(f"  ✓ Mean: {adata.obs['stress_score'].mean():.3f}, Median: {adata.obs['stress_score'].median():.3f}")
else:
    adata.obs['stress_score'] = 0.0
    print("  ⚠️  Too few stress genes, set to 0")

del adata_temp
gc.collect()

# ⭐ 2.5 Cell cycle scores (from allcells line 737-770)
print("\n⭐ 2.5 Calculating cell cycle scores...")
...
if len(s_genes) > 10 and len(g2m_genes) > 10:
    ...
    adata.obs['phase'] = 'G1'
    print("  ⚠️  Too few cell cycle genes, set to 0")
...
if unmapped.any():
    ...
    print(f"⚠️  Unmapped types: {len(unmapped_types)}")
...
print(f"✓ Force-included {len(available_markers)} markers ({n_force_added} new)")
...
print(f"✓ Saved to .raw: {adata.raw.shape}")

# Subset to HVG
adata = adata[:, adata.var['highly_variable']].copy()
print(f"✓ Training shape: {adata.shape}")
...
# ===== STEP 6: scVI Training =====

print("\n" + "="*80)
print("STEP 6: scVI Training (with Covariates)")
print("="*80)
...
print(f"✓ Latent: {adata.obsm['X_scvi'].shape}")


# ⭐ STEP 6.5: Unknown Cleaning (Strategy B+C) =====

print("\n" + "="*80)
print("⭐ STEP 6.5: Unknown Cleaning (Before scANVI Training)")
print("="*80)
...
# ⭐ Strategy B: Neighborhood purity (from allcells line 1100-1150)
...
# ⭐ Strategy C: Quality gating (from allcells line 1200-1250)
...
print("Major:")
print(adata.obs['scanvi_labels_major'].value_counts())
print("\nFine:")
print(adata.obs['scanvi_labels_fine'].value_counts())


# ===== STEP 7: scANVI_major Training =====

print("\n" + "="*80)
print("STEP 7: scANVI_major Training")
print("="*80)
...
scanvi_major_model.save(scanvi_major_model_path, overwrite=True)
...
# ===== STEP 8: scANVI_fine Training =====

print("\n" + "="*80)
print("STEP 8: scANVI_fine Training")
print("="*80)
...
scanvi_fine_model.save(scanvi_fine_model_path, overwrite=True)
...
plt.close()
print("✓ Purity distribution plot")
