...
MANUAL_COL <- "Manual_Annotation"   # 若不存在，将从 Idents() 自动创建
BATCH_COL  <- "sample"             # Harmony batch key

# 需要保留的 Manual_Annotation 类别；设为 NULL 表示不做子集（保留全部）
TARGET_MANUAL <- NULL
# 示例：
...
ROGUE_MAX_CELLS_GROUP <- 2000   # 组内太多细胞则下采样（加速+控内存）
ROGUE_MAX_GENES_GROUP <- 3000   # 组内基因上限（先 sparse 过滤再截断）
...
seurat_sub <- RunHarmony(
  ...
  reduction = "pca",
  dims.use = 1:N_PCS,
  assay.use = DefaultAssay(seurat_sub),
  verbose = FALSE
)
...
markers <- FindAllMarkers(
  ...
  slot = "data"   # 使用 log-normalized data layer
)
...
# 只对热图基因做 scale，避免 scale.data 变成全基因巨大矩阵
