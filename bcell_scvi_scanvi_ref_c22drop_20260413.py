...
"""
B-cell reference scVI/scANVI rerun after removing CHOIR cluster 22.

Goals
-----
1. Start from the reference-only B-cell h5ad used by tissue comparison.
2. Remove the fibroblast-like contamination cells identified as CHOIR cluster 22.
3. Guarantee both `layers['counts']` and `raw.X` exist before training.
4. Re-run reference scVI/scANVI with L3 supervision (`cell_type_expert`).
5. Save a fresh reference h5ad for downstream tissue comparison.

Notes
-----
- This script overwrites `cell_type_scanvi_pred` with the fresh rerun L3
    predictions so the downstream R tissue-comparison pipeline uses the rerun
    labels instead of the stale input column.
- `raw.X` is created from the full-gene counts matrix before HVG subsetting, so
  the saved output preserves full-gene raw counts while the working matrix keeps
  the HVG training view.
"""
...
CHOIR_CLUSTER_CSV = Path(
    "/home/h2048/data/R/0412/bcell_tissue_comparison_v2_6_4_20260412/reports/choir/choir_clusters.csv"
)
DROP_CHOIR_CLUSTER = 22
OUTPUT_DIR = Path("/home/h2048/data/py/0415/bcell_scvi_scanvi_ref_c22drop_l3_20260415")
OUTPUT_H5AD = OUTPUT_DIR / "bcell_reference_c22drop_scanvi_L3_ref_20260415.h5ad"
OUTPUT_SCVI_DIR = OUTPUT_DIR / "scvi_bcell_ref_c22drop_l3_20260415"
OUTPUT_SCANVI_DIR = OUTPUT_DIR / "scanvi_bcell_L3_ref_c22drop_20260415"
OUTPUT_UMAP = OUTPUT_DIR / "umap_operator_scanvi.joblib"
OUTPUT_REMOVED = OUTPUT_DIR / "removed_choir_cluster_22_cells.csv"
OUTPUT_CONFIG = OUTPUT_DIR / "training_config.json"
OUTPUT_HVG = OUTPUT_DIR / "hvg_genes.txt"
REUSE_EXISTING_SCVI_MODEL = True
INPUT_SCVI_MODEL_DIR = Path(
    "/home/h2048/data/py/0413/bcell_scvi_scanvi_ref_c22drop_20260413/scvi_bcell_L2_ref_c22drop_20260413"
)
...
def load_cluster_cells(cluster_csv: Path, cluster_id: int) -> pd.Index:
    ...
    keep = df.loc[pd.to_numeric(df["choir_cluster"], errors="coerce") == cluster_id, "cell"].astype(str)
    if keep.empty:
        raise ValueError(f"No cells found for CHOIR cluster {cluster_id} in {cluster_csv}")
    return pd.Index(keep.unique())
...
def remove_c22_cells(adata: sc.AnnData, c22_cells: pd.Index) -> tuple[sc.AnnData, pd.DataFrame]:
    obs_names = pd.Index(adata.obs_names.astype(str))
    barcode_idx = pd.Index(adata.obs["barcode"].astype(str)) if "barcode" in adata.obs.columns else pd.Index([])

    hit_obs = obs_names.isin(c22_cells)
    hit_barcode = barcode_idx.isin(c22_cells) if len(barcode_idx) == adata.n_obs else np.zeros(adata.n_obs, dtype=bool)
    drop_mask = hit_obs | hit_barcode

    n_drop = int(drop_mask.sum())
    if n_drop == 0:
        raise ValueError("CHOIR cluster 22 cells were not found in the reference h5ad")

    removed = adata.obs.loc[drop_mask].copy()
    removed.insert(0, "obs_name", adata.obs_names[drop_mask].astype(str))
    removed.insert(1, "matched_by_obs_name", hit_obs[drop_mask].astype(bool))
    removed.insert(2, "matched_by_barcode", hit_barcode[drop_mask].astype(bool))

    print(f"[OK] removing {n_drop:,} CHOIR cluster 22 cells")
...
def map_l3_to_l2(adata: sc.AnnData) -> None:
    ...
    if n_missing > 0:
        ...
        raise ValueError(f"Unmapped L3 labels after c22 filtering: {unmapped}")
...
def main() -> None:
    ...
    print_banner("B-cell reference scVI/scANVI rerun after removing CHOIR cluster 22")
    print(f"Input reference : {INPUT_REF_H5AD}")
    print(f"CHOIR cluster   : {DROP_CHOIR_CLUSTER} ({CHOIR_CLUSTER_CSV})")
    ...
    print_banner("Step 1: load reference h5ad")
    adata = sc.read_h5ad(INPUT_REF_H5AD)
    adata.var_names_make_unique()
    adata.obs_names_make_unique()
    print(f"Loaded shape    : {adata.shape}")

    c22_cells = load_cluster_cells(CHOIR_CLUSTER_CSV, DROP_CHOIR_CLUSTER)
    adata, removed = remove_c22_cells(adata, c22_cells)
    ...
    print_banner("Step 4: scVI setup / reuse")
    ...
    if REUSE_EXISTING_SCVI_MODEL and INPUT_SCVI_MODEL_DIR.exists():
        scvi_model = scvi.model.SCVI.load(INPUT_SCVI_MODEL_DIR, adata=adata_hvg)
        print(f"[OK] Reused existing scVI model: {INPUT_SCVI_MODEL_DIR}")
    else:
        print("[WARN] Existing scVI model unavailable; falling back to retraining")
        ...
        print(f"[OK] Fresh scVI model trained")
    scvi_model.save(OUTPUT_SCVI_DIR, overwrite=True)
    print(f"[OK] scVI model saved: {OUTPUT_SCVI_DIR}")

    print_banner("Step 5: scANVI training")
    ...
    print(f"[OK] scANVI model saved: {OUTPUT_SCANVI_DIR}")
    ...
    config = {
        ...
        "choir_cluster_csv": str(CHOIR_CLUSTER_CSV),
        "drop_choir_cluster": DROP_CHOIR_CLUSTER,
        ...
        "reuse_existing_scvi_model": bool(REUSE_EXISTING_SCVI_MODEL),
        "input_scvi_model_dir": str(INPUT_SCVI_MODEL_DIR),
    ...
    print_banner("DONE")
    print(f"Removed c22 cells : {removed.shape[0]:,}")
